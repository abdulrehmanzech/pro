...
import { utils as ce, OverlayMode as h0, ActionType as T1, LineType as o1, init as P9, FormatDateType as Dt, DomPosition as R1, dispose as zn, TooltipIconPosition as Nt, CandleType as O9, YAxisType as D9, registerOverlay as N9 } from "klinecharts";
function ft(e, n, t) {
...
const I9 = {
  ...
  createPointFigures: ({ coordinates: e, overlay: n, precision: t }) => {
    const r = [], a = [];
    if (e.length > 1) {
      const s = e[1].x > e[0].x ? e[0].x : e[1].x, d = [1, 0.786, 0.618, 0.5, 0.382, 0.236, 0], u = e[0].y - e[1].y, p = n.points, k = p[0].value - p[1].value;
      d.forEach((m) => {
        const w = e[1].y + u * m, A = (p[1].value + k * m).toFixed(t.price);
        r.push({ coordinates: [{ x: e[0].x, y: w }, { x: e[1].x, y: w }] }), a.push({
          x: s,
          y: w,
          text: `${A} (${(m * 100).toFixed(1)}%)`,
          baseline: "bottom"
        });
      });
    }
    ...
    if (e.length > 1) {
      ...
      for (let w = 2; w < 9; w++) {
        ...
        switch (w % 4) {
          ...
          case 3: {
            O = s + Math.PI / 2 * 3, m += p[w - 2].r;
            break;
          }
        }
        const F = O + Math.PI / 2, B = ft({ x: k, y: m }, e[0], s);
        p.push({
          ...B,
          r: A,
          startAngle: O,
  ...
  createPointFigures: ({ coordinates: e, bounding: n }) => {
    ...
    if (e.length > 1) {
      ...
      [1, 0.75, 0.618, 0.5, 0.382, 0.25, 0].forEach((m) => {
        const w = e[1].x - u * m, A = e[1].y - p * m;
        t.push({ coordinates: [{ x: w, y: e[0].y }, { x: w, y: e[1].y }] }), t.push({ coordinates: [{ x: e[0].x, y: A }, { x: e[1].x, y: A }] }), r = r.concat(v0([e[0], { x: w, y: e[1].y }], n)), r = r.concat(v0([e[0], { x: e[1].x, y: A }], n)), a.unshift({
          x: e[0].x + s,
          y: A + 10,
          text: `${m.toFixed(3)}`
        }), a.unshift({
  ...
  createPointFigures: ({ coordinates: e, overlay: n, precision: t }) => {
    const r = [], a = [];
    if (e.length > 2) {
      const s = n.points, d = s[1].value - s[0].value, u = e[1].y - e[0].y, p = [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1], k = e[2].x > e[1].x ? e[1].x : e[2].x;
      p.forEach((m) => {
        const w = e[2].y + u * m, A = (s[2].value + d * m).toFixed(t.price);
...
const e5 = (e, n) => e === n, C0 = Symbol("solid-proxy"), t5 = Symbol("solid-track"), Rt = {
  equals: e5
};
let Pr = Ir;
const f1 = 1, jt = 2, Or = {
...
var Ae = null;
let O1 = null, me = null, Fe = null, d1 = null, T0 = 0;
function gt(e, n) {
  const t = me, r = Ae, a = e.length === 0, s = a ? Or : {
    owned: null,
    cleanups: null,
    context: null,
    owner: n === void 0 ? r : n
...
function n5(e, n, t) {
  ...
  let d = null, u = f0, p = null, k = !1, m = "initialValue" in s, w = typeof r == "function" && Z(r);
  const A = /* @__PURE__ */ new Set(), [O, F] = (s.storage || T)(s.initialValue), [B, D] = T(void 0), [K, X] = T(void 0, {
    equals: !1
  }), [U, Y] = T(m ? "ready" : "unresolved");
  if (De.context) {
  ...
  function V(q, ne, ie, $e) {
    return d === q && (d = null, m = !0, (q === u || ne === u) && s.onHydrated && queueMicrotask(() => s.onHydrated($e, {
      value: ne
    })), u = f0, ue(ne, ie)), ne;
  }
  function ue(q, ne) {
  ...
  function R() {
    const q = o5, ne = O(), ie = B();
    if (ie !== void 0 && !d)
      throw ie;
    return me && !me.user && q && Kn(() => {
...
function r5(e) {
  ...
  return Promise.resolve().then(() => {
    me = n, Ae = t;
    let r;
    return v1(e, !1), me = Ae = null, r ? r.done : void 0;
  });
}
let o5;
function Dr() {
...
function Z1(e) {
  if (!e.fn)
    return;
  Xt(e);
  const n = Ae, t = me, r = T0;
  me = Ae = e, i5(e, e.value, r), me = t, Ae = n;
}
function i5(e, n, t) {
  let r;
  try {
...
function c5(e, n, t = {}) {
  ...
  return N1(() => Rn(s)), () => {
    ...
    return p[t5], h1(() => {
      ...
      if (A === 0)
        d !== 0 && (Rn(s), s = [], r = [], a = [], d = 0, u && (u = [])), t.fallback && (r = [l5], a[0] = gt((ue) => (s[0] = ue, t.fallback())), d = 1);
      else if (d === 0) {
        for (a = new Array(A), m = 0; m < A; m++)
          r[m] = p[m], a[m] = gt(w);
        d = A;
      } else {
        ...
        for (O = /* @__PURE__ */ new Map(), F = new Array(Y + 1), m = Y; m >= X; m--)
          V = p[m], k = O.get(V), F[m] = k === void 0 ? -1 : k, O.set(V, m);
        for (k = X; k <= U; k++)
          V = r[k], m = O.get(V), m !== void 0 && m !== -1 ? (B[m] = a[k], D[m] = s[k], u && (K[m] = u[k]), m = F[m], O.set(V, m)) : s[k]();
        for (m = X; m < A; m++)
          m in B ? (a[m] = B[m], s[m] = D[m], u && (u[m] = K[m], u[m](m))) : a[m] = gt(w);
...
const u5 = {
  get(e, n, t) {
    return n === C0 ? t : e.get(n);
  },
  has(e, n) {
    return n === C0 ? !0 : e.has(n);
  },
  set: It,
  deleteProperty: It,
...
function Ur(...e) {
  ...
  for (let r = 0; r < e.length; r++) {
    const a = e[r];
    n = n || !!a && C0 in a, e[r] = typeof a == "function" ? (n = !0, Z(a)) : a;
  }
  if (n)
    return new Proxy({
      get(r) {
...
function d5(e, n, t) {
  let r = t.length, a = n.length, s = r, d = 0, u = 0, p = n[a - 1].nextSibling, k = null;
  for (; d < a || u < s; ) {
    if (n[d] === t[u]) {
      d++, u++;
      continue;
    ...
    for (; n[a - 1] === t[s - 1]; )
      a--, s--;
    if (a === d) {
      const m = s < r ? u ? t[u - 1].nextSibling : t[s - u] : p;
      for (; u < s; )
        e.insertBefore(t[u++], m);
    } else if (s === u)
      for (; d < a; )
        (!k || !k.has(n[d])) && n[d].remove(), d++;
    else if (n[d] === t[s - 1] && t[u] === n[a - 1]) {
      const m = n[--a].nextSibling;
      e.insertBefore(t[u++], n[d++].nextSibling), e.insertBefore(t[--s], m), n[a] = t[s];
    } else {
      if (!k) {
        k = /* @__PURE__ */ new Map();
        let w = u;
      ...
      if (m != null)
        if (u < m && m < s) {
          let w = d, A = 1, O;
          for (; ++w < a && w < s && !((O = k.get(n[w])) == null || O !== m + A); )
            A++;
          if (A > m - u) {
            const F = n[d];
            for (; u < m; )
...
function h5(e, n, t, r = {}) {
  let a;
  return gt((s) => {
    a = s, n === document ? e() : v(n, e(), n.firstChild ? null : void 0, t);
  }, r.owner), () => {
    a(), n.textContent = "";
  };
}
...
function je(e, n = window.document) {
  ...
  for (let r = 0, a = e.length; r < a; r++) {
    const s = e[r];
    t.has(s) || (t.add(s), n.addEventListener(s, f5));
  }
}
function Pe(e, n, t) {
  ...
  if (e = d && t[0] && t[0].parentNode || e, s === "string" || s === "number") {
    ...
    if (Array.isArray(n)) {
      ...
      if (u.length === 0) {
        ...
        p ? t.length === 0 ? Qn(e, u, r) : d5(e, t, u) : (t && j1(e), Qn(e, u));
      t = u;
    ...
    if (u instanceof Node)
      ...
      if (Array.isArray(u))
        ...
        if (r) {
          for (; typeof u == "function"; )
            u = u();
          a = b0(e, Array.isArray(u) ? u : [u], Array.isArray(p) ? p : [p]) || a;
        } else
          e.push(u), a = !0;
      else {
        const k = String(u);
  ...
  if (n.length) {
    ...
    for (let d = n.length - 1; d >= 0; d--) {
      ...
      if (a !== u) {
        ...
        !s && !d ? p ? e.replaceChild(a, u) : e.insertBefore(a, t) : p && u.remove();
      } else
        s = !0;
    }
  } else
...
const g5 = "http://www.w3.org/2000/svg";
function m5(e, n = !1) {
  return n ? document.createElementNS(g5, e) : document.createElement(e);
}
function y5(e) {
  const {
    useShadow: n
  ...
  if (r instanceof HTMLHeadElement) {
    ...
    const s = m5(e.isSVG ? "g" : "div", e.isSVG), d = n && s.attachShadow ? s.attachShadow({
      mode: "open"
    }) : s;
...
var p5 = typeof Et == "object" && Et && Et.Object === Object && Et, Kr = p5, v5 = Kr, C5 = typeof self == "object" && self && self.Object === Object && self, $5 = v5 || C5 || Function("return this")(), a1 = $5, b5 = a1, _5 = b5.Symbol, Jt = _5, Zn = Jt, Rr = Object.prototype, k5 = Rr.hasOwnProperty, x5 = Rr.toString, ht = Zn ? Zn.toStringTag : void 0;
function L5(e) {
...
var S5 = T5, Vn = Jt, P5 = w5, O5 = S5, D5 = "[object Null]", N5 = "[object Undefined]", Hn = Vn ? Vn.toStringTag : void 0;
function I5(e) {
  return e == null ? e === void 0 ? N5 : D5 : Hn && Hn in Object(e) ? P5(e) : O5(e);
}
var mt = I5;
function E5(e) {
  var n = typeof e;
...
var V1 = E5, B5 = mt, F5 = V1, U5 = "[object AsyncFunction]", z5 = "[object Function]", K5 = "[object GeneratorFunction]", R5 = "[object Proxy]";
function j5(e) {
  if (!F5(e))
    return !1;
  var n = B5(e);
...
var jr = j5, Q5 = a1, Z5 = Q5["__core-js_shared__"], V5 = Z5, m0 = V5, qn = function() {
  var e = /[^.]+$/.exec(m0 && m0.keys && m0.keys.IE_PROTO || "");
...
var h6 = d6, f6 = u6, g6 = h6;
function m6(e, n) {
  var t = g6(e, n);
  return f6(t) ? t : void 0;
}
var E1 = m6, y6 = E1, p6 = function() {
  try {
...
var P0 = L6, w6 = Array.isArray, H1 = w6;
function A6(e) {
  return e != null && typeof e == "object";
}
var q1 = A6, M6 = mt, T6 = q1, S6 = "[object Symbol]";
function P6(e) {
  return typeof e == "symbol" || T6(e) && M6(e) == S6;
}
var O0 = P6, O6 = H1, D6 = O0, N6 = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/, I6 = /^\w*$/;
function E6(e, n) {
  if (O6(e))
    return !1;
  var t = typeof e;
...
var B6 = E6, F6 = E1, U6 = F6(Object, "create"), e0 = U6, Wn = e0;
function z6() {
...
var j6 = R6, Q6 = e0, Z6 = "__lodash_hash_undefined__", V6 = Object.prototype, H6 = V6.hasOwnProperty;
function q6(e) {
  var n = this.__data__;
  if (Q6) {
    var t = n[e];
...
var Y6 = q6, W6 = e0, G6 = Object.prototype, X6 = G6.hasOwnProperty;
function J6(e) {
  var n = this.__data__;
  return W6 ? n[e] !== void 0 : X6.call(n, e);
}
var e2 = J6, t2 = e0, n2 = "__lodash_hash_undefined__";
function r2(e, n) {
  var t = this.__data__;
...
var t0 = g2, m2 = t0, y2 = Array.prototype, p2 = y2.splice;
function v2(e) {
...
W1.prototype.has = O2;
W1.prototype.set = D2;
var n0 = W1, N2 = E1, I2 = a1, E2 = N2(I2, "Map"), D0 = E2, Gn = u2, B2 = n0, F2 = D0;
function U2() {
...
function N0(e, n) {
  ...
  return t.cache = new (N0.Cache || qr)(), t;
}
N0.Cache = qr;
var co = N0, uo = co, ho = 500;
function fo(e) {
...
var go = fo, mo = go, yo = /[^.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|$))/g, po = /\\(\\)?/g, vo = mo(function(e) {
  var n = [];
  return e.charCodeAt(0) === 46 && n.push(""), e.replace(yo, function(t, r, a, s) {
    n.push(a ? s.replace(po, "$1") : r || t);
  }), n;
}), Co = vo;
function $o(e, n) {
...
var bo = $o, Xn = Jt, _o = bo, ko = H1, xo = O0, Lo = 1 / 0, Jn = Xn ? Xn.prototype : void 0, er = Jn ? Jn.toString : void 0;
function Yr(e) {
  if (typeof e == "string")
    return e;
  if (ko(e))
...
var Ro = Ko, jo = P0, Qo = Io, Zo = Wr, tr = V1, Vo = Ro;
function Ho(e, n, t, r) {
  if (!tr(e))
    return e;
  n = Qo(n, e);
  for (var a = -1, s = n.length, d = s - 1, u = e; u != null && ++a < s; ) {
    ...
    if (a != d) {
      var m = u[p];
      k = r ? r(m, p, u) : void 0, k === void 0 && (k = tr(m) ? m : Zo(n[a + 1]) ? [] : {});
    }
    jo(u, p, k), u = u[p];
  }
  return e;
...
var a3 = i3, s3 = n0, l3 = D0, c3 = Hr, u3 = 200;
function d3(e, n) {
  var t = this.__data__;
  if (t instanceof s3) {
    var r = t.__data__;
    ...
    t = this.__data__ = new c3(r);
  }
  return t.set(e, n), this.size = t.size, this;
}
var h3 = d3, f3 = n0, g3 = e3, m3 = n3, y3 = o3, p3 = a3, v3 = h3;
function X1(e) {
  var n = this.__data__ = new f3(e);
  this.size = n.size;
}
X1.prototype.clear = g3;
X1.prototype.delete = m3;
X1.prototype.get = y3;
X1.prototype.has = p3;
X1.prototype.set = v3;
var C3 = X1;
...
var b3 = $3, _3 = P0, k3 = Zr;
function x3(e, n, t, r) {
  var a = !t;
  t || (t = {});
  for (var s = -1, d = n.length; ++s < d; ) {
    var u = n[s], p = r ? r(t[u], e[u], u, t, e) : void 0;
    p === void 0 && (p = e[u]), a ? k3(t, u, p) : _3(t, u, p);
  }
  return t;
}
var o0 = x3;
...
var w3 = L3, A3 = mt, M3 = q1, T3 = "[object Arguments]";
function S3(e) {
  return M3(e) && A3(e) == T3;
}
var P3 = S3, nr = P3, O3 = q1, Gr = Object.prototype, D3 = Gr.hasOwnProperty, N3 = Gr.propertyIsEnumerable, I3 = nr(function() {
  return arguments;
}()) ? nr : function(e) {
  return O3(e) && D3.call(e, "callee") && !N3.call(e, "callee");
}, E3 = I3, Ht = { exports: {} };
function B3() {
  return !1;
}
...
Ht.exports;
(function(e, n) {
  var t = a1, r = F3, a = n && !n.nodeType && n, s = a && !0 && e && !e.nodeType && e, d = s && s.exports === a, u = d ? t.Buffer : void 0, p = u ? u.isBuffer : void 0, k = p || r;
  e.exports = k;
})(Ht, Ht.exports);
var Xr = Ht.exports, U3 = 9007199254740991;
function z3(e) {
  return typeof e == "number" && e > -1 && e % 1 == 0 && e <= U3;
}
var Jr = z3, K3 = mt, R3 = Jr, j3 = q1, Q3 = "[object Arguments]", Z3 = "[object Array]", V3 = "[object Boolean]", H3 = "[object Date]", q3 = "[object Error]", Y3 = "[object Function]", W3 = "[object Map]", G3 = "[object Number]", X3 = "[object Object]", J3 = "[object RegExp]", ei = "[object Set]", ti = "[object String]", ni = "[object WeakMap]", ri = "[object ArrayBuffer]", oi = "[object DataView]", ii = "[object Float32Array]", ai = "[object Float64Array]", si = "[object Int8Array]", li = "[object Int16Array]", ci = "[object Int32Array]", ui = "[object Uint8Array]", di = "[object Uint8ClampedArray]", hi = "[object Uint16Array]", fi = "[object Uint32Array]", ge = {};
ge[ii] = ge[ai] = ge[si] = ge[li] = ge[ci] = ge[ui] = ge[di] = ge[hi] = ge[fi] = !0;
ge[Q3] = ge[Z3] = ge[ri] = ge[V3] = ge[oi] = ge[H3] = ge[q3] = ge[Y3] = ge[W3] = ge[G3] = ge[X3] = ge[J3] = ge[ei] = ge[ti] = ge[ni] = !1;
function gi(e) {
  return j3(e) && R3(e.length) && !!ge[K3(e)];
}
var mi = gi;
function yi(e) {
  return function(n) {
...
var E0 = qt.exports, pi = mi, vi = I0, rr = E0, or = rr && rr.isTypedArray, Ci = or ? vi(or) : pi, $i = Ci, bi = w3, _i = E3, ki = H1, xi = Xr, Li = Wr, wi = $i, Ai = Object.prototype, Mi = Ai.hasOwnProperty;
function Ti(e, n) {
  var t = ki(e), r = !t && _i(e), a = !t && !r && xi(e), s = !t && !r && !a && wi(e), d = t || r || a || s, u = d ? bi(e.length, String) : [], p = u.length;
  for (var k in e)
    (n || Mi.call(e, k)) && !(d && // Safari 9 has enumerable `arguments.length` in strict mode.
...
var t9 = Oi, Di = t9, Ni = Di(Object.keys, Object), Ii = Ni, Ei = B0, Bi = Ii, Fi = Object.prototype, Ui = Fi.hasOwnProperty;
function zi(e) {
  if (!Ei(e))
    return Bi(e);
  var n = [];
  for (var t in Object(e))
    Ui.call(e, t) && t != "constructor" && n.push(t);
  return n;
}
var Ki = zi, Ri = jr, ji = Jr;
function Qi(e) {
  return e != null && ji(e.length) && !Ri(e);
}
var n9 = Qi, Zi = e9, Vi = Ki, Hi = n9;
function qi(e) {
  return Hi(e) ? Zi(e) : Vi(e);
}
var F0 = qi, Yi = o0, Wi = F0;
function Gi(e, n) {
  return e && Yi(n, Wi(n), e);
}
var Xi = Gi;
function Ji(e) {
  var n = [];
...
var e8 = Ji, t8 = V1, n8 = B0, r8 = e8, o8 = Object.prototype, i8 = o8.hasOwnProperty;
function a8(e) {
  if (!t8(e))
    return r8(e);
  var n = n8(e), t = [];
  for (var r in e)
    r == "constructor" && (n || !i8.call(e, r)) || t.push(r);
  return t;
}
var s8 = a8, l8 = e9, c8 = s8, u8 = n9;
function d8(e) {
  return u8(e) ? l8(e, !0) : c8(e);
}
var U0 = d8, h8 = o0, f8 = U0;
function g8(e, n) {
  return e && h8(n, f8(n), e);
}
var m8 = g8, Yt = { exports: {} };
Yt.exports;
(function(e, n) {
...
var r9 = b8, _8 = $8, k8 = r9, x8 = Object.prototype, L8 = x8.propertyIsEnumerable, ir = Object.getOwnPropertySymbols, w8 = ir ? function(e) {
  return e == null ? [] : (e = Object(e), _8(ir(e), function(n) {
    return L8.call(e, n);
  }));
} : k8, z0 = w8, A8 = o0, M8 = z0;
function T8(e, n) {
  return A8(e, M8(e), n);
}
var S8 = T8;
...
var o9 = P8, O8 = t9, D8 = O8(Object.getPrototypeOf, Object), i9 = D8, N8 = o9, I8 = i9, E8 = z0, B8 = r9, F8 = Object.getOwnPropertySymbols, U8 = F8 ? function(e) {
  for (var n = []; e; )
    N8(n, E8(e)), e = I8(e);
  return n;
} : B8, a9 = U8, z8 = o0, K8 = a9;
function R8(e, n) {
  return z8(e, K8(e), n);
}
var j8 = R8, Q8 = o9, Z8 = H1;
function V8(e, n, t) {
  var r = n(e);
  return Z8(e) ? r : Q8(r, t(e));
}
var s9 = V8, H8 = s9, q8 = z0, Y8 = F0;
function W8(e) {
  return H8(e, Y8, q8);
}
var G8 = W8, X8 = s9, J8 = a9, ea = U0;
function ta(e) {
  return X8(e, ea, J8);
}
var na = ta, ra = E1, oa = a1, ia = ra(oa, "DataView"), aa = ia, sa = E1, la = a1, ca = sa(la, "Promise"), ua = ca, da = E1, ha = a1, fa = da(ha, "Set"), ga = fa, ma = E1, ya = a1, pa = ma(ya, "WeakMap"), va = pa, k0 = aa, x0 = D0, L0 = ua, w0 = ga, A0 = va, l9 = mt, J1 = Qr, ar = "[object Map]", Ca = "[object Object]", sr = "[object Promise]", lr = "[object Set]", cr = "[object WeakMap]", ur = "[object DataView]", $a = J1(k0), ba = J1(x0), _a = J1(L0), ka = J1(w0), xa = J1(A0), S1 = l9;
(k0 && S1(new k0(new ArrayBuffer(1))) != ur || x0 && S1(new x0()) != ar || L0 && S1(L0.resolve()) != sr || w0 && S1(new w0()) != lr || A0 && S1(new A0()) != cr) && (S1 = function(e) {
  var n = l9(e), t = n == Ca ? e.constructor : void 0, r = t ? J1(t) : "";
  if (r)
    switch (r) {
      case $a:
        return ur;
...
var K0 = S1, La = Object.prototype, wa = La.hasOwnProperty;
function Aa(e) {
  var n = e.length, t = new e.constructor(n);
  return n && typeof e[0] == "string" && wa.call(e, "index") && (t.index = e.index, t.input = e.input), t;
}
var Ma = Aa, Ta = a1, Sa = Ta.Uint8Array, Pa = Sa, dr = Pa;
function Oa(e) {
...
var Ia = Na, Ea = /\w*$/;
function Ba(e) {
  var n = new e.constructor(e.source, Ea.exec(e));
  return n.lastIndex = e.lastIndex, n;
}
var Fa = Ba, hr = Jt, fr = hr ? hr.prototype : void 0, gr = fr ? fr.valueOf : void 0;
function Ua(e) {
...
var ms = gs, ys = V1, mr = Object.create, ps = function() {
  function e() {
  }
  return function(n) {
    if (!ys(n))
...
}(), vs = ps, Cs = vs, $s = i9, bs = B0;
function _s(e) {
  return typeof e.constructor == "function" && !bs(e) ? Cs($s(e)) : {};
}
var ks = _s, xs = K0, Ls = q1, ws = "[object Map]";
function As(e) {
  return Ls(e) && xs(e) == ws;
}
var Ms = As, Ts = Ms, Ss = I0, yr = E0, pr = yr && yr.isMap, Ps = pr ? Ss(pr) : Ts, Os = Ps, Ds = K0, Ns = q1, Is = "[object Set]";
function Es(e) {
  return Ns(e) && Ds(e) == Is;
}
var Bs = Es, Fs = Bs, Us = I0, vr = E0, Cr = vr && vr.isSet, zs = Cr ? Us(Cr) : Fs, Ks = zs, Rs = C3, js = b3, Qs = P0, Zs = Xi, Vs = m8, Hs = y8, qs = v8, Ys = S8, Ws = j8, Gs = G8, Xs = na, Js = K0, e7 = Ma, t7 = ms, n7 = ks, r7 = H1, o7 = Xr, i7 = Os, a7 = V1, s7 = Ks, l7 = F0, c7 = U0, u7 = 1, d7 = 2, h7 = 4, c9 = "[object Arguments]", f7 = "[object Array]", g7 = "[object Boolean]", m7 = "[object Date]", y7 = "[object Error]", u9 = "[object Function]", p7 = "[object GeneratorFunction]", v7 = "[object Map]", C7 = "[object Number]", d9 = "[object Object]", $7 = "[object RegExp]", b7 = "[object Set]", _7 = "[object String]", k7 = "[object Symbol]", x7 = "[object WeakMap]", L7 = "[object ArrayBuffer]", w7 = "[object DataView]", A7 = "[object Float32Array]", M7 = "[object Float64Array]", T7 = "[object Int8Array]", S7 = "[object Int16Array]", P7 = "[object Int32Array]", O7 = "[object Uint8Array]", D7 = "[object Uint8ClampedArray]", N7 = "[object Uint16Array]", I7 = "[object Uint32Array]", fe = {};
fe[c9] = fe[f7] = fe[L7] = fe[w7] = fe[g7] = fe[m7] = fe[A7] = fe[M7] = fe[T7] = fe[S7] = fe[P7] = fe[v7] = fe[C7] = fe[d9] = fe[$7] = fe[b7] = fe[_7] = fe[k7] = fe[O7] = fe[D7] = fe[N7] = fe[I7] = !0;
fe[y7] = fe[u9] = fe[x7] = !1;
function Kt(e, n, t, r, a, s) {
  var d, u = n & u7, p = n & d7, k = n & h7;
  ...
  if (m) {
    ...
    if (w == d9 || w == c9 || A && !a) {
      if (d = p || A ? {} : n7(e), !u)
        return p ? Ws(e, Vs(d, e)) : Ys(e, Zs(d, e));
    } else {
      if (!fe[w])
        return a ? e : {};
      d = t7(e, w, u);
  ...
  if (O)
    return O;
  s.set(e, d), s7(e) ? e.forEach(function(D) {
    d.add(Kt(D, n, t, D, e, s));
  }) : i7(e) && e.forEach(function(D, K) {
    d.set(K, Kt(D, n, t, K, e, s));
  });
  var F = k ? p ? Xs : Gs : p ? c7 : l7, B = m ? void 0 : F(e);
  return js(B || e, function(D, K) {
    B && (K = D, D = e[K]), Qs(d, K, Kt(D, n, t, K, e, s));
  }), d;
}
  ...
  return v(n, L(J, {
    ...
    get children() {
      ...
      return (t = e.dataSource) == null ? void 0 : t.map((r) => {
        var a;
        return ((a = e.renderItem) == null ? void 0 : a.call(e, r)) ?? t4.cloneNode(!0);
      });
    }
  }), null), I((t) => {
  ...
  return n.$$click = (d) => {
    ...
    return () => d() && (() => {
      ...
      return v(u, () => e.buttons.map((p) => L(Q7, Ur(p, {
        ...
        const k = e.btnParentStyle, m = !!e.isMobile;
        return p._v$8 = I1(u, k, p._v$8), m !== p._v$9 && u.classList.toggle("mobile-buttons", p._v$9 = m), p;
      }, {
        _v$8: void 0,
        _v$9: void 0
      }), u;
    ...
    const u = !!e.isMobile, p = e.isMobile ? "100%" : `${e.width ?? 400}px`, k = (e.isMobile, "auto"), m = e.isMobile ? "60vh" : "90vh", w = !!e.isMobile, A = !!e.isMobile, O = !!e.isMobile;
    return u !== d._v$ && n.classList.toggle("mobile-modal", d._v$ = u), p !== d._v$2 && t.style.setProperty("width", d._v$2 = p), k !== d._v$3 && t.style.setProperty("height", d._v$3 = k), m !== d._v$4 && t.style.setProperty("max-height", d._v$4 = m), w !== d._v$5 && t.classList.toggle("mobile-inner", d._v$5 = w), A !== d._v$6 && r.classList.toggle("mobile-title", d._v$6 = A), O !== d._v$7 && s.classList.toggle("mobile-content", d._v$7 = O), d;
  }, {
    _v$: void 0,
    _v$2: void 0,
    _v$3: void 0,
...
je(["click"]);
const o4 = /* @__PURE__ */ $('<div tabindex="0"><div class="selector-container"><span class="value"></span><i class="arrow"></i></div></div>'), i4 = /* @__PURE__ */ $('<div class="drop-down-container"><ul></ul></div>'), a4 = /* @__PURE__ */ $('<div><input type="text"></div>'), s4 = /* @__PURE__ */ $("<li></li>"), f9 = (e) => {
  const [n, t] = T(!1), [r, a] = T("");
  let s, d;
  const u = Z(() => {
    if (!e.dataSource || !e.searchable)
      return e.dataSource;
    const m = r().toLowerCase().trim();
    return m ? typeof e.dataSource[0] == "string" ? e.dataSource.filter((A) => A.toLowerCase().includes(m)) : e.dataSource.filter((A) => {
      var B, D;
      const O = ((B = A.text) == null ? void 0 : B.toString().toLowerCase()) || "", F = ((D = A.key) == null ? void 0 : D.toLowerCase()) || "";
      return O.includes(m) || F.includes(m);
    }) : e.dataSource;
  }), p = () => {
    const m = !n();
    t(m), a(""), m && e.searchable && setTimeout(() => s == null ? void 0 : s.focus(), 50);
  }, k = (m) => {
    const w = m.relatedTarget;
    d && w && d.contains(w) || (t(!1), a(""));
  };
  return (() => {
    const m = o4.cloneNode(!0), w = m.firstChild, A = w.firstChild;
    m.addEventListener("blur", k), m.$$click = (F) => {
      F.stopPropagation(), p();
    };
    const O = d;
    return typeof O == "function" ? D1(O, m) : d = m, v(A, () => e.value), v(m, (() => {
      const F = Z(() => !!(e.dataSource && e.dataSource.length > 0));
      return () => F() && (() => {
        const B = i4.cloneNode(!0), D = B.firstChild;
        return B.$$mousedown = (K) => K.preventDefault(), v(B, (() => {
          const K = Z(() => !!e.searchable);
          return () => K() && (() => {
            const X = a4.cloneNode(!0), U = X.firstChild;
          ...
          return (K = u()) == null ? void 0 : K.map((X) => {
            ...
            return (() => {
              const V = s4.cloneNode(!0);
              return V.$$click = (ue) => {
                var R;
                ue.stopPropagation(), e.value !== Y && ((R = e.onSelected) == null || R.call(e, X)), t(!1), a("");
      ...
      return F._v$ = I1(m, B, F._v$), D !== F._v$2 && oe(m, F._v$2 = D), F;
    }, {
      _v$: void 0,
      _v$2: void 0
    }), m;
...
const l4 = /* @__PURE__ */ $('<span class="prefix"></span>'), c4 = /* @__PURE__ */ $('<span class="suffix"></span>'), u4 = /* @__PURE__ */ $('<div><input class="value"></div>'), g9 = (e) => {
  const n = Ur({
  ...
  const [r, a] = T("normal");
  return (() => {
    const s = u4.cloneNode(!0), d = s.firstChild;
    return s.$$click = () => {
      t == null || t.focus();
    }, v(s, L(J, {
      get when() {
      ...
      get children() {
        const u = c4.cloneNode(!0);
        return v(u, () => n.suffix), u;
      }
    }), null), I((u) => {
      const p = n.style, k = `klinecharts-pro-input ${n.class ?? ""}`, m = r(), w = n.placeholder ?? "";
      return u._v$ = I1(s, p, u._v$), k !== u._v$2 && oe(s, u._v$2 = k), m !== u._v$3 && Pe(s, "data-status", u._v$3 = m), w !== u._v$4 && Pe(d, "placeholder", u._v$4 = w), u;
    }, {
      _v$: void 0,
      _v$2: void 0,
      _v$3: void 0,
...
je(["click"]);
const d4 = /* @__PURE__ */ $('<div><i class="thumb"></i></div>'), h4 = (e) => (() => {
  const n = d4.cloneNode(!0);
  return n.$$click = (t) => {
...
je(["click"]);
const f4 = "指标", g4 = "更多", m4 = "主图指标", y4 = "副图指标", p4 = "设置", v4 = "时区", C4 = "截屏", $4 = "全屏", b4 = "退出全屏", _4 = "保存", k4 = "确定", x4 = "取消", L4 = "MA(移动平均线)", w4 = "EMA(指数平滑移动平均线)", A4 = "SMA", M4 = "BOLL(布林线)", T4 = "BBI(多空指数)", S4 = "SAR(停损点指向指标)", P4 = "VOL(成交量)", O4 = "MACD(指数平滑异同移动平均线)", D4 = "KDJ(随机指标)", N4 = "RSI(相对强弱指标)", I4 = "BIAS(乖离率)", E4 = "BRAR(情绪指标)", B4 = "CCI(顺势指标)", F4 = "DMI(动向指标)", U4 = "CR(能量指标)", z4 = "PSY(心理线)", K4 = "DMA(平行线差指标)", R4 = "TRIX(三重指数平滑平均线)", j4 = "OBV(能量潮指标)", Q4 = "VR(成交量变异率)", Z4 = "WR(威廉指标)", V4 = "MTM(动量指标)", H4 = "EMV(简易波动指标)", q4 = "ROC(变动率指标)", Y4 = "PVT(价量趋势指标)", W4 = "AO(动量震荡指标)", G4 = "世界统一时间", X4 = "(UTC-10) 檀香山", J4 = "(UTC-8) 朱诺", el = "(UTC-7) 洛杉矶", tl = "(UTC-5) 芝加哥", nl = "(UTC-4) 多伦多", rl = "(UTC-3) 圣保罗", ol = "(UTC+1) 伦敦", il = "(UTC+2) 柏林", al = "(UTC+3) 巴林", sl = "(UTC+4) 迪拜", ll = "(UTC+5) 阿什哈巴德", cl = "(UTC+6) 阿拉木图", ul = "(UTC+7) 曼谷", dl = "(UTC+8) 上海", hl = "(UTC+9) 东京", fl = "(UTC+10) 悉尼", gl = "(UTC+12) 诺福克岛", ml = "水平直线", yl = "水平射线", pl = "水平线段", vl = "垂直直线", Cl = "垂直射线", $l = "垂直线段", bl = "直线", _l = "射线", kl = "线段", xl = "箭头", Ll = "价格线", wl = "价格通道线", Al = "平行直线", Ml = "斐波那契回调直线", Tl = "斐波那契回调线段", Sl = "斐波那契圆环", Pl = "斐波那契螺旋", Ol = "斐波那契速度阻力扇", Dl = "斐波那契趋势扩展", Nl = "江恩箱", Il = "矩形", El = "平行四边形", Bl = "圆", Fl = "三角形", Ul = "三浪", zl = "五浪", Kl = "八浪", Rl = "任意浪", jl = "ABCD形态", Ql = "XABCD形态", Zl = "弱磁模式", Vl = "强磁模式", Hl = "商品搜索", ql = "商品代码", Yl = "参数1", Wl = "参数2", Gl = "参数3", Xl = "参数4", Jl = "参数5", ec = "周期", tc = "标准差", nc = "蜡烛图类型", rc = "全实心", oc = "全空心", ic = "涨空心", ac = "跌空心", sc = "OHLC", lc = "面积图", cc = "最新价显示", uc = "最高价显示", dc = "最低价显示", hc = "指标最新值显示", fc = "价格轴类型", gc = "线性轴", mc = "百分比轴", yc = "对数轴", pc = "倒置坐标", vc = "网格线显示", Cc = "恢复默认", $c = {
  indicator: f4,
  more: g4,
  main_indicator: m4,
  sub_indicator: y4,
  ...
  sydney: fl,
  norfolk: gl,
  horizontal_straight_line: ml,
  horizontal_ray_line: yl,
  horizontal_segment: pl,
  vertical_straight_line: vl,
  ...
  arrow: xl,
  price_line: Ll,
  price_channel_line: wl,
  parallel_straight_line: Al,
  fibonacci_line: Ml,
  fibonacci_segment: Tl,
  fibonacci_circle: Sl,
  fibonacci_spiral: Pl,
  fibonacci_speed_resistance_fan: Ol,
  fibonacci_extension: Dl,
  gann_box: Nl,
  rect: Il,
  parallelogram: El,
  circle: Bl,
...
const c = (e, n) => {
  var t;
  return ((t = m9[n]) == null ? void 0 : t[e]) ?? e;
  ...
  let n, t, r;
  const [a, s] = T(window.innerWidth < 768), [d, u] = T(localStorage.getItem("klinechart_secondary_period") || ""), [p, k] = T(!1), [m, w] = T(!1), [A, O] = T(!1), [F, B] = T(!1), [D, K] = T(!1), [X, U] = T({
    top: 0,
    left: 0,
    minWidth: 220
  }), Y = () => {
    s(window.innerWidth < 768), requestAnimationFrame(re), p() && Oe();
  }, [V, ue] = T(!1), R = () => document.fullscreenElement ?? document.body, j = () => {
    ue(!!document.fullscreenElement);
  }, [q, ne] = T(!1), [ie, $e] = T(!1), Oe = () => {
    if (!t)
      return;
    const N = t.getBoundingClientRect(), E = Math.max(220, Math.ceil(N.width)), ve = window.innerWidth, Ne = Math.min(Math.max(8, N.right - E), Math.max(8, ve - E - 8));
    U({
    ...
    if (!p())
      return;
    const E = N.target;
    E && (t != null && t.contains(E) || r != null && r.contains(E) || (be(), k(!1)));
  }, ye = () => {
    p() && Oe();
  }, re = () => {
    if (!n) {
  ...
  S0(() => {
    window.addEventListener("resize", Y), document.addEventListener("fullscreenchange", j), document.addEventListener("mousedown", _e), window.addEventListener("scroll", ye, !0), document.addEventListener("mozfullscreenchange", j), document.addEventListener("webkitfullscreenchange", j), document.addEventListener("msfullscreenchange", j), n && (n.addEventListener("scroll", re), setTimeout(re, 100));
  }), N1(() => {
    window.removeEventListener("resize", Y), document.removeEventListener("fullscreenchange", j), document.removeEventListener("mousedown", _e), window.removeEventListener("scroll", ye, !0), document.removeEventListener("mozfullscreenchange", j), document.removeEventListener("webkitfullscreenchange", j), document.removeEventListener("msfullscreenchange", j), n && n.removeEventListener("scroll", re);
  });
  const pe = Z(() => {
  ...
  return ze(() => {
    ...
    const N = Qd.cloneNode(!0), E = N.firstChild, ve = E.firstChild, Ne = ve.firstChild, se = ve.nextSibling, Le = se.firstChild;
        ...
        return D1((de) => {
          ...
          get children() {
            return L(y5, {
              ...
              get children() {
                const de = Bd.cloneNode(!0), g1 = de.firstChild, l1 = g1.firstChild, et = l1.firstChild, Ie = et.firstChild, yt = Ie.firstChild, B1 = l1.nextSibling, $1 = B1.firstChild, b1 = $1.firstChild, t1 = b1.firstChild, pt = $1.nextSibling, Ze = pt.firstChild, tt = Ze.firstChild, nt = g1.nextSibling, m1 = nt.firstChild, _1 = m1.firstChild, rt = _1.firstChild, vt = rt.firstChild, F1 = m1.nextSibling, ot = F1.firstChild, Ct = ot.firstChild, Ue = Ct.nextSibling, Ke = ot.nextSibling, He = Ke.firstChild, Re = He.nextSibling, it = Re.firstChild, U1 = it.firstChild, z1 = nt.nextSibling, $t = z1.firstChild, xe = $t.firstChild, Ee = z1.nextSibling, bt = Ee.nextSibling, qe = bt.firstChild, y1 = qe.firstChild, p1 = bt.nextSibling, _t = p1.nextSibling, k1 = _t.firstChild, kt = k1.firstChild, Ye = _t.nextSibling, x1 = Ye.firstChild, i0 = x1.firstChild, L1 = i0.firstChild, w1 = L1.firstChild, We = x1.nextSibling, xt = We.firstChild, Lt = xt.firstChild, c1 = Lt.firstChild, K1 = xt.nextSibling, a0 = K1.firstChild, wt = a0.firstChild, s0 = K1.nextSibling, at = s0.firstChild, st = at.firstChild, A1 = Ye.nextSibling, l0 = A1.firstChild, u1 = l0.firstChild;
...
const Ve = (e) => Cf[e.name](e.class), wf = /* @__PURE__ */ $('<div class="klinecharts-pro-drawing-bar"><span class="split-line"></span><div class="item" tabindex="0"><span style="width:32px;height:32px"></span><div class="icon-arrow"><svg viewBox="0 0 4 6"><path d="M1.07298,0.159458C0.827521,-0.0531526,0.429553,-0.0531526,0.184094,0.159458C-0.0613648,0.372068,-0.0613648,0.716778,0.184094,0.929388L2.61275,3.03303L0.260362,5.07061C0.0149035,5.28322,0.0149035,5.62793,0.260362,5.84054C0.505822,6.05315,0.903789,6.05315,1.14925,5.84054L3.81591,3.53075C4.01812,3.3556,4.05374,3.0908,3.92279,2.88406C3.93219,2.73496,3.87113,2.58315,3.73964,2.46925L1.07298,0.159458Z" stroke="none" stroke-opacity="0"></path></svg></div></div><div class="item"><span style="width:32px;height:32px"></span></div><div class="item"><span style="width:32px;height:32px"></span></div><span class="split-line"></span><div class="item"><span style="width:32px;height:32px"></span></div></div>'), Af = /* @__PURE__ */ $('<div class="item" tabindex="0"><span style="width:32px;height:32px"></span><div class="icon-arrow"><svg viewBox="0 0 4 6"><path d="M1.07298,0.159458C0.827521,-0.0531526,0.429553,-0.0531526,0.184094,0.159458C-0.0613648,0.372068,-0.0613648,0.716778,0.184094,0.929388L2.61275,3.03303L0.260362,5.07061C0.0149035,5.28322,0.0149035,5.62793,0.260362,5.84054C0.505822,6.05315,0.903789,6.05315,1.14925,5.84054L3.81591,3.53075C4.01812,3.3556,4.05374,3.0908,3.92279,2.88406C3.93219,2.73496,3.87113,2.58315,3.73964,2.46925L1.07298,0.159458Z" stroke="none" stroke-opacity="0"></path></svg></div></div>'), _r = /* @__PURE__ */ $('<li><span style="padding-left:8px"></span></li>'), kr = "drawing_tools", Mf = (e) => {
  const [n, t] = T("horizontalStraightLine"), [r, a] = T("priceChannelLine"), [s, d] = T("circle"), [u, p] = T("fibonacciLine"), [k, m] = T("xabcd"), [w, A] = T("weak_magnet"), [O, F] = T("normal"), [B, D] = T(!1), [K, X] = T(!0), [U, Y] = T(""), V = Z(() => [{
    key: "singleLine",
    icon: n(),
    list: $f(e.locale),
    setter: t
  ...
  return (() => {
    const R = wf.cloneNode(!0), j = R.firstChild, q = j.nextSibling, ne = q.firstChild, ie = ne.nextSibling, $e = ie.firstChild, Oe = q.nextSibling, be = Oe.firstChild, Qe = Oe.nextSibling, _e = Qe.firstChild, ye = Qe.nextSibling, re = ye.nextSibling, pe = re.firstChild;
    return v(R, () => V().map((H) => (() => {
      const N = Af.cloneNode(!0), E = N.firstChild, ve = E.nextSibling, Ne = ve.firstChild;
  ...
  return L(C1, {
    ...
    get children() {
      ...
      return v(k, L($0, {
        ...
        children: (m) => {
          let w;
          const A = ce.formatValue(n(), m.key);
          switch (m.component) {
            case "select": {
              const O = m.key === "candle.type" ? "170px" : "120px";
              w = L(f9, {
                get style() {
                  return {
                    width: s() ? "100%" : O,
          ...
          return (() => {
            ...
            return v(F, () => m.text), v(B, w), I(() => O.classList.toggle("mobile-item", !!s())), O;
          })();
        }
  ...
  return L(C1, {
    ...
    get children() {
      return [L(g9, {
        ...
        renderItem: (a) => (() => {
          const s = jf.cloneNode(!0), d = s.firstChild, u = d.firstChild;
          return s.$$click = () => {
...
const Zf = /* @__PURE__ */ $('<div class="klinecharts-pro-mobile-more-modal-content"><div class="item"><svg viewBox="0 0 22 22" xmlns="http://www.w3.org/2000/svg"><rect x="1" y="1" width="20" height="20" rx="4" ry="4" fill="none" stroke="currentColor" stroke-width="2"></rect><path d="m15.92 13.9-3.65-3.55a.773.773 0 0 1 0-1.12c.32-.33.79-.33 1.11 0l3.02 3.38c.31.32.31.81 0 1.13-.16.16-.48.16-.48.16Z" fill="currentColor" stroke="1"></path><path d="m12.43 14.23-.48-.17q-.48-.48 0-.96l4.13-4.36c.31-.32.63-.32.95 0s.32.65 0 .97l-4.13 4.35c-.15.17-.31.17-.47.17Zm-1.11-9.52c.31 0 1.11.16 1.11.97 0 .64-.95.48-1.27.48-1.43 0-1.91.81-2.07 1.45l-.31 1.94h1.58c.32 0 .64.32.64.64 0 .33-.32.65-.64.65H8.62l-1.11 4.99c-.16 1.46-1.11 1.46-1.59 0L7.03 11H5.6c-.31 0-.63-.32-.63-.65 0-.64.32-.8.63-.8h1.75l.48-2.1c.31-2.74 2.69-2.9 3.49-2.74Z" fill="noncurrentColore" stroke="1"></path></svg><span></span></div><div class="item"><svg width="20" height="20" viewBox="0 0 20 20"><path d="M18.5446,9.09091C18.3333,6.61616,17.2887,4.31818,15.5751,2.63889C13.8498,0.94697,11.6197,0,9.28404,0C8.02817,0,6.81925,0.265151,5.66901,0.782828C5.65728,0.782828,5.65728,0.795454,5.64554,0.795454C5.6338,0.795454,5.6338,0.808081,5.62207,0.808081C4.53052,1.31313,3.55634,2.0202,2.71127,2.92929C1.85446,3.85101,1.18545,4.91162,0.715963,6.11111C0.246479,7.33586,0,8.64899,0,10C0,10.8712,0.105634,11.7172,0.305164,12.5379C0.305164,12.5631,0.316901,12.5884,0.328638,12.6136C0.739437,14.2298,1.51408,15.7197,2.62911,16.9571C4.07277,18.548,5.92723,19.5581,7.93427,19.8737C7.95775,19.8737,7.96948,19.8864,7.99296,19.8864C8.3216,19.9369,8.66197,19.9747,9.00235,19.9747L9.21362,19.9747C9.61268,19.9747,10.3756,19.9369,11.0094,19.697C11.1737,19.6338,11.3028,19.5076,11.3732,19.3434C11.4437,19.1793,11.4554,18.9899,11.3967,18.8131C11.3028,18.5354,11.0563,18.346,10.7864,18.346C10.716,18.346,10.6338,18.3586,10.5634,18.3838C10.0939,18.5606,9.46009,18.5859,9.20188,18.5859L9.09624,18.5859C9.20188,18.2702,9.23709,17.9167,9.15493,17.5505C9.00235,16.8939,8.50939,16.3384,7.58216,15.7955L7.19484,15.5682C6.57277,15.2146,6.23239,15.0253,6.03286,14.7348C5.83333,14.4444,5.69249,13.9899,5.51643,12.9798C5.38732,12.298,5.04695,11.7677,4.50704,11.4646C4.14319,11.2626,3.70892,11.149,3.19249,11.149C2.82864,11.149,2.42958,11.1995,2.00704,11.3005C1.79578,11.351,1.59624,11.4141,1.42019,11.4646C1.33803,10.9848,1.30282,10.4798,1.30282,9.97475C1.30282,6.93182,2.76995,4.26768,4.98826,2.72727C5,3.00505,5.05869,3.29545,5.17606,3.57323C5.48122,4.26768,6.10329,4.7096,7.01878,4.89899C7.06573,4.91162,7.10094,4.91162,7.13615,4.91162L7.1831,4.91162C7.26526,4.91162,7.57042,4.92424,7.88732,5.0505C8.3216,5.2399,8.56808,5.55555,8.65023,6.04798C8.84977,7.61364,9.07277,10.4293,8.79108,11.3384C8.76761,11.4141,8.75587,11.4899,8.75587,11.5657C8.75587,11.9444,9.0493,12.2601,9.40141,12.2601C9.57747,12.2601,9.74179,12.1843,9.85915,12.0581C9.97653,11.9318,12.6174,9.05303,13.3216,8.09343C13.4038,7.97979,13.4859,7.87878,13.5798,7.76515C13.9202,7.33586,14.2723,6.90656,14.4014,6.26262C14.554,5.56818,14.4014,4.79798,13.9437,3.85101C13.615,3.16919,13.5563,2.86616,13.5446,2.75252C13.5563,2.7399,13.5798,2.72727,13.6033,2.71464C15.6221,4.10353,17.0188,6.43939,17.2535,9.19192C17.2887,9.55808,17.5587,9.82323,17.8991,9.82323L17.9577,9.82323C18.3099,9.8106,18.5681,9.48232,18.5446,9.09091ZM3.19249,12.5631C3.48592,12.5631,3.72066,12.6136,3.89671,12.7146C4.08451,12.8283,4.19014,12.9924,4.23709,13.2702C4.43662,14.3434,4.61268,15.0631,5,15.6061C5.37559,16.1364,5.85681,16.4015,6.58451,16.8182L6.60798,16.8308C6.71362,16.8939,6.84272,16.9571,6.96009,17.0328C7.69953,17.4621,7.86385,17.7525,7.89906,17.8914C7.93427,18.0303,7.85211,18.2323,7.74648,18.4343C4.91784,17.8535,2.65258,15.6944,1.73709,12.8283C2.15962,12.702,2.71127,12.5631,3.19249,12.5631ZM12.7934,4.5202C13.4272,5.83333,13.1455,6.18687,12.5822,6.89394C12.4883,7.00758,12.3944,7.12121,12.3005,7.24747C11.9484,7.72727,11.0211,8.77525,10.2113,9.68434C10.2113,9.24242,10.1878,8.73737,10.1526,8.19444C10.0704,6.95707,9.92958,5.90909,9.92958,5.87121L9.92958,5.83333C9.75352,4.83586,9.20188,4.11616,8.3216,3.76263C7.82864,3.56061,7.37089,3.53535,7.19484,3.53535C6.73709,3.43434,6.4554,3.24495,6.33803,2.99242C6.19718,2.68939,6.29108,2.24747,6.38498,1.9697C7.28873,1.59091,8.26291,1.37626,9.28404,1.37626C10.3873,1.37626,11.4437,1.61616,12.4061,2.04545C12.3357,2.18434,12.277,2.34848,12.2535,2.5505C12.2066,3.04293,12.3709,3.64899,12.7934,4.5202Z"></path></svg><span></span></div><div class="item"><svg viewBox="0 0 20 20"><path d="M19.7361,12.542L18.1916,11.2919C18.2647,10.8678,18.3025,10.4347,18.3025,10.0017C18.3025,9.56861,18.2647,9.13555,18.1916,8.71142L19.7361,7.46135C19.9743,7.26938,20.0615,6.95686,19.9554,6.6756L19.9342,6.61756C19.5074,5.49026,18.8755,4.45449,18.0549,3.53926L18.0124,3.49238C17.8096,3.26692,17.4819,3.1821,17.1848,3.28032L15.2677,3.92544C14.5603,3.3763,13.7704,2.94324,12.9168,2.63966L12.5466,0.742229C12.49,0.449802,12.2472,0.222111,11.9383,0.168536L11.8746,0.157375C10.6461,-0.0524583,9.35391,-0.0524583,8.1254,0.157375L8.06174,0.168536C7.75284,0.222111,7.50997,0.449802,7.45338,0.742229L7.08082,2.64859C6.2343,2.95217,5.44909,3.383,4.74641,3.92991L2.81522,3.28032C2.52047,3.1821,2.19036,3.26469,1.98757,3.49238L1.94513,3.53926C1.12455,4.45672,0.492609,5.49249,0.0658141,6.61756L0.0445921,6.6756C-0.0615171,6.95463,0.0257283,7.26715,0.263885,7.46135L1.82723,8.72482C1.75413,9.14448,1.71876,9.57308,1.71876,9.99944C1.71876,10.428,1.75413,10.8566,1.82723,11.2741L0.263885,12.5375C0.025729,12.7295,-0.0615164,13.042,0.0445929,13.3233L0.0658148,13.3813C0.49261,14.5064,1.12455,15.5444,1.94513,16.4596L1.98757,16.5065C2.19036,16.732,2.51812,16.8168,2.81522,16.7186L4.74641,16.069C5.44909,16.6159,6.2343,17.0489,7.08082,17.3503L7.45338,19.2567C7.50997,19.5491,7.75284,19.7768,8.06174,19.8303L8.1254,19.8415C8.74084,19.9464,9.37042,20,10,20C10.6296,20,11.2615,19.9464,11.8746,19.8415L11.9383,19.8303C12.2472,19.7768,12.49,19.5491,12.5466,19.2567L12.9168,17.3592C13.7704,17.0556,14.5603,16.6248,15.2677,16.0734L17.1848,16.7186C17.4795,16.8168,17.8096,16.7342,18.0124,16.5065L18.0549,16.4596C18.8755,15.5422,19.5074,14.5064,19.9342,13.3813L19.9554,13.3233C20.0615,13.0487,19.9743,12.7362,19.7361,12.542ZM16.5175,8.97483C16.5764,9.3119,16.6071,9.65791,16.6071,10.0039C16.6071,10.3499,16.5764,10.6959,16.5175,11.033L16.3618,11.9281L18.1233,13.3545C17.8568,13.9372,17.5196,14.4863,17.1188,14.9975L14.9305,14.2631L14.1901,14.839C13.6266,15.2765,12.9994,15.6203,12.3203,15.8614L11.4219,16.1806L10.9998,18.3459C10.3372,18.4173,9.66045,18.4173,8.9955,18.3459L8.57342,16.1761L7.6821,15.8524C7.01008,15.6114,6.38521,15.2676,5.82637,14.8323L5.08596,14.2541L2.88361,14.9953C2.48275,14.4841,2.14791,13.9327,1.8791,13.3523L3.65938,11.9125L3.50611,11.0196C3.44952,10.687,3.41887,10.3432,3.41887,10.0039C3.41887,9.66237,3.44716,9.32083,3.50611,8.98822L3.65938,8.09531L1.8791,6.6555C2.14556,6.07288,2.48275,5.52374,2.88361,5.01255L5.08596,5.75367L5.82637,5.17551C6.38521,4.74022,7.01008,4.39645,7.6821,4.15536L8.57578,3.83615L8.99786,1.66638C9.66045,1.59495,10.3372,1.59495,11.0021,1.66638L11.4242,3.83168L12.3226,4.1509C12.9994,4.39198,13.6289,4.73575,14.1925,5.17328L14.9329,5.7492L17.1211,5.01479C17.522,5.52598,17.8568,6.07734,18.1256,6.65773L16.3642,8.08416L16.5175,8.97483ZM10.0024,5.85189C7.7104,5.85189,5.85231,7.61092,5.85231,9.78068C5.85231,11.9504,7.7104,13.7095,10.0024,13.7095C12.2943,13.7095,14.1524,11.9504,14.1524,9.78068C14.1524,7.61092,12.2943,5.85189,10.0024,5.85189ZM11.8699,11.5486C11.37,12.0196,10.7074,12.2808,10.0024,12.2808C9.29732,12.2808,8.63473,12.0196,8.13483,11.5486C7.6373,11.0754,7.36142,10.4481,7.36142,9.78068C7.36142,9.11323,7.6373,8.48596,8.13483,8.01272C8.63473,7.53948,9.29732,7.28054,10.0024,7.28054C10.7074,7.28054,11.37,7.53948,11.8699,8.01272C12.3674,8.48596,12.6433,9.11323,12.6433,9.78068C12.6433,10.4481,12.3674,11.0754,11.8699,11.5486Z"></path></svg><span></span></div></div>'), Vf = (e) => L(C1, {
  ...
  get children() {
    const n = Zf.cloneNode(!0), t = n.firstChild, r = t.firstChild, a = r.nextSibling, s = t.nextSibling, d = s.firstChild, u = d.nextSibling, p = s.nextSibling, k = p.firstChild, m = k.nextSibling;
    return t.$$click = () => {
...
const Hf = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-picker"></div>'), qf = /* @__PURE__ */ $('<label class="klinecharts-pro-time-tools-field"><span></span><button type="button" class="klinecharts-pro-time-tools-input"><span></span><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="5" width="16" height="15" rx="2"></rect><path d="M8 3v4M16 3v4M4 10h16"></path></svg></button></label>'), Yf = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-calendar"><div class="klinecharts-pro-time-tools-month"><button type="button">&lt;&lt;</button><button type="button">&lt;</button><button type="button" class="calendar-title"></button><button type="button">></button><button type="button">>></button></div></div>'), Wf = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-grid"></div>'), Gf = /* @__PURE__ */ $('<span class="weekday"></span>'), P1 = /* @__PURE__ */ $('<button type="button"></button>'), Xf = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-month-grid"></div>'), Jf = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-month-grid year-grid"></div>'), eg = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-spinners"><div class="spinner-column scrollable"></div><div class="spinner-column scrollable"></div><div class="spinner-column"></div></div>'), tg = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-content"></div>'), ng = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-tabs"></div>'), rg = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-range-panel"><div class="klinecharts-pro-time-tools-range-header"><button type="button"></button><span class="klinecharts-pro-time-tools-range-arrow">-></span><button type="button"></button><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="5" width="16" height="15" rx="2"></rect><path d="M8 3v4M16 3v4M4 10h16"></path></svg></div></div>'), og = /* @__PURE__ */ $('<div class="klinecharts-pro-time-tools-panel"><div class="klinecharts-pro-time-tools-row"><div><strong>Time Anchor</strong><span>Anchor to a time on the chart when switching between intervals</span></div><button type="button"><span></span></button></div><div class="klinecharts-pro-time-tools-row"><div><strong>Anchor Point</strong></div><select><option value="date">Date</option><option value="current">Current</option></select></div><div class="klinecharts-pro-time-tools-row with-divider"><div><strong>Anchor line</strong><span>Mark the anchored time on the chart with a vertical line</span></div><button type="button"><span></span></button></div><div class="klinecharts-pro-time-tools-row with-divider"><div><strong>Across Tokens</strong><span>Retain onscreen chart range when switching symbols</span></div><button type="button"><span></span></button></div></div>'), ig = [{
  ...
  key: "timeAnchor",
  label: "Time Anchor"
}], ag = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"], Mr = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], Q1 = (e) => String(e).padStart(2, "0"), Tr = (e, n, t) => Math.min(t, new Date(e, n + 1, 0).getDate()), y0 = (e) => {
  const n = new Date(e);
  return {
    year: n.getFullYear(),
    month: n.getMonth(),
...
}, Bt = (e) => new Date(e.year, e.month, e.day, e.hour, e.minute, 0, 0).getTime(), p0 = (e) => e.year * 1e4 + (e.month + 1) * 100 + e.day, M0 = (e) => {
  ...
  const [n, t] = T(!0), [r, a] = T("date"), [s, d] = T(e.value.year), [u, p] = T(e.value.month), k = Z(() => sg(s(), u())), m = Z(() => Math.floor(s() / 10) * 10), w = Z(() => Array.from({
    length: 12
  }, (R, j) => m() - 1 + j)), A = Z(() => e.value.hour % 12 || 12), O = Z(() => e.value.hour >= 12 ? "PM" : "AM"), F = Array.from({
    length: 12
  }, (R, j) => j + 1), B = Array.from({
    length: 60
  }, (R, j) => j), D = (R) => {
    const j = new Date(s(), u() + R, 1);
    d(j.getFullYear()), p(j.getMonth());
  }, K = () => {
    ...
    e.onChange({
      ...
      month: R.getMonth(),
      day: R.getDate()
    }), (j = e.onDateSelect) == null || j.call(e), d(R.getFullYear()), p(R.getMonth());
  }, U = (R) => {
    p(R), e.onChange({
      ...e.value,
      year: s(),
      month: R,
      day: Tr(s(), R, e.value.day)
    }), a("date");
  }, Y = (R) => {
    d(R), e.onChange({
      ...e.value,
      year: R,
      day: Tr(R, e.value.month, e.value.day)
    }), a("month");
  }, V = (R) => {
    const j = O() === "PM";
    e.onChange({
      ...e.value,
      hour: j ? R === 12 ? 12 : R + 12 : R === 12 ? 0 : R
    });
  }, ue = (R) => {
    const j = A();
    e.onChange({
      ...e.value,
      hour: R === "PM" ? j === 12 ? 12 : j + 12 : j === 12 ? 0 : j
    });
  };
  return (() => {
    ...
    return v(R, (() => {
      ...
      return () => j() && (() => {
        ...
        return v(ne, () => e.label), ie.$$click = () => t(!n()), v($e, () => M0(e.value)), q;
      })();
    })(), null), v(R, (() => {
      const j = Z(() => !!n());
      return () => j() && (() => {
        const q = Yf.cloneNode(!0), ne = q.firstChild, ie = ne.firstChild, $e = ie.nextSibling, Oe = $e.nextSibling, be = Oe.nextSibling, Qe = be.nextSibling;
        return ie.$$click = () => {
          ...
          const _e = Z(() => r() === "date");
          return () => _e() && (() => {
            const ye = Wf.cloneNode(!0);
            return v(ye, () => ag.map((re) => (() => {
              const pe = Gf.cloneNode(!0);
              return v(pe, re), pe;
            })()), null), v(ye, () => k().map(({
              date: re,
              current: pe
              ...
              const H = p0({
                ...
                month: re.getMonth(),
                day: re.getDate()
              }), N = e.range ? p0(e.range.from) : NaN, E = e.range ? p0(e.range.to) : NaN, ve = Math.min(N, E), Ne = Math.max(N, E), se = Number.isFinite(ve) && H >= ve && H <= Ne, Le = Number.isFinite(ve) && (H === ve || H === Ne), _ = re.getFullYear() === e.value.year && re.getMonth() === e.value.month && re.getDate() === e.value.day;
              return (() => {
          ...
          const _e = Z(() => r() === "month");
          return () => _e() && (() => {
            const ye = Xf.cloneNode(!0);
            return v(ye, () => Mr.map((re, pe) => (() => {
          ...
          const _e = Z(() => r() === "year");
          return () => _e() && (() => {
            const ye = Jf.cloneNode(!0);
            return v(ye, () => w().map((re) => (() => {
              const pe = P1.cloneNode(!0);
              return pe.$$click = () => Y(re), v(pe, re), I(() => oe(pe, `${re < m() || re > m() + 9 ? "muted" : ""} ${re === e.value.year ? "selected" : ""}`)), pe;
            })())), ye;
          })();
        })(), null), v(q, (() => {
          const _e = Z(() => r() === "date");
          return () => _e() && (() => {
            const ye = eg.cloneNode(!0), re = ye.firstChild, pe = re.nextSibling, H = pe.nextSibling;
            return v(re, () => F.map((N) => (() => {
  ...
  const [n, t] = T(e.initialTab ?? "goToDate"), [r, a] = T(y0(e.initialTimestamp)), [s, d] = T(y0(e.initialRange.from)), [u, p] = T(y0(e.initialRange.to)), [k, m] = T("from"), [w, A] = T({
    ...e.anchorSettings
  }), O = (B) => {
    A((D) => ({
      ...D,
  ...
  return L(C1, {
    ...
    get title() {
      return (() => {
        const B = ng.cloneNode(!0);
        return v(B, () => ig.map((D) => (() => {
          const K = P1.cloneNode(!0);
          return K.$$click = () => t(D.key), v(K, () => D.label), I(() => oe(K, n() === D.key ? "active" : "")), K;
        })())), B;
      })();
    },
    get buttons() {
    ...
    get children() {
      ...
      return v(B, (() => {
        ...
        return () => D() && (() => {
          const K = rg.cloneNode(!0), X = K.firstChild, U = X.firstChild, Y = U.nextSibling, V = Y.nextSibling;
          return U.$$click = () => m("from"), v(U, () => M0(s())), V.$$click = () => m("to"), v(V, () => M0(u())), v(K, (() => {
            const ue = Z(() => k() === "from");
            return () => ue() ? L(Ft, {
              label: "Start",
              get value() {
            ...
            const R = k() === "from" ? "active" : "", j = k() === "to" ? "active" : "";
            return R !== ue._v$ && oe(U, ue._v$ = R), j !== ue._v$2 && oe(V, ue._v$2 = j), ue;
          }, {
            _v$: void 0,
            _v$2: void 0
          }), K;
        ...
        return () => D() && (() => {
          const K = og.cloneNode(!0), X = K.firstChild, U = X.firstChild, Y = U.nextSibling, V = X.nextSibling, ue = V.firstChild, R = ue.nextSibling, j = V.nextSibling, q = j.firstChild, ne = q.nextSibling, ie = j.nextSibling, $e = ie.firstChild, Oe = $e.nextSibling;
          return Y.$$click = () => O({
            enabled: !w().enabled
          }), R.addEventListener("change", (be) => O({
            ...
            const Qe = `klinecharts-pro-time-tools-switch${w().enabled ? " on" : ""}`, _e = `klinecharts-pro-time-tools-switch${w().anchorLine ? " on" : ""}`, ye = `klinecharts-pro-time-tools-switch${w().acrossTokens ? " on" : ""}`;
            return Qe !== be._v$3 && oe(Y, be._v$3 = Qe), _e !== be._v$4 && oe(ne, be._v$4 = _e), ye !== be._v$5 && oe(Oe, be._v$5 = ye), be;
          }, {
            _v$3: void 0,
            _v$4: void 0,
            _v$5: void 0
          }), I(() => R.value = w().anchorPoint), K;
        })();
      })(), null), B;
    }
  });
...
je(["click"]);
const cg = /* @__PURE__ */ $('<i class="icon-close klinecharts-pro-load-icon"></i>'), ug = /* @__PURE__ */ $('<div class="klinecharts-pro-content"><div class="klinecharts-pro-widget"></div></div>'), dg = /* @__PURE__ */ $('<div class="klinecharts-pro-time-navigation-tooltip"></div>'), hg = /* @__PURE__ */ $('<div class="klinecharts-pro-countdown-price-mark"><span class="klinecharts-pro-countdown-price-mark-price"></span><span class="klinecharts-pro-countdown-price-mark-timer"></span></div>'), fg = /* @__PURE__ */ $('<div class="overlay-toolbar-color-popover"><div class="overlay-toolbar-color-grid"></div><div class="overlay-toolbar-color-footer"><button type="button" class="overlay-toolbar-add-color">+</button><div class="overlay-toolbar-color-slider"><span></span></div></div></div>'), gg = /* @__PURE__ */ $('<div class="overlay-toolbar-dropdown width-menu"></div>'), mg = /* @__PURE__ */ $('<div class="overlay-toolbar-dropdown style-menu"><button type="button"><span class="overlay-toolbar-style-sample solid"></span></button><button type="button"><span class="overlay-toolbar-style-sample dashed"></span></button><button type="button"><span class="overlay-toolbar-style-sample dotted"></span></button></div>'), yg = /* @__PURE__ */ $('<div class="klinecharts-pro-overlay-toolbar"><button type="button" class="overlay-toolbar-icon drag" title="Move"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="6" r="1.5"></circle><circle cx="15" cy="6" r="1.5"></circle><circle cx="9" cy="12" r="1.5"></circle><circle cx="15" cy="12" r="1.5"></circle><circle cx="9" cy="18" r="1.5"></circle><circle cx="15" cy="18" r="1.5"></circle></svg></button><button type="button" class="overlay-toolbar-icon refresh" title="Reset"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 12a9 9 0 0 1 15.4-6.36L21 8"></path><path d="M21 3v5h-5"></path><path d="M21 12a9 9 0 0 1-15.4 6.36L3 16"></path><path d="M3 21v-5h5"></path></svg></button><div class="overlay-toolbar-picker"><button type="button" title="Color"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20h9"></path><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"></path></svg></button></div><div class="overlay-toolbar-picker"><button type="button" title="Line width"><span class="overlay-toolbar-line-preview"></span><span>px</span></button></div><div class="overlay-toolbar-picker"><button type="button" title="Line style"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 12h16"></path></svg></button></div><button type="button"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="5" y="5" width="14" height="14" rx="2"></rect><path d="M20 4 4 20"></path></svg></button><button type="button"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="5" y="10" width="14" height="10" rx="2"></rect><path d="M8 10V7a4 4 0 0 1 8 0v3"></path></svg></button><button type="button" class="overlay-toolbar-icon delete" title="Delete"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18"></path><path d="M8 6V4h8v2"></path><path d="M19 6 18 20H6L5 6"></path><path d="M10 11v5"></path><path d="M14 11v5"></path></svg></button></div>'), pg = /* @__PURE__ */ $('<button type="button"></button>'), vg = /* @__PURE__ */ $('<button type="button"><span class="overlay-toolbar-width-sample"></span></button>'), Cg = /* @__PURE__ */ $('<div class="klinecharts-pro-quick-order-marker"><button type="button" class="klinecharts-pro-quick-order-plus"></button></div>'), $g = /* @__PURE__ */ $('<span class="klinecharts-pro-quick-order-plus-icon"></span>'), bg = /* @__PURE__ */ $('<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e3e3e3" aria-hidden="true"><path d="M460-300h40v-160h160v-40H500v-160h-40v160H300v40h160v160ZM224.62-160q-27.62 0-46.12-18.5Q160-197 160-224.62v-510.76q0-27.62 18.5-46.12Q197-800 224.62-800h510.76q27.62 0 46.12 18.5Q800-763 800-735.38v510.76q0 27.62-18.5 46.12Q763-160 735.38-160H224.62Zm0-40h510.76q9.24 0 16.93-7.69 7.69-7.69 7.69-16.93v-510.76q0-9.24-7.69-16.93-7.69-7.69-16.93-7.69H224.62q-9.24 0-16.93 7.69-7.69 7.69-7.69 16.93v510.76q0 9.24 7.69 16.93 7.69 7.69 16.93 7.69ZM200-760v560-560Z"></path></svg>'), _g = /* @__PURE__ */ $('<div class="klinecharts-pro-quick-order-menu-anchor"><div class="klinecharts-pro-quick-order-menu"><button type="button">Trade <!> @ <!> Limit</button><button type="button">Trade <!> @ <!> Stop</button><button type="button">Create new order...</button><button type="button">Copy Price (<!>)</button><button type="button">Draw horizontal line on </button></div></div>');
function Ut(e, n, t, r) {
  n === "VOL" && (r = {
    gap: {
      bottom: 2
...
const xg = (e) => {
  var cn, un, dn, hn, fn, gn, mn, yn, pn, vn, Cn, $n, bn, _n, kn, xn, Ln, wn, An, Mn, Tn, Sn, Pn, On, Dn, Nn, In, En;
  let n, t = null, r;
  const [a, s] = T(!1), [d, u] = T(e.theme), [p, k] = T(e.styles), [m, w] = T(e.locale), [A, O] = T(e.symbol), [F, B] = T(e.period), D = () => {
    var o, i, l, h;
    return {
  ...
  }, [K, X] = T(!1), [U, Y] = T([...e.mainIndicators]), [V, ue] = T({}), [R, j] = T(!1), [q, ne] = T({
    key: e.timezone,
    text: wr(e.timezone, e.locale)
  }), [ie, $e] = T(!1), [Oe, be] = T(), [Qe, _e] = T(""), [ye, re] = T(!1), [pe, H] = T(Date.now()), [N, E] = T({
  ...
  let se = null;
  const [Le, _] = T(e.drawingBarVisible), [ae, Me] = T(!1), [s1, de] = T(!1), [g1, l1] = T(!1), et = ((cn = e.orderTools) == null ? void 0 : cn.quickOrder) ?? !0, [Ie, yt] = T({
    quickOrder: et,
    ...
    positions: ((mn = e.orderTools) == null ? void 0 : mn.positions) ?? !0,
  ...
  const bt = (o, i, l) => {
    ...
    try {
      ...
      const y = _t(g);
      if (y) {
        const f = xe.get(o), C = ((l = f == null ? void 0 : f.points) == null ? void 0 : l.length) || 0, x = ((h = y.points) == null ? void 0 : h.length) || 0;
        xe.set(o, y);
        const M = y1(y.type);
        if (x >= M) {
          const P = Ee.get(o);
    ...
    document.addEventListener("mouseup", h), document.addEventListener("touchend", h), setTimeout(() => {
      ...
      if (g && !g.complete) {
        ...
        const f = xe.get(o);
        if (f) {
          const C = y1(f.type), x = ((y = f.points) == null ? void 0 : y.length) || 0;
      ...
      try {
        const l = `kline_drawings_${o}`, g = {
          drawings: i.map((y) => {
            ...
            const C = y1(y.type), x = ((M = y.points) == null ? void 0 : M.length) || 0;
            return x < C && console.warn(`âš ï¸ Saving ${y.type} with only ${x} point(s), needs ${C}`), f;
          }),
          timestamp: Date.now()
        };
    ...
    loadDrawings: (o) => {
      try {
        ...
        if (l) {
          ...
          return Array.isArray(h.drawings) && h.drawings.forEach((y) => {
            var x;
            const f = y1(y.type);
            (((x = y.points) == null ? void 0 : x.length) || 0) >= f && g.push(y);
          }), g;
        }
      } catch (i) {
  ...
  const x1 = () => {
    ...
    }), xe.clear(), Ee.clear(), Ke(null), Re(null), Ye.loadDrawings(o).forEach((l) => {
      var h;
      try {
        const g = A1({
          name: l.type,
          ...
          visible: l.visible ?? !0,
          lock: l.lock ?? !1,
          mode: l.mode || h0.Normal
        }), y = t == null ? void 0 : t.createOverlay(g), f = typeof y == "string" ? y : null;
        f && (xe.set(f, {
          ...l,
          id: f
        }), Ee.set(f, {
    ...
    const g = (e1 = t.convertToPixel) == null ? void 0 : e1.call(t, [{
      ...
      paneId: "candle_pane",
      absolute: !0
    }), y = Number((ut = g == null ? void 0 : g[0]) == null ? void 0 : ut.y), f = (Bn = t.getSize) == null ? void 0 : Bn.call(t, "candle_pane"), C = (f == null ? void 0 : f.height) ?? n.clientHeight;
    if (!Number.isFinite(y) || y < 0 || y > C) {
      F1(null);
      return;
    }
    ...
    }), P = (Un = t.getSize) == null ? void 0 : Un.call(t, "candle_pane", R1.YAxis), W = P != null && P.width && Number.isFinite(P.width) ? Math.max(74, Math.floor(P.width) - 2) : 96, te = zt(F()), ee = o % te, Q = ee === 0 ? te : te - ee, G = Number(l.close), he = Number(l.open), Be = t.getStyles().candle.priceMark.last, z = Be.text, Ce = Number(z.size) || 12, le = Number(z.paddingTop) || 2, ke = Number(z.paddingBottom) || 2, we = Math.min(Number(z.paddingLeft) || 4, 3), Ge = Math.min(Number(z.paddingRight) || 4, 3), Xe = Math.max(34, Ce * 2 + le + ke + 6), r1 = Math.max(0, Math.min(y - Xe / 2, C - Xe));
    F1({
      top: r1,
    ...
    try {
      const g = t == null ? void 0 : t.convertFromPixel([{
        ...
        paneId: "candle_pane"
      }), y = Number((l = g == null ? void 0 : g[0]) == null ? void 0 : l.value);
    ...
    try {
      const g = t == null ? void 0 : t.convertFromPixel([{
        ...
        paneId: "candle_pane",
        absolute: !0
      }), y = Number((h = g == null ? void 0 : g[0]) == null ? void 0 : h.value);
      if (Number.isFinite(y) && y > 0)
        return y;
    } catch {
    }
    ...
    const i = (y = t == null ? void 0 : t.getSize) == null ? void 0 : y.call(t, "candle_pane", R1.YAxis);
    i != null && i.width && Number.isFinite(i.width) && nt(Math.max(44, Math.ceil(i.width)));
    const l = Number(o.y), h = xt(o), g = n.clientHeight;
    ...
    if (i) {
      if (Number.isFinite(o == null ? void 0 : o.pageX) ? y = o.pageX - i.left : Number.isFinite(o == null ? void 0 : o.x) && l && (y = l.left - i.left + o.x), Number.isFinite(o == null ? void 0 : o.pageY))
        ...
        try {
          const ee = (W = t == null ? void 0 : t.convertToPixel) == null ? void 0 : W.call(t, [{
            value: g.value
          }], {
            paneId: "candle_pane",
            absolute: !0
    ...
    return (!i || ((h = Ue()) == null ? void 0 : h.id) === i) && (Ke(null), Re(null)), !1;
  }, A1 = (o) => {
    var f, C, x, M, P, W, te, ee, Q;
    if (o.name !== "horizontalStraightLine")
      return o;
    const i = o.onClick, l = o.onSelected, h = o.onDeselected, g = o.onRemoved, y = o.onPressedMoveEnd;
    return {
      ...o,
      styles: {
        ...o.styles,
      ...
      onPressedMoveEnd: (G) => (at(G), (y == null ? void 0 : y(G)) ?? !1),
    ...
    if (!i || !n)
      return;
    o.preventDefault(), o.stopPropagation(), Re(null);
    const l = (P = (M = n.parentElement) == null ? void 0 : M.getBoundingClientRect) == null ? void 0 : P.call(M);
    if (!l)
      return;
    const h = o.clientX, g = o.clientY, y = i.x, f = i.y, C = (W) => {
      W.preventDefault();
      const te = y + W.clientX - h, ee = f + W.clientY - g;
      Ke({
        ...i,
  ...
  let ct = (kn = e.orderTools) == null ? void 0 : kn.quickOrder, j0 = (xn = e.orderTools) == null ? void 0 : xn.quickOrderFloatingWindow, Q0 = (Ln = e.orderTools) == null ? void 0 : Ln.quickOrderPlusButton, Z0 = (wn = e.orderTools) == null ? void 0 : wn.openOrders, V0 = (An = e.orderTools) == null ? void 0 : An.openOrdersExtendedPriceLine, H0 = (Mn = e.orderTools) == null ? void 0 : Mn.openOrdersDisplay, q0 = (Tn = e.orderTools) == null ? void 0 : Tn.positions, Y0 = (Sn = e.orderTools) == null ? void 0 : Sn.breakevenPrice, W0 = (Pn = e.orderTools) == null ? void 0 : Pn.liquidationPrice, G0 = (On = e.orderTools) == null ? void 0 : On.priceLine, X0 = (Dn = e.orderTools) == null ? void 0 : Dn.marketPriceLine, J0 = (Nn = e.orderTools) == null ? void 0 : Nn.countDown, en = (In = e.orderTools) == null ? void 0 : In.bidAskPrice, tn = (En = e.orderTools) == null ? void 0 : En.orderHistory;
  ze(() => {
    var G, he, Be, z, Ce, le, ke, we, Ge, Xe, r1, Je, e1, ut;
    const o = (G = e.orderTools) == null ? void 0 : G.quickOrder, i = (he = e.orderTools) == null ? void 0 : he.quickOrderFloatingWindow, l = (Be = e.orderTools) == null ? void 0 : Be.quickOrderPlusButton, h = (z = e.orderTools) == null ? void 0 : z.openOrders, g = (Ce = e.orderTools) == null ? void 0 : Ce.openOrdersExtendedPriceLine, y = (le = e.orderTools) == null ? void 0 : le.openOrdersDisplay, f = (ke = e.orderTools) == null ? void 0 : ke.positions, C = (we = e.orderTools) == null ? void 0 : we.breakevenPrice, x = (Ge = e.orderTools) == null ? void 0 : Ge.liquidationPrice, M = (Xe = e.orderTools) == null ? void 0 : Xe.priceLine, P = (r1 = e.orderTools) == null ? void 0 : r1.marketPriceLine, W = (Je = e.orderTools) == null ? void 0 : Je.countDown, te = (e1 = e.orderTools) == null ? void 0 : e1.bidAskPrice, ee = (ut = e.orderTools) == null ? void 0 : ut.orderHistory, Q = {};
    typeof o == "boolean" && o !== ct && (ct = o, Q.quickOrder = o, typeof i != "boolean" && (Q.quickOrderFloatingWindow = o), typeof l != "boolean" && (Q.quickOrderPlusButton = o)), typeof i == "boolean" && i !== j0 && (j0 = i, Q.quickOrderFloatingWindow = i), typeof l == "boolean" && l !== Q0 && (Q0 = l, Q.quickOrderPlusButton = l), typeof h == "boolean" && h !== Z0 && (Z0 = h, Q.openOrders = h), typeof g == "boolean" && g !== V0 && (V0 = g, Q.openOrdersExtendedPriceLine = g), y !== void 0 && y !== H0 && (H0 = y, Q.openOrdersDisplay = y), typeof f == "boolean" && f !== q0 && (q0 = f, Q.positions = f), typeof C == "boolean" && C !== Y0 && (Y0 = C, Q.breakevenPrice = C), typeof x == "boolean" && x !== W0 && (W0 = x, Q.liquidationPrice = x), typeof M == "boolean" && M !== G0 && (G0 = M, Q.priceLine = M, typeof P != "boolean" && (Q.marketPriceLine = M), typeof W != "boolean" && (Q.countDown = M), typeof te != "boolean" && (Q.bidAskPrice = M)), typeof P == "boolean" && P !== X0 && (X0 = P, Q.marketPriceLine = P), typeof W == "boolean" && W !== J0 && (J0 = W, Q.countDown = W), typeof te == "boolean" && te !== en && (en = te, Q.bidAskPrice = te), typeof ee == "boolean" && ee !== tn && (tn = ee, Q.orderHistory = ee), Object.keys(Q).length > 0 && L1(Q);
  }), ze(() => {
    ...
    getSettings: () => {
      var l, h, g, y, f, C, x, M, P, W, te, ee, Q, G, he, Be;
      if (!t)
        return {};
      ...
      return {
        ...
        showLastPrice: (f = (y = (g = o.candle) == null ? void 0 : g.priceMark) == null ? void 0 : y.last) == null ? void 0 : f.show,
    ...
    setSettings: (o) => {
      var l, h, g, y, f, C, x, M, P, W, te, ee, Q, G;
      if (!t)
        return;
      const i = {};
      ...
      o.showLastPrice !== void 0 && (i.candle = {
        ...
        priceMark: {
          ...
          last: {
            ...(y = (g = i.candle) == null ? void 0 : g.priceMark) == null ? void 0 : y.last,
    ...
    saveDrawings: (o) => {
      ...
      i.forEach((l, h) => {
        var f;
        const g = y1(l.type), y = ((f = l.points) == null ? void 0 : f.length) || 0;
        y < g && console.warn(`âš ï¸ ${l.type} ${l.id} has only ${y} point(s), should have ${g}`);
      }), Ye.saveDrawings(o, i);
    },
    loadDrawings: (o) => {
      Ye.loadDrawings(o).forEach((l, h) => {
        ...
        try {
          const y = {
            ...
            lock: l.lock ?? !1,
            mode: l.mode ?? h0.Normal
          }, f = t == null ? void 0 : t.createOverlay(y), C = typeof f == "string" ? f : null;
          C && (xe.set(C, {
            ...l,
            id: C
          }), Ee.set(C, {
  ...
  const nn = () => {
    t == null || t.resize(), We(), sn();
  }, rn = () => {
    We(), sn();
  }, on = [T1.OnVisibleRangeChange, T1.OnZoom, T1.OnScroll];
  let Tt, St;
  const y9 = (o) => {
    const i = new Date(o), l = i.getFullYear(), h = `${i.getMonth() + 1}`.padStart(2, "0"), g = `${i.getDate()}`.padStart(2, "0"), y = `${i.getHours()}`.padStart(2, "0"), f = `${i.getMinutes()}`.padStart(2, "0");
    return `${l}/${h}/${g} ${y}:${f}`;
  }, p9 = (o) => {
    var f;
    ...
    if (i.length === 0)
      return null;
    let l = i[0], h = 0, g = Number(l == null ? void 0 : l.timestamp), y = Math.abs(g - o);
    ...
    if (!t || !n)
      return null;
    const i = p9(o), l = i == null ? void 0 : i.candle, h = Number((l == null ? void 0 : l.timestamp) ?? o), g = Number((l == null ? void 0 : l.high) ?? (l == null ? void 0 : l.close) ?? (l == null ? void 0 : l.open)), y = ((ee = t == null ? void 0 : t.getDataList) == null ? void 0 : ee.call(t)) ?? [], f = i && i.dataIndex < y.length - 1 ? i.dataIndex + 1 : i == null ? void 0 : i.dataIndex, C = i && Number.isFinite(g) ? {
      dataIndex: f,
      value: g
    } : {
      timestamp: h
      ...
      paneId: "candle_pane",
      absolute: !0
    }), M = Number((G = x == null ? void 0 : x[0]) == null ? void 0 : G.x), P = Number((he = x == null ? void 0 : x[0]) == null ? void 0 : he.y), W = n.clientWidth, te = n.clientHeight;
    ...
    i < 6 && (St = window.setTimeout(() => c0(o, i + 1), 80));
  }, u0 = (o, i, l) => {
    let h = i, g = h;
    switch (o.timespan) {
      case "minute": {
      ...
      case "month": {
        const y = new Date(h), f = y.getFullYear(), C = y.getMonth() + 1;
    ...
    const l = zt(F()), h = Math.max(1, Math.floor(i / 2)) * l;
    return {
      from: o - h,
      to: o + h
    };
    ...
    for (let g = o.length - 1; g >= 0; g -= 1) {
      const y = Number((h = o[g]) == null ? void 0 : h.timestamp);
    ...
    const l = zt(i), h = Math.abs(o.to - o.from), g = Math.max(1, Math.ceil(h / l) + 1), y = Math.max(g, 120) * l;
    return {
      from: o.from,
    ...
    if (!t || !n || o.length === 0)
      return;
    const i = ((y = t.getSize("candle_pane", R1.YAxis)) == null ? void 0 : y.width) ?? 0, l = ((f = t.getSize("candle_pane", R1.Main)) == null ? void 0 : f.width) ?? n.clientWidth - i, h = Math.max(1, l - 8), g = Math.max(2, h / Math.max(1, o.length));
    t.setOffsetRightDistance(0), t.setLeftMinVisibleBarCount(0), t.setRightMinVisibleBarCount(0), t.setBarSpace(g);
  }, d0 = (o) => {
    var i;
    !t || !Number.isFinite(o) || ((i = t.scrollToTimestamp) == null || i.call(t, o, 250), requestAnimationFrame(() => c0(o)), We());
  }, Pt = () => {
    var l, h;
    ...
    if (t) {
      ...
      try {
        const l = F(), h = o.from <= o.to ? o : {
          from: o.to,
          to: o.from
        }, g = C9(h), y = i ? g : k9(g, l), f = await e.datafeed.getHistoryKLineData(A(), l, y.from, y.to), C = $9(f, g);
        t.applyNewData(f, f.length > 0), E(g), requestAnimationFrame(() => {
          const x = _9(f, g);
          i ? d0(i) : (x9(C), t == null || t.scrollToDataIndex(x, 0), c0(b9(C, g))), Pt();
        });
      } finally {
        s(!1), de(!1);
      }
    ...
    Ne(o), o.enabled && (H(o.timestamp), requestAnimationFrame(() => d0(o.timestamp))), requestAnimationFrame(Pt);
  };
  S0(() => {
    if (window.addEventListener("resize", nn), t = P9(n, {
      customApi: {
        formatDate: (f, C, x, M) => {
          switch (F().timespan) {
            case "minute":
    ...
    e.subIndicators.forEach((f) => {
      ...
      s(!0), (async () => {
        try {
          const x = F(), [M] = u0(x, f, 1), [P] = u0(x, M, 500), W = await e.datafeed.getHistoryKLineData(A(), x, P, M);
          t == null || t.applyMoreData(W, W.length > 0);
        } finally {
          s(!1);
        }
      })();
    ...
    }), t == null || t.subscribeAction(T1.OnCrosshairChange, Lt), on.forEach((f) => {
      t == null || t.subscribeAction(f, rn);
    }), Tt = window.setInterval(() => We(), 1e3), We(), document.addEventListener("mousedown", lt);
    const y = t == null ? void 0 : t.createOverlay;
    t && y && (t.createOverlay = function(...f) {
      const C = A1(f[0]), x = y.apply(this, [C, ...f.slice(1)]), M = typeof x == "string" ? x : null;
      return M && (kt(M, C.name || "unknown"), k1(M), x1()), x;
    });
  }), N1(() => {
    window.removeEventListener("resize", nn), t == null || t.unsubscribeAction(T1.OnCrosshairChange, Lt), on.forEach((o) => {
      t == null || t.unsubscribeAction(o, rn);
    }), Tt && (window.clearInterval(Tt), Tt = void 0), St && (window.clearTimeout(St), St = void 0), document.removeEventListener("mousedown", lt), Ee.clear(), xe.clear(), zn(n);
  }), ze(() => {
    const o = A();
  ...
  const A9 = (o) => {
    const i = new Date(o), l = i.getFullYear(), h = `${i.getMonth() + 1}`.padStart(2, "0"), g = `${i.getDate()}`.padStart(2, "0"), y = `${i.getHours()}`.padStart(2, "0"), f = `${i.getMinutes()}`.padStart(2, "0"), C = `${l}-${h}-${g}`;
    switch (F().timespan) {
      case "minute":
      case "hour":
        return `${C} ${y}:${f}`;
    ...
    const {
      current: l
    } = o, h = i.tooltip.text.color, g = l.close > l.open ? i.bar.upColor : l.close < l.open ? i.bar.downColor : i.bar.noChangeColor, y = Math.min(Math.max(((x = A()) == null ? void 0 : x.pricePrecision) ?? 2, 0), 8), f = Math.min(Math.max(((M = A()) == null ? void 0 : M.volumePrecision) ?? 0, 0), 8), C = (P) => ({
      text: ce.formatPrecision(P, y),
      color: g
    });
    return [{
      title: "time",
  ...
  return ze((o) => {
    ...
    return N1(() => {
      ...
      try {
        const y = ve(), f = y.enabled && (!o || o.symbol.ticker === i.ticker || y.acrossTokens), C = f ? y.timestamp + zt(l) * 250 : (/* @__PURE__ */ new Date()).getTime(), [x, M] = u0(l, C, 500), P = await e.datafeed.getHistoryKLineData(i, l, x, M);
        if (!h)
          return;
        t == null || t.applyNewData(P, P.length > 0), f ? requestAnimationFrame(() => {
          d0(y.timestamp), Pt();
        }) : Pt(), We(), setTimeout(() => {
          h && (i0(i == null ? void 0 : i.ticker), We());
        }, 0), e.datafeed.subscribe(i, l, (W) => {
          t == null || t.updateData(W), We();
        });
      } finally {
        h && (s(!1), de(!1));
      }
    ...
    t == null || t.setLocale(m());
  }), ze(() => {
    t == null || t.setTimezone(q().key);
  }), ze(() => {
    p() && (t == null || t.setStyles(p()), Ot(), be(R7(t.getStyles())));
  }), [cg.cloneNode(!0), L(Gd, {
    get locale() {
      return e.locale;
    },
    ...
    const o = ug.cloneNode(!0), i = o.firstChild;
    return o.addEventListener("mouseleave", () => {
    ...
    }), i), D1((l) => n = l, i), v(o, L(J, {
      get when() {
        return ot();
      },
      keyed: !0,
      children: (l) => (() => {
        const h = dg.cloneNode(!0);
        return v(h, () => l.text), I((g) => {
          const y = `${l.left}px`, f = `${l.top}px`;
          return y !== g._v$ && h.style.setProperty("left", g._v$ = y), f !== g._v$2 && h.style.setProperty("top", g._v$2 = f), g;
        }, {
          _v$: void 0,
          _v$2: void 0
        }), h;
      ...
      keyed: !0,
      children: (l) => (() => {
        const h = hg.cloneNode(!0), g = h.firstChild, y = g.nextSibling;
        return h.style.setProperty("right", "0px"), v(g, () => l.priceText), v(y, () => l.text), I((f) => {
          ...
          return C !== f._v$3 && h.style.setProperty("top", f._v$3 = C), x !== f._v$4 && h.style.setProperty("width", f._v$4 = x), M !== f._v$5 && h.style.setProperty("background", f._v$5 = M), P !== f._v$6 && h.style.setProperty("border-radius", f._v$6 = P), W !== f._v$7 && h.style.setProperty("font-family", f._v$7 = W), te !== f._v$8 && h.style.setProperty("font-weight", f._v$8 = te), ee !== f._v$9 && h.style.setProperty("padding-left", f._v$9 = ee), Q !== f._v$10 && h.style.setProperty("padding-right", f._v$10 = Q), G !== f._v$11 && h.style.setProperty("padding-top", f._v$11 = G), he !== f._v$12 && h.style.setProperty("padding-bottom", f._v$12 = he), Be !== f._v$13 && g.style.setProperty("font-size", f._v$13 = Be), z !== f._v$14 && y.style.setProperty("font-size", f._v$14 = z), f;
        }, {
          _v$3: void 0,
          _v$4: void 0,
          _v$5: void 0,
      ...
      keyed: !0,
      children: (l) => (() => {
        const h = yg.cloneNode(!0), g = h.firstChild, y = g.nextSibling, f = y.nextSibling, C = f.firstChild, x = f.nextSibling, M = x.firstChild, P = M.firstChild, W = P.nextSibling, te = W.firstChild, ee = x.nextSibling, Q = ee.firstChild, G = ee.nextSibling, he = G.nextSibling, Be = he.nextSibling;
        return h.$$click = (z) => {
          z.stopPropagation();
        }, h.$$mousedown = (z) => {
          z.preventDefault(), z.stopPropagation();
        }, g.$$mousedown = At, y.$$click = n1, C.$$click = () => Re(He() === "color" ? null : "color"), v(f, L(J, {
          get when() {
            return He() === "color";
          },
          get children() {
            const z = fg.cloneNode(!0), Ce = z.firstChild;
            return v(Ce, L($0, {
              each: it,
              children: (le) => (() => {
                const ke = pg.cloneNode(!0);
        ...
        }), null), M.$$click = () => Re(He() === "width" ? null : "width"), v(W, () => l.lineSize, te), v(x, L(J, {
          get when() {
            return He() === "width";
          },
          get children() {
            ...
            return v(z, L($0, {
              each: [1, 2, 3, 4],
              children: (Ce) => (() => {
                const le = vg.cloneNode(!0), ke = le.firstChild;
                return le.$$click = () => Te(Ce), ke.style.setProperty("height", `${Ce}px`), I(() => oe(le, l.lineSize === Ce ? "selected" : "")), le;
              })()
            })), z;
          }
        }), null), Q.$$click = () => Re(He() === "style" ? null : "style"), v(ee, L(J, {
          get when() {
            return He() === "style";
          },
          get children() {
            const z = mg.cloneNode(!0), Ce = z.firstChild, le = Ce.nextSibling, ke = le.nextSibling;
        ...
        }), null), G.$$click = S, he.$$click = b, Be.$$click = l0, I((z) => {
      ...
      keyed: !0,
      children: (l) => (() => {
        const h = Cg.cloneNode(!0), g = h.firstChild;
        return h.addEventListener("mouseleave", () => {
          b1() || Ze(!1);
        }), h.$$mousemove = (y) => {
          y.stopPropagation(), c1();
        }, h.addEventListener("mouseenter", () => {
          Ze(!0), c1();
        }), g.$$click = (y) => {
          y.stopPropagation(), Ze(!0), _1({
            y: l.y,
            price: l.price,
            yAxisWidth: tt()
          }), t1(!0), c1();
        }, g.$$mousedown = (y) => {
          y.preventDefault(), y.stopPropagation(), c1();
        }, v(g, (() => {
          const y = Z(() => {
            var f;
          ...
          return f !== y._v$27 && h.style.setProperty("top", y._v$27 = f), C !== y._v$28 && h.style.setProperty("right", y._v$28 = C), x !== y._v$29 && h.style.setProperty("display", y._v$29 = x), y;
        }, {
          _v$27: void 0,
          _v$28: void 0,
          _v$29: void 0
      ...
      keyed: !0,
      children: (l) => (() => {
        const h = _g.cloneNode(!0), g = h.firstChild, y = g.firstChild, f = y.firstChild, C = f.nextSibling, x = C.nextSibling, M = x.nextSibling;
        M.nextSibling;
        const P = y.nextSibling, W = P.firstChild, te = W.nextSibling, ee = te.nextSibling, Q = ee.nextSibling;
        Q.nextSibling;
        ...
        }, y.$$click = () => K1("limit"), v(y, () => A().shortName ?? A().name ?? A().ticker, C), v(y, () => w1(l.price), M), P.$$click = () => K1("stop"), v(P, () => A().shortName ?? A().name ?? A().ticker, te), v(P, () => w1(l.price), Q), G.$$click = () => K1("create"), he.$$click = a0, v(he, () => w1(l.price), z), Ce.$$click = wt, v(Ce, () => w1(l.price), null), I((le) => {
