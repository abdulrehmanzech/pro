var y9 = Object.defineProperty;
var f9 = (e, t, n) => t in e ? y9(e, t, { enumerable: !0, configurable: !0, writable: !0, value: n }) : e[t] = n;
var ce = (e, t, n) => (f9(e, typeof t != "symbol" ? t + "" : t, n), n);
import { utils as n1, OverlayMode as N0, init as g9, FormatDateType as me, DomPosition as D0, ActionType as Ze, dispose as E0, TooltipIconPosition as pe, CandleType as m9, YAxisType as p9, registerOverlay as v9 } from "klinecharts";
function ue(e, t, n) {
  const r = (e.x - t.x) * Math.cos(n) - (e.y - t.y) * Math.sin(n) + t.x, i = (e.x - t.x) * Math.sin(n) + (e.y - t.y) * Math.cos(n) + t.y;
  return { x: r, y: i };
}
function Ye(e, t) {
  if (e.length > 1) {
    let n;
    return e[0].x === e[1].x && e[0].y !== e[1].y ? e[0].y < e[1].y ? n = {
      x: e[0].x,
      y: t.height
    } : n = {
      x: e[0].x,
      y: 0
    } : e[0].x > e[1].x ? n = {
      x: 0,
      y: n1.getLinearYFromCoordinates(e[0], e[1], { x: 0, y: e[0].y })
    } : n = {
      x: t.width,
      y: n1.getLinearYFromCoordinates(e[0], e[1], { x: t.width, y: e[0].y })
    }, { coordinates: [e[0], n] };
  }
  return [];
}
function Lt(e, t) {
  const n = Math.abs(e.x - t.x), r = Math.abs(e.y - t.y);
  return Math.sqrt(n * n + r * r);
}
const _9 = {
  name: "arrow",
  totalStep: 3,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e }) => {
    if (e.length > 1) {
      const t = e[1].x > e[0].x ? 0 : 1, n = n1.getLinearSlopeIntercept(e[0], e[1]);
      let r;
      n ? r = Math.atan(n[0]) + Math.PI * t : e[1].y > e[0].y ? r = Math.PI / 2 : r = Math.PI / 2 * 3;
      const i = ue({ x: e[1].x - 8, y: e[1].y + 4 }, e[1], r), s = ue({ x: e[1].x - 8, y: e[1].y - 4 }, e[1], r);
      return [
        {
          type: "line",
          attrs: { coordinates: e }
        },
        {
          type: "line",
          ignoreEvent: !0,
          attrs: { coordinates: [i, e[1], s] }
        }
      ];
    }
    return [];
  }
}, $9 = {
  name: "circle",
  totalStep: 3,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  styles: {
    circle: {
      color: "rgba(22, 119, 255, 0.15)"
    }
  },
  createPointFigures: ({ coordinates: e }) => {
    if (e.length > 1) {
      const t = Lt(e[0], e[1]);
      return {
        type: "circle",
        attrs: {
          ...e[0],
          r: t
        },
        styles: { style: "stroke_fill" }
      };
    }
    return [];
  }
}, b9 = {
  name: "rect",
  totalStep: 3,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  styles: {
    polygon: {
      color: "rgba(22, 119, 255, 0.15)"
    }
  },
  createPointFigures: ({ coordinates: e }) => e.length > 1 ? [
    {
      type: "polygon",
      attrs: {
        coordinates: [
          e[0],
          { x: e[1].x, y: e[0].y },
          e[1],
          { x: e[0].x, y: e[1].y }
        ]
      },
      styles: { style: "stroke_fill" }
    }
  ] : []
}, L9 = {
  name: "parallelogram",
  totalStep: 4,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  styles: {
    polygon: {
      color: "rgba(22, 119, 255, 0.15)"
    }
  },
  createPointFigures: ({ coordinates: e }) => {
    if (e.length === 2)
      return [
        {
          type: "line",
          ignoreEvent: !0,
          attrs: { coordinates: e }
        }
      ];
    if (e.length === 3) {
      const t = { x: e[0].x + (e[2].x - e[1].x), y: e[2].y };
      return [
        {
          type: "polygon",
          attrs: { coordinates: [e[0], e[1], e[2], t] },
          styles: { style: "stroke_fill" }
        }
      ];
    }
    return [];
  },
  performEventPressedMove: ({ points: e, performPointIndex: t, performPoint: n }) => {
    t < 2 && (e[0].price = n.price, e[1].price = n.price);
  },
  performEventMoveForDrawing: ({ currentStep: e, points: t, performPoint: n }) => {
    e === 2 && (t[0].price = n.price);
  }
}, x9 = {
  name: "triangle",
  totalStep: 4,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  styles: {
    polygon: {
      color: "rgba(22, 119, 255, 0.15)"
    }
  },
  createPointFigures: ({ coordinates: e }) => [
    {
      type: "polygon",
      attrs: { coordinates: e },
      styles: { style: "stroke_fill" }
    }
  ]
}, k9 = {
  name: "fibonacciCircle",
  totalStep: 3,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e }) => {
    if (e.length > 1) {
      const t = Math.abs(e[0].x - e[1].x), n = Math.abs(e[0].y - e[1].y), r = Math.sqrt(t * t + n * n), i = [0.236, 0.382, 0.5, 0.618, 0.786, 1], s = [], l = [];
      return i.forEach((c) => {
        const h = r * c;
        s.push(
          { ...e[0], r: h }
        ), l.push({
          x: e[0].x,
          y: e[0].y + h + 6,
          text: `${(c * 100).toFixed(1)}%`
        });
      }), [
        {
          type: "circle",
          attrs: s,
          styles: { style: "stroke" }
        },
        {
          type: "text",
          ignoreEvent: !0,
          attrs: l
        }
      ];
    }
    return [];
  }
}, A9 = {
  name: "fibonacciSegment",
  totalStep: 3,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e, overlay: t, precision: n }) => {
    const r = [], i = [];
    if (e.length > 1) {
      const s = e[1].x > e[0].x ? e[0].x : e[1].x, l = [1, 0.786, 0.618, 0.5, 0.382, 0.236, 0], c = e[0].y - e[1].y, h = t.points, m = h[0].value - h[1].value;
      l.forEach((d) => {
        const $ = e[1].y + c * d, v = (h[1].value + m * d).toFixed(n.price);
        r.push({ coordinates: [{ x: e[0].x, y: $ }, { x: e[1].x, y: $ }] }), i.push({
          x: s,
          y: $,
          text: `${v} (${(d * 100).toFixed(1)}%)`,
          baseline: "bottom"
        });
      });
    }
    return [
      {
        type: "line",
        attrs: r
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: i
      }
    ];
  }
}, w9 = {
  name: "fibonacciSpiral",
  totalStep: 3,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e, bounding: t }) => {
    if (e.length > 1) {
      const n = Lt(e[0], e[1]) / Math.sqrt(24), r = e[1].x > e[0].x ? 0 : 1, i = n1.getLinearSlopeIntercept(e[0], e[1]);
      let s;
      i ? s = Math.atan(i[0]) + Math.PI * r : e[1].y > e[0].y ? s = Math.PI / 2 : s = Math.PI / 2 * 3;
      const l = ue(
        { x: e[0].x - n, y: e[0].y },
        e[0],
        s
      ), c = ue(
        { x: e[0].x - n, y: e[0].y - n },
        e[0],
        s
      ), h = [{
        ...l,
        r: n,
        startAngle: s,
        endAngle: s + Math.PI / 2
      }, {
        ...c,
        r: n * 2,
        startAngle: s + Math.PI / 2,
        endAngle: s + Math.PI
      }];
      let m = e[0].x - n, d = e[0].y - n;
      for (let $ = 2; $ < 9; $++) {
        const v = h[$ - 2].r + h[$ - 1].r;
        let k = 0;
        switch ($ % 4) {
          case 0: {
            k = s, m -= h[$ - 2].r;
            break;
          }
          case 1: {
            k = s + Math.PI / 2, d -= h[$ - 2].r;
            break;
          }
          case 2: {
            k = s + Math.PI, m += h[$ - 2].r;
            break;
          }
          case 3: {
            k = s + Math.PI / 2 * 3, d += h[$ - 2].r;
            break;
          }
        }
        const T = k + Math.PI / 2, D = ue({ x: m, y: d }, e[0], s);
        h.push({
          ...D,
          r: v,
          startAngle: k,
          endAngle: T
        });
      }
      return [
        {
          type: "arc",
          attrs: h
        },
        {
          type: "line",
          attrs: Ye(e, t)
        }
      ];
    }
    return [];
  }
}, M9 = {
  name: "fibonacciSpeedResistanceFan",
  totalStep: 3,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e, bounding: t }) => {
    const n = [];
    let r = [];
    const i = [];
    if (e.length > 1) {
      const s = e[1].x > e[0].x ? -38 : 4, l = e[1].y > e[0].y ? -2 : 20, c = e[1].x - e[0].x, h = e[1].y - e[0].y;
      [1, 0.75, 0.618, 0.5, 0.382, 0.25, 0].forEach((d) => {
        const $ = e[1].x - c * d, v = e[1].y - h * d;
        n.push({ coordinates: [{ x: $, y: e[0].y }, { x: $, y: e[1].y }] }), n.push({ coordinates: [{ x: e[0].x, y: v }, { x: e[1].x, y: v }] }), r = r.concat(Ye([e[0], { x: $, y: e[1].y }], t)), r = r.concat(Ye([e[0], { x: e[1].x, y: v }], t)), i.unshift({
          x: e[0].x + s,
          y: v + 10,
          text: `${d.toFixed(3)}`
        }), i.unshift({
          x: $ - 18,
          y: e[0].y + l,
          text: `${d.toFixed(3)}`
        });
      });
    }
    return [
      {
        type: "line",
        attrs: n
      },
      {
        type: "line",
        attrs: r
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: i
      }
    ];
  }
}, S9 = {
  name: "fibonacciExtension",
  totalStep: 4,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e, overlay: t, precision: n }) => {
    const r = [], i = [];
    if (e.length > 2) {
      const s = t.points, l = s[1].value - s[0].value, c = e[1].y - e[0].y, h = [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1], m = e[2].x > e[1].x ? e[1].x : e[2].x;
      h.forEach((d) => {
        const $ = e[2].y + c * d, v = (s[2].value + l * d).toFixed(n.price);
        r.push({ coordinates: [{ x: e[1].x, y: $ }, { x: e[2].x, y: $ }] }), i.push({
          x: m,
          y: $,
          text: `${v} (${(d * 100).toFixed(1)}%)`,
          baseline: "bottom"
        });
      });
    }
    return [
      {
        type: "line",
        attrs: { coordinates: e },
        styles: { style: "dashed" }
      },
      {
        type: "line",
        attrs: r
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: i
      }
    ];
  }
}, T9 = {
  name: "gannBox",
  totalStep: 3,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  styles: {
    polygon: {
      color: "rgba(22, 119, 255, 0.15)"
    }
  },
  createPointFigures: ({ coordinates: e }) => {
    if (e.length > 1) {
      const t = (e[1].y - e[0].y) / 4, n = e[1].x - e[0].x, r = [
        { coordinates: [e[0], { x: e[1].x, y: e[1].y - t }] },
        { coordinates: [e[0], { x: e[1].x, y: e[1].y - t * 2 }] },
        { coordinates: [{ x: e[0].x, y: e[1].y }, { x: e[1].x, y: e[0].y + t }] },
        { coordinates: [{ x: e[0].x, y: e[1].y }, { x: e[1].x, y: e[0].y + t * 2 }] },
        { coordinates: [{ ...e[0] }, { x: e[0].x + n * 0.236, y: e[1].y }] },
        { coordinates: [{ ...e[0] }, { x: e[0].x + n * 0.5, y: e[1].y }] },
        { coordinates: [{ x: e[0].x, y: e[1].y }, { x: e[0].x + n * 0.236, y: e[0].y }] },
        { coordinates: [{ x: e[0].x, y: e[1].y }, { x: e[0].x + n * 0.5, y: e[0].y }] }
      ], i = [
        { coordinates: [e[0], e[1]] },
        { coordinates: [{ x: e[0].x, y: e[1].y }, { x: e[1].x, y: e[0].y }] }
      ];
      return [
        {
          type: "line",
          attrs: [
            { coordinates: [e[0], { x: e[1].x, y: e[0].y }] },
            { coordinates: [{ x: e[1].x, y: e[0].y }, e[1]] },
            { coordinates: [e[1], { x: e[0].x, y: e[1].y }] },
            { coordinates: [{ x: e[0].x, y: e[1].y }, e[0]] }
          ]
        },
        {
          type: "polygon",
          ignoreEvent: !0,
          attrs: {
            coordinates: [
              e[0],
              { x: e[1].x, y: e[0].y },
              e[1],
              { x: e[0].x, y: e[1].y }
            ]
          },
          styles: { style: "fill" }
        },
        {
          type: "line",
          attrs: r,
          styles: { style: "dashed" }
        },
        {
          type: "line",
          attrs: i
        }
      ];
    }
    return [];
  }
}, P9 = {
  name: "threeWaves",
  totalStep: 5,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e }) => {
    const t = e.map((n, r) => ({
      ...n,
      text: `(${r})`,
      baseline: "bottom"
    }));
    return [
      {
        type: "line",
        attrs: { coordinates: e }
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: t
      }
    ];
  }
}, I9 = {
  name: "fiveWaves",
  totalStep: 7,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e }) => {
    const t = e.map((n, r) => ({
      ...n,
      text: `(${r})`,
      baseline: "bottom"
    }));
    return [
      {
        type: "line",
        attrs: { coordinates: e }
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: t
      }
    ];
  }
}, O9 = {
  name: "eightWaves",
  totalStep: 10,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e }) => {
    const t = e.map((n, r) => ({
      ...n,
      text: `(${r})`,
      baseline: "bottom"
    }));
    return [
      {
        type: "line",
        attrs: { coordinates: e }
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: t
      }
    ];
  }
}, N9 = {
  name: "anyWaves",
  totalStep: Number.MAX_SAFE_INTEGER,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e }) => {
    const t = e.map((n, r) => ({
      ...n,
      text: `(${r})`,
      baseline: "bottom"
    }));
    return [
      {
        type: "line",
        attrs: { coordinates: e }
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: t
      }
    ];
  }
}, D9 = {
  name: "abcd",
  totalStep: 5,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  createPointFigures: ({ coordinates: e }) => {
    let t = [], n = [];
    const r = ["A", "B", "C", "D"], i = e.map((s, l) => ({
      ...s,
      baseline: "bottom",
      text: `(${r[l]})`
    }));
    return e.length > 2 && (t = [e[0], e[2]], e.length > 3 && (n = [e[1], e[3]])), [
      {
        type: "line",
        attrs: { coordinates: e }
      },
      {
        type: "line",
        attrs: [{ coordinates: t }, { coordinates: n }],
        styles: { style: "dashed" }
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: i
      }
    ];
  }
}, E9 = {
  name: "xabcd",
  totalStep: 6,
  needDefaultPointFigure: !0,
  needDefaultXAxisFigure: !0,
  needDefaultYAxisFigure: !0,
  styles: {
    polygon: {
      color: "rgba(22, 119, 255, 0.15)"
    }
  },
  createPointFigures: ({ coordinates: e, overlay: t }) => {
    const n = [], r = [], i = ["X", "A", "B", "C", "D"], s = e.map((l, c) => ({
      ...l,
      baseline: "bottom",
      text: `(${i[c]})`
    }));
    return e.length > 2 && (n.push({ coordinates: [e[0], e[2]] }), r.push({ coordinates: [e[0], e[1], e[2]] }), e.length > 3 && (n.push({ coordinates: [e[1], e[3]] }), e.length > 4 && (n.push({ coordinates: [e[2], e[4]] }), r.push({ coordinates: [e[2], e[3], e[4]] })))), [
      {
        type: "line",
        attrs: { coordinates: e }
      },
      {
        type: "line",
        attrs: n,
        styles: { style: "dashed" }
      },
      {
        type: "polygon",
        ignoreEvent: !0,
        attrs: r
      },
      {
        type: "text",
        ignoreEvent: !0,
        attrs: s
      }
    ];
  }
}, B9 = [
  _9,
  $9,
  b9,
  x9,
  L9,
  k9,
  A9,
  w9,
  M9,
  S9,
  T9,
  P9,
  I9,
  O9,
  N9,
  D9,
  E9
];
class Ed {
  constructor(t) {
    ce(this, "_apiKey");
    ce(this, "_prevSymbolMarket");
    ce(this, "_ws");
    this._apiKey = t;
  }
  async searchSymbols(t) {
    return await ((await (await fetch(`https://api.polygon.io/v3/reference/tickers?apiKey=${this._apiKey}&active=true&search=${t ?? ""}`)).json()).results || []).map((i) => ({
      ticker: i.ticker,
      name: i.name,
      shortName: i.ticker,
      market: i.market,
      exchange: i.primary_exchange,
      priceCurrency: i.currency_name,
      type: i.type,
      logo: "data:image/png;base64,"
    }));
  }
  async getHistoryKLineData(t, n, r, i) {
    return await ((await (await fetch(`https://api.polygon.io/v2/aggs/ticker/${t.ticker}/range/${n.multiplier}/${n.timespan}/${r}/${i}?apiKey=${this._apiKey}`)).json()).results || []).map((c) => ({
      timestamp: c.t,
      open: c.o,
      high: c.h,
      low: c.l,
      close: c.c,
      volume: c.v,
      turnover: c.vw
    }));
  }
  subscribe(t, n, r) {
    var i, s;
    this._prevSymbolMarket !== t.market ? ((i = this._ws) == null || i.close(), this._ws = new WebSocket(`wss://delayed.polygon.io/${t.market}`), this._ws.onopen = () => {
      var l;
      (l = this._ws) == null || l.send(JSON.stringify({ action: "auth", params: this._apiKey }));
    }, this._ws.onmessage = (l) => {
      var h;
      const c = JSON.parse(l.data);
      c[0].ev === "status" ? c[0].status === "auth_success" && ((h = this._ws) == null || h.send(JSON.stringify({ action: "subscribe", params: `T.${t.ticker}` }))) : "sym" in c && r({
        timestamp: c.s,
        open: c.o,
        high: c.h,
        low: c.l,
        close: c.c,
        volume: c.v,
        turnover: c.vw
      });
    }) : (s = this._ws) == null || s.send(JSON.stringify({ action: "subscribe", params: `T.${t.ticker}` })), this._prevSymbolMarket = t.market;
  }
  unsubscribe(t, n) {
  }
}
const s1 = {};
function U9(e) {
  s1.context = e;
}
const F9 = (e, t) => e === t, Ge = Symbol("solid-proxy"), K9 = Symbol("solid-track"), Le = {
  equals: F9
};
let xt = Mt;
const S1 = 1, xe = 2, kt = {
  owned: null,
  cleanups: null,
  context: null,
  owner: null
}, Re = {};
var i1 = null;
let F1 = null, e1 = null, c1 = null, w1 = null, r0 = 0;
function de(e, t) {
  const n = e1, r = i1, i = e.length === 0, s = i ? kt : {
    owned: null,
    cleanups: null,
    context: null,
    owner: t === void 0 ? r : t
  }, l = i ? e : () => e(() => M1(() => Oe(s)));
  i1 = s, e1 = null;
  try {
    return I1(l, !0);
  } finally {
    e1 = n, i1 = r;
  }
}
function w(e, t) {
  t = t ? Object.assign({}, Le, t) : Le;
  const n = {
    value: e,
    observers: null,
    observerSlots: null,
    comparator: t.equals || void 0
  }, r = (i) => (typeof i == "function" && (i = i(n.value)), wt(n, i));
  return [At.bind(n), r];
}
function B0(e, t, n) {
  const r = Ie(e, t, !0, S1);
  Y1(r);
}
function Q(e, t, n) {
  const r = Ie(e, t, !1, S1);
  Y1(r);
}
function m1(e, t, n) {
  xt = H9;
  const r = Ie(e, t, !1, S1);
  r.user = !0, w1 ? w1.push(r) : Y1(r);
}
function V(e, t, n) {
  n = n ? Object.assign({}, Le, n) : Le;
  const r = Ie(e, t, !0, 0);
  return r.observers = null, r.observerSlots = null, r.comparator = n.equals || void 0, Y1(r), At.bind(r);
}
function j9(e, t, n) {
  let r, i, s;
  arguments.length === 2 && typeof t == "object" || arguments.length === 1 ? (r = !0, i = e, s = t || {}) : (r = e, i = t, s = n || {});
  let l = null, c = Re, h = null, m = !1, d = "initialValue" in s, $ = typeof r == "function" && V(r);
  const v = /* @__PURE__ */ new Set(), [k, T] = (s.storage || w)(s.initialValue), [D, I] = w(void 0), [F, K] = w(void 0, {
    equals: !1
  }), [P, B] = w(d ? "ready" : "unresolved");
  if (s1.context) {
    h = `${s1.context.id}${s1.context.count++}`;
    let O;
    s.ssrLoadFrom === "initial" ? c = s.initialValue : s1.load && (O = s1.load(h)) && (c = O[0]);
  }
  function z(O, G, R, N) {
    return l === O && (l = null, d = !0, (O === c || G === c) && s.onHydrated && queueMicrotask(() => s.onHydrated(N, {
      value: G
    })), c = Re, $1(G, R)), G;
  }
  function $1(O, G) {
    I1(() => {
      G === void 0 && T(() => O), B(G !== void 0 ? "errored" : "ready"), I(G);
      for (const R of v.keys())
        R.decrement();
      v.clear();
    }, !1);
  }
  function a1() {
    const O = Q9, G = k(), R = D();
    if (R !== void 0 && !l)
      throw R;
    return e1 && !e1.user && O && B0(() => {
      F(), l && (O.resolved || v.has(O) || (O.increment(), v.add(O)));
    }), G;
  }
  function f1(O = !0) {
    if (O !== !1 && m)
      return;
    m = !1;
    const G = $ ? $() : r;
    if (G == null || G === !1) {
      z(l, M1(k));
      return;
    }
    const R = c !== Re ? c : M1(() => i(G, {
      value: k(),
      refetching: O
    }));
    return typeof R != "object" || !(R && "then" in R) ? (z(l, R, void 0, G), R) : (l = R, m = !0, queueMicrotask(() => m = !1), I1(() => {
      B(d ? "refreshing" : "pending"), K();
    }, !1), R.then((N) => z(R, N, void 0, G), (N) => z(R, void 0, Tt(N), G)));
  }
  return Object.defineProperties(a1, {
    state: {
      get: () => P()
    },
    error: {
      get: () => D()
    },
    loading: {
      get() {
        const O = P();
        return O === "pending" || O === "refreshing";
      }
    },
    latest: {
      get() {
        if (!d)
          return a1();
        const O = D();
        if (O && !l)
          throw O;
        return k();
      }
    }
  }), $ ? B0(() => f1(!1)) : f1(!1), [a1, {
    refetch: f1,
    mutate: T
  }];
}
function M1(e) {
  if (e1 === null)
    return e();
  const t = e1;
  e1 = null;
  try {
    return e();
  } finally {
    e1 = t;
  }
}
function i0(e) {
  m1(() => M1(e));
}
function j1(e) {
  return i1 === null || (i1.cleanups === null ? i1.cleanups = [e] : i1.cleanups.push(e)), e;
}
function z9(e) {
  const t = e1, n = i1;
  return Promise.resolve().then(() => {
    e1 = t, i1 = n;
    let r;
    return I1(e, !1), e1 = i1 = null, r ? r.done : void 0;
  });
}
let Q9;
function At() {
  const e = F1;
  if (this.sources && (this.state || e))
    if (this.state === S1 || e)
      Y1(this);
    else {
      const t = c1;
      c1 = null, I1(() => Ae(this), !1), c1 = t;
    }
  if (e1) {
    const t = this.observers ? this.observers.length : 0;
    e1.sources ? (e1.sources.push(this), e1.sourceSlots.push(t)) : (e1.sources = [this], e1.sourceSlots = [t]), this.observers ? (this.observers.push(e1), this.observerSlots.push(e1.sources.length - 1)) : (this.observers = [e1], this.observerSlots = [e1.sources.length - 1]);
  }
  return this.value;
}
function wt(e, t, n) {
  let r = e.value;
  return (!e.comparator || !e.comparator(r, t)) && (e.value = t, e.observers && e.observers.length && I1(() => {
    for (let i = 0; i < e.observers.length; i += 1) {
      const s = e.observers[i], l = F1 && F1.running;
      l && F1.disposed.has(s), (l && !s.tState || !l && !s.state) && (s.pure ? c1.push(s) : w1.push(s), s.observers && St(s)), l || (s.state = S1);
    }
    if (c1.length > 1e6)
      throw c1 = [], new Error();
  }, !1)), t;
}
function Y1(e) {
  if (!e.fn)
    return;
  Oe(e);
  const t = i1, n = e1, r = r0;
  e1 = i1 = e, Z9(e, e.value, r), e1 = n, i1 = t;
}
function Z9(e, t, n) {
  let r;
  try {
    r = e.fn(t);
  } catch (i) {
    e.pure && (e.state = S1, e.owned && e.owned.forEach(Oe), e.owned = null), Pt(i);
  }
  (!e.updatedAt || e.updatedAt <= n) && (e.updatedAt != null && "observers" in e ? wt(e, r) : e.value = r, e.updatedAt = n);
}
function Ie(e, t, n, r = S1, i) {
  const s = {
    fn: e,
    state: r,
    updatedAt: null,
    owned: null,
    sources: null,
    sourceSlots: null,
    cleanups: null,
    value: t,
    owner: i1,
    context: null,
    pure: n
  };
  return i1 === null || i1 !== kt && (i1.owned ? i1.owned.push(s) : i1.owned = [s]), s;
}
function ke(e) {
  const t = F1;
  if (e.state === 0 || t)
    return;
  if (e.state === xe || t)
    return Ae(e);
  if (e.suspense && M1(e.suspense.inFallback))
    return e.suspense.effects.push(e);
  const n = [e];
  for (; (e = e.owner) && (!e.updatedAt || e.updatedAt < r0); )
    (e.state || t) && n.push(e);
  for (let r = n.length - 1; r >= 0; r--)
    if (e = n[r], e.state === S1 || t)
      Y1(e);
    else if (e.state === xe || t) {
      const i = c1;
      c1 = null, I1(() => Ae(e, n[0]), !1), c1 = i;
    }
}
function I1(e, t) {
  if (c1)
    return e();
  let n = !1;
  t || (c1 = []), w1 ? n = !0 : w1 = [], r0++;
  try {
    const r = e();
    return R9(n), r;
  } catch (r) {
    n || (w1 = null), c1 = null, Pt(r);
  }
}
function R9(e) {
  if (c1 && (Mt(c1), c1 = null), e)
    return;
  const t = w1;
  w1 = null, t.length && I1(() => xt(t), !1);
}
function Mt(e) {
  for (let t = 0; t < e.length; t++)
    ke(e[t]);
}
function H9(e) {
  let t, n = 0;
  for (t = 0; t < e.length; t++) {
    const r = e[t];
    r.user ? e[n++] = r : ke(r);
  }
  for (s1.context && U9(), t = 0; t < n; t++)
    ke(e[t]);
}
function Ae(e, t) {
  const n = F1;
  e.state = 0;
  for (let r = 0; r < e.sources.length; r += 1) {
    const i = e.sources[r];
    i.sources && (i.state === S1 || n ? i !== t && ke(i) : (i.state === xe || n) && Ae(i, t));
  }
}
function St(e) {
  const t = F1;
  for (let n = 0; n < e.observers.length; n += 1) {
    const r = e.observers[n];
    (!r.state || t) && (r.state = xe, r.pure ? c1.push(r) : w1.push(r), r.observers && St(r));
  }
}
function Oe(e) {
  let t;
  if (e.sources)
    for (; e.sources.length; ) {
      const n = e.sources.pop(), r = e.sourceSlots.pop(), i = n.observers;
      if (i && i.length) {
        const s = i.pop(), l = n.observerSlots.pop();
        r < i.length && (s.sourceSlots[l] = r, i[r] = s, n.observerSlots[r] = l);
      }
    }
  if (e.owned) {
    for (t = 0; t < e.owned.length; t++)
      Oe(e.owned[t]);
    e.owned = null;
  }
  if (e.cleanups) {
    for (t = 0; t < e.cleanups.length; t++)
      e.cleanups[t]();
    e.cleanups = null;
  }
  e.state = 0, e.context = null;
}
function Tt(e) {
  return e instanceof Error || typeof e == "string" ? e : new Error("Unknown error");
}
function Pt(e) {
  throw e = Tt(e), e;
}
const V9 = Symbol("fallback");
function U0(e) {
  for (let t = 0; t < e.length; t++)
    e[t]();
}
function Y9(e, t, n = {}) {
  let r = [], i = [], s = [], l = 0, c = t.length > 1 ? [] : null;
  return j1(() => U0(s)), () => {
    let h = e() || [], m, d;
    return h[K9], M1(() => {
      let v = h.length, k, T, D, I, F, K, P, B, z;
      if (v === 0)
        l !== 0 && (U0(s), s = [], r = [], i = [], l = 0, c && (c = [])), n.fallback && (r = [V9], i[0] = de(($1) => (s[0] = $1, n.fallback())), l = 1);
      else if (l === 0) {
        for (i = new Array(v), d = 0; d < v; d++)
          r[d] = h[d], i[d] = de($);
        l = v;
      } else {
        for (D = new Array(v), I = new Array(v), c && (F = new Array(v)), K = 0, P = Math.min(l, v); K < P && r[K] === h[K]; K++)
          ;
        for (P = l - 1, B = v - 1; P >= K && B >= K && r[P] === h[B]; P--, B--)
          D[B] = i[P], I[B] = s[P], c && (F[B] = c[P]);
        for (k = /* @__PURE__ */ new Map(), T = new Array(B + 1), d = B; d >= K; d--)
          z = h[d], m = k.get(z), T[d] = m === void 0 ? -1 : m, k.set(z, d);
        for (m = K; m <= P; m++)
          z = r[m], d = k.get(z), d !== void 0 && d !== -1 ? (D[d] = i[m], I[d] = s[m], c && (F[d] = c[m]), d = T[d], k.set(z, d)) : s[m]();
        for (d = K; d < v; d++)
          d in D ? (i[d] = D[d], s[d] = I[d], c && (c[d] = F[d], c[d](d))) : i[d] = de($);
        i = i.slice(0, l = v), r = h.slice(0);
      }
      return i;
    });
    function $(v) {
      if (s[d] = v, c) {
        const [k, T] = w(d);
        return c[d] = T, t(h[d], k);
      }
      return t(h[d]);
    }
  };
}
function L(e, t) {
  return M1(() => e(t || {}));
}
function ve() {
  return !0;
}
const G9 = {
  get(e, t, n) {
    return t === Ge ? n : e.get(t);
  },
  has(e, t) {
    return t === Ge ? !0 : e.has(t);
  },
  set: ve,
  deleteProperty: ve,
  getOwnPropertyDescriptor(e, t) {
    return {
      configurable: !0,
      enumerable: !0,
      get() {
        return e.get(t);
      },
      set: ve,
      deleteProperty: ve
    };
  },
  ownKeys(e) {
    return e.keys();
  }
};
function He(e) {
  return (e = typeof e == "function" ? e() : e) ? e : {};
}
function It(...e) {
  let t = !1;
  for (let r = 0; r < e.length; r++) {
    const i = e[r];
    t = t || !!i && Ge in i, e[r] = typeof i == "function" ? (t = !0, V(i)) : i;
  }
  if (t)
    return new Proxy({
      get(r) {
        for (let i = e.length - 1; i >= 0; i--) {
          const s = He(e[i])[r];
          if (s !== void 0)
            return s;
        }
      },
      has(r) {
        for (let i = e.length - 1; i >= 0; i--)
          if (r in He(e[i]))
            return !0;
        return !1;
      },
      keys() {
        const r = [];
        for (let i = 0; i < e.length; i++)
          r.push(...Object.keys(He(e[i])));
        return [...new Set(r)];
      }
    }, G9);
  const n = {};
  for (let r = e.length - 1; r >= 0; r--)
    if (e[r]) {
      const i = Object.getOwnPropertyDescriptors(e[r]);
      for (const s in i)
        s in n || Object.defineProperty(n, s, {
          enumerable: !0,
          get() {
            for (let l = e.length - 1; l >= 0; l--) {
              const c = (e[l] || {})[s];
              if (c !== void 0)
                return c;
            }
          }
        });
    }
  return n;
}
function q9(e) {
  const t = "fallback" in e && {
    fallback: () => e.fallback
  };
  return V(Y9(() => e.each, e.children, t || void 0));
}
function H(e) {
  let t = !1;
  const n = e.keyed, r = V(() => e.when, void 0, {
    equals: (i, s) => t ? i === s : !i == !s
  });
  return V(() => {
    const i = r();
    if (i) {
      const s = e.children, l = typeof s == "function" && s.length > 0;
      return t = n || l, l ? M1(() => s(i)) : s;
    }
    return e.fallback;
  }, void 0, void 0);
}
function W9(e, t, n) {
  let r = n.length, i = t.length, s = r, l = 0, c = 0, h = t[i - 1].nextSibling, m = null;
  for (; l < i || c < s; ) {
    if (t[l] === n[c]) {
      l++, c++;
      continue;
    }
    for (; t[i - 1] === n[s - 1]; )
      i--, s--;
    if (i === l) {
      const d = s < r ? c ? n[c - 1].nextSibling : n[s - c] : h;
      for (; c < s; )
        e.insertBefore(n[c++], d);
    } else if (s === c)
      for (; l < i; )
        (!m || !m.has(t[l])) && t[l].remove(), l++;
    else if (t[l] === n[s - 1] && n[c] === t[i - 1]) {
      const d = t[--i].nextSibling;
      e.insertBefore(n[c++], t[l++].nextSibling), e.insertBefore(n[--s], d), t[i] = n[s];
    } else {
      if (!m) {
        m = /* @__PURE__ */ new Map();
        let $ = c;
        for (; $ < s; )
          m.set(n[$], $++);
      }
      const d = m.get(t[l]);
      if (d != null)
        if (c < d && d < s) {
          let $ = l, v = 1, k;
          for (; ++$ < i && $ < s && !((k = m.get(t[$])) == null || k !== d + v); )
            v++;
          if (v > d - c) {
            const T = t[l];
            for (; c < d; )
              e.insertBefore(n[c++], T);
          } else
            e.replaceChild(n[c++], t[l++]);
        } else
          l++;
      else
        t[l++].remove();
    }
  }
}
const F0 = "_$DX_DELEGATE";
function X9(e, t, n, r = {}) {
  let i;
  return de((s) => {
    i = s, t === document ? e() : _(t, e(), t.firstChild ? null : void 0, n);
  }, r.owner), () => {
    i(), t.textContent = "";
  };
}
function p(e, t, n) {
  const r = document.createElement("template");
  r.innerHTML = e;
  let i = r.content.firstChild;
  return n && (i = i.firstChild), i;
}
function _1(e, t = window.document) {
  const n = t[F0] || (t[F0] = /* @__PURE__ */ new Set());
  for (let r = 0, i = e.length; r < i; r++) {
    const s = e[r];
    n.has(s) || (n.add(s), t.addEventListener(s, J9));
  }
}
function y1(e, t, n) {
  n == null ? e.removeAttribute(t) : e.setAttribute(t, n);
}
function z1(e, t) {
  t == null ? e.removeAttribute("class") : e.className = t;
}
function P1(e, t, n, r) {
  if (r)
    Array.isArray(n) ? (e[`$$${t}`] = n[0], e[`$$${t}Data`] = n[1]) : e[`$$${t}`] = n;
  else if (Array.isArray(n)) {
    const i = n[0];
    e.addEventListener(t, n[0] = (s) => i.call(e, n[1], s));
  } else
    e.addEventListener(t, n);
}
function Q1(e, t, n) {
  if (!t)
    return n ? y1(e, "style") : t;
  const r = e.style;
  if (typeof t == "string")
    return r.cssText = t;
  typeof n == "string" && (r.cssText = n = void 0), n || (n = {}), t || (t = {});
  let i, s;
  for (s in n)
    t[s] == null && r.removeProperty(s), delete n[s];
  for (s in t)
    i = t[s], i !== n[s] && (r.setProperty(s, i), n[s] = i);
  return n;
}
function K1(e, t, n) {
  return M1(() => e(t, n));
}
function _(e, t, n, r) {
  if (n !== void 0 && !r && (r = []), typeof t != "function")
    return we(e, t, r, n);
  Q((i) => we(e, t(), i, n), r);
}
function J9(e) {
  const t = `$$${e.type}`;
  let n = e.composedPath && e.composedPath()[0] || e.target;
  for (e.target !== n && Object.defineProperty(e, "target", {
    configurable: !0,
    value: n
  }), Object.defineProperty(e, "currentTarget", {
    configurable: !0,
    get() {
      return n || document;
    }
  }), s1.registry && !s1.done && (s1.done = !0, document.querySelectorAll("[id^=pl-]").forEach((r) => {
    for (; r && r.nodeType !== 8 && r.nodeValue !== "pl-" + e; ) {
      let i = r.nextSibling;
      r.remove(), r = i;
    }
    r && r.remove();
  })); n; ) {
    const r = n[t];
    if (r && !n.disabled) {
      const i = n[`${t}Data`];
      if (i !== void 0 ? r.call(n, i, e) : r.call(n, e), e.cancelBubble)
        return;
    }
    n = n._$host || n.parentNode || n.host;
  }
}
function we(e, t, n, r, i) {
  for (s1.context && !n && (n = [...e.childNodes]); typeof n == "function"; )
    n = n();
  if (t === n)
    return n;
  const s = typeof t, l = r !== void 0;
  if (e = l && n[0] && n[0].parentNode || e, s === "string" || s === "number") {
    if (s1.context)
      return n;
    if (s === "number" && (t = t.toString()), l) {
      let c = n[0];
      c && c.nodeType === 3 ? c.data = t : c = document.createTextNode(t), n = V1(e, n, r, c);
    } else
      n !== "" && typeof n == "string" ? n = e.firstChild.data = t : n = e.textContent = t;
  } else if (t == null || s === "boolean") {
    if (s1.context)
      return n;
    n = V1(e, n, r);
  } else {
    if (s === "function")
      return Q(() => {
        let c = t();
        for (; typeof c == "function"; )
          c = c();
        n = we(e, c, n, r);
      }), () => n;
    if (Array.isArray(t)) {
      const c = [], h = n && Array.isArray(n);
      if (qe(c, t, n, i))
        return Q(() => n = we(e, c, n, r, !0)), () => n;
      if (s1.context) {
        if (!c.length)
          return n;
        for (let m = 0; m < c.length; m++)
          if (c[m].parentNode)
            return n = c;
      }
      if (c.length === 0) {
        if (n = V1(e, n, r), l)
          return n;
      } else
        h ? n.length === 0 ? K0(e, c, r) : W9(e, n, c) : (n && V1(e), K0(e, c));
      n = c;
    } else if (t instanceof Node) {
      if (s1.context && t.parentNode)
        return n = l ? [t] : t;
      if (Array.isArray(n)) {
        if (l)
          return n = V1(e, n, r, t);
        V1(e, n, null, t);
      } else
        n == null || n === "" || !e.firstChild ? e.appendChild(t) : e.replaceChild(t, e.firstChild);
      n = t;
    }
  }
  return n;
}
function qe(e, t, n, r) {
  let i = !1;
  for (let s = 0, l = t.length; s < l; s++) {
    let c = t[s], h = n && n[s];
    if (c instanceof Node)
      e.push(c);
    else if (!(c == null || c === !0 || c === !1))
      if (Array.isArray(c))
        i = qe(e, c, h) || i;
      else if (typeof c == "function")
        if (r) {
          for (; typeof c == "function"; )
            c = c();
          i = qe(e, Array.isArray(c) ? c : [c], Array.isArray(h) ? h : [h]) || i;
        } else
          e.push(c), i = !0;
      else {
        const m = String(c);
        h && h.nodeType === 3 && h.data === m ? e.push(h) : e.push(document.createTextNode(m));
      }
  }
  return i;
}
function K0(e, t, n = null) {
  for (let r = 0, i = t.length; r < i; r++)
    e.insertBefore(t[r], n);
}
function V1(e, t, n, r) {
  if (n === void 0)
    return e.textContent = "";
  const i = r || document.createTextNode("");
  if (t.length) {
    let s = !1;
    for (let l = t.length - 1; l >= 0; l--) {
      const c = t[l];
      if (i !== c) {
        const h = c.parentNode === e;
        !s && !l ? h ? e.replaceChild(i, c) : e.insertBefore(i, n) : h && c.remove();
      } else
        s = !0;
    }
  } else
    e.insertBefore(i, n);
  return [i];
}
const e5 = "http://www.w3.org/2000/svg";
function t5(e, t = !1) {
  return t ? document.createElementNS(e5, e) : document.createElement(e);
}
function n5(e) {
  const {
    useShadow: t
  } = e, n = document.createTextNode(""), r = e.mount || document.body;
  function i() {
    if (s1.context) {
      const [s, l] = w(!1);
      return queueMicrotask(() => l(!0)), () => s() && e.children;
    } else
      return () => e.children;
  }
  if (r instanceof HTMLHeadElement) {
    const [s, l] = w(!1), c = () => l(!0);
    de((h) => _(r, () => s() ? h() : i()(), null)), j1(() => {
      s1.context ? queueMicrotask(c) : c();
    });
  } else {
    const s = t5(e.isSVG ? "g" : "div", e.isSVG), l = t && s.attachShadow ? s.attachShadow({
      mode: "open"
    }) : s;
    Object.defineProperty(s, "_$host", {
      get() {
        return n.parentNode;
      },
      configurable: !0
    }), _(l, i()), r.appendChild(s), e.ref && e.ref(s), j1(() => r.removeChild(s));
  }
  return n;
}
var _e = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
function Ot(e) {
  return e && e.__esModule && Object.prototype.hasOwnProperty.call(e, "default") ? e.default : e;
}
var r5 = typeof _e == "object" && _e && _e.Object === Object && _e, Nt = r5, i5 = Nt, a5 = typeof self == "object" && self && self.Object === Object && self, s5 = i5 || a5 || Function("return this")(), k1 = s5, o5 = k1, c5 = o5.Symbol, Ne = c5, j0 = Ne, Dt = Object.prototype, l5 = Dt.hasOwnProperty, u5 = Dt.toString, le = j0 ? j0.toStringTag : void 0;
function d5(e) {
  var t = l5.call(e, le), n = e[le];
  try {
    e[le] = void 0;
    var r = !0;
  } catch {
  }
  var i = u5.call(e);
  return r && (t ? e[le] = n : delete e[le]), i;
}
var h5 = d5, C5 = Object.prototype, y5 = C5.toString;
function f5(e) {
  return y5.call(e);
}
var g5 = f5, z0 = Ne, m5 = h5, p5 = g5, v5 = "[object Null]", _5 = "[object Undefined]", Q0 = z0 ? z0.toStringTag : void 0;
function $5(e) {
  return e == null ? e === void 0 ? _5 : v5 : Q0 && Q0 in Object(e) ? m5(e) : p5(e);
}
var he = $5;
function b5(e) {
  var t = typeof e;
  return e != null && (t == "object" || t == "function");
}
var G1 = b5, L5 = he, x5 = G1, k5 = "[object AsyncFunction]", A5 = "[object Function]", w5 = "[object GeneratorFunction]", M5 = "[object Proxy]";
function S5(e) {
  if (!x5(e))
    return !1;
  var t = L5(e);
  return t == A5 || t == w5 || t == k5 || t == M5;
}
var Et = S5, T5 = k1, P5 = T5["__core-js_shared__"], I5 = P5, Ve = I5, Z0 = function() {
  var e = /[^.]+$/.exec(Ve && Ve.keys && Ve.keys.IE_PROTO || "");
  return e ? "Symbol(src)_1." + e : "";
}();
function O5(e) {
  return !!Z0 && Z0 in e;
}
var N5 = O5, D5 = Function.prototype, E5 = D5.toString;
function B5(e) {
  if (e != null) {
    try {
      return E5.call(e);
    } catch {
    }
    try {
      return e + "";
    } catch {
    }
  }
  return "";
}
var Bt = B5, U5 = Et, F5 = N5, K5 = G1, j5 = Bt, z5 = /[\\^$.*+?()[\]{}|]/g, Q5 = /^\[object .+?Constructor\]$/, Z5 = Function.prototype, R5 = Object.prototype, H5 = Z5.toString, V5 = R5.hasOwnProperty, Y5 = RegExp(
  "^" + H5.call(V5).replace(z5, "\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") + "$"
);
function G5(e) {
  if (!K5(e) || F5(e))
    return !1;
  var t = U5(e) ? Y5 : Q5;
  return t.test(j5(e));
}
var q5 = G5;
function W5(e, t) {
  return e == null ? void 0 : e[t];
}
var X5 = W5, J5 = q5, e6 = X5;
function t6(e, t) {
  var n = e6(e, t);
  return J5(n) ? n : void 0;
}
var Z1 = t6, n6 = Z1, r6 = function() {
  try {
    var e = n6(Object, "defineProperty");
    return e({}, "", {}), e;
  } catch {
  }
}(), i6 = r6, R0 = i6;
function a6(e, t, n) {
  t == "__proto__" && R0 ? R0(e, t, {
    configurable: !0,
    enumerable: !0,
    value: n,
    writable: !0
  }) : e[t] = n;
}
var Ut = a6;
function s6(e, t) {
  return e === t || e !== e && t !== t;
}
var Ft = s6, o6 = Ut, c6 = Ft, l6 = Object.prototype, u6 = l6.hasOwnProperty;
function d6(e, t, n) {
  var r = e[t];
  (!(u6.call(e, t) && c6(r, n)) || n === void 0 && !(t in e)) && o6(e, t, n);
}
var a0 = d6, h6 = Array.isArray, q1 = h6;
function C6(e) {
  return e != null && typeof e == "object";
}
var W1 = C6, y6 = he, f6 = W1, g6 = "[object Symbol]";
function m6(e) {
  return typeof e == "symbol" || f6(e) && y6(e) == g6;
}
var s0 = m6, p6 = q1, v6 = s0, _6 = /\.|\[(?:[^[\]]*|(["'])(?:(?!\1)[^\\]|\\.)*?\1)\]/, $6 = /^\w*$/;
function b6(e, t) {
  if (p6(e))
    return !1;
  var n = typeof e;
  return n == "number" || n == "symbol" || n == "boolean" || e == null || v6(e) ? !0 : $6.test(e) || !_6.test(e) || t != null && e in Object(t);
}
var L6 = b6, x6 = Z1, k6 = x6(Object, "create"), De = k6, H0 = De;
function A6() {
  this.__data__ = H0 ? H0(null) : {}, this.size = 0;
}
var w6 = A6;
function M6(e) {
  var t = this.has(e) && delete this.__data__[e];
  return this.size -= t ? 1 : 0, t;
}
var S6 = M6, T6 = De, P6 = "__lodash_hash_undefined__", I6 = Object.prototype, O6 = I6.hasOwnProperty;
function N6(e) {
  var t = this.__data__;
  if (T6) {
    var n = t[e];
    return n === P6 ? void 0 : n;
  }
  return O6.call(t, e) ? t[e] : void 0;
}
var D6 = N6, E6 = De, B6 = Object.prototype, U6 = B6.hasOwnProperty;
function F6(e) {
  var t = this.__data__;
  return E6 ? t[e] !== void 0 : U6.call(t, e);
}
var K6 = F6, j6 = De, z6 = "__lodash_hash_undefined__";
function Q6(e, t) {
  var n = this.__data__;
  return this.size += this.has(e) ? 0 : 1, n[e] = j6 && t === void 0 ? z6 : t, this;
}
var Z6 = Q6, R6 = w6, H6 = S6, V6 = D6, Y6 = K6, G6 = Z6;
function X1(e) {
  var t = -1, n = e == null ? 0 : e.length;
  for (this.clear(); ++t < n; ) {
    var r = e[t];
    this.set(r[0], r[1]);
  }
}
X1.prototype.clear = R6;
X1.prototype.delete = H6;
X1.prototype.get = V6;
X1.prototype.has = Y6;
X1.prototype.set = G6;
var q6 = X1;
function W6() {
  this.__data__ = [], this.size = 0;
}
var X6 = W6, J6 = Ft;
function e2(e, t) {
  for (var n = e.length; n--; )
    if (J6(e[n][0], t))
      return n;
  return -1;
}
var Ee = e2, t2 = Ee, n2 = Array.prototype, r2 = n2.splice;
function i2(e) {
  var t = this.__data__, n = t2(t, e);
  if (n < 0)
    return !1;
  var r = t.length - 1;
  return n == r ? t.pop() : r2.call(t, n, 1), --this.size, !0;
}
var a2 = i2, s2 = Ee;
function o2(e) {
  var t = this.__data__, n = s2(t, e);
  return n < 0 ? void 0 : t[n][1];
}
var c2 = o2, l2 = Ee;
function u2(e) {
  return l2(this.__data__, e) > -1;
}
var d2 = u2, h2 = Ee;
function C2(e, t) {
  var n = this.__data__, r = h2(n, e);
  return r < 0 ? (++this.size, n.push([e, t])) : n[r][1] = t, this;
}
var y2 = C2, f2 = X6, g2 = a2, m2 = c2, p2 = d2, v2 = y2;
function J1(e) {
  var t = -1, n = e == null ? 0 : e.length;
  for (this.clear(); ++t < n; ) {
    var r = e[t];
    this.set(r[0], r[1]);
  }
}
J1.prototype.clear = f2;
J1.prototype.delete = g2;
J1.prototype.get = m2;
J1.prototype.has = p2;
J1.prototype.set = v2;
var Be = J1, _2 = Z1, $2 = k1, b2 = _2($2, "Map"), o0 = b2, V0 = q6, L2 = Be, x2 = o0;
function k2() {
  this.size = 0, this.__data__ = {
    hash: new V0(),
    map: new (x2 || L2)(),
    string: new V0()
  };
}
var A2 = k2;
function w2(e) {
  var t = typeof e;
  return t == "string" || t == "number" || t == "symbol" || t == "boolean" ? e !== "__proto__" : e === null;
}
var M2 = w2, S2 = M2;
function T2(e, t) {
  var n = e.__data__;
  return S2(t) ? n[typeof t == "string" ? "string" : "hash"] : n.map;
}
var Ue = T2, P2 = Ue;
function I2(e) {
  var t = P2(this, e).delete(e);
  return this.size -= t ? 1 : 0, t;
}
var O2 = I2, N2 = Ue;
function D2(e) {
  return N2(this, e).get(e);
}
var E2 = D2, B2 = Ue;
function U2(e) {
  return B2(this, e).has(e);
}
var F2 = U2, K2 = Ue;
function j2(e, t) {
  var n = K2(this, e), r = n.size;
  return n.set(e, t), this.size += n.size == r ? 0 : 1, this;
}
var z2 = j2, Q2 = A2, Z2 = O2, R2 = E2, H2 = F2, V2 = z2;
function ee(e) {
  var t = -1, n = e == null ? 0 : e.length;
  for (this.clear(); ++t < n; ) {
    var r = e[t];
    this.set(r[0], r[1]);
  }
}
ee.prototype.clear = Q2;
ee.prototype.delete = Z2;
ee.prototype.get = R2;
ee.prototype.has = H2;
ee.prototype.set = V2;
var Kt = ee, jt = Kt, Y2 = "Expected a function";
function c0(e, t) {
  if (typeof e != "function" || t != null && typeof t != "function")
    throw new TypeError(Y2);
  var n = function() {
    var r = arguments, i = t ? t.apply(this, r) : r[0], s = n.cache;
    if (s.has(i))
      return s.get(i);
    var l = e.apply(this, r);
    return n.cache = s.set(i, l) || s, l;
  };
  return n.cache = new (c0.Cache || jt)(), n;
}
c0.Cache = jt;
var G2 = c0, q2 = G2, W2 = 500;
function X2(e) {
  var t = q2(e, function(r) {
    return n.size === W2 && n.clear(), r;
  }), n = t.cache;
  return t;
}
var J2 = X2, e3 = J2, t3 = /[^.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|$))/g, n3 = /\\(\\)?/g, r3 = e3(function(e) {
  var t = [];
  return e.charCodeAt(0) === 46 && t.push(""), e.replace(t3, function(n, r, i, s) {
    t.push(i ? s.replace(n3, "$1") : r || n);
  }), t;
}), i3 = r3;
function a3(e, t) {
  for (var n = -1, r = e == null ? 0 : e.length, i = Array(r); ++n < r; )
    i[n] = t(e[n], n, e);
  return i;
}
var s3 = a3, Y0 = Ne, o3 = s3, c3 = q1, l3 = s0, u3 = 1 / 0, G0 = Y0 ? Y0.prototype : void 0, q0 = G0 ? G0.toString : void 0;
function zt(e) {
  if (typeof e == "string")
    return e;
  if (c3(e))
    return o3(e, zt) + "";
  if (l3(e))
    return q0 ? q0.call(e) : "";
  var t = e + "";
  return t == "0" && 1 / e == -u3 ? "-0" : t;
}
var d3 = zt, h3 = d3;
function C3(e) {
  return e == null ? "" : h3(e);
}
var y3 = C3, f3 = q1, g3 = L6, m3 = i3, p3 = y3;
function v3(e, t) {
  return f3(e) ? e : g3(e, t) ? [e] : m3(p3(e));
}
var _3 = v3, $3 = 9007199254740991, b3 = /^(?:0|[1-9]\d*)$/;
function L3(e, t) {
  var n = typeof e;
  return t = t ?? $3, !!t && (n == "number" || n != "symbol" && b3.test(e)) && e > -1 && e % 1 == 0 && e < t;
}
var Qt = L3, x3 = s0, k3 = 1 / 0;
function A3(e) {
  if (typeof e == "string" || x3(e))
    return e;
  var t = e + "";
  return t == "0" && 1 / e == -k3 ? "-0" : t;
}
var w3 = A3, M3 = a0, S3 = _3, T3 = Qt, W0 = G1, P3 = w3;
function I3(e, t, n, r) {
  if (!W0(e))
    return e;
  t = S3(t, e);
  for (var i = -1, s = t.length, l = s - 1, c = e; c != null && ++i < s; ) {
    var h = P3(t[i]), m = n;
    if (h === "__proto__" || h === "constructor" || h === "prototype")
      return e;
    if (i != l) {
      var d = c[h];
      m = r ? r(d, h, c) : void 0, m === void 0 && (m = W0(d) ? d : T3(t[i + 1]) ? [] : {});
    }
    M3(c, h, m), c = c[h];
  }
  return e;
}
var O3 = I3, N3 = O3;
function D3(e, t, n) {
  return e == null ? e : N3(e, t, n);
}
var E3 = D3;
const We = /* @__PURE__ */ Ot(E3);
var B3 = Be;
function U3() {
  this.__data__ = new B3(), this.size = 0;
}
var F3 = U3;
function K3(e) {
  var t = this.__data__, n = t.delete(e);
  return this.size = t.size, n;
}
var j3 = K3;
function z3(e) {
  return this.__data__.get(e);
}
var Q3 = z3;
function Z3(e) {
  return this.__data__.has(e);
}
var R3 = Z3, H3 = Be, V3 = o0, Y3 = Kt, G3 = 200;
function q3(e, t) {
  var n = this.__data__;
  if (n instanceof H3) {
    var r = n.__data__;
    if (!V3 || r.length < G3 - 1)
      return r.push([e, t]), this.size = ++n.size, this;
    n = this.__data__ = new Y3(r);
  }
  return n.set(e, t), this.size = n.size, this;
}
var W3 = q3, X3 = Be, J3 = F3, e8 = j3, t8 = Q3, n8 = R3, r8 = W3;
function te(e) {
  var t = this.__data__ = new X3(e);
  this.size = t.size;
}
te.prototype.clear = J3;
te.prototype.delete = e8;
te.prototype.get = t8;
te.prototype.has = n8;
te.prototype.set = r8;
var i8 = te;
function a8(e, t) {
  for (var n = -1, r = e == null ? 0 : e.length; ++n < r && t(e[n], n, e) !== !1; )
    ;
  return e;
}
var s8 = a8, o8 = a0, c8 = Ut;
function l8(e, t, n, r) {
  var i = !n;
  n || (n = {});
  for (var s = -1, l = t.length; ++s < l; ) {
    var c = t[s], h = r ? r(n[c], e[c], c, n, e) : void 0;
    h === void 0 && (h = e[c]), i ? c8(n, c, h) : o8(n, c, h);
  }
  return n;
}
var Fe = l8;
function u8(e, t) {
  for (var n = -1, r = Array(e); ++n < e; )
    r[n] = t(n);
  return r;
}
var d8 = u8, h8 = he, C8 = W1, y8 = "[object Arguments]";
function f8(e) {
  return C8(e) && h8(e) == y8;
}
var g8 = f8, X0 = g8, m8 = W1, Zt = Object.prototype, p8 = Zt.hasOwnProperty, v8 = Zt.propertyIsEnumerable, _8 = X0(function() {
  return arguments;
}()) ? X0 : function(e) {
  return m8(e) && p8.call(e, "callee") && !v8.call(e, "callee");
}, $8 = _8, Me = { exports: {} };
function b8() {
  return !1;
}
var L8 = b8;
Me.exports;
(function(e, t) {
  var n = k1, r = L8, i = t && !t.nodeType && t, s = i && !0 && e && !e.nodeType && e, l = s && s.exports === i, c = l ? n.Buffer : void 0, h = c ? c.isBuffer : void 0, m = h || r;
  e.exports = m;
})(Me, Me.exports);
var Rt = Me.exports, x8 = 9007199254740991;
function k8(e) {
  return typeof e == "number" && e > -1 && e % 1 == 0 && e <= x8;
}
var Ht = k8, A8 = he, w8 = Ht, M8 = W1, S8 = "[object Arguments]", T8 = "[object Array]", P8 = "[object Boolean]", I8 = "[object Date]", O8 = "[object Error]", N8 = "[object Function]", D8 = "[object Map]", E8 = "[object Number]", B8 = "[object Object]", U8 = "[object RegExp]", F8 = "[object Set]", K8 = "[object String]", j8 = "[object WeakMap]", z8 = "[object ArrayBuffer]", Q8 = "[object DataView]", Z8 = "[object Float32Array]", R8 = "[object Float64Array]", H8 = "[object Int8Array]", V8 = "[object Int16Array]", Y8 = "[object Int32Array]", G8 = "[object Uint8Array]", q8 = "[object Uint8ClampedArray]", W8 = "[object Uint16Array]", X8 = "[object Uint32Array]", J = {};
J[Z8] = J[R8] = J[H8] = J[V8] = J[Y8] = J[G8] = J[q8] = J[W8] = J[X8] = !0;
J[S8] = J[T8] = J[z8] = J[P8] = J[Q8] = J[I8] = J[O8] = J[N8] = J[D8] = J[E8] = J[B8] = J[U8] = J[F8] = J[K8] = J[j8] = !1;
function J8(e) {
  return M8(e) && w8(e.length) && !!J[A8(e)];
}
var e7 = J8;
function t7(e) {
  return function(t) {
    return e(t);
  };
}
var l0 = t7, Se = { exports: {} };
Se.exports;
(function(e, t) {
  var n = Nt, r = t && !t.nodeType && t, i = r && !0 && e && !e.nodeType && e, s = i && i.exports === r, l = s && n.process, c = function() {
    try {
      var h = i && i.require && i.require("util").types;
      return h || l && l.binding && l.binding("util");
    } catch {
    }
  }();
  e.exports = c;
})(Se, Se.exports);
var u0 = Se.exports, n7 = e7, r7 = l0, J0 = u0, et = J0 && J0.isTypedArray, i7 = et ? r7(et) : n7, a7 = i7, s7 = d8, o7 = $8, c7 = q1, l7 = Rt, u7 = Qt, d7 = a7, h7 = Object.prototype, C7 = h7.hasOwnProperty;
function y7(e, t) {
  var n = c7(e), r = !n && o7(e), i = !n && !r && l7(e), s = !n && !r && !i && d7(e), l = n || r || i || s, c = l ? s7(e.length, String) : [], h = c.length;
  for (var m in e)
    (t || C7.call(e, m)) && !(l && // Safari 9 has enumerable `arguments.length` in strict mode.
    (m == "length" || // Node.js 0.10 has enumerable non-index properties on buffers.
    i && (m == "offset" || m == "parent") || // PhantomJS 2 has enumerable non-index properties on typed arrays.
    s && (m == "buffer" || m == "byteLength" || m == "byteOffset") || // Skip index properties.
    u7(m, h))) && c.push(m);
  return c;
}
var Vt = y7, f7 = Object.prototype;
function g7(e) {
  var t = e && e.constructor, n = typeof t == "function" && t.prototype || f7;
  return e === n;
}
var d0 = g7;
function m7(e, t) {
  return function(n) {
    return e(t(n));
  };
}
var Yt = m7, p7 = Yt, v7 = p7(Object.keys, Object), _7 = v7, $7 = d0, b7 = _7, L7 = Object.prototype, x7 = L7.hasOwnProperty;
function k7(e) {
  if (!$7(e))
    return b7(e);
  var t = [];
  for (var n in Object(e))
    x7.call(e, n) && n != "constructor" && t.push(n);
  return t;
}
var A7 = k7, w7 = Et, M7 = Ht;
function S7(e) {
  return e != null && M7(e.length) && !w7(e);
}
var Gt = S7, T7 = Vt, P7 = A7, I7 = Gt;
function O7(e) {
  return I7(e) ? T7(e) : P7(e);
}
var h0 = O7, N7 = Fe, D7 = h0;
function E7(e, t) {
  return e && N7(t, D7(t), e);
}
var B7 = E7;
function U7(e) {
  var t = [];
  if (e != null)
    for (var n in Object(e))
      t.push(n);
  return t;
}
var F7 = U7, K7 = G1, j7 = d0, z7 = F7, Q7 = Object.prototype, Z7 = Q7.hasOwnProperty;
function R7(e) {
  if (!K7(e))
    return z7(e);
  var t = j7(e), n = [];
  for (var r in e)
    r == "constructor" && (t || !Z7.call(e, r)) || n.push(r);
  return n;
}
var H7 = R7, V7 = Vt, Y7 = H7, G7 = Gt;
function q7(e) {
  return G7(e) ? V7(e, !0) : Y7(e);
}
var C0 = q7, W7 = Fe, X7 = C0;
function J7(e, t) {
  return e && W7(t, X7(t), e);
}
var en = J7, Te = { exports: {} };
Te.exports;
(function(e, t) {
  var n = k1, r = t && !t.nodeType && t, i = r && !0 && e && !e.nodeType && e, s = i && i.exports === r, l = s ? n.Buffer : void 0, c = l ? l.allocUnsafe : void 0;
  function h(m, d) {
    if (d)
      return m.slice();
    var $ = m.length, v = c ? c($) : new m.constructor($);
    return m.copy(v), v;
  }
  e.exports = h;
})(Te, Te.exports);
var tn = Te.exports;
function nn(e, t) {
  var n = -1, r = e.length;
  for (t || (t = Array(r)); ++n < r; )
    t[n] = e[n];
  return t;
}
var rn = nn;
function an(e, t) {
  for (var n = -1, r = e == null ? 0 : e.length, i = 0, s = []; ++n < r; ) {
    var l = e[n];
    t(l, n, e) && (s[i++] = l);
  }
  return s;
}
var sn = an;
function on() {
  return [];
}
var qt = on, cn = sn, ln = qt, un = Object.prototype, dn = un.propertyIsEnumerable, tt = Object.getOwnPropertySymbols, hn = tt ? function(e) {
  return e == null ? [] : (e = Object(e), cn(tt(e), function(t) {
    return dn.call(e, t);
  }));
} : ln, y0 = hn, Cn = Fe, yn = y0;
function fn(e, t) {
  return Cn(e, yn(e), t);
}
var gn = fn;
function mn(e, t) {
  for (var n = -1, r = t.length, i = e.length; ++n < r; )
    e[i + n] = t[n];
  return e;
}
var Wt = mn, pn = Yt, vn = pn(Object.getPrototypeOf, Object), Xt = vn, _n = Wt, $n = Xt, bn = y0, Ln = qt, xn = Object.getOwnPropertySymbols, kn = xn ? function(e) {
  for (var t = []; e; )
    _n(t, bn(e)), e = $n(e);
  return t;
} : Ln, Jt = kn, An = Fe, wn = Jt;
function Mn(e, t) {
  return An(e, wn(e), t);
}
var Sn = Mn, Tn = Wt, Pn = q1;
function In(e, t, n) {
  var r = t(e);
  return Pn(e) ? r : Tn(r, n(e));
}
var e9 = In, On = e9, Nn = y0, Dn = h0;
function En(e) {
  return On(e, Dn, Nn);
}
var Bn = En, Un = e9, Fn = Jt, Kn = C0;
function jn(e) {
  return Un(e, Kn, Fn);
}
var zn = jn, Qn = Z1, Zn = k1, Rn = Qn(Zn, "DataView"), Hn = Rn, Vn = Z1, Yn = k1, Gn = Vn(Yn, "Promise"), qn = Gn, Wn = Z1, Xn = k1, Jn = Wn(Xn, "Set"), er = Jn, tr = Z1, nr = k1, rr = tr(nr, "WeakMap"), ir = rr, Xe = Hn, Je = o0, e0 = qn, t0 = er, n0 = ir, t9 = he, ne = Bt, nt = "[object Map]", ar = "[object Object]", rt = "[object Promise]", it = "[object Set]", at = "[object WeakMap]", st = "[object DataView]", sr = ne(Xe), or = ne(Je), cr = ne(e0), lr = ne(t0), ur = ne(n0), U1 = t9;
(Xe && U1(new Xe(new ArrayBuffer(1))) != st || Je && U1(new Je()) != nt || e0 && U1(e0.resolve()) != rt || t0 && U1(new t0()) != it || n0 && U1(new n0()) != at) && (U1 = function(e) {
  var t = t9(e), n = t == ar ? e.constructor : void 0, r = n ? ne(n) : "";
  if (r)
    switch (r) {
      case sr:
        return st;
      case or:
        return nt;
      case cr:
        return rt;
      case lr:
        return it;
      case ur:
        return at;
    }
  return t;
});
var f0 = U1, dr = Object.prototype, hr = dr.hasOwnProperty;
function Cr(e) {
  var t = e.length, n = new e.constructor(t);
  return t && typeof e[0] == "string" && hr.call(e, "index") && (n.index = e.index, n.input = e.input), n;
}
var yr = Cr, fr = k1, gr = fr.Uint8Array, mr = gr, ot = mr;
function pr(e) {
  var t = new e.constructor(e.byteLength);
  return new ot(t).set(new ot(e)), t;
}
var g0 = pr, vr = g0;
function _r(e, t) {
  var n = t ? vr(e.buffer) : e.buffer;
  return new e.constructor(n, e.byteOffset, e.byteLength);
}
var $r = _r, br = /\w*$/;
function Lr(e) {
  var t = new e.constructor(e.source, br.exec(e));
  return t.lastIndex = e.lastIndex, t;
}
var xr = Lr, ct = Ne, lt = ct ? ct.prototype : void 0, ut = lt ? lt.valueOf : void 0;
function kr(e) {
  return ut ? Object(ut.call(e)) : {};
}
var Ar = kr, wr = g0;
function Mr(e, t) {
  var n = t ? wr(e.buffer) : e.buffer;
  return new e.constructor(n, e.byteOffset, e.length);
}
var Sr = Mr, Tr = g0, Pr = $r, Ir = xr, Or = Ar, Nr = Sr, Dr = "[object Boolean]", Er = "[object Date]", Br = "[object Map]", Ur = "[object Number]", Fr = "[object RegExp]", Kr = "[object Set]", jr = "[object String]", zr = "[object Symbol]", Qr = "[object ArrayBuffer]", Zr = "[object DataView]", Rr = "[object Float32Array]", Hr = "[object Float64Array]", Vr = "[object Int8Array]", Yr = "[object Int16Array]", Gr = "[object Int32Array]", qr = "[object Uint8Array]", Wr = "[object Uint8ClampedArray]", Xr = "[object Uint16Array]", Jr = "[object Uint32Array]";
function e4(e, t, n) {
  var r = e.constructor;
  switch (t) {
    case Qr:
      return Tr(e);
    case Dr:
    case Er:
      return new r(+e);
    case Zr:
      return Pr(e, n);
    case Rr:
    case Hr:
    case Vr:
    case Yr:
    case Gr:
    case qr:
    case Wr:
    case Xr:
    case Jr:
      return Nr(e, n);
    case Br:
      return new r();
    case Ur:
    case jr:
      return new r(e);
    case Fr:
      return Ir(e);
    case Kr:
      return new r();
    case zr:
      return Or(e);
  }
}
var t4 = e4, n4 = G1, dt = Object.create, r4 = function() {
  function e() {
  }
  return function(t) {
    if (!n4(t))
      return {};
    if (dt)
      return dt(t);
    e.prototype = t;
    var n = new e();
    return e.prototype = void 0, n;
  };
}(), i4 = r4, a4 = i4, s4 = Xt, o4 = d0;
function c4(e) {
  return typeof e.constructor == "function" && !o4(e) ? a4(s4(e)) : {};
}
var l4 = c4, u4 = f0, d4 = W1, h4 = "[object Map]";
function C4(e) {
  return d4(e) && u4(e) == h4;
}
var y4 = C4, f4 = y4, g4 = l0, ht = u0, Ct = ht && ht.isMap, m4 = Ct ? g4(Ct) : f4, p4 = m4, v4 = f0, _4 = W1, $4 = "[object Set]";
function b4(e) {
  return _4(e) && v4(e) == $4;
}
var L4 = b4, x4 = L4, k4 = l0, yt = u0, ft = yt && yt.isSet, A4 = ft ? k4(ft) : x4, w4 = A4, M4 = i8, S4 = s8, T4 = a0, P4 = B7, I4 = en, O4 = tn, N4 = rn, D4 = gn, E4 = Sn, B4 = Bn, U4 = zn, F4 = f0, K4 = yr, j4 = t4, z4 = l4, Q4 = q1, Z4 = Rt, R4 = p4, H4 = G1, V4 = w4, Y4 = h0, G4 = C0, q4 = 1, W4 = 2, X4 = 4, n9 = "[object Arguments]", J4 = "[object Array]", ei = "[object Boolean]", ti = "[object Date]", ni = "[object Error]", r9 = "[object Function]", ri = "[object GeneratorFunction]", ii = "[object Map]", ai = "[object Number]", i9 = "[object Object]", si = "[object RegExp]", oi = "[object Set]", ci = "[object String]", li = "[object Symbol]", ui = "[object WeakMap]", di = "[object ArrayBuffer]", hi = "[object DataView]", Ci = "[object Float32Array]", yi = "[object Float64Array]", fi = "[object Int8Array]", gi = "[object Int16Array]", mi = "[object Int32Array]", pi = "[object Uint8Array]", vi = "[object Uint8ClampedArray]", _i = "[object Uint16Array]", $i = "[object Uint32Array]", W = {};
W[n9] = W[J4] = W[di] = W[hi] = W[ei] = W[ti] = W[Ci] = W[yi] = W[fi] = W[gi] = W[mi] = W[ii] = W[ai] = W[i9] = W[si] = W[oi] = W[ci] = W[li] = W[pi] = W[vi] = W[_i] = W[$i] = !0;
W[ni] = W[r9] = W[ui] = !1;
function be(e, t, n, r, i, s) {
  var l, c = t & q4, h = t & W4, m = t & X4;
  if (n && (l = i ? n(e, r, i, s) : n(e)), l !== void 0)
    return l;
  if (!H4(e))
    return e;
  var d = Q4(e);
  if (d) {
    if (l = K4(e), !c)
      return N4(e, l);
  } else {
    var $ = F4(e), v = $ == r9 || $ == ri;
    if (Z4(e))
      return O4(e, c);
    if ($ == i9 || $ == n9 || v && !i) {
      if (l = h || v ? {} : z4(e), !c)
        return h ? E4(e, I4(l, e)) : D4(e, P4(l, e));
    } else {
      if (!W[$])
        return i ? e : {};
      l = j4(e, $, c);
    }
  }
  s || (s = new M4());
  var k = s.get(e);
  if (k)
    return k;
  s.set(e, l), V4(e) ? e.forEach(function(I) {
    l.add(be(I, t, n, I, e, s));
  }) : R4(e) && e.forEach(function(I, F) {
    l.set(F, be(I, t, n, F, e, s));
  });
  var T = m ? h ? U4 : B4 : h ? G4 : Y4, D = d ? void 0 : T(e);
  return S4(D || e, function(I, F) {
    D && (F = I, I = e[F]), T4(l, F, be(I, t, n, F, e, s));
  }), l;
}
var bi = be, Li = bi, xi = 1, ki = 4;
function Ai(e) {
  return Li(e, xi | ki);
}
var wi = Ai;
const Mi = /* @__PURE__ */ Ot(wi), Si = /* @__PURE__ */ p("<button></button>"), Ti = (e) => (() => {
  const t = Si.cloneNode(!0);
  return P1(t, "click", e.onClick, !0), _(t, () => e.children), Q((n) => {
    const r = e.style, i = `klinecharts-pro-button ${e.type ?? "confirm"} ${e.class ?? ""}`;
    return n._v$ = Q1(t, r, n._v$), i !== n._v$2 && z1(t, n._v$2 = i), n;
  }, {
    _v$: void 0,
    _v$2: void 0
  }), t;
})();
_1(["click"]);
const Pi = /* @__PURE__ */ p('<svg viewBox="0 0 1024 1024" class="icon"><path d="M810.666667 128H213.333333c-46.933333 0-85.333333 38.4-85.333333 85.333333v597.333334c0 46.933333 38.4 85.333333 85.333333 85.333333h597.333334c46.933333 0 85.333333-38.4 85.333333-85.333333V213.333333c0-46.933333-38.4-85.333333-85.333333-85.333333z m-353.706667 567.04a42.496 42.496 0 0 1-60.16 0L243.626667 541.866667c-8.106667-8.106667-12.373333-18.773333-12.373334-29.866667s4.693333-22.186667 12.373334-29.866667a42.496 42.496 0 0 1 60.16 0L426.666667 604.586667l293.546666-293.546667a42.496 42.496 0 1 1 60.16 60.16l-323.413333 323.84z"></path></svg>'), Ii = /* @__PURE__ */ p('<svg viewBox="0 0 1024 1024" class="icon"><path d="M245.333333 128h533.333334A117.333333 117.333333 0 0 1 896 245.333333v533.333334A117.333333 117.333333 0 0 1 778.666667 896H245.333333A117.333333 117.333333 0 0 1 128 778.666667V245.333333A117.333333 117.333333 0 0 1 245.333333 128z m0 64c-29.44 0-53.333333 23.893333-53.333333 53.333333v533.333334c0 29.44 23.893333 53.333333 53.333333 53.333333h533.333334c29.44 0 53.333333-23.893333 53.333333-53.333333V245.333333c0-29.44-23.893333-53.333333-53.333333-53.333333H245.333333z"></path></svg>'), Oi = /* @__PURE__ */ p("<div></div>"), Ni = /* @__PURE__ */ p('<span class="label"></span>'), Di = () => Pi.cloneNode(!0), Ei = () => Ii.cloneNode(!0), gt = (e) => {
  const [t, n] = w(e.checked ?? !1);
  return m1(() => {
    "checked" in e && n(e.checked);
  }), (() => {
    const r = Oi.cloneNode(!0);
    return r.$$click = (i) => {
      const s = !t();
      e.onChange && e.onChange(s), n(s);
    }, _(r, (() => {
      const i = V(() => !!t());
      return () => i() ? L(Di, {}) : L(Ei, {});
    })(), null), _(r, (() => {
      const i = V(() => !!e.label);
      return () => i() && (() => {
        const s = Ni.cloneNode(!0);
        return _(s, () => e.label), s;
      })();
    })(), null), Q((i) => {
      const s = e.style, l = `klinecharts-pro-checkbox ${t() && "checked" || ""} ${e.class || ""}`;
      return i._v$ = Q1(r, s, i._v$), l !== i._v$2 && z1(r, i._v$2 = l), i;
    }, {
      _v$: void 0,
      _v$2: void 0
    }), r;
  })();
};
_1(["click"]);
const Bi = /* @__PURE__ */ p('<div class="klinecharts-pro-loading"><i class="circle1"></i><i class="circle2"></i><i class="circle3"></i></div>'), a9 = () => Bi.cloneNode(!0), Ui = /* @__PURE__ */ p('<div class="klinecharts-pro-empty"><svg class="icon" viewBox="0 0 1024 1024"><path d="M855.6 427.2H168.5c-12.7 0-24.4 6.9-30.6 18L4.4 684.7C1.5 689.9 0 695.8 0 701.8v287.1c0 19.4 15.7 35.1 35.1 35.1H989c19.4 0 35.1-15.7 35.1-35.1V701.8c0-6-1.5-11.8-4.4-17.1L886.2 445.2c-6.2-11.1-17.9-18-30.6-18zM673.4 695.6c-16.5 0-30.8 11.5-34.3 27.7-12.7 58.5-64.8 102.3-127.2 102.3s-114.5-43.8-127.2-102.3c-3.5-16.1-17.8-27.7-34.3-27.7H119c-26.4 0-43.3-28-31.1-51.4l81.7-155.8c6.1-11.6 18-18.8 31.1-18.8h622.4c13 0 25 7.2 31.1 18.8l81.7 155.8c12.2 23.4-4.7 51.4-31.1 51.4H673.4zM819.9 209.5c-1-1.8-2.1-3.7-3.2-5.5-9.8-16.6-31.1-22.2-47.8-12.6L648.5 261c-17 9.8-22.7 31.6-12.6 48.4 0.9 1.4 1.7 2.9 2.5 4.4 9.5 17 31.2 22.8 48 13L807 257.3c16.7-9.7 22.4-31 12.9-47.8zM375.4 261.1L255 191.6c-16.7-9.6-38-4-47.8 12.6-1.1 1.8-2.1 3.6-3.2 5.5-9.5 16.8-3.8 38.1 12.9 47.8L337.3 327c16.9 9.7 38.6 4 48-13.1 0.8-1.5 1.7-2.9 2.5-4.4 10.2-16.8 4.5-38.6-12.4-48.4zM512 239.3h2.5c19.5 0.3 35.5-15.5 35.5-35.1v-139c0-19.3-15.6-34.9-34.8-35.1h-6.4C489.6 30.3 474 46 474 65.2v139c0 19.5 15.9 35.4 35.5 35.1h2.5z"></path></svg></div>'), Fi = () => Ui.cloneNode(!0), Ki = /* @__PURE__ */ p("<ul></ul>"), ji = /* @__PURE__ */ p("<li></li>"), Pe = (e) => (() => {
  const t = Ki.cloneNode(!0);
  return _(t, L(H, {
    get when() {
      return e.loading;
    },
    get children() {
      return L(a9, {});
    }
  }), null), _(t, L(H, {
    get when() {
      var n;
      return !e.loading && !e.children && !((n = e.dataSource) != null && n.length);
    },
    get children() {
      return L(Fi, {});
    }
  }), null), _(t, L(H, {
    get when() {
      return e.children;
    },
    get children() {
      return e.children;
    }
  }), null), _(t, L(H, {
    get when() {
      return !e.children;
    },
    get children() {
      var n;
      return (n = e.dataSource) == null ? void 0 : n.map((r) => {
        var i;
        return ((i = e.renderItem) == null ? void 0 : i.call(e, r)) ?? ji.cloneNode(!0);
      });
    }
  }), null), Q((n) => {
    const r = e.style, i = `klinecharts-pro-list ${e.class ?? ""}`;
    return n._v$ = Q1(t, r, n._v$), i !== n._v$2 && z1(t, n._v$2 = i), n;
  }, {
    _v$: void 0,
    _v$2: void 0
  }), t;
})(), zi = /* @__PURE__ */ p('<div class="klinecharts-pro-modal"><div class="inner"><div class="title-container"><svg class="close-icon" viewBox="0 0 1024 1024"><path d="M934.184927 199.723787 622.457206 511.452531l311.727721 311.703161c14.334473 14.229073 23.069415 33.951253 23.069415 55.743582 0 43.430138-35.178197 78.660524-78.735226 78.660524-21.664416 0-41.361013-8.865925-55.642275-23.069415L511.149121 622.838388 199.420377 934.490384c-14.204513 14.20349-33.901111 23.069415-55.642275 23.069415-43.482327 0-78.737272-35.230386-78.737272-78.660524 0-21.792329 8.864902-41.513486 23.094998-55.743582l311.677579-311.703161L88.135828 199.723787c-14.230096-14.255679-23.094998-33.92567-23.094998-55.642275 0-43.430138 35.254945-78.762855 78.737272-78.762855 21.741163 0 41.437761 8.813736 55.642275 23.069415l311.727721 311.727721L822.876842 88.389096c14.281261-14.255679 33.977859-23.069415 55.642275-23.069415 43.557028 0 78.735226 35.332716 78.735226 78.762855C957.254342 165.798117 948.5194 185.468109 934.184927 199.723787"></path></svg></div><div class="content-container"></div></div></div>'), Qi = /* @__PURE__ */ p('<div class="button-container"></div>'), R1 = (e) => (() => {
  const t = zi.cloneNode(!0), n = t.firstChild, r = n.firstChild, i = r.firstChild, s = r.nextSibling;
  return t.$$click = (l) => {
    l.target === l.currentTarget && e.onClose && e.onClose();
  }, _(r, () => e.title, i), P1(i, "click", e.onClose, !0), _(s, () => e.children), _(n, (() => {
    const l = V(() => !!(e.buttons && e.buttons.length > 0));
    return () => l() && (() => {
      const c = Qi.cloneNode(!0);
      return _(c, () => e.buttons.map((h) => L(Ti, It(h, {
        get style() {
          return {
            ...e.minButtonWidth ? {
              "min-width": `${e.minButtonWidth}px`
            } : {},
            width: e.isMobile ? "100%" : "auto"
          };
        },
        get children() {
          return h.children;
        }
      })))), Q((h) => {
        const m = e.btnParentStyle, d = !!e.isMobile;
        return h._v$8 = Q1(c, m, h._v$8), d !== h._v$9 && c.classList.toggle("mobile-buttons", h._v$9 = d), h;
      }, {
        _v$8: void 0,
        _v$9: void 0
      }), c;
    })();
  })(), null), Q((l) => {
    const c = !!e.isMobile, h = e.isMobile ? "100%" : `${e.width ?? 400}px`, m = (e.isMobile, "auto"), d = e.isMobile ? "60vh" : "90vh", $ = !!e.isMobile, v = !!e.isMobile, k = !!e.isMobile;
    return c !== l._v$ && t.classList.toggle("mobile-modal", l._v$ = c), h !== l._v$2 && n.style.setProperty("width", l._v$2 = h), m !== l._v$3 && n.style.setProperty("height", l._v$3 = m), d !== l._v$4 && n.style.setProperty("max-height", l._v$4 = d), $ !== l._v$5 && n.classList.toggle("mobile-inner", l._v$5 = $), v !== l._v$6 && r.classList.toggle("mobile-title", l._v$6 = v), k !== l._v$7 && s.classList.toggle("mobile-content", l._v$7 = k), l;
  }, {
    _v$: void 0,
    _v$2: void 0,
    _v$3: void 0,
    _v$4: void 0,
    _v$5: void 0,
    _v$6: void 0,
    _v$7: void 0
  }), t;
})();
_1(["click"]);
const Zi = /* @__PURE__ */ p('<div tabindex="0"><div class="selector-container"><span class="value"></span><i class="arrow"></i></div></div>'), Ri = /* @__PURE__ */ p('<div class="drop-down-container"><ul></ul></div>'), Hi = /* @__PURE__ */ p('<div><input type="text"></div>'), Vi = /* @__PURE__ */ p("<li></li>"), s9 = (e) => {
  const [t, n] = w(!1), [r, i] = w("");
  let s, l;
  const c = V(() => {
    if (!e.dataSource || !e.searchable)
      return e.dataSource;
    const d = r().toLowerCase().trim();
    return d ? typeof e.dataSource[0] == "string" ? e.dataSource.filter((v) => v.toLowerCase().includes(d)) : e.dataSource.filter((v) => {
      var D, I;
      const k = ((D = v.text) == null ? void 0 : D.toString().toLowerCase()) || "", T = ((I = v.key) == null ? void 0 : I.toLowerCase()) || "";
      return k.includes(d) || T.includes(d);
    }) : e.dataSource;
  }), h = () => {
    const d = !t();
    n(d), i(""), d && e.searchable && setTimeout(() => s == null ? void 0 : s.focus(), 50);
  }, m = (d) => {
    const $ = d.relatedTarget;
    l && $ && l.contains($) || (n(!1), i(""));
  };
  return (() => {
    const d = Zi.cloneNode(!0), $ = d.firstChild, v = $.firstChild;
    d.addEventListener("blur", m), d.$$click = (T) => {
      T.stopPropagation(), h();
    };
    const k = l;
    return typeof k == "function" ? K1(k, d) : l = d, _(v, () => e.value), _(d, (() => {
      const T = V(() => !!(e.dataSource && e.dataSource.length > 0));
      return () => T() && (() => {
        const D = Ri.cloneNode(!0), I = D.firstChild;
        return D.$$mousedown = (F) => F.preventDefault(), _(D, (() => {
          const F = V(() => !!e.searchable);
          return () => F() && (() => {
            const K = Hi.cloneNode(!0), P = K.firstChild;
            K.style.setProperty("padding", "8px"), K.style.setProperty("border-bottom", "1px solid #333"), P.$$click = (z) => z.stopPropagation(), P.$$input = (z) => i(z.currentTarget.value);
            const B = s;
            return typeof B == "function" ? K1(B, P) : s = P, P.style.setProperty("width", "100%"), P.style.setProperty("padding", "6px 10px"), P.style.setProperty("border", "1px solid var(--klinecharts-pro-border-color)"), P.style.setProperty("border-radius", "4px"), P.style.setProperty("background-color", "var(--klinecharts-pro-popover-background-color)"), P.style.setProperty("color", "#fff"), P.style.setProperty("font-size", "13px"), P.style.setProperty("outline", "none"), Q(() => y1(P, "placeholder", e.searchPlaceholder || "Search...")), Q(() => P.value = r()), K;
          })();
        })(), I), _(I, () => {
          var F;
          return (F = c()) == null ? void 0 : F.map((K) => {
            const B = K[e.valueKey ?? "text"] ?? K;
            return (() => {
              const z = Vi.cloneNode(!0);
              return z.$$click = ($1) => {
                var a1;
                $1.stopPropagation(), e.value !== B && ((a1 = e.onSelected) == null || a1.call(e, K)), n(!1), i("");
              }, _(z, B), z;
            })();
          });
        }), D;
      })();
    })(), null), Q((T) => {
      const D = e.style, I = `klinecharts-pro-select ${e.class ?? ""} ${t() ? "klinecharts-pro-select-show" : ""}`;
      return T._v$ = Q1(d, D, T._v$), I !== T._v$2 && z1(d, T._v$2 = I), T;
    }, {
      _v$: void 0,
      _v$2: void 0
    }), d;
  })();
};
_1(["click", "mousedown", "input"]);
const Yi = /* @__PURE__ */ p('<span class="prefix"></span>'), Gi = /* @__PURE__ */ p('<span class="suffix"></span>'), qi = /* @__PURE__ */ p('<div><input class="value"></div>'), o9 = (e) => {
  const t = It({
    min: Number.MIN_SAFE_INTEGER,
    max: Number.MAX_SAFE_INTEGER
  }, e);
  let n;
  const [r, i] = w("normal");
  return (() => {
    const s = qi.cloneNode(!0), l = s.firstChild;
    return s.$$click = () => {
      n == null || n.focus();
    }, _(s, L(H, {
      get when() {
        return t.prefix;
      },
      get children() {
        const c = Yi.cloneNode(!0);
        return _(c, () => t.prefix), c;
      }
    }), l), l.addEventListener("change", (c) => {
      var m, d;
      const h = c.target.value;
      if ("precision" in t) {
        let $;
        const v = Math.max(0, Math.floor(t.precision));
        v <= 0 ? $ = new RegExp(/^[1-9]\d*$/) : $ = new RegExp("^\\d+\\.?\\d{0," + v + "}$"), (h === "" || $.test(h) && +h >= t.min && +h <= t.max) && ((m = t.onChange) == null || m.call(t, h === "" ? h : +h));
      } else
        (d = t.onChange) == null || d.call(t, h);
    }), l.addEventListener("blur", () => {
      i("normal");
    }), l.addEventListener("focus", () => {
      i("focus");
    }), K1((c) => {
      n = c;
    }, l), _(s, L(H, {
      get when() {
        return t.suffix;
      },
      get children() {
        const c = Gi.cloneNode(!0);
        return _(c, () => t.suffix), c;
      }
    }), null), Q((c) => {
      const h = t.style, m = `klinecharts-pro-input ${t.class ?? ""}`, d = r(), $ = t.placeholder ?? "";
      return c._v$ = Q1(s, h, c._v$), m !== c._v$2 && z1(s, c._v$2 = m), d !== c._v$3 && y1(s, "data-status", c._v$3 = d), $ !== c._v$4 && y1(l, "placeholder", c._v$4 = $), c;
    }, {
      _v$: void 0,
      _v$2: void 0,
      _v$3: void 0,
      _v$4: void 0
    }), Q(() => l.value = t.value), s;
  })();
};
_1(["click"]);
const Wi = /* @__PURE__ */ p('<div><i class="thumb"></i></div>'), Xi = (e) => (() => {
  const t = Wi.cloneNode(!0);
  return t.$$click = (n) => {
    e.onChange && e.onChange();
  }, Q((n) => {
    const r = e.style, i = `klinecharts-pro-switch ${e.open ? "turn-on" : "turn-off"} ${e.class ?? ""}`;
    return n._v$ = Q1(t, r, n._v$), i !== n._v$2 && z1(t, n._v$2 = i), n;
  }, {
    _v$: void 0,
    _v$2: void 0
  }), t;
})();
_1(["click"]);
const Ji = "指标", ea = "更多", ta = "主图指标", na = "副图指标", ra = "设置", ia = "时区", aa = "截屏", sa = "全屏", oa = "退出全屏", ca = "保存", la = "确定", ua = "取消", da = "MA(移动平均线)", ha = "EMA(指数平滑移动平均线)", Ca = "SMA", ya = "BOLL(布林线)", fa = "BBI(多空指数)", ga = "SAR(停损点指向指标)", ma = "VOL(成交量)", pa = "MACD(指数平滑异同移动平均线)", va = "KDJ(随机指标)", _a = "RSI(相对强弱指标)", $a = "BIAS(乖离率)", ba = "BRAR(情绪指标)", La = "CCI(顺势指标)", xa = "DMI(动向指标)", ka = "CR(能量指标)", Aa = "PSY(心理线)", wa = "DMA(平行线差指标)", Ma = "TRIX(三重指数平滑平均线)", Sa = "OBV(能量潮指标)", Ta = "VR(成交量变异率)", Pa = "WR(威廉指标)", Ia = "MTM(动量指标)", Oa = "EMV(简易波动指标)", Na = "ROC(变动率指标)", Da = "PVT(价量趋势指标)", Ea = "AO(动量震荡指标)", Ba = "世界统一时间", Ua = "(UTC-10) 檀香山", Fa = "(UTC-8) 朱诺", Ka = "(UTC-7) 洛杉矶", ja = "(UTC-5) 芝加哥", za = "(UTC-4) 多伦多", Qa = "(UTC-3) 圣保罗", Za = "(UTC+1) 伦敦", Ra = "(UTC+2) 柏林", Ha = "(UTC+3) 巴林", Va = "(UTC+4) 迪拜", Ya = "(UTC+5) 阿什哈巴德", Ga = "(UTC+6) 阿拉木图", qa = "(UTC+7) 曼谷", Wa = "(UTC+8) 上海", Xa = "(UTC+9) 东京", Ja = "(UTC+10) 悉尼", es = "(UTC+12) 诺福克岛", ts = "水平直线", ns = "水平射线", rs = "水平线段", is = "垂直直线", as = "垂直射线", ss = "垂直线段", os = "直线", cs = "射线", ls = "线段", us = "箭头", ds = "价格线", hs = "价格通道线", Cs = "平行直线", ys = "斐波那契回调直线", fs = "斐波那契回调线段", gs = "斐波那契圆环", ms = "斐波那契螺旋", ps = "斐波那契速度阻力扇", vs = "斐波那契趋势扩展", _s = "江恩箱", $s = "矩形", bs = "平行四边形", Ls = "圆", xs = "三角形", ks = "三浪", As = "五浪", ws = "八浪", Ms = "任意浪", Ss = "ABCD形态", Ts = "XABCD形态", Ps = "弱磁模式", Is = "强磁模式", Os = "商品搜索", Ns = "商品代码", Ds = "参数1", Es = "参数2", Bs = "参数3", Us = "参数4", Fs = "参数5", Ks = "周期", js = "标准差", zs = "蜡烛图类型", Qs = "全实心", Zs = "全空心", Rs = "涨空心", Hs = "跌空心", Vs = "OHLC", Ys = "面积图", Gs = "最新价显示", qs = "最高价显示", Ws = "最低价显示", Xs = "指标最新值显示", Js = "价格轴类型", eo = "线性轴", to = "百分比轴", no = "对数轴", ro = "倒置坐标", io = "网格线显示", ao = "恢复默认", so = {
  indicator: Ji,
  more: ea,
  main_indicator: ta,
  sub_indicator: na,
  setting: ra,
  timezone: ia,
  screenshot: aa,
  full_screen: sa,
  exit_full_screen: oa,
  save: ca,
  confirm: la,
  cancel: ua,
  ma: da,
  ema: ha,
  sma: Ca,
  boll: ya,
  bbi: fa,
  sar: ga,
  vol: ma,
  macd: pa,
  kdj: va,
  rsi: _a,
  bias: $a,
  brar: ba,
  cci: La,
  dmi: xa,
  cr: ka,
  psy: Aa,
  dma: wa,
  trix: Ma,
  obv: Sa,
  vr: Ta,
  wr: Pa,
  mtm: Ia,
  emv: Oa,
  roc: Na,
  pvt: Da,
  ao: Ea,
  utc: Ba,
  honolulu: Ua,
  juneau: Fa,
  los_angeles: Ka,
  chicago: ja,
  toronto: za,
  sao_paulo: Qa,
  london: Za,
  berlin: Ra,
  bahrain: Ha,
  dubai: Va,
  ashkhabad: Ya,
  almaty: Ga,
  bangkok: qa,
  shanghai: Wa,
  tokyo: Xa,
  sydney: Ja,
  norfolk: es,
  horizontal_straight_line: ts,
  horizontal_ray_line: ns,
  horizontal_segment: rs,
  vertical_straight_line: is,
  vertical_ray_line: as,
  vertical_segment: ss,
  straight_line: os,
  ray_line: cs,
  segment: ls,
  arrow: us,
  price_line: ds,
  price_channel_line: hs,
  parallel_straight_line: Cs,
  fibonacci_line: ys,
  fibonacci_segment: fs,
  fibonacci_circle: gs,
  fibonacci_spiral: ms,
  fibonacci_speed_resistance_fan: ps,
  fibonacci_extension: vs,
  gann_box: _s,
  rect: $s,
  parallelogram: bs,
  circle: Ls,
  triangle: xs,
  three_waves: ks,
  five_waves: As,
  eight_waves: ws,
  any_waves: Ms,
  abcd: Ss,
  xabcd: Ts,
  weak_magnet: Ps,
  strong_magnet: Is,
  symbol_search: Os,
  symbol_code: Ns,
  params_1: Ds,
  params_2: Es,
  params_3: Bs,
  params_4: Us,
  params_5: Fs,
  period: Ks,
  standard_deviation: js,
  candle_type: zs,
  candle_solid: Qs,
  candle_stroke: Zs,
  candle_up_stroke: Rs,
  candle_down_stroke: Hs,
  ohlc: Vs,
  area: Ys,
  last_price_show: Gs,
  high_price_show: qs,
  low_price_show: Ws,
  indicator_last_value_show: Xs,
  price_axis_type: Js,
  normal: eo,
  percentage: to,
  log: no,
  reverse_coordinate: ro,
  grid_show: io,
  restore_default: ao
}, oo = "Indicator", co = "More", lo = "Main Indicator", uo = "Sub Indicator", ho = "Setting", Co = "Timezone", yo = "Screenshot", fo = "Full Screen", go = "Exit", mo = "Save", po = "Confirm", vo = "Cancel", _o = "MA(Moving Average)", $o = "EMA(Exponential Moving Average)", bo = "SMA", Lo = "BOLL(Bolinger Bands)", xo = "BBI(Bull And Bearlndex)", ko = "SAR(Stop and Reverse)", Ao = "VOL(Volume)", wo = "MACD(Moving Average Convergence / Divergence)", Mo = "KDJ(KDJ Index)", So = "RSI(Relative Strength Index)", To = "BIAS(Bias Ratio)", Po = "BRAR(情绪指标)", Io = "CCI(Commodity Channel Index)", Oo = "DMI(Directional Movement Index)", No = "CR(能量指标)", Do = "PSY(Psychological Line)", Eo = "DMA(Different of Moving Average)", Bo = "TRIX(Triple Exponentially Smoothed Moving Average)", Uo = "OBV(On Balance Volume)", Fo = "VR(Volatility Volume Ratio)", Ko = "WR(Williams %R)", jo = "MTM(Momentum Index)", zo = "EMV(Ease of Movement Value)", Qo = "ROC(Price Rate of Change)", Zo = "PVT(Price and Volume Trend)", Ro = "AO(Awesome Oscillator)", Ho = "UTC", Vo = "(UTC-10) Honolulu", Yo = "(UTC-8) Juneau", Go = "(UTC-7) Los Angeles", qo = "(UTC-5) Chicago", Wo = "(UTC-4) Toronto", Xo = "(UTC-3) Sao Paulo", Jo = "(UTC+1) London", ec = "(UTC+2) Berlin", tc = "(UTC+3) Bahrain", nc = "(UTC+4) Dubai", rc = "(UTC+5) Ashkhabad", ic = "(UTC+6) Almaty", ac = "(UTC+7) Bangkok", sc = "(UTC+8) Shanghai", oc = "(UTC+9) Tokyo", cc = "(UTC+10) Sydney", lc = "(UTC+12) Norfolk", uc = "Horizontal Line", dc = "Horizontal Ray", hc = "Horizontal Segment", Cc = "Vertical Line", yc = "Vertical Ray", fc = "Vertical Segment", gc = "Trend Line", mc = "Ray", pc = "Segment", vc = "Arrow", _c = "Price Line", $c = "Price Channel Line", bc = "Parallel Line", Lc = "Fibonacci Line", xc = "Fibonacci Segment", kc = "Fibonacci Circle", Ac = "Fibonacci Spiral", wc = "Fibonacci Sector", Mc = "Fibonacci Extension", Sc = "Gann Box", Tc = "Rect", Pc = "Parallelogram", Ic = "Circle", Oc = "Triangle", Nc = "Three Waves", Dc = "Five Waves", Ec = "Eight Waves", Bc = "Any Waves", Uc = "ABCD Pattern", Fc = "XABCD Pattern", Kc = "Weak Magnet", jc = "Strong Magnet", zc = "Symbol Search", Qc = "Symbol Code", Zc = "Parameter 1", Rc = "Parameter 2", Hc = "Parameter 3", Vc = "Parameter 4", Yc = "Parameter 5", Gc = "Period", qc = "Standard Deviation", Wc = "Candle Type", Xc = "Candle Solid", Jc = "Candle Stroke", el = "Candle Up Stroke", tl = "Candle Down Stroke", nl = "OHLC", rl = "Area", il = "Show Last Price", al = "Show Highest Price", sl = "Show Lowest Price", ol = "Show indicator's last value", cl = "Price Axis Type", ll = "Normal", ul = "Percentage", dl = "Log", hl = "Reverse Coordinate", Cl = "Show Grids", yl = "Restore Defaults", fl = {
  indicator: oo,
  more: co,
  main_indicator: lo,
  sub_indicator: uo,
  setting: ho,
  timezone: Co,
  screenshot: yo,
  full_screen: fo,
  exit_full_screen: go,
  save: mo,
  confirm: po,
  cancel: vo,
  ma: _o,
  ema: $o,
  sma: bo,
  boll: Lo,
  bbi: xo,
  sar: ko,
  vol: Ao,
  macd: wo,
  kdj: Mo,
  rsi: So,
  bias: To,
  brar: Po,
  cci: Io,
  dmi: Oo,
  cr: No,
  psy: Do,
  dma: Eo,
  trix: Bo,
  obv: Uo,
  vr: Fo,
  wr: Ko,
  mtm: jo,
  emv: zo,
  roc: Qo,
  pvt: Zo,
  ao: Ro,
  utc: Ho,
  honolulu: Vo,
  juneau: Yo,
  los_angeles: Go,
  chicago: qo,
  toronto: Wo,
  sao_paulo: Xo,
  london: Jo,
  berlin: ec,
  bahrain: tc,
  dubai: nc,
  ashkhabad: rc,
  almaty: ic,
  bangkok: ac,
  shanghai: sc,
  tokyo: oc,
  sydney: cc,
  norfolk: lc,
  horizontal_straight_line: uc,
  horizontal_ray_line: dc,
  horizontal_segment: hc,
  vertical_straight_line: Cc,
  vertical_ray_line: yc,
  vertical_segment: fc,
  straight_line: gc,
  ray_line: mc,
  segment: pc,
  arrow: vc,
  price_line: _c,
  price_channel_line: $c,
  parallel_straight_line: bc,
  fibonacci_line: Lc,
  fibonacci_segment: xc,
  fibonacci_circle: kc,
  fibonacci_spiral: Ac,
  fibonacci_speed_resistance_fan: wc,
  fibonacci_extension: Mc,
  gann_box: Sc,
  rect: Tc,
  parallelogram: Pc,
  circle: Ic,
  triangle: Oc,
  three_waves: Nc,
  five_waves: Dc,
  eight_waves: Ec,
  any_waves: Bc,
  abcd: Uc,
  xabcd: Fc,
  weak_magnet: Kc,
  strong_magnet: jc,
  symbol_search: zc,
  symbol_code: Qc,
  params_1: Zc,
  params_2: Rc,
  params_3: Hc,
  params_4: Vc,
  params_5: Yc,
  period: Gc,
  standard_deviation: qc,
  candle_type: Wc,
  candle_solid: Xc,
  candle_stroke: Jc,
  candle_up_stroke: el,
  candle_down_stroke: tl,
  ohlc: nl,
  area: rl,
  last_price_show: il,
  high_price_show: al,
  low_price_show: sl,
  indicator_last_value_show: ol,
  price_axis_type: cl,
  normal: ll,
  percentage: ul,
  log: dl,
  reverse_coordinate: hl,
  grid_show: Cl,
  restore_default: yl
}, c9 = {
  "zh-CN": so,
  "en-US": fl
};
function Bd(e, t) {
  c9[e] = t;
}
const a = (e, t) => {
  var n;
  return ((n = c9[t]) == null ? void 0 : n[e]) ?? e;
}, gl = /* @__PURE__ */ p('<div class="scroll-indicator left"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"></path></svg></div>'), ml = /* @__PURE__ */ p('<img alt="symbol">'), pl = /* @__PURE__ */ p('<div class="symbol"><span></span></div>'), vl = /* @__PURE__ */ p('<span class="down-arrow-icon mobile-separate-arrow"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9L12 15L18 9"></path></svg></span>'), _l = /* @__PURE__ */ p('<span class="mobile-more-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M9.671 4.136a2.34 2.34 0 0 1 4.659 0 2.34 2.34 0 0 0 3.319 1.915 2.34 2.34 0 0 1 2.33 4.033 2.34 2.34 0 0 0 0 3.831 2.34 2.34 0 0 1-2.33 4.033 2.34 2.34 0 0 0-3.319 1.915 2.34 2.34 0 0 1-4.659 0 2.34 2.34 0 0 0-3.32-1.915 2.34 2.34 0 0 1-2.33-4.033 2.34 2.34 0 0 0 0-3.831A2.34 2.34 0 0 1 6.35 6.051a2.34 2.34 0 0 0 3.319-1.915"></path><circle cx="12" cy="12" r="3"></circle></svg></span>'), $l = /* @__PURE__ */ p('<div class="item tools"><svg viewBox="0 0 22 22" xmlns="http://www.w3.org/2000/svg"><rect x="1" y="1" width="20" height="20" rx="4" ry="4" fill="none" stroke="currentColor" stroke-width="2"></rect><path d="m15.92 13.9-3.65-3.55a.773.773 0 0 1 0-1.12c.32-.33.79-.33 1.11 0l3.02 3.38c.31.32.31.81 0 1.13-.16.16-.48.16-.48.16Z" fill="currentColor" stroke="1"></path><path d="m12.43 14.23-.48-.17q-.48-.48 0-.96l4.13-4.36c.31-.32.63-.32.95 0s.32.65 0 .97l-4.13 4.35c-.15.17-.31.17-.47.17Zm-1.11-9.52c.31 0 1.11.16 1.11.97 0 .64-.95.48-1.27.48-1.43 0-1.91.81-2.07 1.45l-.31 1.94h1.58c.32 0 .64.32.64.64 0 .33-.32.65-.64.65H8.62l-1.11 4.99c-.16 1.46-1.11 1.46-1.59 0L7.03 11H5.6c-.31 0-.63-.32-.63-.65 0-.64.32-.8.63-.8h1.75l.48-2.1c.31-2.74 2.69-2.9 3.49-2.74Z" fill="noncurrentColore" stroke="1"></path></svg><span></span></div>'), bl = /* @__PURE__ */ p('<div class="klinecharts-pro-order-tools-popover"><label class="klinecharts-pro-order-tools-item"><span class="klinecharts-pro-order-tools-checkbox-box"><input class="klinecharts-pro-order-tools-checkbox-input" type="checkbox"><span class="klinecharts-pro-order-tools-checkbox-fill"></span></span><span class="klinecharts-pro-order-tools-label">Quick Order</span></label> <label class="klinecharts-pro-order-tools-item"><span class="klinecharts-pro-order-tools-checkbox-box"><input class="klinecharts-pro-order-tools-checkbox-input" type="checkbox"><span class="klinecharts-pro-order-tools-checkbox-fill"></span></span><span class="klinecharts-pro-order-tools-label">Open Orders</span></label> <label class="klinecharts-pro-order-tools-item"><span class="klinecharts-pro-order-tools-checkbox-box"><input class="klinecharts-pro-order-tools-checkbox-input" type="checkbox"><span class="klinecharts-pro-order-tools-checkbox-fill"></span></span><span class="klinecharts-pro-order-tools-label">Positions</span></label> <label class="klinecharts-pro-order-tools-item"><span class="klinecharts-pro-order-tools-checkbox-box"><input class="klinecharts-pro-order-tools-checkbox-input" type="checkbox"><span class="klinecharts-pro-order-tools-checkbox-fill"></span></span><span class="klinecharts-pro-order-tools-label">Order History</span></label></div>'), Ll = /* @__PURE__ */ p('<div><div class="item tools order-tools-trigger"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-label="Orders"><path d="M4 7h10"></path><path d="M4 12h8"></path><path d="M4 17h6"></path><path d="M18 6v12"></path><path d="M15 15l3 3 3-3"></path></svg><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9L12 15L18 9"></path></svg></div></div>'), xl = /* @__PURE__ */ p('<div class="item tools"><svg width="20" height="20" viewBox="0 0 20 20"><path d="M18.5446,9.09091C18.3333,6.61616,17.2887,4.31818,15.5751,2.63889C13.8498,0.94697,11.6197,0,9.28404,0C8.02817,0,6.81925,0.265151,5.66901,0.782828C5.65728,0.782828,5.65728,0.795454,5.64554,0.795454C5.6338,0.795454,5.6338,0.808081,5.62207,0.808081C4.53052,1.31313,3.55634,2.0202,2.71127,2.92929C1.85446,3.85101,1.18545,4.91162,0.715963,6.11111C0.246479,7.33586,0,8.64899,0,10C0,10.8712,0.105634,11.7172,0.305164,12.5379C0.305164,12.5631,0.316901,12.5884,0.328638,12.6136C0.739437,14.2298,1.51408,15.7197,2.62911,16.9571C4.07277,18.548,5.92723,19.5581,7.93427,19.8737C7.95775,19.8737,7.96948,19.8864,7.99296,19.8864C8.3216,19.9369,8.66197,19.9747,9.00235,19.9747L9.21362,19.9747C9.61268,19.9747,10.3756,19.9369,11.0094,19.697C11.1737,19.6338,11.3028,19.5076,11.3732,19.3434C11.4437,19.1793,11.4554,18.9899,11.3967,18.8131C11.3028,18.5354,11.0563,18.346,10.7864,18.346C10.716,18.346,10.6338,18.3586,10.5634,18.3838C10.0939,18.5606,9.46009,18.5859,9.20188,18.5859L9.09624,18.5859C9.20188,18.2702,9.23709,17.9167,9.15493,17.5505C9.00235,16.8939,8.50939,16.3384,7.58216,15.7955L7.19484,15.5682C6.57277,15.2146,6.23239,15.0253,6.03286,14.7348C5.83333,14.4444,5.69249,13.9899,5.51643,12.9798C5.38732,12.298,5.04695,11.7677,4.50704,11.4646C4.14319,11.2626,3.70892,11.149,3.19249,11.149C2.82864,11.149,2.42958,11.1995,2.00704,11.3005C1.79578,11.351,1.59624,11.4141,1.42019,11.4646C1.33803,10.9848,1.30282,10.4798,1.30282,9.97475C1.30282,6.93182,2.76995,4.26768,4.98826,2.72727C5,3.00505,5.05869,3.29545,5.17606,3.57323C5.48122,4.26768,6.10329,4.7096,7.01878,4.89899C7.06573,4.91162,7.10094,4.91162,7.13615,4.91162L7.1831,4.91162C7.26526,4.91162,7.57042,4.92424,7.88732,5.0505C8.3216,5.2399,8.56808,5.55555,8.65023,6.04798C8.84977,7.61364,9.07277,10.4293,8.79108,11.3384C8.76761,11.4141,8.75587,11.4899,8.75587,11.5657C8.75587,11.9444,9.0493,12.2601,9.40141,12.2601C9.57747,12.2601,9.74179,12.1843,9.85915,12.0581C9.97653,11.9318,12.6174,9.05303,13.3216,8.09343C13.4038,7.97979,13.4859,7.87878,13.5798,7.76515C13.9202,7.33586,14.2723,6.90656,14.4014,6.26262C14.554,5.56818,14.4014,4.79798,13.9437,3.85101C13.615,3.16919,13.5563,2.86616,13.5446,2.75252C13.5563,2.7399,13.5798,2.72727,13.6033,2.71464C15.6221,4.10353,17.0188,6.43939,17.2535,9.19192C17.2887,9.55808,17.5587,9.82323,17.8991,9.82323L17.9577,9.82323C18.3099,9.8106,18.5681,9.48232,18.5446,9.09091ZM3.19249,12.5631C3.48592,12.5631,3.72066,12.6136,3.89671,12.7146C4.08451,12.8283,4.19014,12.9924,4.23709,13.2702C4.43662,14.3434,4.61268,15.0631,5,15.6061C5.37559,16.1364,5.85681,16.4015,6.58451,16.8182L6.60798,16.8308C6.71362,16.8939,6.84272,16.9571,6.96009,17.0328C7.69953,17.4621,7.86385,17.7525,7.89906,17.8914C7.93427,18.0303,7.85211,18.2323,7.74648,18.4343C4.91784,17.8535,2.65258,15.6944,1.73709,12.8283C2.15962,12.702,2.71127,12.5631,3.19249,12.5631ZM12.7934,4.5202C13.4272,5.83333,13.1455,6.18687,12.5822,6.89394C12.4883,7.00758,12.3944,7.12121,12.3005,7.24747C11.9484,7.72727,11.0211,8.77525,10.2113,9.68434C10.2113,9.24242,10.1878,8.73737,10.1526,8.19444C10.0704,6.95707,9.92958,5.90909,9.92958,5.87121L9.92958,5.83333C9.75352,4.83586,9.20188,4.11616,8.3216,3.76263C7.82864,3.56061,7.37089,3.53535,7.19484,3.53535C6.73709,3.43434,6.4554,3.24495,6.33803,2.99242C6.19718,2.68939,6.29108,2.24747,6.38498,1.9697C7.28873,1.59091,8.26291,1.37626,9.28404,1.37626C10.3873,1.37626,11.4437,1.61616,12.4061,2.04545C12.3357,2.18434,12.277,2.34848,12.2535,2.5505C12.2066,3.04293,12.3709,3.64899,12.7934,4.5202Z"></path><path d="M15.22299772857666,9.722223632261718C12.59389772857666,9.722223632261718,10.44600772857666,12.020201374511718,10.44600772857666,14.861111374511719C10.44600772857666,17.70202137451172,12.58215772857666,20.000021374511718,15.223007728576661,20.000021374511718C17.86384772857666,20.000021374511718,19.99999772857666,17.70202137451172,19.99999772857666,14.861111374511719C19.99999772857666,12.020201374511718,17.85211772857666,9.72222212709572,15.22299772857666,9.722223632261718ZM15.22299772857666,18.598491374511717C13.30985772857666,18.598491374511717,11.737087728576661,16.91919137451172,11.737087728576661,14.848481374511719C11.737087728576661,12.777781374511719,13.29811772857666,11.098491374511719,15.22299772857666,11.098491374511719C17.14787772857666,11.098491374511719,18.708917728576658,12.777781374511719,18.708917728576658,14.848481374511719C18.708917728576658,16.91919137451172,17.13614772857666,18.59848137451172,15.22299772857666,18.598491374511717Z"></path><path d="M15.692486288146974,15.050496970825195L15.692486288146974,12.676760970825196C15.692486288146974,12.297972970825196,15.399058288146973,11.982316970825195,15.046945288146972,11.982316970825195C14.694833288146972,11.982316970825195,14.401406288146973,12.297972970825196,14.401406288146973,12.676760970825196L14.401406288146973,15.340896970825195C14.401406288146973,15.530296970825194,14.471829288146973,15.694436970825196,14.589200288146973,15.833326970825196L15.751176288146972,17.095956970825195C15.868546288146973,17.222216970825194,16.032866288146973,17.297976970825196,16.208916288146973,17.297976970825196C16.384976288146973,17.297976970825196,16.537556288146973,17.222216970825194,16.666666288146974,17.095956970825195C16.78403628814697,16.969686970825194,16.854456288146974,16.792916970825196,16.854456288146974,16.603526970825193C16.854456288146974,16.414136970825197,16.78403628814697,16.237366970825196,16.666666288146974,16.111106970825197L15.692486288146974,15.050496970825195Z"></path></svg></div>'), kl = /* @__PURE__ */ p('<div class="item tools"><svg viewBox="0 0 20 20"><path d="M19.7361,12.542L18.1916,11.2919C18.2647,10.8678,18.3025,10.4347,18.3025,10.0017C18.3025,9.56861,18.2647,9.13555,18.1916,8.71142L19.7361,7.46135C19.9743,7.26938,20.0615,6.95686,19.9554,6.6756L19.9342,6.61756C19.5074,5.49026,18.8755,4.45449,18.0549,3.53926L18.0124,3.49238C17.8096,3.26692,17.4819,3.1821,17.1848,3.28032L15.2677,3.92544C14.5603,3.3763,13.7704,2.94324,12.9168,2.63966L12.5466,0.742229C12.49,0.449802,12.2472,0.222111,11.9383,0.168536L11.8746,0.157375C10.6461,-0.0524583,9.35391,-0.0524583,8.1254,0.157375L8.06174,0.168536C7.75284,0.222111,7.50997,0.449802,7.45338,0.742229L7.08082,2.64859C6.2343,2.95217,5.44909,3.383,4.74641,3.92991L2.81522,3.28032C2.52047,3.1821,2.19036,3.26469,1.98757,3.49238L1.94513,3.53926C1.12455,4.45672,0.492609,5.49249,0.0658141,6.61756L0.0445921,6.6756C-0.0615171,6.95463,0.0257283,7.26715,0.263885,7.46135L1.82723,8.72482C1.75413,9.14448,1.71876,9.57308,1.71876,9.99944C1.71876,10.428,1.75413,10.8566,1.82723,11.2741L0.263885,12.5375C0.025729,12.7295,-0.0615164,13.042,0.0445929,13.3233L0.0658148,13.3813C0.49261,14.5064,1.12455,15.5444,1.94513,16.4596L1.98757,16.5065C2.19036,16.732,2.51812,16.8168,2.81522,16.7186L4.74641,16.069C5.44909,16.6159,6.2343,17.0489,7.08082,17.3503L7.45338,19.2567C7.50997,19.5491,7.75284,19.7768,8.06174,19.8303L8.1254,19.8415C8.74084,19.9464,9.37042,20,10,20C10.6296,20,11.2615,19.9464,11.8746,19.8415L11.9383,19.8303C12.2472,19.7768,12.49,19.5491,12.5466,19.2567L12.9168,17.3592C13.7704,17.0556,14.5603,16.6248,15.2677,16.0734L17.1848,16.7186C17.4795,16.8168,17.8096,16.7342,18.0124,16.5065L18.0549,16.4596C18.8755,15.5422,19.5074,14.5064,19.9342,13.3813L19.9554,13.3233C20.0615,13.0487,19.9743,12.7362,19.7361,12.542ZM16.5175,8.97483C16.5764,9.3119,16.6071,9.65791,16.6071,10.0039C16.6071,10.3499,16.5764,10.6959,16.5175,11.033L16.3618,11.9281L18.1233,13.3545C17.8568,13.9372,17.5196,14.4863,17.1188,14.9975L14.9305,14.2631L14.1901,14.839C13.6266,15.2765,12.9994,15.6203,12.3203,15.8614L11.4219,16.1806L10.9998,18.3459C10.3372,18.4173,9.66045,18.4173,8.9955,18.3459L8.57342,16.1761L7.6821,15.8524C7.01008,15.6114,6.38521,15.2676,5.82637,14.8323L5.08596,14.2541L2.88361,14.9953C2.48275,14.4841,2.14791,13.9327,1.8791,13.3523L3.65938,11.9125L3.50611,11.0196C3.44952,10.687,3.41887,10.3432,3.41887,10.0039C3.41887,9.66237,3.44716,9.32083,3.50611,8.98822L3.65938,8.09531L1.8791,6.6555C2.14556,6.07288,2.48275,5.52374,2.88361,5.01255L5.08596,5.75367L5.82637,5.17551C6.38521,4.74022,7.01008,4.39645,7.6821,4.15536L8.57578,3.83615L8.99786,1.66638C9.66045,1.59495,10.3372,1.59495,11.0021,1.66638L11.4242,3.83168L12.3226,4.1509C12.9994,4.39198,13.6289,4.73575,14.1925,5.17328L14.9329,5.7492L17.1211,5.01479C17.522,5.52598,17.8568,6.07734,18.1256,6.65773L16.3642,8.08416L16.5175,8.97483ZM10.0024,5.85189C7.7104,5.85189,5.85231,7.61092,5.85231,9.78068C5.85231,11.9504,7.7104,13.7095,10.0024,13.7095C12.2943,13.7095,14.1524,11.9504,14.1524,9.78068C14.1524,7.61092,12.2943,5.85189,10.0024,5.85189ZM11.8699,11.5486C11.37,12.0196,10.7074,12.2808,10.0024,12.2808C9.29732,12.2808,8.63473,12.0196,8.13483,11.5486C7.6373,11.0754,7.36142,10.4481,7.36142,9.78068C7.36142,9.11323,7.6373,8.48596,8.13483,8.01272C8.63473,7.53948,9.29732,7.28054,10.0024,7.28054C10.7074,7.28054,11.37,7.53948,11.8699,8.01272C12.3674,8.48596,12.6433,9.11323,12.6433,9.78068C12.6433,10.4481,12.3674,11.0754,11.8699,11.5486Z"></path></svg></div>'), Al = /* @__PURE__ */ p('<div class="item tools"><svg viewBox="0 0 20 20"><path d="M6.50977,1L13.4902,1C13.6406,1,13.7695,1.1104910000000001,13.7969,1.2631700000000001L14.0273,2.52277C14.1387,3.13147,14.6543,3.57143,15.2559,3.57143L17.5,3.57143C18.8809,3.57143,20,4.72254,20,6.14286L20,16.4286C20,17.8489,18.8809,19,17.5,19L2.5,19C1.11914,19,0,17.8489,0,16.4286L0,6.14286C0,4.72254,1.11914,3.57143,2.5,3.57143L4.74414,3.57143C5.3457,3.57143,5.86133,3.13147,5.97266,2.52277L6.20312,1.2631700000000001C6.23047,1.1104910000000001,6.35937,1,6.50977,1ZM15.2559,4.857139999999999C14.0547,4.857139999999999,13.0215,3.97522,12.7988,2.75982L12.7129,2.28571L7.28711,2.28571L7.20117,2.75982C6.98047,3.97522,5.94727,4.857139999999999,4.74414,4.857139999999999L2.5,4.857139999999999C1.81055,4.857139999999999,1.25,5.43371,1.25,6.14286L1.25,16.4286C1.25,17.1377,1.81055,17.7143,2.5,17.7143L17.5,17.7143C18.1895,17.7143,18.75,17.1377,18.75,16.4286L18.75,6.14286C18.75,5.43371,18.1895,4.857139999999999,17.5,4.857139999999999L15.2559,4.857139999999999ZM4.375,6.78571L3.125,6.78571C2.7793,6.78571,2.5,6.49844,2.5,6.14286C2.5,5.78728,2.7793,5.5,3.125,5.5L4.375,5.5C4.7207,5.5,5,5.78728,5,6.14286C5,6.49844,4.7207,6.78571,4.375,6.78571ZM10,6.14286C7.06641,6.14286,4.6875,8.58973,4.6875,11.6071C4.6875,14.6246,7.06641,17.0714,10,17.0714C12.9336,17.0714,15.3125,14.6246,15.3125,11.6071C15.3125,8.58973,12.9336,6.14286,10,6.14286ZM10,7.42857C11.0859,7.42857,12.1055,7.8625,12.873,8.65201C13.6406,9.44152,14.0625,10.49018,14.0625,11.6071C14.0625,12.7241,13.6406,13.7728,12.873,14.5623C12.1055,15.3518,11.0859,15.7857,10,15.7857C8.91406,15.7857,7.89453,15.3518,7.12695,14.5623C6.35937,13.7728,5.9375,12.7241,5.9375,11.6071C5.9375,10.49018,6.35938,9.44152,7.12695,8.65201C7.89453,7.8625,8.91406,7.42857,10,7.42857ZM10,9.67857C8.96484,9.67857,8.125,10.54241,8.125,11.6071C8.125,12.6719,8.96484,13.5357,10,13.5357C11.0352,13.5357,11.875,12.6719,11.875,11.6071C11.875,10.54241,11.0352,9.67857,10,9.67857ZM10,10.96429C10.3438,10.96429,10.625,11.2536,10.625,11.6071C10.625,11.9607,10.3438,12.25,10,12.25C9.65625,12.25,9.375,11.9607,9.375,11.6071C9.375,11.2536,9.65625,10.96429,10,10.96429Z"></path></svg></div>'), wl = /* @__PURE__ */ p('<div class="scroll-indicator right"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18l6-6-6-6"></path></svg></div>'), Ml = /* @__PURE__ */ p('<div><div class="klinecharts-pro-period-bar"><div class="menu-container"><svg viewBox="0 0 1024 1024"><path d="M192.037 287.953h640.124c17.673 0 32-14.327 32-32s-14.327-32-32-32H192.037c-17.673 0-32 14.327-32 32s14.327 32 32 32zM832.161 479.169H438.553c-17.673 0-32 14.327-32 32s14.327 32 32 32h393.608c17.673 0 32-14.327 32-32s-14.327-32-32-32zM832.161 735.802H192.037c-17.673 0-32 14.327-32 32s14.327 32 32 32h640.124c17.673 0 32-14.327 32-32s-14.327-32-32-32zM319.028 351.594l-160 160 160 160z"></path></svg></div><div><div class="item tools"></div></div></div></div>'), Sl = /* @__PURE__ */ p("<span></span>"), Tl = /* @__PURE__ */ p('<svg viewBox="0 0 20 20"><path d="M1.08108,0L0,1.079L4.18919,5.27938L2.54826,6.91715L6.9112,6.91715L6.9112,2.56262L5.28957,4.18112L1.08108,0ZM15.8108,5.27938L20,1.079L18.9189,0L14.7104,4.18112L13.0888,2.56262L13.0888,6.91715L17.4517,6.91715L15.8108,5.27938ZM4.16988,14.7014L0.07722,18.8054L1.1583,20L5.27027,15.7996L6.9112,17.4374L6.9112,13.0829L2.54826,13.0829L4.16988,14.7014ZM17.4517,13.0829L13.0888,13.0829L13.0888,17.4374L14.7297,15.7996L18.8417,20L19.9228,18.8054L15.8301,14.7013L17.4517,13.0829Z"></path></svg>'), Pl = /* @__PURE__ */ p('<svg viewBox="0 0 20 20"><path d="M2.93444,1.76899L7.57544,6.40999L6.38918,7.59626L1.76899,2.93444L0,4.70343L0,0L4.70343,0L2.93444,1.76899ZM6.40999,12.4037L1.76899,17.0447L0,15.2758L0,19.9792L4.70343,19.9792L2.93444,18.2102L7.57544,13.5692L6.40999,12.4037ZM15.2758,0L17.0447,1.76899L12.4037,6.40999L13.59,7.59626L18.231,2.95526L20,4.72425L20,0L15.2758,0ZM13.5692,12.4037L12.3829,13.59L17.0239,18.231L15.2549,20L19.9792,20L19.9792,15.2758L18.2102,17.0447L13.5692,12.4037Z"></path></svg>'), Il = (e) => {
  let t, n, r;
  const [i, s] = w(window.innerWidth < 768), [l, c] = w(localStorage.getItem("klinechart_secondary_period") || ""), [h, m] = w(!1), [d, $] = w({
    top: 0,
    left: 0,
    minWidth: 220
  }), v = () => {
    s(window.innerWidth < 768), h() && z();
  }, [k, T] = w(!1), D = () => document.fullscreenElement ?? document.body, I = () => {
    T(!!document.fullscreenElement);
  }, [F, K] = w(!1), [P, B] = w(!1), z = () => {
    if (!n)
      return;
    const N = n.getBoundingClientRect(), Z = Math.max(220, Math.ceil(N.width)), u1 = window.innerWidth, l1 = Math.min(Math.max(8, N.right - Z), Math.max(8, u1 - Z - 8));
    $({
      top: Math.ceil(N.bottom + 8),
      left: Math.ceil(l1),
      minWidth: Z
    });
  }, $1 = () => {
    m((N) => {
      const Z = !N;
      return Z && queueMicrotask(z), Z;
    });
  }, a1 = (N) => {
    if (!h())
      return;
    const Z = N.target;
    Z && (n != null && n.contains(Z) || r != null && r.contains(Z) || m(!1));
  }, f1 = () => {
    h() && z();
  }, O = () => {
    if (t && i()) {
      const N = t;
      K(N.scrollLeft > 10), B(N.scrollLeft + N.clientWidth < N.scrollWidth - 10);
    } else
      K(!1), B(!1);
  };
  i0(() => {
    window.addEventListener("resize", v), document.addEventListener("fullscreenchange", I), document.addEventListener("mousedown", a1), window.addEventListener("scroll", f1, !0), document.addEventListener("mozfullscreenchange", I), document.addEventListener("webkitfullscreenchange", I), document.addEventListener("msfullscreenchange", I), t && (t.addEventListener("scroll", O), setTimeout(O, 100));
  }), j1(() => {
    window.removeEventListener("resize", v), document.removeEventListener("fullscreenchange", I), document.removeEventListener("mousedown", a1), window.removeEventListener("scroll", f1, !0), document.removeEventListener("mozfullscreenchange", I), document.removeEventListener("webkitfullscreenchange", I), document.removeEventListener("msfullscreenchange", I), t && t.removeEventListener("scroll", O);
  });
  const G = V(() => {
    const N = e.periods.filter((Z) => {
      if (!i() || k())
        return !0;
      const u1 = e.period.text, l1 = l();
      if (Z.text === u1 || l1 && Z.text === l1)
        return !0;
      if (!l1 || l1 === u1) {
        const q = e.periods.find((b1) => b1.text !== u1);
        return Z.text === (q == null ? void 0 : q.text);
      }
      return !1;
    }).slice(0, i() && !k() ? 2 : e.periods.length);
    return setTimeout(O, 50), N;
  });
  let R = e.period.text;
  return m1(() => {
    const N = e.period.text;
    N !== R && (i() && (c(R), localStorage.setItem("klinechart_secondary_period", R)), R = N), setTimeout(O, 50);
  }), m1(() => {
    k(), setTimeout(O, 100);
  }), m1(() => {
    if (!e.showOrderToolsMenu) {
      m(!1);
      return;
    }
    h() && queueMicrotask(z);
  }), (() => {
    const N = Ml.cloneNode(!0), Z = N.firstChild, u1 = Z.firstChild, l1 = u1.firstChild, q = u1.nextSibling, b1 = q.firstChild;
    return N.style.setProperty("position", "relative"), N.style.setProperty("width", "100%"), N.style.setProperty("display", "flex"), N.style.setProperty("align-items", "center"), _(N, L(H, {
      get when() {
        return V(() => !!i())() && F();
      },
      get children() {
        const g = gl.cloneNode(!0);
        return g.$$click = () => t.scrollBy({
          left: -100,
          behavior: "smooth"
        }), g.style.setProperty("position", "absolute"), g.style.setProperty("left", "0"), g.style.setProperty("top", "0"), g.style.setProperty("bottom", "1px"), g.style.setProperty("width", "30px"), g.style.setProperty("display", "flex"), g.style.setProperty("align-items", "center"), g.style.setProperty("justify-content", "center"), g.style.setProperty("background", "var(--klinecharts-pro-popover-background-color)"), g.style.setProperty("z-index", "10"), g.style.setProperty("cursor", "pointer"), g.style.setProperty("color", "var(--klinecharts-pro-primary-color)"), g;
      }
    }), Z), K1((g) => {
      t = g;
    }, Z), Z.style.setProperty("width", "100%"), P1(l1, "click", e.onMenuClick, !0), _(Z, L(H, {
      get when() {
        return e.symbol;
      },
      get children() {
        const g = pl.cloneNode(!0), t1 = g.firstChild;
        return P1(g, "click", e.onSymbolClick, !0), _(g, L(H, {
          get when() {
            return e.symbol.logo;
          },
          get children() {
            const S = ml.cloneNode(!0);
            return Q(() => y1(S, "src", e.symbol.logo)), S;
          }
        }), t1), _(t1, () => e.symbol.shortName ?? e.symbol.name ?? e.symbol.ticker), g;
      }
    }), q), _(Z, () => G().map((g, t1) => {
      const S = g.text === e.period.text;
      return (() => {
        const Y = Sl.cloneNode(!0);
        return Y.$$click = (j) => {
          i() && S && !k() ? (e.onMobilePeriodClick ? e.onMobilePeriodClick(g) : e.onMenuClick(), j.stopPropagation()) : e.onPeriodChange(g);
        }, z1(Y, `item period ${S ? "selected" : ""}`), _(Y, () => g.text), Y;
      })();
    }), q), _(Z, L(H, {
      get when() {
        return V(() => !!(i() && !k()))() && G().length > 1;
      },
      get children() {
        const g = vl.cloneNode(!0);
        return g.$$click = (t1) => {
          t1.stopPropagation(), e.onMobilePeriodClick ? e.onMobilePeriodClick(e.period) : e.onMenuClick();
        }, g.style.setProperty("margin-left", "4px"), g.style.setProperty("display", "inline-flex"), g.style.setProperty("align-items", "center"), g;
      }
    }), q), _(Z, L(H, {
      get when() {
        return V(() => !!i())() && !k();
      },
      get children() {
        const g = _l.cloneNode(!0);
        return g.$$click = (t1) => {
          var S;
          t1.stopPropagation(), (S = e.onMobileMoreClick) == null || S.call(e);
        }, g.style.setProperty("margin-left", "8px"), g.style.setProperty("display", "inline-flex"), g.style.setProperty("align-items", "center"), g.style.setProperty("cursor", "pointer"), g.style.setProperty("padding", "0 4px"), g;
      }
    }), q), _(Z, L(H, {
      get when() {
        return !i();
      },
      get children() {
        const g = $l.cloneNode(!0), t1 = g.firstChild, S = t1.nextSibling;
        return P1(g, "click", e.onIndicatorClick, !0), _(S, () => a("indicator", e.locale)), g;
      }
    }), q), q.style.setProperty("display", "flex"), q.style.setProperty("gap", "4px"), q.style.setProperty("margin-left", "auto"), q.style.setProperty("align-items", "center"), _(q, L(H, {
      get when() {
        return e.showOrderToolsMenu;
      },
      get children() {
        const g = Ll.cloneNode(!0), t1 = g.firstChild, S = t1.firstChild, Y = S.nextSibling;
        return K1((j) => {
          n = j;
        }, g), g.style.setProperty("display", "flex"), g.style.setProperty("align-items", "center"), t1.$$click = (j) => {
          j.stopPropagation(), $1();
        }, t1.style.setProperty("gap", "6px"), Y.style.setProperty("transition", "transform 0.2s ease"), _(g, L(H, {
          get when() {
            return h();
          },
          get children() {
            return L(n5, {
              get mount() {
                return D();
              },
              get children() {
                const j = bl.cloneNode(!0), d1 = j.firstChild, g1 = d1.firstChild, O1 = g1.firstChild, L1 = d1.nextSibling, x1 = L1.nextSibling, T1 = x1.firstChild, A1 = T1.firstChild, Ce = x1.nextSibling, re = Ce.nextSibling, ie = re.firstChild, ae = ie.firstChild, o1 = re.nextSibling, h1 = o1.nextSibling, Ke = h1.firstChild, p1 = Ke.firstChild;
                return j.$$mousedown = (E) => E.stopPropagation(), K1((E) => {
                  r = E;
                }, j), j.style.setProperty("position", "fixed"), j.style.setProperty("z-index", "9999"), O1.addEventListener("change", (E) => {
                  var r1;
                  (r1 = e.onOrderToolsStateChange) == null || r1.call(e, {
                    quickOrder: E.currentTarget.checked
                  });
                }), A1.addEventListener("change", (E) => {
                  var r1;
                  (r1 = e.onOrderToolsStateChange) == null || r1.call(e, {
                    openOrders: E.currentTarget.checked
                  });
                }), ae.addEventListener("change", (E) => {
                  var r1;
                  (r1 = e.onOrderToolsStateChange) == null || r1.call(e, {
                    positions: E.currentTarget.checked
                  });
                }), p1.addEventListener("change", (E) => {
                  var r1;
                  (r1 = e.onOrderToolsStateChange) == null || r1.call(e, {
                    orderHistory: E.currentTarget.checked
                  });
                }), Q((E) => {
                  const r1 = `${d().top}px`, ye = `${d().left}px`, N1 = `${d().minWidth}px`;
                  return r1 !== E._v$ && j.style.setProperty("top", E._v$ = r1), ye !== E._v$2 && j.style.setProperty("left", E._v$2 = ye), N1 !== E._v$3 && j.style.setProperty("width", E._v$3 = N1), E;
                }, {
                  _v$: void 0,
                  _v$2: void 0,
                  _v$3: void 0
                }), Q(() => {
                  var E;
                  return O1.checked = ((E = e.orderToolsState) == null ? void 0 : E.quickOrder) ?? !0;
                }), Q(() => {
                  var E;
                  return A1.checked = ((E = e.orderToolsState) == null ? void 0 : E.openOrders) ?? !0;
                }), Q(() => {
                  var E;
                  return ae.checked = ((E = e.orderToolsState) == null ? void 0 : E.positions) ?? !0;
                }), Q(() => {
                  var E;
                  return p1.checked = ((E = e.orderToolsState) == null ? void 0 : E.orderHistory) ?? !0;
                }), j;
              }
            });
          }
        }), null), Q((j) => {
          const d1 = i() ? "0 8px" : "0 10px", g1 = h() ? "rotate(180deg)" : "rotate(0deg)";
          return d1 !== j._v$4 && t1.style.setProperty("padding", j._v$4 = d1), g1 !== j._v$5 && Y.style.setProperty("transform", j._v$5 = g1), j;
        }, {
          _v$4: void 0,
          _v$5: void 0
        }), g;
      }
    }), b1), _(q, L(H, {
      get when() {
        return !i();
      },
      get children() {
        return [(() => {
          const g = xl.cloneNode(!0);
          return P1(g, "click", e.onTimezoneClick, !0), g;
        })(), (() => {
          const g = kl.cloneNode(!0);
          return P1(g, "click", e.onSettingClick, !0), g;
        })()];
      }
    }), b1), _(q, L(H, {
      get when() {
        return !i();
      },
      get children() {
        const g = Al.cloneNode(!0);
        return P1(g, "click", e.onScreenshotClick, !0), g;
      }
    }), b1), b1.$$click = () => {
      if (k())
        (document.exitFullscreen || document.msExitFullscreen || document.mozCancelFullScreen || document.webkitExitFullscreen).call(document);
      else {
        const g = t == null ? void 0 : t.closest(".klinecharts-pro");
        g && ((g == null ? void 0 : g.requestFullscreen) ?? (g == null ? void 0 : g.webkitRequestFullscreen) ?? (g == null ? void 0 : g.mozRequestFullScreen) ?? (g == null ? void 0 : g.msRequestFullscreen)).call(g);
      }
    }, _(b1, (() => {
      const g = V(() => !!k());
      return () => g() ? Tl.cloneNode(!0) : Pl.cloneNode(!0);
    })()), _(N, L(H, {
      get when() {
        return V(() => !!i())() && P();
      },
      get children() {
        const g = wl.cloneNode(!0);
        return g.$$click = () => t.scrollBy({
          left: 100,
          behavior: "smooth"
        }), g.style.setProperty("position", "absolute"), g.style.setProperty("right", "0"), g.style.setProperty("top", "0"), g.style.setProperty("bottom", "1px"), g.style.setProperty("width", "30px"), g.style.setProperty("display", "flex"), g.style.setProperty("align-items", "center"), g.style.setProperty("justify-content", "center"), g.style.setProperty("background", "var(--klinecharts-pro-popover-background-color)"), g.style.setProperty("z-index", "10"), g.style.setProperty("cursor", "pointer"), g.style.setProperty("color", "var(--klinecharts-pro-primary-color)"), g;
      }
    }), null), Q((g) => {
      const t1 = i() ? "auto" : "visible", S = e.spread ? "" : "rotate", Y = k() ? "0px" : "var(--klinecharts-pro-period-bar-padding-right)";
      return t1 !== g._v$6 && Z.style.setProperty("overflow", g._v$6 = t1), S !== g._v$7 && y1(l1, "class", g._v$7 = S), Y !== g._v$8 && q.style.setProperty("padding-right", g._v$8 = Y), g;
    }, {
      _v$6: void 0,
      _v$7: void 0,
      _v$8: void 0
    }), N;
  })();
};
_1(["click", "mousedown"]);
const Ol = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M12.41465,11L18.5,11C18.7761,11,19,11.22386,19,11.5C19,11.77614,18.7761,12,18.5,12L12.41465,12C12.20873,12.5826,11.65311,13,11,13C10.34689,13,9.79127,12.5826,9.58535,12L3.5,12C3.223857,12,3,11.77614,3,11.5C3,11.22386,3.223857,11,3.5,11L9.58535,11C9.79127,10.417404,10.34689,10,11,10C11.65311,10,12.20873,10.417404,12.41465,11Z" stroke-opacity="0" stroke="none"></path></svg>'), Nl = () => Ol.cloneNode(!0), Dl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M6.91465,11L11.08535,11C11.29127,10.417404,11.84689,10,12.5,10C13.15311,10,13.70873,10.417404,13.91465,11L18.5,11C18.7761,11,19,11.22386,19,11.5C19,11.77614,18.7761,12,18.5,12L13.91465,12C13.70873,12.5826,13.15311,13,12.5,13C11.84689,13,11.29127,12.5826,11.08535,12L6.91465,12C6.70873,12.5826,6.15311,13,5.5,13C4.671573,13,4,12.32843,4,11.5C4,10.671573,4.671573,10,5.5,10C6.15311,10,6.70873,10.417404,6.91465,11Z" stroke-opacity="0" stroke="none"></path></svg>'), El = () => Dl.cloneNode(!0), Bl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M6.91465,12.5C6.70873,13.0826,6.15311,13.5,5.5,13.5C4.671573,13.5,4,12.82843,4,12C4,11.171573,4.671573,10.5,5.5,10.5C6.15311,10.5,6.70873,10.917404,6.91465,11.5L16.0853,11.5C16.2913,10.917404,16.846899999999998,10.5,17.5,10.5C18.328400000000002,10.5,19,11.171573,19,12C19,12.82843,18.328400000000002,13.5,17.5,13.5C16.846899999999998,13.5,16.2913,13.0826,16.0853,12.5L6.91465,12.5Z" stroke-opacity="0" stroke="none"></path></svg>'), Ul = () => Bl.cloneNode(!0), Fl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M11,12.41465L11,18.5C11,18.7761,11.22386,19,11.5,19C11.77614,19,12,18.7761,12,18.5L12,12.41465C12.5826,12.20873,13,11.65311,13,11C13,10.34689,12.5826,9.79127,12,9.58535L12,3.5C12,3.223857,11.77614,3,11.5,3C11.22386,3,11,3.223857,11,3.5L11,9.58535C10.417404,9.79127,10,10.34689,10,11C10,11.65311,10.417404,12.20873,11,12.41465Z" stroke-opacity="0" stroke="none"></path></svg>'), Kl = () => Fl.cloneNode(!0), jl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M11.66558837890625,19C10.83716137890625,19,10.16558837890625,18.328400000000002,10.16558837890625,17.5C10.16558837890625,16.846899999999998,10.58298437890625,16.2913,11.16557337890625,16.0854L11.16557337890625,11.91464C10.58298437890625,11.70872,10.16558837890625,11.1531,10.16558837890625,10.5C10.16558837890625,9.8469,10.58298437890625,9.29128,11.16557337890625,9.08536L11.16557337890625,4.5C11.16557337890625,4.223857,11.38942837890625,4,11.66556837890625,4C11.94171837890625,4,12.16556837890625,4.223857,12.16556837890625,4.5L12.16556837890625,9.08535C12.74817837890625,9.291260000000001,13.16558837890625,9.846879999999999,13.16558837890625,10.5C13.16558837890625,11.153120000000001,12.74817837890625,11.708739999999999,12.16556837890625,11.91465L12.16556837890625,16.0854C12.74817837890625,16.2913,13.16558837890625,16.846899999999998,13.16558837890625,17.5C13.16558837890625,18.328400000000002,12.49401837890625,19,11.66558837890625,19Z" stroke-opacity="0" stroke="none"></path></svg>'), zl = () => jl.cloneNode(!0), Ql = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M11.165603637695312,6.91465C11.748203637695312,6.70873,12.165603637695312,6.15311,12.165603637695312,5.5C12.165603637695312,4.671573,11.494033637695313,4,10.665603637695312,4C9.837176637695313,4,9.165603637695312,4.671573,9.165603637695312,5.5C9.165603637695312,6.15311,9.583007637695312,6.70873,10.165603637695312,6.91465L10.165603637695312,16.0854C9.583007637695312,16.2913,9.165603637695312,16.846899999999998,9.165603637695312,17.5C9.165603637695312,18.328400000000002,9.837176637695313,19,10.665603637695312,19C11.494033637695313,19,12.165603637695312,18.328400000000002,12.165603637695312,17.5C12.165603637695312,16.846899999999998,11.748203637695312,16.2913,11.165603637695312,16.0854L11.165603637695312,6.91465Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), Zl = () => Ql.cloneNode(!0), Rl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M5.146447,15.753C4.9511845,15.9483,4.9511845,16.2649,5.146447,16.4602C5.341709,16.6554,5.658291,16.6554,5.853554,16.4602L8.156600000000001,14.15711C8.352409999999999,14.25082,8.57173,14.3033,8.8033,14.3033C9.631730000000001,14.3033,10.3033,13.63172,10.3033,12.80329C10.3033,12.57172,10.250820000000001,12.352409999999999,10.157119999999999,12.15659L12.156600000000001,10.15711C12.352409999999999,10.250820000000001,12.571729999999999,10.30329,12.8033,10.30329C13.63173,10.30329,14.3033,9.63172,14.3033,8.80329C14.3033,8.57172,14.25082,8.352409999999999,14.15712,8.15659L16.4602,5.853553C16.6554,5.658291,16.6554,5.341709,16.4602,5.146447C16.2649,4.9511843,15.9483,4.9511843,15.753,5.146447L13.45001,7.449479999999999C13.25419,7.35577,13.03487,7.3032900000000005,12.8033,7.3032900000000005C11.97487,7.3032900000000005,11.3033,7.97487,11.3033,8.80329C11.3033,9.03487,11.35578,9.254190000000001,11.44949,9.450009999999999L9.450009999999999,11.449480000000001C9.254190000000001,11.35577,9.03487,11.30329,8.8033,11.30329C7.97487,11.30329,7.3033,11.97487,7.3033,12.80329C7.3033,13.03487,7.35578,13.25419,7.44949,13.45001L5.146447,15.753Z" stroke-opacity="0" stroke="none"></path></svg>'), Hl = () => Rl.cloneNode(!0), Vl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M7.573332939453125,14.54567903564453C7.667042939453125,14.741499035644532,7.719512939453125,14.960809035644532,7.719512939453125,15.19239903564453C7.719512939453125,16.02079903564453,7.047942939453125,16.69239903564453,6.219512939453125,16.69239903564453C5.391085939453125,16.69239903564453,4.719512939453125,16.02079903564453,4.719512939453125,15.19239903564453C4.719512939453125,14.36394903564453,5.391085939453125,13.692379035644532,6.219512939453125,13.692379035644532C6.451092939453125,13.692379035644532,6.670412939453125,13.74485903564453,6.866232939453125,13.83856903564453L9.865702939453126,10.83909903564453C9.771992939453124,10.643279035644532,9.719512939453125,10.42395903564453,9.719512939453125,10.192379035644532C9.719512939453125,9.36394903564453,10.391082939453124,8.692379035644532,11.219512939453125,8.692379035644532C11.451092939453126,8.692379035644532,11.670412939453126,8.74485903564453,11.866232939453125,8.838569035644532L15.462112939453124,5.242645035644531C15.657412939453126,5.047383335644532,15.974012939453125,5.047383335644532,16.169212939453125,5.242645035644531C16.364512939453125,5.437907035644531,16.364512939453125,5.754489035644531,16.169212939453125,5.949752035644531L12.573332939453124,9.545679035644532C12.667042939453125,9.74149903564453,12.719512939453125,9.96080903564453,12.719512939453125,10.192379035644532C12.719512939453125,11.020809035644533,12.047942939453126,11.692379035644532,11.219512939453125,11.692379035644532C10.987942939453125,11.692379035644532,10.768632939453125,11.639909035644532,10.572812939453126,11.54619903564453L7.573332939453125,14.54567903564453Z" stroke-opacity="0" stroke="none"></path></svg>'), Yl = () => Vl.cloneNode(!0), Gl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M15.719512939453125,8.461776733398438C16.547912939453127,8.461776733398438,17.219512939453125,7.7902067333984375,17.219512939453125,6.9617767333984375C17.219512939453125,6.133349733398438,16.547912939453127,5.4617767333984375,15.719512939453125,5.4617767333984375C14.891082939453124,5.4617767333984375,14.219512939453125,6.133349733398438,14.219512939453125,6.9617767333984375C14.219512939453125,7.193346733398437,14.271992939453124,7.412666733398438,14.365692939453124,7.608486733398438L7.366222939453126,14.607956733398437C7.170402939453125,14.514256733398437,6.951082939453125,14.461776733398438,6.719512939453125,14.461776733398438C5.891085939453125,14.461776733398438,5.219512939453125,15.133346733398437,5.219512939453125,15.961776733398438C5.219512939453125,16.79017673339844,5.891085939453125,17.461776733398438,6.719512939453125,17.461776733398438C7.547942939453125,17.461776733398438,8.219512939453125,16.79017673339844,8.219512939453125,15.961776733398438C8.219512939453125,15.730176733398437,8.167032939453126,15.510876733398437,8.073322939453124,15.315066733398437L15.072802939453124,8.315586733398437C15.268612939453124,8.409296733398438,15.487912939453125,8.461776733398438,15.719512939453125,8.461776733398438Z" stroke-opacity="0" stroke="none"></path></svg>'), ql = () => Gl.cloneNode(!0), Wl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M17.0643,7.033864912109375L18,3.585784912109375L14.5078,4.509695912109375L15.3537,5.344934912109375L6.02026,14.560584912109375C5.87635,14.517484912109374,5.72366,14.494284912109375,5.5655,14.494284912109375C4.7009,14.494284912109375,4,15.186384912109375,4,16.040084912109375C4,16.893784912109375,4.7009,17.585784912109375,5.5655,17.585784912109375C6.43011,17.585784912109375,7.13101,16.893784912109375,7.13101,16.040084912109375C7.13101,15.722284912109375,7.03392,15.426984912109376,6.86744,15.181384912109374L16.0917,6.073604912109375L17.0643,7.033864912109375Z" stroke-opacity="0" stroke="none"></path></svg>'), Xl = () => Wl.cloneNode(!0), Jl = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M6.91465,13.00505L18.5,13.00505C18.7761,13.00505,19,13.228909999999999,19,13.50505C19,13.781189999999999,18.7761,14.00505,18.5,14.00505L6.91465,14.00505C6.70873,14.58765,6.15311,15.00505,5.5,15.00505C4.671573,15.00505,4,14.33348,4,13.50505C4,12.67662,4.671573,12.00505,5.5,12.00505C6.15311,12.00505,6.70873,12.422450000000001,6.91465,13.00505ZM7.81404,11.625L10.48591,11.625L10.48591,10.90625L9.65193,10.90625L9.65193,7.125L8.997630000000001,7.125C8.71443,7.306641,8.415600000000001,7.419922,7.96443,7.498047L7.96443,8.05078L8.77497,8.05078L8.77497,10.90625L7.81404,10.90625L7.81404,11.625ZM11.081620000000001,11.625L14.0562,11.625L14.0562,10.88281L13.09724,10.88281C12.8863,10.88281,12.59333,10.90625,12.36482,10.93555C13.17537,10.11328,13.84724,9.2207,13.84724,8.39062C13.84724,7.541016,13.28865,7,12.4488,7C11.84333,7,11.446850000000001,7.234375,11.03279,7.679688L11.52497,8.16797C11.747630000000001,7.914062,12.0113,7.697266,12.33552,7.697266C12.7613,7.697266,13.00154,7.982422,13.00154,8.43359C13.00154,9.14648,12.29255,10.00781,11.081620000000001,11.11523L11.081620000000001,11.625ZM15.9605,11.75C16.8121,11.75,17.526899999999998,11.2832,17.526899999999998,10.4375C17.526899999999998,9.82031,17.142200000000003,9.43945,16.6441,9.30078L16.6441,9.27148C17.1129,9.08594,17.3824,8.7207,17.3824,8.21289C17.3824,7.421875,16.8004,7,15.9429,7C15.4215,7,14.9957,7.210938,14.6109,7.541016L15.066,8.11133C15.3258,7.849609,15.5836,7.697266,15.9019,7.697266C16.2789,7.697266,16.4957,7.914062,16.4957,8.28125C16.4957,8.70898,16.2301,9,15.4215,9L15.4215,9.63672C16.3804,9.63672,16.6383,9.91992,16.6383,10.38086C16.6383,10.79688,16.3336,11.03125,15.8824,11.03125C15.4742,11.03125,15.1578,10.82227,14.8922,10.55078L14.4781,11.13281C14.7906,11.486329999999999,15.2652,11.75,15.9605,11.75Z" stroke-opacity="0" stroke="none"></path></svg>'), eu = () => Jl.cloneNode(!0), tu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M3.146447,14.178126025390625C2.9511847,13.982826025390626,2.9511847,13.666226025390625,3.146447,13.470926025390625L7.39146,9.225966025390626C7.35417,9.095106025390624,7.33421,8.956946025390625,7.33421,8.814116025390625C7.33421,7.985696025390625,8.00578,7.314116025390625,8.834209999999999,7.314116025390625C8.97703,7.314116025390625,9.11519,7.334086025390625,9.24605,7.371366025390625L13.753,2.864373025390625C13.9483,2.669110325390625,14.2649,2.669110325390625,14.4602,2.864373025390625C14.6554,3.059635025390625,14.6554,3.376217025390625,14.4602,3.571479025390625L10.06916,7.962476025390625C10.23631,8.204386025390626,10.334209999999999,8.497826025390625,10.334209999999999,8.814116025390625C10.334209999999999,9.642546025390626,9.66264,10.314116025390625,8.834209999999999,10.314116025390625C8.51791,10.314116025390625,8.22448,10.216226025390625,7.98256,10.049076025390626L3.853554,14.178126025390625C3.658291,14.373326025390625,3.341709,14.373326025390625,3.146447,14.178126025390625ZM7.67736,19.188526025390626C7.4821,18.993226025390626,7.4821,18.676626025390625,7.67736,18.481426025390626L9.9804,16.178326025390625C9.88669,15.982526025390625,9.834209999999999,15.763226025390624,9.834209999999999,15.531626025390626C9.834209999999999,14.703226025390626,10.50578,14.031626025390626,11.33421,14.031626025390626C11.56579,14.031626025390626,11.78511,14.084126025390624,11.98093,14.177826025390624L13.9804,12.178356025390626C13.8867,11.982536025390624,13.8342,11.763216025390625,13.8342,11.531636025390625C13.8342,10.703206025390624,14.5058,10.031636025390625,15.3342,10.031636025390625C15.5658,10.031636025390625,15.7851,10.084116025390625,15.9809,10.177826025390626L18.284,7.874796025390625C18.4792,7.679536025390625,18.7958,7.679536025390625,18.9911,7.874796025390625C19.1863,8.070056025390624,19.1863,8.386636025390626,18.9911,8.581906025390625L16.688000000000002,10.884936025390624C16.7817,11.080756025390626,16.8342,11.300066025390626,16.8342,11.531636025390625C16.8342,12.360066025390624,16.162599999999998,13.031626025390626,15.3342,13.031626025390626C15.1026,13.031626025390626,14.8833,12.979126025390626,14.6875,12.885426025390625L12.68803,14.884926025390625C12.78174,15.080726025390625,12.83421,15.300026025390626,12.83421,15.531626025390626C12.83421,16.360026025390624,12.16264,17.031626025390626,11.33421,17.031626025390626C11.10264,17.031626025390626,10.88333,16.979126025390627,10.68751,16.885426025390625L8.38446,19.188526025390626C8.1892,19.383726025390626,7.87262,19.383726025390626,7.67736,19.188526025390626Z" stroke-opacity="0" stroke="none"></path></svg>'), nu = () => tu.cloneNode(!0), ru = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M3.3367688759765626,12.63173C3.5320318759765623,12.82699,3.8486138759765627,12.82699,4.043876875976562,12.63173L11.822052875976562,4.853553C12.017312875976563,4.658291,12.017312875976563,4.341708,11.822052875976562,4.146446C11.626792875976562,3.9511843,11.310202875976563,3.9511843,11.114942875976563,4.146446L3.3367688759765626,11.92462C3.1415071759765625,12.11988,3.1415071759765625,12.43647,3.3367688759765626,12.63173ZM5.001492875976562,17.0351C4.806232875976562,16.8399,4.806232875976562,16.5233,5.001492875976562,16.328L7.304532875976562,14.025C7.210822875976563,13.82916,7.158352875976563,13.60984,7.158352875976563,13.37827C7.158352875976563,12.54984,7.829922875976562,11.87827,8.658352875976561,11.87827C8.889922875976563,11.87827,9.109232875976563,11.93075,9.305052875976562,12.02446L11.304532875976562,10.02498C11.210822875976563,9.82916,11.158352875976561,9.60984,11.158352875976561,9.37827C11.158352875976561,8.54984,11.829922875976562,7.8782700000000006,12.658352875976563,7.8782700000000006C12.889922875976563,7.8782700000000006,13.109232875976563,7.93075,13.305022875976562,8.024460000000001L15.608122875976562,5.72142C15.803322875976562,5.5261499999999995,16.119922875976563,5.5261499999999995,16.315222875976563,5.72142C16.510422875976563,5.9166799999999995,16.510422875976563,6.23326,16.315222875976563,6.42852L14.012122875976562,8.73156C14.105822875976562,8.92738,14.158322875976562,9.1467,14.158322875976562,9.37827C14.158322875976562,10.2067,13.486822875976562,10.87827,12.658352875976563,10.87827C12.426772875976562,10.87827,12.207452875976562,10.82579,12.011642875976563,10.73209L10.012162875976562,12.73156C10.105872875976562,12.92738,10.158352875976561,13.1467,10.158352875976561,13.37827C10.158352875976561,14.2067,9.486772875976563,14.8783,8.658352875976561,14.8783C8.426772875976562,14.8783,8.207452875976562,14.8258,8.011642875976563,14.7321L5.708602875976562,17.0351C5.513342875976562,17.2304,5.196752875976562,17.2304,5.001492875976562,17.0351ZM10.415712875976563,18.328C10.220452875976562,18.5233,9.903862875976563,18.5233,9.708602875976563,18.328C9.513342875976562,18.1328,9.513342875976562,17.816200000000002,9.708602875976563,17.6209L12.304532875976562,15.025C12.210822875976563,14.8292,12.158352875976563,14.6098,12.158352875976563,14.3783C12.158352875976563,13.54984,12.829922875976562,12.87827,13.658322875976562,12.87827C13.889922875976563,12.87827,14.109222875976563,12.93075,14.305022875976562,13.02446L17.486822875976564,9.84274C17.682022875976564,9.64747,17.99862287597656,9.64747,18.19392287597656,9.84274C18.38912287597656,10.038,18.38912287597656,10.35458,18.19392287597656,10.54984L15.012122875976562,13.73156C15.105822875976562,13.92738,15.158322875976562,14.1467,15.158322875976562,14.3783C15.158322875976562,15.2067,14.486822875976562,15.8783,13.658322875976562,15.8783C13.426822875976562,15.8783,13.207422875976562,15.8258,13.011642875976563,15.7321L10.415712875976563,18.328Z" stroke-opacity="0" stroke="none"></path></svg>'), iu = () => ru.cloneNode(!0), au = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M13.1889,6C12.98303,6.582599999999999,12.42741,7,11.7743,7C11.12119,7,10.565570000000001,6.582599999999999,10.35965,6L3.5,6C3.223857,6,3,5.77614,3,5.5C3,5.22386,3.223857,5,3.5,5L10.35965,5C10.565570000000001,4.417404,11.12119,4,11.7743,4C12.42741,4,12.98303,4.417404,13.1889,5L18.5,5C18.7761,5,19,5.22386,19,5.5C19,5.77614,18.7761,6,18.5,6L13.1889,6ZM3,8.5C3,8.22386,3.223857,8,3.5,8L18.5,8C18.7761,8,19,8.22386,19,8.5C19,8.77614,18.7761,9,18.5,9L3.5,9C3.223857,9,3,8.77614,3,8.5ZM3.278549,11.5C3.278549,11.22386,3.502407,11,3.778549,11L18.7785,11C19.0547,11,19.2785,11.22386,19.2785,11.5C19.2785,11.77614,19.0547,12,18.7785,12L3.778549,12C3.502407,12,3.278549,11.77614,3.278549,11.5ZM3.139267,14.5C3.139267,14.2239,3.363124,14,3.6392670000000003,14L18.6393,14C18.915399999999998,14,19.1393,14.2239,19.1393,14.5C19.1393,14.7761,18.915399999999998,15,18.6393,15L3.6392670000000003,15C3.363124,15,3.139267,14.7761,3.139267,14.5ZM13.1889,18C12.98303,18.5826,12.42741,19,11.7743,19C11.12119,19,10.565570000000001,18.5826,10.35965,18L3.778549,18C3.502407,18,3.278549,17.7761,3.278549,17.5C3.278549,17.2239,3.502407,17,3.778549,17L10.35965,17C10.565570000000001,16.4174,11.12119,16,11.7743,16C12.42741,16,12.98303,16.4174,13.1889,17L18.7785,17C19.0547,17,19.2785,17.2239,19.2785,17.5C19.2785,17.7761,19.0547,18,18.7785,18L13.1889,18Z" stroke-opacity="0" stroke="none"></path></svg>'), su = () => au.cloneNode(!0), ou = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M4.91465,6C4.70873,6.582599999999999,4.15311,7,3.5,7C2.671573,7,2,6.32843,2,5.5C2,4.671573,2.671573,4,3.5,4C4.15311,4,4.70873,4.417404,4.91465,5L18.2257,5C18.5018,5,18.7257,5.22386,18.7257,5.5C18.7257,5.77614,18.5018,6,18.2257,6L4.91465,6ZM2.7257,8.5C2.7257,8.22386,2.949558,8,3.2257,8L18.2257,8C18.5018,8,18.7257,8.22386,18.7257,8.5C18.7257,8.77614,18.5018,9,18.2257,9L3.2257,9C2.949558,9,2.7257,8.77614,2.7257,8.5ZM3.00425,11.5C3.00425,11.22386,3.22811,11,3.50425,11L18.5042,11C18.7804,11,19.0042,11.22386,19.0042,11.5C19.0042,11.77614,18.7804,12,18.5042,12L3.50425,12C3.22811,12,3.00425,11.77614,3.00425,11.5ZM2.864967,14.5C2.864967,14.2239,3.08882,14,3.36497,14L18.365,14C18.6411,14,18.865,14.2239,18.865,14.5C18.865,14.7761,18.6411,15,18.365,15L3.36497,15C3.08882,15,2.864967,14.7761,2.864967,14.5ZM20,17.5C20,18.328400000000002,19.3284,19,18.5,19C17.846899999999998,19,17.2913,18.5826,17.0854,18L3.50425,18C3.22811,18,3.00425,17.7761,3.00425,17.5C3.00425,17.2239,3.22811,17,3.50425,17L17.0854,17C17.2913,16.4174,17.846899999999998,16,18.5,16C19.3284,16,20,16.671599999999998,20,17.5Z" stroke-opacity="0" stroke="none"></path></svg>'), cu = () => ou.cloneNode(!0), lu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><ellipse cx="10.5" cy="11.5" rx="1.5" ry="1.5" stroke-opacity="0" stroke="none"></ellipse><ellipse cx="17.5" cy="11.5" rx="1.5" ry="1.5" stroke-opacity="0" stroke="none"></ellipse><ellipse cx="10.5" cy="11.5" rx="7" ry="7" fill-opacity="0" stroke-opacity="1" fill="none" stroke-width="1"></ellipse><ellipse cx="10.5" cy="11.5" rx="5" ry="5" fill-opacity="0" stroke-opacity="1" fill="none" stroke-width="1"></ellipse><ellipse cx="10.5" cy="11.5" rx="3" ry="3" fill-opacity="0" stroke-opacity="1" fill="none" stroke-width="1"></ellipse></svg>'), uu = () => lu.cloneNode(!0), du = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M3,7.32468C5.90649,3.3893050000000002,11.49833,2.81306,14.6674,6.31944C14.9056,6.1554199999999994,15.192,6.05979,15.5,6.05979C15.845,6.05979,16.1628,6.17974,16.4162,6.381349999999999L18.4509,4.23827L19,4.816615L16.8945,7.03429C16.962600000000002,7.21075,17,7.40319,17,7.60463C17,8.45782,16.328400000000002,9.14947,15.5,9.14947C14.6716,9.14947,14,8.45782,14,7.60463C14,7.36402,14.0534,7.13625,14.1487,6.93322C11.32695,3.748365,6.25159,4.253956,3.612785,7.82695L3,7.32468ZM14.09,15.4717C15.7427,13.78985,16.244500000000002,11.524740000000001,15.5633,9.30134L15.5618,9.30134L16.3012,9.0502C17.072400000000002,11.56646,16.497700000000002,14.158,14.6282,16.0599C12.28737,18.442,8.62386,18.6988,6.41348,16.4501C4.5526,14.5572,4.52076,11.19671,6.36766,9.3177C7.89069,7.76754,10.07544,7.706189999999999,11.56741,9.22363C11.95453,9.61742,12.24817,10.08363,12.43369,10.57677L14.1451,8.77421L14.6942,9.35256L12.64982,11.50582C12.65827,11.59712,12.66295,11.68839,12.66378,11.77936C12.87398,12.04523,13,12.38451,13,12.7541C13,13.60729,12.32843,14.2989,11.5,14.2989C10.67157,14.2989,10,13.60729,10,12.7541C10,11.90091,10.67157,11.20926,11.5,11.20926C11.60387,11.20926,11.70528,11.220130000000001,11.8032,11.240829999999999L11.81763,11.22564C11.69858,10.71874,11.42858,10.21929,11.0284,9.81179C9.844000000000001,8.60765,8.136890000000001,8.65592,6.90822,9.90586C5.37975,11.460930000000001,5.40693,14.288,6.95404,15.8619C8.84598,17.7867,12.03496,17.5626,14.09,15.4717Z" stroke-opacity="0" stroke="none"></path></svg>'), hu = () => du.cloneNode(!0), Cu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M4,17.0854L4,3.5C4,3.223858,4.22386,3,4.5,3C4.77614,3,5,3.223858,5,3.5L5,10L7.57584,10L9.8127,4.46359C9.91614,4.20756,10.20756,4.08386,10.46359,4.1873000000000005C10.71963,4.29075,10.84333,4.58216,10.73988,4.8382000000000005L8.65438,10L11.08535,10C11.29127,9.4174,11.84689,9,12.5,9C12.65154,9,12.79784,9.02247,12.93573,9.06427L16.6464,5.35355C16.8417,5.15829,17.1583,5.15829,17.3536,5.35355C17.5488,5.54882,17.5488,5.8654,17.3536,6.06066L13.7475,9.66675C13.907,9.90508,14,10.19168,14,10.5C14,11.15311,13.5826,11.70873,13,11.91465L13,14.3638L18.3714,12.1936C18.6274,12.09015,18.918799999999997,12.21385,19.0222,12.46989C19.1257,12.72592,19.002,13.0173,18.746000000000002,13.1208L13,15.4423L13,18L19.5,18C19.7761,18,20,18.2239,20,18.5C20,18.7761,19.7761,19,19.5,19L5.91465,19C5.70873,19.5826,5.15311,20,4.5,20C3.671573,20,3,19.3284,3,18.5C3,17.846899999999998,3.417404,17.2913,4,17.0854ZM6.3729499999999994,17.0413L12,14.7678L12,11.91465C11.88136,11.87271,11.76956,11.81627,11.66675,11.74746L6.3729499999999994,17.0413ZM12,15.8463L6.6694700000000005,18L12,18L12,15.8463ZM6.38629,15.6137L8.250350000000001,11L11,11L6.38629,15.6137ZM5,11L7.17182,11L5,16.3754L5,11Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), yu = () => Cu.cloneNode(!0), fu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M17,4.5C17,5.32843,16.328400000000002,6,15.5,6C15.0931,6,14.7241,5.83802,14.4539,5.57503L5.98992,8.32515C5.99658,8.38251,6,8.440850000000001,6,8.5C6,9.15311,5.582599999999999,9.70873,5,9.91465L5,11.08535C5.42621,11.236,5.763999999999999,11.57379,5.91465,12L19.5,12C19.7761,12,20,12.22386,20,12.5C20,12.77614,19.7761,13,19.5,13L5.91465,13C5.70873,13.5826,5.15311,14,4.5,14C3.671573,14,3,13.3284,3,12.5C3,11.84689,3.417404,11.29127,4,11.08535L4,9.91465C3.417404,9.70873,3,9.15311,3,8.5C3,7.67157,3.671573,7,4.5,7C4.90411,7,5.2709,7.15981,5.5406200000000005,7.41967L14.0093,4.66802C14.0032,4.6128599999999995,14,4.5568,14,4.5C14,3.671573,14.6716,3,15.5,3C16.328400000000002,3,17,3.671573,17,4.5ZM4,15.5C4,15.2239,4.22386,15,4.5,15L19.5,15C19.7761,15,20,15.2239,20,15.5C20,15.7761,19.7761,16,19.5,16L4.5,16C4.22386,16,4,15.7761,4,15.5ZM4,18.5C4,18.2239,4.22386,18,4.5,18L19.5,18C19.7761,18,20,18.2239,20,18.5C20,18.7761,19.7761,19,19.5,19L4.5,19C4.22386,19,4,18.7761,4,18.5Z" stroke-opacity="0" stroke="none"></path></svg>'), gu = () => fu.cloneNode(!0), mu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M20,3.5C20,4.15311,19.5826,4.70873,19,4.91465L19,18.5C19,18.7761,18.7761,19,18.5,19L4.91465,19C4.70873,19.5826,4.15311,20,3.5,20C2.671573,20,2,19.3284,2,18.5C2,17.846899999999998,2.417404,17.2913,3,17.0854L3,3.5C3,3.22386,3.22386,3,3.5,3L17.0854,3C17.2913,2.417404,17.846899999999998,2,18.5,2C19.3284,2,20,2.671573,20,3.5ZM17.0854,4C17.236,4.42621,17.5738,4.763999999999999,18,4.91465L18,8L14,8L14,4L17.0854,4ZM13,4L13,8L9,8L9,4L13,4ZM13,9L9,9L9,13L13,13L13,9ZM13,14L9,14L9,18L13,18L13,14ZM14,18L14,14L18,14L18,18L14,18ZM18,13L14,13L14,9L18,9L18,13ZM4.91465,18C4.763999999999999,17.5738,4.42621,17.236,4,17.0854L4,14L8,14L8,18L4.91465,18ZM4,8L4,4L8,4L8,8L4,8ZM8,9L8,13L4,13L4,9L8,9Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), pu = () => mu.cloneNode(!0), vu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><ellipse cx="10.5" cy="11.5" rx="1.5" ry="1.5" stroke-opacity="0" stroke="none"></ellipse><ellipse cx="17.5" cy="11.5" rx="1.5" ry="1.5" stroke-opacity="0" stroke="none"></ellipse><ellipse cx="10.5" cy="11.5" rx="7" ry="7" fill-opacity="0" fill="none" stroke-opacity="1" stroke-width="1"></ellipse></svg>'), _u = () => vu.cloneNode(!0), $u = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M11.57625,6.9981C11.55099,6.999359999999999,11.52557,7,11.5,7C11.34,7,11.18584,6.97495,11.04125,6.9285499999999995L5.55401,16.4327C5.713760000000001,16.5905,5.83826,16.7839,5.91465,17L16.0854,17C16.2187,16.622700000000002,16.4987,16.314700000000002,16.8569,16.1445L11.57625,6.9981ZM12.50759,6.611219999999999C12.81005,6.336790000000001,13,5.94058,13,5.5C13,4.671573,12.32843,4,11.5,4C10.67157,4,10,4.671573,10,5.5C10,5.80059,10.08841,6.08052,10.24066,6.31522L4.64514,16.0069C4.59738,16.002299999999998,4.54896,16,4.5,16C3.671573,16,3,16.671599999999998,3,17.5C3,18.328400000000002,3.671573,19,4.5,19C5.15311,19,5.70873,18.5826,5.91465,18L16.0854,18C16.2913,18.5826,16.846899999999998,19,17.5,19C18.328400000000002,19,19,18.328400000000002,19,17.5C19,16.8365,18.5691,16.2735,17.971899999999998,16.075699999999998L12.50759,6.611219999999999Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), bu = () => $u.cloneNode(!0), Lu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M19,4.5C19,5.15311,18.5826,5.70873,18,5.91465L18,18.5C18,18.7761,17.7761,19,17.5,19L5.91465,19C5.70873,19.5826,5.15311,20,4.5,20C3.671573,20,3,19.3284,3,18.5C3,17.846899999999998,3.417404,17.2913,4,17.0854L4,4.5C4,4.22386,4.22386,4,4.5,4L16.0854,4C16.2913,3.417404,16.846899999999998,3,17.5,3C18.328400000000002,3,19,3.671573,19,4.5ZM5,5L16.0854,5C16.236,5.42621,16.5738,5.763999999999999,17,5.91465L17,18L5.91465,18C5.763999999999999,17.5738,5.42621,17.236,5,17.0854L5,5Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), xu = () => Lu.cloneNode(!0), ku = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M19.6401,7.99355C20.4028,7.92291,21,7.2811900000000005,21,6.5C21,5.671573,20.3284,5,19.5,5C18.8469,5,18.2913,5.417404,18.0854,6L7.62067,6C7.34453,6,7.12067,6.22386,7.12067,6.5C7.12067,6.5479,7.12741,6.59423,7.13999,6.63809L3.2294099999999997,15.0243C2.530138,15.1517,2,15.764,2,16.5C2,17.328400000000002,2.671573,18,3.5,18C4.15311,18,4.70873,17.5826,4.91465,17L14.5963,17C14.6456,17.076,14.7162,17.1396,14.8044,17.1807C15.0546,17.2974,15.3521,17.1891,15.4688,16.9388L19.6401,7.99355ZM14.7896,16.0293L18.6551,7.739599999999999C18.3942,7.56144,18.1925,7.30307,18.0854,7L8.0746,7L4.25044,15.2009C4.55701,15.3784,4.79493,15.6613,4.91465,16L14.6207,16C14.68,16,14.7368,16.0103,14.7896,16.0293Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), Au = () => ku.cloneNode(!0), wu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M8.134443814697265,7.494615087890625L8.764323814697265,7.494615087890625L8.764323814697265,3.414215087890625L8.310223814697267,3.414215087890625L7.294603814697266,4.005035087890625L7.289713814697266,4.634915087890625L8.134443814697265,4.149892087890625L8.134443814697265,7.494615087890625ZM18.832003814697266,6.933095087890624Q19.004603814697266,6.635245087890625,19.004603814697266,6.2543850878906255Q19.004603814697266,5.884915087890625,18.845103814697264,5.593575087890625Q18.685503814697267,5.3006050878906255,18.399103814697266,5.136225087890625Q18.114303814697266,4.9702050878906245,17.754603814697266,4.9653250878906245L18.820603814697265,3.840647087890625L18.820603814697265,3.414215087890625L16.519203814697264,3.414215087890625L16.519203814697264,3.939931087890625L18.050803814697264,3.939931087890625L16.719403814697266,5.334785087890625L17.074203814697263,5.7205350878906245Q17.254903814697265,5.484525087890625,17.619503814697268,5.484525087890625Q17.980803814697268,5.484525087890625,18.187503814697266,5.689605087890625Q18.394203814697267,5.894685087890625,18.394203814697267,6.2543850878906255Q18.394203814697267,6.604315087890625,18.187503814697266,6.822415087890625Q17.980803814697268,7.0405150878906255,17.640603814697265,7.0405150878906255Q17.334603814697267,7.0405150878906255,17.124703814697266,6.890775087890625Q16.914703814697265,6.739415087890626,16.820303814697265,6.469225087890624L16.354803814697263,6.744295087890626Q16.480103814697266,7.125155087890625,16.821903814697265,7.341625087890625Q17.165403814697264,7.559725087890625,17.640603814697265,7.559725087890625Q18.039403814697266,7.559725087890625,18.348603814697267,7.393705087890625Q18.659503814697267,7.229315087890625,18.832003814697266,6.933095087890624ZM10.000003814697266,10.634915087890626C10.000003814697266,11.024655087890626,9.851363814697265,11.379685087890625,9.607683814697266,11.646395087890625L12.168903814697266,15.171615087890626C12.275403814697265,15.147615087890625,12.386203814697266,15.134915087890626,12.500003814697266,15.134915087890626C12.596503814697266,15.134915087890626,12.690803814697265,15.144015087890624,12.782303814697265,15.161415087890624L16.108803814697268,11.196955087890625C16.038703814697264,11.023375087890624,16.000003814697266,10.833655087890625,16.000003814697266,10.634915087890626C16.000003814697266,9.806495087890625,16.671603814697264,9.134915087890626,17.500003814697266,9.134915087890626C18.328403814697264,9.134915087890626,19.000003814697266,9.806495087890625,19.000003814697266,10.634915087890626C19.000003814697266,11.463345087890625,18.328403814697264,12.134915087890626,17.500003814697266,12.134915087890626C17.239503814697265,12.134915087890626,16.994503814697268,12.068495087890625,16.781003814697264,11.951675087890624L13.654703814697266,15.677415087890624C13.870303814697266,15.937215087890625,14.000003814697266,16.270915087890625,14.000003814697266,16.634915087890626C14.000003814697266,17.463315087890624,13.328403814697266,18.134915087890626,12.500003814697266,18.134915087890626C11.671573814697265,18.134915087890626,11.000003814697266,17.463315087890624,11.000003814697266,16.634915087890626C11.000003814697266,16.284415087890626,11.120193814697265,15.962015087890626,11.321603814697266,15.706715087890625L8.715393814697265,12.119565087890624C8.645053814697267,12.129685087890625,8.573143814697266,12.134915087890626,8.500003814697266,12.134915087890626C8.162103814697264,12.134915087890626,7.8503038146972655,12.023195087890626,7.599523814697266,11.834665087890626L4.505583814697266,15.521915087890624C4.809213814697266,15.796415087890624,5.000003814697266,16.193415087890624,5.000003814697266,16.634915087890626C5.000003814697266,17.463315087890624,4.328433814697266,18.134915087890626,3.5000038146972656,18.134915087890626C2.6715768146972656,18.134915087890626,2.0000038146972656,17.463315087890624,2.0000038146972656,16.634915087890626C2.0000038146972656,15.806515087890626,2.6715768146972656,15.134915087890626,3.5000038146972656,15.134915087890626C3.508253814697266,15.134915087890626,3.5164838146972657,15.135015087890626,3.524703814697266,15.135115087890625L7.033823814697266,10.953115087890625C7.011673814697265,10.850565087890626,7.000003814697266,10.744105087890624,7.000003814697266,10.634915087890626C7.000003814697266,9.806495087890625,7.671573814697266,9.134915087890626,8.500003814697266,9.134915087890626C9.328433814697267,9.134915087890626,10.000003814697266,9.806495087890625,10.000003814697266,10.634915087890626Z" stroke-opacity="0" stroke="none"></path></svg>'), Mu = () => wu.cloneNode(!0), Su = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M8.13444,7.494615087890625L8.76432,7.494615087890625L8.76432,3.414215087890625L8.310220000000001,3.414215087890625L7.2946,4.005035087890625L7.28971,4.634915087890625L8.13444,4.149892087890625L8.13444,7.494615087890625ZM18.832,6.929835087890625Q19.0046,6.635245087890625,19.0046,6.2543850878906255Q19.0046,5.889805087890625,18.8451,5.5952050878906245Q18.6855,5.3006050878906255,18.3975,5.132965087890625Q18.1094,4.9653250878906245,17.7399,4.9653250878906245Q17.435499999999998,4.9653250878906245,17.1556,5.149245087890625L17.2793,3.939931087890625L18.8304,3.939931087890625L18.8304,3.414215087890625L16.7406,3.414215087890625L16.5094,5.665195087890625L17.0156,5.795405087890625Q17.095399999999998,5.655425087890626,17.2516,5.570795087890625Q17.4095,5.484525087890625,17.6357,5.484525087890625Q17.9694,5.484525087890625,18.1842,5.697745087890625Q18.4007,5.909335087890625,18.4007,6.2543850878906255Q18.4007,6.604315087890625,18.1842,6.822415087890625Q17.9694,7.0405150878906255,17.6292,7.0405150878906255Q17.3298,7.0405150878906255,17.119799999999998,6.890775087890625Q16.9098,6.739415087890626,16.825200000000002,6.474115087890625L16.3597,6.749175087890626Q16.470399999999998,7.110505087890624,16.807299999999998,7.335115087890625Q17.144199999999998,7.559725087890625,17.6292,7.559725087890625Q18.0296,7.559725087890625,18.3438,7.392075087890625Q18.6595,7.224435087890625,18.832,6.929835087890625ZM10,10.634915087890626C10,11.024655087890626,9.85136,11.379685087890625,9.60768,11.646395087890625L12.1689,15.171615087890626C12.2754,15.147615087890625,12.3862,15.134915087890626,12.5,15.134915087890626C12.5965,15.134915087890626,12.6908,15.144015087890624,12.7823,15.161415087890624L16.108800000000002,11.196955087890625C16.0387,11.023375087890624,16,10.833655087890625,16,10.634915087890626C16,9.806495087890625,16.671599999999998,9.134915087890626,17.5,9.134915087890626C18.3284,9.134915087890626,19,9.806495087890625,19,10.634915087890626C19,11.463345087890625,18.3284,12.134915087890626,17.5,12.134915087890626C17.2395,12.134915087890626,16.994500000000002,12.068505087890625,16.781,11.951675087890624L13.6547,15.677415087890624C13.8703,15.937215087890625,14,16.270915087890625,14,16.634915087890626C14,17.463315087890624,13.3284,18.134915087890626,12.5,18.134915087890626C11.67157,18.134915087890626,11,17.463315087890624,11,16.634915087890626C11,16.284415087890626,11.12019,15.962015087890626,11.3216,15.706715087890625L8.71539,12.119565087890624C8.645050000000001,12.129685087890625,8.57314,12.134915087890626,8.5,12.134915087890626C8.162099999999999,12.134915087890626,7.8503,12.023195087890626,7.59952,11.834665087890626L4.50558,15.521915087890624C4.80921,15.796415087890624,5,16.193415087890624,5,16.634915087890626C5,17.463315087890624,4.32843,18.134915087890626,3.5,18.134915087890626C2.671573,18.134915087890626,2,17.463315087890624,2,16.634915087890626C2,15.806515087890626,2.671573,15.134915087890626,3.5,15.134915087890626C3.5082500000000003,15.134915087890626,3.51648,15.135015087890626,3.5247,15.135115087890625L7.03382,10.953115087890625C7.01167,10.850565087890626,7,10.744105087890624,7,10.634915087890626C7,9.806495087890625,7.67157,9.134915087890626,8.5,9.134915087890626C9.32843,9.134915087890626,10,9.806495087890625,10,10.634915087890626Z" stroke-opacity="0" stroke="none"></path></svg>'), Tu = () => Su.cloneNode(!0), Pu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M18.8532,7.020985087890625Q19.0257,6.734525087890625,19.0257,6.369945087890625Q19.0257,6.020005087890625,18.8499,5.754705087890625Q18.6758,5.489415087890626,18.3649,5.339675087890625Q18.5944,5.209465087890625,18.7214,4.994615087890625Q18.8499,4.779775087890625,18.8499,4.5193550878906255Q18.8499,4.2003480878906245,18.7002,3.951324087890625Q18.5505,3.700673087890625,18.277,3.557444087890625Q18.0052,3.414215087890625,17.6455,3.414215087890625Q17.285800000000002,3.414215087890625,17.0107,3.557444087890625Q16.7357,3.700673087890625,16.5843,3.951324087890625Q16.4346,4.2003480878906245,16.4346,4.5193550878906255Q16.4346,4.779775087890625,16.561500000000002,4.994615087890625Q16.6901,5.209465087890625,16.919600000000003,5.339675087890625Q16.6055,5.489415087890626,16.4297,5.757965087890625Q16.255499999999998,6.024895087890625,16.255499999999998,6.369945087890625Q16.255499999999998,6.734525087890625,16.4297,7.020985087890625Q16.6055,7.305815087890625,16.919600000000003,7.465325087890625Q17.2354,7.624825087890625,17.6455,7.624825087890625Q18.0557,7.624825087890625,18.3682,7.465325087890625Q18.6807,7.305815087890625,18.8532,7.020985087890625ZM8.76432,7.559725087890625L8.13444,7.559725087890625L8.13444,4.214996087890625L7.28971,4.700025087890625L7.2946,4.070139087890625L8.310220000000001,3.479319087890625L8.76432,3.479319087890625L8.76432,7.559725087890625ZM17.1816,4.955555087890625Q17.0042,4.784655087890625,17.0042,4.5095950878906255Q17.0042,4.229645087890625,17.18,4.057119087890625Q17.355800000000002,3.884592087890625,17.6455,3.884592087890625Q17.935200000000002,3.884592087890625,18.1077,4.057119087890625Q18.2803,4.229645087890625,18.2803,4.5095950878906255Q18.2803,4.784655087890625,18.1045,4.955555087890625Q17.930300000000003,5.124825087890625,17.6455,5.124825087890625Q17.3607,5.124825087890625,17.1816,4.955555087890625ZM18.2217,5.7953950878906255Q18.4398,6.005365087890625,18.4398,6.3552950878906245Q18.4398,6.705235087890625,18.2217,6.915195087890625Q18.0052,7.125155087890625,17.6455,7.125155087890625Q17.285800000000002,7.125155087890625,17.067700000000002,6.915195087890625Q16.849600000000002,6.705235087890625,16.849600000000002,6.3552950878906245Q16.849600000000002,6.005365087890625,17.064500000000002,5.7953950878906255Q17.2793,5.585435087890625,17.6455,5.585435087890625Q18.0052,5.585435087890625,18.2217,5.7953950878906255ZM9.60768,11.711495087890626C9.85136,11.444785087890626,10,11.089765087890626,10,10.700025087890625C10,9.871595087890626,9.32843,9.200025087890625,8.5,9.200025087890625C7.67157,9.200025087890625,7,9.871595087890626,7,10.700025087890625C7,10.809205087890625,7.01167,10.915665087890625,7.03382,11.018215087890624L3.5247,15.200215087890625C3.51648,15.200115087890625,3.5082500000000003,15.200015087890625,3.5,15.200015087890625C2.671573,15.200015087890625,2,15.871615087890625,2,16.700015087890627C2,17.528415087890625,2.671573,18.200015087890627,3.5,18.200015087890627C4.32843,18.200015087890627,5,17.528415087890625,5,16.700015087890627C5,16.258515087890625,4.80921,15.861515087890625,4.50558,15.587015087890626L7.59952,11.899765087890625C7.8503,12.088295087890625,8.162099999999999,12.200025087890625,8.5,12.200025087890625C8.57314,12.200025087890625,8.645050000000001,12.194785087890626,8.71539,12.184675087890625L11.3216,15.771815087890625C11.12019,16.027215087890625,11,16.349515087890623,11,16.700015087890627C11,17.528415087890625,11.67157,18.200015087890627,12.5,18.200015087890627C13.3284,18.200015087890627,14,17.528415087890625,14,16.700015087890627C14,16.336015087890623,13.8703,16.002315087890626,13.6547,15.742515087890625L16.781,12.016775087890625C16.994500000000002,12.133605087890626,17.2395,12.200025087890625,17.5,12.200025087890625C18.3284,12.200025087890625,19,11.528445087890624,19,10.700025087890625C19,9.871595087890626,18.3284,9.200025087890625,17.5,9.200025087890625C16.671599999999998,9.200025087890625,16,9.871595087890626,16,10.700025087890625C16,10.898765087890624,16.0387,11.088475087890625,16.108800000000002,11.262055087890625L12.7823,15.226515087890625C12.6908,15.209115087890625,12.5965,15.200015087890625,12.5,15.200015087890625C12.3862,15.200015087890625,12.2754,15.212715087890626,12.1689,15.236715087890625L9.60768,11.711495087890626Z" stroke-opacity="0" stroke="none"></path></svg>'), Iu = () => Pu.cloneNode(!0), Ou = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M9.474616630859375,7.494615087890625L8.844736630859375,7.494615087890625L8.844736630859375,4.149892087890625L8.000006630859374,4.634915087890625L8.004896630859374,4.005035087890625L9.020516630859376,3.414215087890625L9.474616630859375,3.414215087890625L9.474616630859375,7.494615087890625ZM18.529296630859378,4.8318550878906255Q18.307996630859375,5.028795087890625,18.122396630859377,5.385245087890625Q17.868496630859376,5.019035087890625,17.629196630859376,4.8269750878906255Q17.389996630859375,4.634915087890625,17.168596630859376,4.634915087890625Q16.794296630859375,4.634915087890625,16.522496630859376,4.976715087890625Q16.252296630859377,5.3168850878906255,16.252296630859377,5.7856350878906255Q16.252296630859377,6.218575087890625,16.502896630859375,6.521315087890625Q16.755196630859373,6.822415087890625,17.114896630859377,6.822415087890625Q17.368796630859375,6.822415087890625,17.588596630859374,6.625475087890624Q17.809896630859377,6.428535087890625,17.998696630859374,6.0688350878906245Q18.249396630859373,6.439935087890625,18.488596630859377,6.631985087890625Q18.727896630859377,6.822415087890625,18.952496630859375,6.822415087890625Q19.326796630859373,6.822415087890625,19.596996630859376,6.482245087890625Q19.868796630859375,6.140455087890626,19.868796630859375,5.671705087890626Q19.868796630859375,5.238755087890625,19.618196630859376,4.937655087890625Q19.367496630859375,4.634915087890625,19.006196630859375,4.634915087890625Q18.750696630859377,4.634915087890625,18.529296630859378,4.8318550878906255ZM18.337296630859377,5.674955087890625L18.278696630859375,5.596835087890625Q18.449596630859375,5.272935087890625,18.622096630859374,5.1101750878906245Q18.794596630859374,4.947415087890625,18.967096630859373,4.947415087890625Q19.194996630859375,4.947415087890625,19.346396630859374,5.1345950878906255Q19.497696630859377,5.320135087890625,19.497696630859377,5.598455087890625Q19.497696630859377,5.8914250878906245,19.360996630859376,6.096505087890625Q19.224296630859374,6.301585087890626,19.027396630859375,6.301585087890626Q18.915096630859374,6.301585087890626,18.742496630859375,6.146965087890624Q18.569996630859375,5.992335087890625,18.337296630859377,5.674955087890625ZM17.785496630859377,5.779125087890625L17.842496630859372,5.857245087890625Q17.668296630859373,6.186025087890625,17.495796630859374,6.348785087890625Q17.324896630859374,6.509915087890625,17.153996630859375,6.509915087890625Q16.926096630859377,6.509915087890625,16.774796630859377,6.324375087890624Q16.623396630859375,6.137195087890625,16.623396630859375,5.858875087890625Q16.623396630859375,5.565905087890625,16.761696630859376,5.360825087890625Q16.900096630859373,5.1557550878906255,17.095396630859376,5.1557550878906255Q17.228896630859374,5.1557550878906255,17.365596630859375,5.2778250878906245Q17.502296630859377,5.399895087890625,17.785496630859377,5.779125087890625ZM10.710296630859375,10.634915087890626C10.710296630859375,11.024655087890626,10.561656630859375,11.379685087890625,10.317976630859375,11.646395087890625L12.879196630859376,15.171615087890626C12.985696630859374,15.147615087890625,13.096496630859376,15.134915087890626,13.210296630859375,15.134915087890626C13.306796630859376,15.134915087890626,13.401096630859374,15.144015087890624,13.492596630859374,15.161415087890624L16.819096630859377,11.196955087890625C16.748996630859374,11.023375087890624,16.710296630859375,10.833655087890625,16.710296630859375,10.634915087890626C16.710296630859375,9.806495087890625,17.381896630859373,9.134915087890626,18.210296630859375,9.134915087890626C19.038696630859373,9.134915087890626,19.710296630859375,9.806495087890625,19.710296630859375,10.634915087890626C19.710296630859375,11.463345087890625,19.038696630859373,12.134915087890626,18.210296630859375,12.134915087890626C17.949796630859375,12.134915087890626,17.704796630859377,12.068505087890625,17.491296630859374,11.951675087890624L14.364996630859375,15.677415087890624C14.580596630859375,15.937215087890625,14.710296630859375,16.270915087890625,14.710296630859375,16.634915087890626C14.710296630859375,17.463315087890624,14.038696630859375,18.134915087890626,13.210296630859375,18.134915087890626C12.381866630859374,18.134915087890626,11.710296630859375,17.463315087890624,11.710296630859375,16.634915087890626C11.710296630859375,16.284415087890626,11.830486630859374,15.962015087890626,12.031896630859375,15.706715087890625L9.425686630859374,12.119565087890624C9.355346630859376,12.129685087890625,9.283436630859375,12.134915087890626,9.210296630859375,12.134915087890626C8.872396630859374,12.134915087890626,8.560596630859376,12.023195087890626,8.309816630859375,11.834665087890626L5.215876630859375,15.521915087890624C5.519506630859375,15.796415087890624,5.710296630859375,16.193415087890624,5.710296630859375,16.634915087890626C5.710296630859375,17.463315087890624,5.038726630859375,18.134915087890626,4.210296630859375,18.134915087890626C3.381869630859375,18.134915087890626,2.710296630859375,17.463315087890624,2.710296630859375,16.634915087890626C2.710296630859375,15.806515087890626,3.381869630859375,15.134915087890626,4.210296630859375,15.134915087890626C4.218546630859375,15.134915087890626,4.226776630859375,15.135015087890626,4.234996630859375,15.135115087890625L7.744116630859375,10.953115087890625C7.721966630859375,10.850565087890626,7.710296630859375,10.744105087890624,7.710296630859375,10.634915087890626C7.710296630859375,9.806495087890625,8.381866630859374,9.134915087890626,9.210296630859375,9.134915087890626C10.038726630859376,9.134915087890626,10.710296630859375,9.806495087890625,10.710296630859375,10.634915087890626Z" stroke-opacity="0" stroke="none"></path></svg>'), Nu = () => Ou.cloneNode(!0), Du = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M21,5.5C21,6.32843,20.3284,7,19.5,7C19.4136,7,19.3289,6.99269,19.2465,6.97866L15.6257,15.5086C15.8587,15.7729,16,16.119999999999997,16,16.5C16,17.328400000000002,15.3284,18,14.5,18C13.8469,18,13.2913,17.5826,13.0854,17L3.91465,17C3.70873,17.5826,3.15311,18,2.5,18C1.671573,18,1,17.328400000000002,1,16.5C1,15.6716,1.671573,15,2.5,15C2.5840199999999998,15,2.66643,15.0069,2.74668,15.0202L6.36934,6.48574C6.13933,6.22213,6,5.87733,6,5.5C6,4.671573,6.67157,4,7.5,4C8.15311,4,8.70873,4.417404,8.91465,5L18.0854,5C18.2913,4.417404,18.8469,4,19.5,4C20.3284,4,21,4.671573,21,5.5ZM18.0854,6L8.91465,6C8.892579999999999,6.06243,8.8665,6.12296,8.83672,6.18128L13.9814,15.0921C14.143,15.0325,14.3177,15,14.5,15C14.584,15,14.6664,15.0069,14.7467,15.0202L18.3693,6.48574C18.2462,6.3446,18.149,6.1802,18.0854,6ZM13.2036,15.745L8.0861,6.8811800000000005C7.90605,6.95768,7.70797,7,7.5,7C7.41359,7,7.32888,6.99269,7.24647,6.97866L3.62571,15.5086C3.7512,15.651,3.8501,15.8174,3.91465,16L13.0854,16C13.1169,15.9108,13.1566,15.8255,13.2036,15.745Z" stroke-opacity="0" stroke="none"></path></svg>'), Eu = () => Du.cloneNode(!0), Bu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M5.92159,5.93994C6.04014,5.90529,6.152620000000001,5.85639,6.25704,5.79523L9.12729,9.89437C9.045449999999999,10.07959,9,10.28449,9,10.5C9,10.79522,9.08529,11.07053,9.232569999999999,11.30262L4.97573,16.7511L5.92159,5.93994ZM4.92259,5.8848400000000005C4.38078,5.658659999999999,4,5.1238,4,4.5C4,3.671573,4.67157,3,5.5,3C6.2157,3,6.81433,3.50124,6.96399,4.17183L15.1309,4.88634C15.3654,4.36387,15.8902,4,16.5,4C17.328400000000002,4,18,4.67157,18,5.5C18,6.08983,17.659599999999998,6.60015,17.1645,6.84518L18.4264,14.0018C18.4508,14.0006,18.4753,14,18.5,14C19.3284,14,20,14.6716,20,15.5C20,16.328400000000002,19.3284,17,18.5,17C17.932499999999997,17,17.4386,16.6849,17.183799999999998,16.22L5.99686,18.5979C5.946429999999999,19.3807,5.29554,20,4.5,20C3.671573,20,3,19.3284,3,18.5C3,17.869300000000003,3.389292,17.3295,3.94071,17.1077L4.92259,5.8848400000000005ZM5.72452,17.6334C5.69799,17.596,5.6698,17.5599,5.64004,17.525100000000002L10.01843,11.92103C10.16958,11.97223,10.33155,12,10.5,12C10.80059,12,11.08052,11.91158,11.31522,11.75934L17.0606,15.0765C17.0457,15.1271,17.0335,15.1789,17.023899999999998,15.2317L5.72452,17.6334ZM11.92855,10.95875L17.4349,14.1379L16.1699,6.96356C15.9874,6.92257,15.8174,6.8483,15.6667,6.74746L11.99771,10.4165C11.99923,10.44414,12,10.47198,12,10.5C12,10.66,11.97495,10.814160000000001,11.92855,10.95875ZM10.5,9C10.259830000000001,9,10.03285,9.05644,9.83159,9.15679L7.04919,5.1831L15.0493,5.88302C15.054,5.90072,15.059,5.91829,15.0643,5.9357299999999995L11.56066,9.43934C11.28921,9.16789,10.91421,9,10.5,9Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), Uu = () => Bu.cloneNode(!0), Fu = /* @__PURE__ */ p('<svg viewBox="0 0 22 22"><path d="M4.727219638671875,8.007996215820313L9.973849638671876,2.7629472158203123C10.167279638671875,2.5696791158203123,10.480729638671875,2.5696791158203123,10.674169638671875,2.7629472158203123L13.223329638671874,5.311756215820313C13.416929638671874,5.505236215820313,13.416929638671874,5.8189862158203125,13.223329638671874,6.012466215820313L7.977129638671875,11.257906215820313C7.379859638671875,11.855176215820313,7.407609638671875,12.909396215820312,8.033809638671876,13.535596215820313C8.660409638671876,14.162596215820313,9.713849638671874,14.189996215820312,10.311129638671876,13.591896215820313L15.556929638671875,8.346066215820311C15.750429638671875,8.152526215820313,16.064229638671875,8.152526215820313,16.257629638671872,8.346066215820311L18.806529638671876,10.895266215820312C19.000029638671876,11.088746215820313,19.000029638671876,11.402496215820312,18.806529638671876,11.595976215820313L13.560629638671875,16.841796215820313C11.165619638671876,19.237196215820312,7.197149638671875,19.19919621582031,4.783499638671875,16.785496215820313C2.3698426386718747,14.371896215820312,2.331397638671875,10.403416215820313,4.727219638671875,8.007996215820313ZM12.172299638671875,5.662106215820312L10.323809638671875,3.8136162158203124L5.4287196386718755,8.709096215820313C3.422893638671875,10.714536215820312,3.4549956386718748,14.055196215820313,5.484999638671875,16.08479621582031C7.514609638671875,18.114796215820313,10.855289638671875,18.146496215820314,12.860719638671876,16.141096215820312L15.465629638671874,13.535796215820312L14.090929638671875,12.160756215820312L14.791629638671875,11.460436215820312L16.166229638671876,12.834996215820313L17.755829638671877,11.245226215820313L15.907729638671874,9.396736215820312L11.011839638671875,14.292596215820312C10.042809638671875,15.262396215820312,8.418249638671874,15.243796215820312,7.406019638671875,14.306496215820312L7.333099638671875,14.236296215820312C6.327599638671876,13.230796215820313,6.284009638671876,11.550396215820312,7.276419638671875,10.557586215820312L9.882199638671874,7.952026215820313L8.501079638671875,6.570906215820313L9.201789638671876,5.870186215820313L10.582939638671874,7.251336215820312L12.172299638671875,5.662106215820312Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), Ku = (e) => (() => {
  const t = Fu.cloneNode(!0);
  return y1(t, "class", `icon-overlay ${e ?? ""}`), t;
})(), ju = /* @__PURE__ */ p('<svg viewBox="0 0 22 22"><defs><clipPath id="master_svg0_151_615"><rect x="0" y="0" width="22" height="22" rx="0"></rect></clipPath></defs><g clip-path="url(#master_svg0_151_615)"><path d="M19.672,3.0673368C19.4417,2.9354008,19.1463,3.00292252,18.9994,3.2210900000000002L17.4588,5.50622L16.743299999999998,3.781253L13.9915,7.4662L13.9618,7.51108C13.8339,7.72862,13.8936,8.005659999999999,14.1004,8.15391L14.1462,8.183430000000001C14.3683,8.308720000000001,14.6511,8.25001,14.8022,8.047229999999999L16.4907,5.78571L17.246299999999998,7.60713L19.8374,3.7635389999999997L19.8651,3.717088C19.9871,3.484615,19.9023,3.199273,19.672,3.0673368ZM4.79974,8.462530000000001L10.117740000000001,3.252975C10.31381,3.0610145,10.63152,3.0610145,10.82759,3.252975L13.4115,5.78453C13.6076,5.976710000000001,13.6076,6.28833,13.4115,6.4805L8.093869999999999,11.69045C7.48847,12.28368,7.51659,13.3308,8.151309999999999,13.9528C8.786439999999999,14.5755,9.85421,14.6027,10.45961,14.0087L15.7768,8.79831C15.9729,8.60609,16.2909,8.60609,16.487099999999998,8.79831L19.0705,11.33026C19.2667,11.52244,19.2667,11.83406,19.0705,12.02623L13.7533,17.2366C11.32572,19.6158,7.30328,19.578,4.85679,17.1807C2.410298,14.7834,2.371331,10.84174,4.79974,8.462530000000001ZM12.3461,6.1325199999999995L10.47246,4.29654L5.51079,9.15889C3.477674,11.15076,3.510214,14.4688,5.56784,16.4847C7.62506,18.500999999999998,11.01117,18.5325,13.0439,16.540599999999998L15.6842,13.9529L14.2908,12.58718L15.0011,11.89161L16.394399999999997,13.2569L18.0056,11.67786L16.1323,9.84188L11.16985,14.7046C10.18764,15.6679,8.540980000000001,15.6494,7.51498,14.7184L7.44107,14.6487C6.4219,13.65,6.37771,11.98096,7.38362,10.994869999999999L10.02485,8.40693L8.624939999999999,7.03516L9.335180000000001,6.33919L10.73512,7.71099L12.3461,6.1325199999999995Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></g></svg>'), zu = (e) => (() => {
  const t = ju.cloneNode(!0);
  return y1(t, "class", `icon-overlay ${e ?? ""}`), t;
})(), Qu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M11,17C5.80945,17,3.667717,12.85,3.113386,11.575C2.9622047,11.2,2.9622047,10.8,3.113386,10.425C3.667717,9.15,5.80945,5,11,5C16.165399999999998,5,18.3323,9.15,18.8866,10.425C19.0378,10.8,19.0378,11.2,18.8866,11.575C18.3323,12.85,16.165399999999998,17,11,17ZM4.04567,10.8C3.995276,10.925,3.995276,11.05,4.04567,11.175C4.52441,12.325,6.43937,16,11,16C15.5606,16,17.4756,12.325,17.9543,11.2C18.0047,11.075,18.0047,10.95,17.9543,10.825C17.4756,9.675,15.5606,6,11,6C6.43937,6,4.52441,9.675,4.04567,10.8ZM11,13.5C9.61417,13.5,8.480319999999999,12.375,8.480319999999999,11C8.480319999999999,9.625,9.61417,8.5,11,8.5C12.38583,8.5,13.5197,9.625,13.5197,11C13.5197,12.375,12.38583,13.5,11,13.5ZM11,9.5C10.1685,9.5,9.48819,10.175,9.48819,11C9.48819,11.825,10.1685,12.5,11,12.5C11.8315,12.5,12.51181,11.825,12.51181,11C12.51181,10.175,11.8315,9.5,11,9.5Z" stroke-opacity="0" fill-opacity="1"></path></svg>'), Zu = () => Qu.cloneNode(!0), Ru = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M5.80417,14.9887L4.62563,16.167299999999997C4.43037,16.3625,4.43037,16.6791,4.62563,16.8744C4.82089,17.0696,5.13748,17.0696,5.332739999999999,16.8744L6.62638,15.5807C7.75595,16.290100000000002,9.19328,16.7929,11,16.7929C16.165399999999998,16.7929,18.3323,12.64289,18.8866,11.36789C19.0378,10.99289,19.0378,10.59289,18.8866,10.21789C18.5549,9.45486,17.6456,7.66212,15.8617,6.34545L17.3536,4.853553C17.5488,4.658291,17.5488,4.341709,17.3536,4.146447C17.1583,3.9511845,16.8417,3.9511845,16.6464,4.146447L15.0014,5.7915399999999995C13.9314,5.1969,12.61166,4.792893,11,4.792893C5.80945,4.792893,3.667717,8.94289,3.113386,10.21789C2.9622049,10.59289,2.9622049,10.99289,3.113386,11.36789C3.424435,12.08333,4.2353000000000005,13.70399,5.80417,14.9887ZM7.36012,14.847C8.32327,15.4074,9.52286,15.7929,11,15.7929C15.5606,15.7929,17.4756,12.11789,17.9543,10.99289C18.0047,10.86789,18.0047,10.74289,17.9543,10.61789C17.659,9.90846,16.8171,8.23812,15.1447,7.06241L12.96929,9.23782C13.3134,9.66543,13.5197,10.20642,13.5197,10.79289C13.5197,12.16789,12.38583,13.29289,11,13.29289C10.41596,13.29289,9.87667,13.09308,9.44815,12.75896L7.36012,14.847ZM8.794609999999999,11.99829L6.520099999999999,14.2728C5.06905,13.12119,4.32057,11.628250000000001,4.04567,10.96789C3.995275,10.84289,3.995275,10.71789,4.04567,10.59289C4.52441,9.46789,6.43937,5.79289,11,5.79289C12.28868,5.79289,13.3661,6.086320000000001,14.2596,6.53329L12.19759,8.5953C11.84086,8.40257,11.43271,8.29289,11,8.29289C9.61417,8.29289,8.480319999999999,9.41789,8.480319999999999,10.79289C8.480319999999999,11.22918,8.594470000000001,11.64029,8.794609999999999,11.99829ZM10.16528,12.04183C10.404869999999999,12.20032,10.692070000000001,12.29289,11,12.29289C11.8315,12.29289,12.51181,11.61789,12.51181,10.79289C12.51181,10.48318,12.41593,10.194600000000001,12.25216,9.95494L10.16528,12.04183ZM11.43602,9.35687L9.55616,11.236740000000001C9.512,11.09633,9.48819,10.94724,9.48819,10.79289C9.48819,9.96789,10.1685,9.29289,11,9.29289C11.15142,9.29289,11.29782,9.31528,11.43602,9.35687Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></svg>'), Hu = () => Ru.cloneNode(!0), Vu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><defs><clipPath id="master_svg0_151_625"><rect x="0" y="0" width="22" height="22" rx="0"></rect></clipPath></defs><g clip-path="url(#master_svg0_151_625)"><path d="M14.5385,9.76923L15.6538,9.76923C16.6538,9.76923,17.4615,10.576920000000001,17.4615,11.576920000000001L17.4615,17.1923C17.4615,18.1923,16.6538,19,15.6538,19L5.80769,19C4.807692,19,4,18.1923,4,17.1923L4,11.576920000000001C4,10.576920000000001,4.807692,9.76923,5.80769,9.76923L7.23077,9.76923L7.23077,7.576919999999999C7.23077,5.61538,8.88462,4,10.88462,4C12.88462,4,14.5385,5.61538,14.5385,7.576919999999999L14.5385,9.76923ZM10.88461,5.15385C9.5,5.15385,8.38461,6.23077,8.38461,7.576919999999999L8.38461,9.76923L13.38462,9.76923L13.38462,7.576919999999999C13.38462,6.23077,12.26923,5.15385,10.88461,5.15385ZM15.6538,17.8462C16,17.8462,16.3077,17.5385,16.3077,17.1923L16.3077,11.576920000000001C16.3077,11.23077,16,10.923079999999999,15.6538,10.923079999999999L5.80769,10.923079999999999C5.46154,10.923079999999999,5.15385,11.23077,5.15385,11.576920000000001L5.15385,17.1923C5.15385,17.5385,5.46154,17.8462,5.80769,17.8462L15.6538,17.8462ZM10.153839999999999,12.65385C10.153839999999999,12.34615,10.42307,12.07692,10.73076,12.07692C11.038450000000001,12.07692,11.307680000000001,12.34615,11.307680000000001,12.65385L11.307680000000001,14.5769C11.307680000000001,14.8846,11.038450000000001,15.1538,10.73076,15.1538C10.42307,15.1538,10.153839999999999,14.8846,10.153839999999999,14.5769L10.153839999999999,12.65385Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></g></svg>'), Yu = () => Vu.cloneNode(!0), Gu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><defs><clipPath id="master_svg0_151_620"><rect x="0" y="0" width="22" height="22" rx="0"></rect></clipPath></defs><g clip-path="url(#master_svg0_151_620)"><path d="M8.38461,9.76923L15.6538,9.76923C16.6538,9.76923,17.4615,10.576920000000001,17.4615,11.576920000000001L17.4615,17.1923C17.4615,18.1923,16.6538,19,15.6538,19L5.80769,19C4.807692,19,4,18.1923,4,17.1923L4,11.576920000000001C4,10.576920000000001,4.807693,9.76923,5.80769,9.76923L7.23077,9.76923L7.23077,7.576919999999999C7.23077,5.61538,8.88462,4,10.88462,4C12.46154,4,13.84615,4.961539,14.3462,6.423080000000001C14.4615,6.73077,14.3077,7.038460000000001,14,7.15385C13.69231,7.26923,13.38461,7.11538,13.26923,6.80769C12.92308,5.80769,11.96154,5.15385,10.88462,5.15385C9.5,5.15385,8.38461,6.23077,8.38461,7.576919999999999L8.38461,9.76923ZM15.6538,17.8462C16,17.8462,16.3077,17.5385,16.3077,17.1923L16.3077,11.576920000000001C16.3077,11.23077,16,10.923079999999999,15.6538,10.923079999999999L5.80769,10.923079999999999C5.46154,10.923079999999999,5.15385,11.23077,5.15385,11.576920000000001L5.15385,17.1923C5.15385,17.5385,5.46154,17.8462,5.80769,17.8462L15.6538,17.8462ZM10.153839999999999,12.65385C10.153839999999999,12.34615,10.42307,12.07692,10.73076,12.07692C11.03846,12.07692,11.307690000000001,12.34615,11.307690000000001,12.65385L11.307690000000001,14.5769C11.307690000000001,14.8846,11.03846,15.1538,10.73076,15.1538C10.42307,15.1538,10.153839999999999,14.8846,10.153839999999999,14.5769L10.153839999999999,12.65385Z" stroke-opacity="0" fill-rule="evenodd" fill-opacity="1"></path></g></svg>'), qu = () => Gu.cloneNode(!0), Wu = /* @__PURE__ */ p('<svg class="icon-overlay" viewBox="0 0 22 22"><path d="M16.966900000000003,8.67144C16.6669,8.67144,16.4247,8.91558,16.4247,9.21802L16.4247,16.631500000000003C16.4247,17.322,16.007199999999997,17.9068,15.5139,17.9068L13.93072,17.9068L13.93072,9.2162C13.93072,8.91741,13.68675,8.67144,13.38855,8.67144C13.09036,8.67144,12.84639,8.91741,12.84639,9.21802L12.84639,17.9068L10.151810000000001,17.9068L10.151810000000001,9.21802C10.151810000000001,8.91741,9.90783,8.67144,9.609639999999999,8.67144C9.31145,8.67144,9.06747,8.91741,9.06747,9.219850000000001L9.06747,17.9068L7.48614,17.9068C6.99277,17.9068,6.5753,17.322,6.5753,16.631500000000003L6.5753,9.21802C6.5753,8.91558,6.333130000000001,8.67144,6.03313,8.67144C5.73313,8.67144,5.49096,8.91558,5.49096,9.21802L5.49096,16.631500000000003C5.49096,17.9378,6.385540000000001,19,7.48614,19L15.512,19C16.6127,19,17.509,17.9378,17.509,16.631500000000003L17.509,9.21802C17.509,8.91558,17.2669,8.67144,16.966900000000003,8.67144ZM18.4578,6.21183L4.542169,6.21183C4.243976,6.21183,4,6.45779,4,6.75841C4,7.05903,4.243976,7.30499,4.542169,7.30499L18.4578,7.30499C18.756,7.30499,19,7.05903,19,6.75841C19,6.45779,18.756,6.21183,18.4578,6.21183ZM8.68072,5.10045L14.3193,5.10045C14.6175,5.10045,14.8614,4.852666,14.8614,4.550225C14.8614,4.247783,14.6175,4,14.3193,4L8.68072,4C8.38253,4,8.13855,4.247783,8.13855,4.550225C8.13855,4.852666,8.38253,5.10045,8.68072,5.10045Z" stroke-opacity="0" fill-opacity="1"></path></svg>'), Xu = () => Wu.cloneNode(!0), Ju = {
  horizontalStraightLine: Nl,
  horizontalRayLine: El,
  horizontalSegment: Ul,
  verticalStraightLine: Kl,
  verticalRayLine: zl,
  verticalSegment: Zl,
  straightLine: Hl,
  rayLine: Yl,
  segment: ql,
  arrow: Xl,
  priceLine: eu,
  priceChannelLine: nu,
  parallelStraightLine: iu,
  fibonacciLine: su,
  fibonacciSegment: cu,
  fibonacciCircle: uu,
  fibonacciSpiral: hu,
  fibonacciSpeedResistanceFan: yu,
  fibonacciExtension: gu,
  gannBox: pu,
  circle: _u,
  triangle: bu,
  rect: xu,
  parallelogram: Au,
  threeWaves: Mu,
  fiveWaves: Tu,
  eightWaves: Iu,
  anyWaves: Nu,
  abcd: Eu,
  xabcd: Uu,
  weak_magnet: Ku,
  strong_magnet: zu,
  lock: Yu,
  unlock: qu,
  visible: Zu,
  invisible: Hu,
  remove: Xu
};
function ed(e) {
  return [
    { key: "horizontalStraightLine", text: a("horizontal_straight_line", e) },
    { key: "horizontalRayLine", text: a("horizontal_ray_line", e) },
    { key: "horizontalSegment", text: a("horizontal_segment", e) },
    { key: "verticalStraightLine", text: a("vertical_straight_line", e) },
    { key: "verticalRayLine", text: a("vertical_ray_line", e) },
    { key: "verticalSegment", text: a("vertical_segment", e) },
    { key: "straightLine", text: a("straight_line", e) },
    { key: "rayLine", text: a("ray_line", e) },
    { key: "segment", text: a("segment", e) },
    { key: "arrow", text: a("arrow", e) },
    { key: "priceLine", text: a("price_line", e) }
  ];
}
function td(e) {
  return [
    { key: "priceChannelLine", text: a("price_channel_line", e) },
    { key: "parallelStraightLine", text: a("parallel_straight_line", e) }
  ];
}
function nd(e) {
  return [
    { key: "circle", text: a("circle", e) },
    { key: "rect", text: a("rect", e) },
    { key: "parallelogram", text: a("parallelogram", e) },
    { key: "triangle", text: a("triangle", e) }
  ];
}
function rd(e) {
  return [
    { key: "fibonacciLine", text: a("fibonacci_line", e) },
    { key: "fibonacciSegment", text: a("fibonacci_segment", e) },
    { key: "fibonacciCircle", text: a("fibonacci_circle", e) },
    { key: "fibonacciSpiral", text: a("fibonacci_spiral", e) },
    { key: "fibonacciSpeedResistanceFan", text: a("fibonacci_speed_resistance_fan", e) },
    { key: "fibonacciExtension", text: a("fibonacci_extension", e) },
    { key: "gannBox", text: a("gann_box", e) }
  ];
}
function id(e) {
  return [
    { key: "xabcd", text: a("xabcd", e) },
    { key: "abcd", text: a("abcd", e) },
    { key: "threeWaves", text: a("three_waves", e) },
    { key: "fiveWaves", text: a("five_waves", e) },
    { key: "eightWaves", text: a("eight_waves", e) },
    { key: "anyWaves", text: a("any_waves", e) }
  ];
}
function ad(e) {
  return [
    { key: "weak_magnet", text: a("weak_magnet", e) },
    { key: "strong_magnet", text: a("strong_magnet", e) }
  ];
}
const v1 = (e) => Ju[e.name](e.class), sd = /* @__PURE__ */ p('<div class="klinecharts-pro-drawing-bar"><span class="split-line"></span><div class="item" tabindex="0"><span style="width:32px;height:32px"></span><div class="icon-arrow"><svg viewBox="0 0 4 6"><path d="M1.07298,0.159458C0.827521,-0.0531526,0.429553,-0.0531526,0.184094,0.159458C-0.0613648,0.372068,-0.0613648,0.716778,0.184094,0.929388L2.61275,3.03303L0.260362,5.07061C0.0149035,5.28322,0.0149035,5.62793,0.260362,5.84054C0.505822,6.05315,0.903789,6.05315,1.14925,5.84054L3.81591,3.53075C4.01812,3.3556,4.05374,3.0908,3.92279,2.88406C3.93219,2.73496,3.87113,2.58315,3.73964,2.46925L1.07298,0.159458Z" stroke="none" stroke-opacity="0"></path></svg></div></div><div class="item"><span style="width:32px;height:32px"></span></div><div class="item"><span style="width:32px;height:32px"></span></div><span class="split-line"></span><div class="item"><span style="width:32px;height:32px"></span></div></div>'), od = /* @__PURE__ */ p('<div class="item" tabindex="0"><span style="width:32px;height:32px"></span><div class="icon-arrow"><svg viewBox="0 0 4 6"><path d="M1.07298,0.159458C0.827521,-0.0531526,0.429553,-0.0531526,0.184094,0.159458C-0.0613648,0.372068,-0.0613648,0.716778,0.184094,0.929388L2.61275,3.03303L0.260362,5.07061C0.0149035,5.28322,0.0149035,5.62793,0.260362,5.84054C0.505822,6.05315,0.903789,6.05315,1.14925,5.84054L3.81591,3.53075C4.01812,3.3556,4.05374,3.0908,3.92279,2.88406C3.93219,2.73496,3.87113,2.58315,3.73964,2.46925L1.07298,0.159458Z" stroke="none" stroke-opacity="0"></path></svg></div></div>'), mt = /* @__PURE__ */ p('<li><span style="padding-left:8px"></span></li>'), pt = "drawing_tools", cd = (e) => {
  const [t, n] = w("horizontalStraightLine"), [r, i] = w("priceChannelLine"), [s, l] = w("circle"), [c, h] = w("fibonacciLine"), [m, d] = w("xabcd"), [$, v] = w("weak_magnet"), [k, T] = w("normal"), [D, I] = w(!1), [F, K] = w(!0), [P, B] = w(""), z = V(() => [{
    key: "singleLine",
    icon: t(),
    list: ed(e.locale),
    setter: n
  }, {
    key: "moreLine",
    icon: r(),
    list: td(e.locale),
    setter: i
  }, {
    key: "polygon",
    icon: s(),
    list: nd(e.locale),
    setter: l
  }, {
    key: "fibonacci",
    icon: c(),
    list: rd(e.locale),
    setter: h
  }, {
    key: "wave",
    icon: m(),
    list: id(e.locale),
    setter: d
  }]), $1 = V(() => ad(e.locale));
  return (() => {
    const a1 = sd.cloneNode(!0), f1 = a1.firstChild, O = f1.nextSibling, G = O.firstChild, R = G.nextSibling, N = R.firstChild, Z = O.nextSibling, u1 = Z.firstChild, l1 = Z.nextSibling, q = l1.firstChild, b1 = l1.nextSibling, g = b1.nextSibling, t1 = g.firstChild;
    return _(a1, () => z().map((S) => (() => {
      const Y = od.cloneNode(!0), j = Y.firstChild, d1 = j.nextSibling, g1 = d1.firstChild;
      return Y.addEventListener("blur", () => {
        B("");
      }), j.$$click = () => {
        e.onDrawingItemClick({
          groupId: pt,
          name: S.icon,
          visible: F(),
          lock: D(),
          mode: k()
        });
      }, _(j, L(v1, {
        get name() {
          return S.icon;
        }
      })), d1.$$click = () => {
        S.key === P() ? B("") : B(S.key);
      }, _(Y, (() => {
        const O1 = V(() => S.key === P());
        return () => O1() && L(Pe, {
          class: "list",
          get children() {
            return S.list.map((L1) => (() => {
              const x1 = mt.cloneNode(!0), T1 = x1.firstChild;
              return x1.$$click = () => {
                S.setter(L1.key), e.onDrawingItemClick({
                  name: L1.key,
                  lock: D(),
                  mode: k()
                }), B("");
              }, _(x1, L(v1, {
                get name() {
                  return L1.key;
                }
              }), T1), _(T1, () => L1.text), x1;
            })());
          }
        });
      })(), null), Q(() => y1(g1, "class", S.key === P() ? "rotate" : "")), Y;
    })()), f1), O.addEventListener("blur", () => {
      B("");
    }), G.$$click = () => {
      let S = $();
      k() !== "normal" && (S = "normal"), T(S), e.onModeChange(S);
    }, _(G, (() => {
      const S = V(() => $() === "weak_magnet");
      return () => S() ? (() => {
        const Y = V(() => k() === "weak_magnet");
        return () => Y() ? L(v1, {
          name: "weak_magnet",
          class: "selected"
        }) : L(v1, {
          name: "weak_magnet"
        });
      })() : (() => {
        const Y = V(() => k() === "strong_magnet");
        return () => Y() ? L(v1, {
          name: "strong_magnet",
          class: "selected"
        }) : L(v1, {
          name: "strong_magnet"
        });
      })();
    })()), R.$$click = () => {
      P() === "mode" ? B("") : B("mode");
    }, _(O, (() => {
      const S = V(() => P() === "mode");
      return () => S() && L(Pe, {
        class: "list",
        get children() {
          return $1().map((Y) => (() => {
            const j = mt.cloneNode(!0), d1 = j.firstChild;
            return j.$$click = () => {
              v(Y.key), T(Y.key), e.onModeChange(Y.key), B("");
            }, _(j, L(v1, {
              get name() {
                return Y.key;
              }
            }), d1), _(d1, () => Y.text), j;
          })());
        }
      });
    })(), null), u1.$$click = () => {
      const S = !D();
      I(S), e.onLockChange(S);
    }, _(u1, (() => {
      const S = V(() => !!D());
      return () => S() ? L(v1, {
        name: "lock"
      }) : L(v1, {
        name: "unlock"
      });
    })()), q.$$click = () => {
      const S = !F();
      K(S), e.onVisibleChange(S);
    }, _(q, (() => {
      const S = V(() => !!F());
      return () => S() ? L(v1, {
        name: "visible"
      }) : L(v1, {
        name: "invisible"
      });
    })()), t1.$$click = () => {
      e.onRemoveClick(pt);
    }, _(t1, L(v1, {
      name: "remove"
    })), Q(() => y1(N, "class", P() === "mode" ? "rotate" : "")), a1;
  })();
};
_1(["click"]);
const vt = /* @__PURE__ */ p('<li class="title"></li>'), _t = /* @__PURE__ */ p('<li class="row"></li>'), ld = (e) => L(R1, {
  get title() {
    return a("indicator", e.locale);
  },
  width: 400,
  get onClose() {
    return e.onClose;
  },
  get children() {
    return L(Pe, {
      class: "klinecharts-pro-indicator-modal-list",
      get children() {
        return [(() => {
          const t = vt.cloneNode(!0);
          return _(t, () => a("main_indicator", e.locale)), t;
        })(), V(() => ["MA", "EMA", "SMA", "BOLL", "SAR", "BBI"].map((t) => {
          const n = e.mainIndicators.includes(t);
          return (() => {
            const r = _t.cloneNode(!0);
            return r.$$click = (i) => {
              e.onMainIndicatorChange({
                name: t,
                paneId: "candle_pane",
                added: !n
              });
            }, _(r, L(gt, {
              checked: n,
              get label() {
                return a(t.toLowerCase(), e.locale);
              }
            })), r;
          })();
        })), (() => {
          const t = vt.cloneNode(!0);
          return _(t, () => a("sub_indicator", e.locale)), t;
        })(), V(() => ["MA", "EMA", "VOL", "MACD", "BOLL", "KDJ", "RSI", "BIAS", "BRAR", "CCI", "DMI", "CR", "PSY", "DMA", "TRIX", "OBV", "VR", "WR", "MTM", "EMV", "SAR", "SMA", "ROC", "PVT", "BBI", "AO"].map((t) => {
          const n = t in e.subIndicators;
          return (() => {
            const r = _t.cloneNode(!0);
            return r.$$click = (i) => {
              e.onSubIndicatorChange({
                name: t,
                paneId: e.subIndicators[t] ?? "",
                added: !n
              });
            }, _(r, L(gt, {
              checked: n,
              get label() {
                return a(t.toLowerCase(), e.locale);
              }
            })), r;
          })();
        }))];
      }
    });
  }
});
_1(["click"]);
function $t(e, t) {
  switch (e) {
    case "Etc/UTC":
      return a("utc", t);
    case "Pacific/Midway":
      return a("midway", t);
    case "Pacific/Honolulu":
      return a("honolulu", t);
    case "America/Anchorage":
      return a("anchorage", t);
    case "America/Juneau":
      return a("juneau", t);
    case "America/Los_Angeles":
      return a("los_angeles", t);
    case "America/Vancouver":
      return a("vancouver", t);
    case "America/Tijuana":
      return a("tijuana", t);
    case "America/Phoenix":
      return a("phoenix", t);
    case "America/Denver":
      return a("denver", t);
    case "America/Chicago":
      return a("chicago", t);
    case "America/Mexico_City":
      return a("mexico_city", t);
    case "America/Guatemala":
      return a("guatemala", t);
    case "America/New_York":
      return a("new_york", t);
    case "America/Toronto":
      return a("toronto", t);
    case "America/Bogota":
      return a("bogota", t);
    case "America/Lima":
      return a("lima", t);
    case "America/Caracas":
      return a("caracas", t);
    case "America/Halifax":
      return a("halifax", t);
    case "America/Santiago":
      return a("santiago", t);
    case "America/La_Paz":
      return a("la_paz", t);
    case "America/Sao_Paulo":
      return a("sao_paulo", t);
    case "America/Buenos_Aires":
      return a("buenos_aires", t);
    case "America/Montevideo":
      return a("montevideo", t);
    case "America/Godthab":
      return a("godthab", t);
    case "Atlantic/Azores":
      return a("azores", t);
    case "Atlantic/Cape_Verde":
      return a("cape_verde", t);
    case "Europe/London":
      return a("london", t);
    case "Europe/Dublin":
      return a("dublin", t);
    case "Europe/Lisbon":
      return a("lisbon", t);
    case "Africa/Casablanca":
      return a("casablanca", t);
    case "Europe/Paris":
      return a("paris", t);
    case "Europe/Berlin":
      return a("berlin", t);
    case "Europe/Amsterdam":
      return a("amsterdam", t);
    case "Europe/Brussels":
      return a("brussels", t);
    case "Europe/Madrid":
      return a("madrid", t);
    case "Europe/Rome":
      return a("rome", t);
    case "Europe/Vienna":
      return a("vienna", t);
    case "Europe/Warsaw":
      return a("warsaw", t);
    case "Africa/Lagos":
      return a("lagos", t);
    case "Europe/Athens":
      return a("athens", t);
    case "Europe/Bucharest":
      return a("bucharest", t);
    case "Europe/Helsinki":
      return a("helsinki", t);
    case "Europe/Istanbul":
      return a("istanbul", t);
    case "Europe/Kiev":
      return a("kiev", t);
    case "Africa/Cairo":
      return a("cairo", t);
    case "Africa/Johannesburg":
      return a("johannesburg", t);
    case "Asia/Jerusalem":
      return a("jerusalem", t);
    case "Europe/Moscow":
      return a("moscow", t);
    case "Asia/Baghdad":
      return a("baghdad", t);
    case "Asia/Kuwait":
      return a("kuwait", t);
    case "Asia/Riyadh":
      return a("riyadh", t);
    case "Asia/Bahrain":
      return a("bahrain", t);
    case "Africa/Nairobi":
      return a("nairobi", t);
    case "Asia/Tehran":
      return a("tehran", t);
    case "Asia/Dubai":
      return a("dubai", t);
    case "Asia/Muscat":
      return a("muscat", t);
    case "Asia/Baku":
      return a("baku", t);
    case "Asia/Kabul":
      return a("kabul", t);
    case "Asia/Karachi":
      return a("karachi", t);
    case "Asia/Tashkent":
      return a("tashkent", t);
    case "Asia/Ashkhabad":
      return a("ashkhabad", t);
    case "Asia/Kolkata":
      return a("kolkata", t);
    case "Asia/Mumbai":
      return a("mumbai", t);
    case "Asia/Colombo":
      return a("colombo", t);
    case "Asia/Kathmandu":
      return a("kathmandu", t);
    case "Asia/Dhaka":
      return a("dhaka", t);
    case "Asia/Almaty":
      return a("almaty", t);
    case "Asia/Yangon":
      return a("yangon", t);
    case "Asia/Bangkok":
      return a("bangkok", t);
    case "Asia/Jakarta":
      return a("jakarta", t);
    case "Asia/Ho_Chi_Minh":
      return a("ho_chi_minh", t);
    case "Asia/Shanghai":
      return a("shanghai", t);
    case "Asia/Hong_Kong":
      return a("hong_kong", t);
    case "Asia/Singapore":
      return a("singapore", t);
    case "Asia/Taipei":
      return a("taipei", t);
    case "Asia/Manila":
      return a("manila", t);
    case "Asia/Kuala_Lumpur":
      return a("kuala_lumpur", t);
    case "Australia/Perth":
      return a("perth", t);
    case "Asia/Tokyo":
      return a("tokyo", t);
    case "Asia/Seoul":
      return a("seoul", t);
    case "Asia/Pyongyang":
      return a("pyongyang", t);
    case "Australia/Adelaide":
      return a("adelaide", t);
    case "Australia/Darwin":
      return a("darwin", t);
    case "Australia/Brisbane":
      return a("brisbane", t);
    case "Australia/Sydney":
      return a("sydney", t);
    case "Australia/Melbourne":
      return a("melbourne", t);
    case "Pacific/Guam":
      return a("guam", t);
    case "Pacific/Port_Moresby":
      return a("port_moresby", t);
    case "Pacific/Norfolk":
      return a("norfolk", t);
    case "Pacific/Guadalcanal":
      return a("guadalcanal", t);
    case "Pacific/Auckland":
      return a("auckland", t);
    case "Pacific/Fiji":
      return a("fiji", t);
    case "Pacific/Tongatapu":
      return a("tongatapu", t);
    case "Pacific/Apia":
      return a("apia", t);
    case "Asia/Karachi":
      return a("karachi", t);
  }
  return e;
}
function ud(e) {
  return [
    // UTC
    { key: "Etc/UTC", text: `(UTC+0:00) ${a("utc", e)}` },
    // Pacific (UTC-12 to UTC-8)
    { key: "Pacific/Midway", text: `(UTC-11:00) ${a("midway", e)}` },
    { key: "Pacific/Honolulu", text: `(UTC-10:00) ${a("honolulu", e)}` },
    { key: "America/Anchorage", text: `(UTC-9:00) ${a("anchorage", e)}` },
    { key: "America/Juneau", text: `(UTC-9:00) ${a("juneau", e)}` },
    // North America West (UTC-8 to UTC-7)
    { key: "America/Los_Angeles", text: `(UTC-8:00) ${a("los_angeles", e)}` },
    { key: "America/Vancouver", text: `(UTC-8:00) ${a("vancouver", e)}` },
    { key: "America/Tijuana", text: `(UTC-8:00) ${a("tijuana", e)}` },
    { key: "America/Phoenix", text: `(UTC-7:00) ${a("phoenix", e)}` },
    { key: "America/Denver", text: `(UTC-7:00) ${a("denver", e)}` },
    // North America Central (UTC-6)
    { key: "America/Chicago", text: `(UTC-6:00) ${a("chicago", e)}` },
    { key: "America/Mexico_City", text: `(UTC-6:00) ${a("mexico_city", e)}` },
    { key: "America/Guatemala", text: `(UTC-6:00) ${a("guatemala", e)}` },
    // North America East (UTC-5 to UTC-4)
    { key: "America/New_York", text: `(UTC-5:00) ${a("new_york", e)}` },
    { key: "America/Toronto", text: `(UTC-5:00) ${a("toronto", e)}` },
    { key: "America/Bogota", text: `(UTC-5:00) ${a("bogota", e)}` },
    { key: "America/Lima", text: `(UTC-5:00) ${a("lima", e)}` },
    { key: "America/Caracas", text: `(UTC-4:00) ${a("caracas", e)}` },
    { key: "America/Halifax", text: `(UTC-4:00) ${a("halifax", e)}` },
    // South America (UTC-4 to UTC-3)
    { key: "America/Santiago", text: `(UTC-4:00) ${a("santiago", e)}` },
    { key: "America/La_Paz", text: `(UTC-4:00) ${a("la_paz", e)}` },
    { key: "America/Sao_Paulo", text: `(UTC-3:00) ${a("sao_paulo", e)}` },
    { key: "America/Buenos_Aires", text: `(UTC-3:00) ${a("buenos_aires", e)}` },
    { key: "America/Montevideo", text: `(UTC-3:00) ${a("montevideo", e)}` },
    // Atlantic (UTC-3 to UTC-1)
    { key: "America/Godthab", text: `(UTC-3:00) ${a("godthab", e)}` },
    { key: "Atlantic/Azores", text: `(UTC-1:00) ${a("azores", e)}` },
    { key: "Atlantic/Cape_Verde", text: `(UTC-1:00) ${a("cape_verde", e)}` },
    // Western Europe (UTC+0 to UTC+1)
    { key: "Europe/London", text: `(UTC+0:00) ${a("london", e)}` },
    { key: "Europe/Dublin", text: `(UTC+0:00) ${a("dublin", e)}` },
    { key: "Europe/Lisbon", text: `(UTC+0:00) ${a("lisbon", e)}` },
    { key: "Africa/Casablanca", text: `(UTC+0:00) ${a("casablanca", e)}` },
    { key: "Europe/Paris", text: `(UTC+1:00) ${a("paris", e)}` },
    { key: "Europe/Berlin", text: `(UTC+1:00) ${a("berlin", e)}` },
    { key: "Europe/Amsterdam", text: `(UTC+1:00) ${a("amsterdam", e)}` },
    { key: "Europe/Brussels", text: `(UTC+1:00) ${a("brussels", e)}` },
    { key: "Europe/Madrid", text: `(UTC+1:00) ${a("madrid", e)}` },
    { key: "Europe/Rome", text: `(UTC+1:00) ${a("rome", e)}` },
    { key: "Europe/Vienna", text: `(UTC+1:00) ${a("vienna", e)}` },
    { key: "Europe/Warsaw", text: `(UTC+1:00) ${a("warsaw", e)}` },
    { key: "Africa/Lagos", text: `(UTC+1:00) ${a("lagos", e)}` },
    // Central Europe (UTC+2)
    { key: "Europe/Athens", text: `(UTC+2:00) ${a("athens", e)}` },
    { key: "Europe/Bucharest", text: `(UTC+2:00) ${a("bucharest", e)}` },
    { key: "Europe/Helsinki", text: `(UTC+2:00) ${a("helsinki", e)}` },
    { key: "Europe/Istanbul", text: `(UTC+2:00) ${a("istanbul", e)}` },
    { key: "Europe/Kiev", text: `(UTC+2:00) ${a("kiev", e)}` },
    { key: "Africa/Cairo", text: `(UTC+2:00) ${a("cairo", e)}` },
    { key: "Africa/Johannesburg", text: `(UTC+2:00) ${a("johannesburg", e)}` },
    { key: "Asia/Jerusalem", text: `(UTC+2:00) ${a("jerusalem", e)}` },
    // Eastern Europe / Middle East (UTC+3)
    { key: "Europe/Moscow", text: `(UTC+3:00) ${a("moscow", e)}` },
    { key: "Asia/Baghdad", text: `(UTC+3:00) ${a("baghdad", e)}` },
    { key: "Asia/Kuwait", text: `(UTC+3:00) ${a("kuwait", e)}` },
    { key: "Asia/Riyadh", text: `(UTC+3:00) ${a("riyadh", e)}` },
    { key: "Asia/Bahrain", text: `(UTC+3:00) ${a("bahrain", e)}` },
    { key: "Africa/Nairobi", text: `(UTC+3:00) ${a("nairobi", e)}` },
    // Middle East (UTC+3:30 to UTC+4)
    { key: "Asia/Tehran", text: `(UTC+3:30) ${a("tehran", e)}` },
    { key: "Asia/Dubai", text: `(UTC+4:00) ${a("dubai", e)}` },
    { key: "Asia/Muscat", text: `(UTC+4:00) ${a("muscat", e)}` },
    { key: "Asia/Baku", text: `(UTC+4:00) ${a("baku", e)}` },
    // Central Asia (UTC+4:30 to UTC+5)
    { key: "Asia/Kabul", text: `(UTC+4:30) ${a("kabul", e)}` },
    { key: "Asia/Karachi", text: `(UTC+5:00) ${a("karachi", e)}` },
    { key: "Asia/Tashkent", text: `(UTC+5:00) ${a("tashkent", e)}` },
    { key: "Asia/Ashkhabad", text: `(UTC+5:00) ${a("ashkhabad", e)}` },
    // South Asia (UTC+5:30 to UTC+6)
    { key: "Asia/Kolkata", text: `(UTC+5:30) ${a("kolkata", e)}` },
    { key: "Asia/Mumbai", text: `(UTC+5:30) ${a("mumbai", e)}` },
    { key: "Asia/Colombo", text: `(UTC+5:30) ${a("colombo", e)}` },
    { key: "Asia/Kathmandu", text: `(UTC+5:45) ${a("kathmandu", e)}` },
    { key: "Asia/Dhaka", text: `(UTC+6:00) ${a("dhaka", e)}` },
    { key: "Asia/Almaty", text: `(UTC+6:00) ${a("almaty", e)}` },
    // Southeast Asia (UTC+6:30 to UTC+7)
    { key: "Asia/Yangon", text: `(UTC+6:30) ${a("yangon", e)}` },
    { key: "Asia/Bangkok", text: `(UTC+7:00) ${a("bangkok", e)}` },
    { key: "Asia/Jakarta", text: `(UTC+7:00) ${a("jakarta", e)}` },
    { key: "Asia/Ho_Chi_Minh", text: `(UTC+7:00) ${a("ho_chi_minh", e)}` },
    // East Asia (UTC+8)
    { key: "Asia/Shanghai", text: `(UTC+8:00) ${a("shanghai", e)}` },
    { key: "Asia/Hong_Kong", text: `(UTC+8:00) ${a("hong_kong", e)}` },
    { key: "Asia/Singapore", text: `(UTC+8:00) ${a("singapore", e)}` },
    { key: "Asia/Taipei", text: `(UTC+8:00) ${a("taipei", e)}` },
    { key: "Asia/Manila", text: `(UTC+8:00) ${a("manila", e)}` },
    { key: "Asia/Kuala_Lumpur", text: `(UTC+8:00) ${a("kuala_lumpur", e)}` },
    { key: "Australia/Perth", text: `(UTC+8:00) ${a("perth", e)}` },
    // East Asia (UTC+9)
    { key: "Asia/Tokyo", text: `(UTC+9:00) ${a("tokyo", e)}` },
    { key: "Asia/Seoul", text: `(UTC+9:00) ${a("seoul", e)}` },
    { key: "Asia/Pyongyang", text: `(UTC+9:00) ${a("pyongyang", e)}` },
    // Australia (UTC+9:30 to UTC+10)
    { key: "Australia/Adelaide", text: `(UTC+9:30) ${a("adelaide", e)}` },
    { key: "Australia/Darwin", text: `(UTC+9:30) ${a("darwin", e)}` },
    { key: "Australia/Brisbane", text: `(UTC+10:00) ${a("brisbane", e)}` },
    { key: "Australia/Sydney", text: `(UTC+10:00) ${a("sydney", e)}` },
    { key: "Australia/Melbourne", text: `(UTC+10:00) ${a("melbourne", e)}` },
    { key: "Pacific/Guam", text: `(UTC+10:00) ${a("guam", e)}` },
    // Pacific (UTC+10 to UTC+13)
    { key: "Pacific/Port_Moresby", text: `(UTC+10:00) ${a("port_moresby", e)}` },
    { key: "Pacific/Norfolk", text: `(UTC+11:00) ${a("norfolk", e)}` },
    { key: "Pacific/Guadalcanal", text: `(UTC+11:00) ${a("guadalcanal", e)}` },
    { key: "Pacific/Auckland", text: `(UTC+12:00) ${a("auckland", e)}` },
    { key: "Pacific/Fiji", text: `(UTC+12:00) ${a("fiji", e)}` },
    { key: "Pacific/Tongatapu", text: `(UTC+13:00) ${a("tongatapu", e)}` },
    { key: "Pacific/Apia", text: `(UTC+13:00) ${a("apia", e)}` }
  ];
}
const dd = (e) => {
  const [t, n] = w(e.timezone), r = V(() => ud(e.locale));
  return L(R1, {
    get title() {
      return a("timezone", e.locale);
    },
    width: 320,
    get buttons() {
      return [{
        children: a("confirm", e.locale),
        onClick: () => {
          e.onConfirm(t()), e.onClose();
        }
      }];
    },
    get onClose() {
      return e.onClose;
    },
    get children() {
      return L(s9, {
        style: {
          width: "100%",
          "margin-top": "20px"
        },
        get value() {
          return t().text;
        },
        onSelected: (i) => {
          n(i);
        },
        get dataSource() {
          return r();
        },
        searchable: !0,
        get searchPlaceholder() {
          return a("Search Timezone", e.locale) || "Search timezone...";
        }
      });
    }
  });
};
function bt(e) {
  return [
    {
      key: "candle.type",
      text: a("candle_type", e),
      component: "select",
      dataSource: [
        { key: "candle_solid", text: a("candle_solid", e) },
        { key: "candle_stroke", text: a("candle_stroke", e) },
        { key: "candle_up_stroke", text: a("candle_up_stroke", e) },
        { key: "candle_down_stroke", text: a("candle_down_stroke", e) },
        { key: "ohlc", text: a("ohlc", e) },
        { key: "area", text: a("area", e) }
      ]
    },
    {
      key: "candle.priceMark.last.show",
      text: a("last_price_show", e),
      component: "switch"
    },
    {
      key: "candle.priceMark.high.show",
      text: a("high_price_show", e),
      component: "switch"
    },
    {
      key: "candle.priceMark.low.show",
      text: a("low_price_show", e),
      component: "switch"
    },
    {
      key: "indicator.lastValueMark.show",
      text: a("indicator_last_value_show", e),
      component: "switch"
    },
    {
      key: "yAxis.type",
      text: a("price_axis_type", e),
      component: "select",
      dataSource: [
        { key: "normal", text: a("normal", e) },
        { key: "percentage", text: a("percentage", e) },
        { key: "log", text: a("log", e) }
      ]
    },
    {
      key: "yAxis.reverse",
      text: a("reverse_coordinate", e),
      component: "switch"
    },
    {
      key: "grid.show",
      text: a("grid_show", e),
      component: "switch"
    }
  ];
}
const hd = /* @__PURE__ */ p('<div class="klinecharts-pro-setting-modal-content"></div>'), Cd = /* @__PURE__ */ p('<div class="setting-item"><span class="setting-label"></span><div class="setting-control"></div></div>'), yd = (e) => {
  const [t, n] = w(e.currentStyles), [r, i] = w(bt(e.locale)), [s, l] = w(!1), c = () => {
    l(window.innerWidth <= 768);
  };
  i0(() => {
    c(), window.addEventListener("resize", c);
  }), j1(() => {
    window.removeEventListener("resize", c);
  }), m1(() => {
    i(bt(e.locale));
  });
  const h = (m, d) => {
    const $ = {};
    We($, m.key, d);
    const v = n1.clone(t());
    We(v, m.key, d), n(v), i(r().map((k) => ({
      ...k
    }))), e.onChange($);
  };
  return L(R1, {
    get title() {
      return a("setting", e.locale);
    },
    width: 690,
    btnParentStyle: {
      display: "flex",
      "justify-content": "center"
    },
    minButtonWidth: 200,
    get isMobile() {
      return s();
    },
    get buttons() {
      return [{
        children: a("restore_default", e.locale),
        onClick: () => {
          e.onRestoreDefault(r()), e.onClose();
        }
      }];
    },
    get onClose() {
      return e.onClose;
    },
    get children() {
      const m = hd.cloneNode(!0);
      return _(m, L(q9, {
        get each() {
          return r();
        },
        children: (d) => {
          let $;
          const v = n1.formatValue(t(), d.key);
          switch (d.component) {
            case "select": {
              $ = L(s9, {
                get style() {
                  return {
                    width: s() ? "100%" : "120px",
                    "min-width": s() ? "auto" : "120px"
                  };
                },
                get value() {
                  return a(v, e.locale);
                },
                get dataSource() {
                  return d.dataSource;
                },
                onSelected: (k) => {
                  const T = k.key;
                  h(d, T);
                }
              });
              break;
            }
            case "switch": {
              const k = !!v;
              $ = L(Xi, {
                open: k,
                onChange: () => {
                  h(d, !k);
                }
              });
              break;
            }
          }
          return (() => {
            const k = Cd.cloneNode(!0), T = k.firstChild, D = T.nextSibling;
            return _(T, () => d.text), _(D, $), Q(() => k.classList.toggle("mobile-item", !!s())), k;
          })();
        }
      })), Q(() => m.classList.toggle("mobile-layout", !!s())), m;
    }
  });
}, fd = /* @__PURE__ */ p('<img style="width:500px;margin-top: 20px">'), gd = (e) => L(R1, {
  get title() {
    return a("screenshot", e.locale);
  },
  width: 540,
  get buttons() {
    return [{
      type: "confirm",
      children: a("save", e.locale),
      onClick: () => {
        const t = document.createElement("a");
        t.download = "screenshot", t.href = e.url, document.body.appendChild(t), t.click(), t.remove();
      }
    }];
  },
  get onClose() {
    return e.onClose;
  },
  get children() {
    const t = fd.cloneNode(!0);
    return Q(() => y1(t, "src", e.url)), t;
  }
}), md = {
  AO: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 5 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 34 }
  ],
  BIAS: [
    { paramNameKey: "BIAS1", precision: 0, min: 1, styleKey: "lines[0].color" },
    { paramNameKey: "BIAS2", precision: 0, min: 1, styleKey: "lines[1].color" },
    { paramNameKey: "BIAS3", precision: 0, min: 1, styleKey: "lines[2].color" },
    { paramNameKey: "BIAS4", precision: 0, min: 1, styleKey: "lines[3].color" },
    { paramNameKey: "BIAS5", precision: 0, min: 1, styleKey: "lines[4].color" }
  ],
  BOLL: [
    { paramNameKey: "period", precision: 0, min: 1, default: 20 },
    { paramNameKey: "standard_deviation", precision: 2, min: 1, default: 2 }
  ],
  BRAR: [
    { paramNameKey: "period", precision: 0, min: 1, default: 26 }
  ],
  BBI: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 3 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 6 },
    { paramNameKey: "params_3", precision: 0, min: 1, default: 12 },
    { paramNameKey: "params_4", precision: 0, min: 1, default: 24 }
  ],
  CCI: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 20 }
  ],
  CR: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 26 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 10 },
    { paramNameKey: "params_3", precision: 0, min: 1, default: 20 },
    { paramNameKey: "params_4", precision: 0, min: 1, default: 40 },
    { paramNameKey: "params_5", precision: 0, min: 1, default: 60 }
  ],
  DMA: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 10 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 50 },
    { paramNameKey: "params_3", precision: 0, min: 1, default: 10 }
  ],
  DMI: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 14 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 6 }
  ],
  EMV: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 14 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 9 }
  ],
  EMA: [
    { paramNameKey: "EMA1", precision: 0, min: 1, styleKey: "lines[0].color" },
    { paramNameKey: "EMA2", precision: 0, min: 1, styleKey: "lines[1].color" },
    { paramNameKey: "EMA3", precision: 0, min: 1, styleKey: "lines[2].color" },
    { paramNameKey: "EMA4", precision: 0, min: 1, styleKey: "lines[3].color" },
    { paramNameKey: "EMA5", precision: 0, min: 1, styleKey: "lines[4].color" }
  ],
  MTM: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 12 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 6 }
  ],
  MA: [
    { paramNameKey: "MA1", precision: 0, min: 1, styleKey: "lines[0].color" },
    { paramNameKey: "MA2", precision: 0, min: 1, styleKey: "lines[1].color" },
    { paramNameKey: "MA3", precision: 0, min: 1, styleKey: "lines[2].color" },
    { paramNameKey: "MA4", precision: 0, min: 1, styleKey: "lines[3].color" },
    { paramNameKey: "MA5", precision: 0, min: 1, styleKey: "lines[4].color" }
  ],
  MACD: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 12 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 26 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 9 }
  ],
  OBV: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 30 }
  ],
  PVT: [],
  PSY: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 12 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 6 }
  ],
  ROC: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 12 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 6 }
  ],
  RSI: [
    { paramNameKey: "RSI1", precision: 0, min: 1, styleKey: "lines[0].color" },
    { paramNameKey: "RSI2", precision: 0, min: 1, styleKey: "lines[1].color" },
    { paramNameKey: "RSI3", precision: 0, min: 1, styleKey: "lines[2].color" },
    { paramNameKey: "RSI4", precision: 0, min: 1, styleKey: "lines[3].color" },
    { paramNameKey: "RSI5", precision: 0, min: 1, styleKey: "lines[4].color" }
  ],
  SMA: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 12 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 2 }
  ],
  KDJ: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 9 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 3 },
    { paramNameKey: "params_3", precision: 0, min: 1, default: 3 }
  ],
  SAR: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 2 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 2 },
    { paramNameKey: "params_3", precision: 0, min: 1, default: 20 }
  ],
  TRIX: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 12 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 9 }
  ],
  VOL: [
    { paramNameKey: "MA1", precision: 0, min: 1, styleKey: "lines[0].color" },
    { paramNameKey: "MA2", precision: 0, min: 1, styleKey: "lines[1].color" },
    { paramNameKey: "MA3", precision: 0, min: 1, styleKey: "lines[2].color" },
    { paramNameKey: "MA4", precision: 0, min: 1, styleKey: "lines[3].color" },
    { paramNameKey: "MA5", precision: 0, min: 1, styleKey: "lines[4].color" }
  ],
  VR: [
    { paramNameKey: "params_1", precision: 0, min: 1, default: 26 },
    { paramNameKey: "params_2", precision: 0, min: 1, default: 6 }
  ],
  WR: [
    { paramNameKey: "WR1", precision: 0, min: 1, styleKey: "lines[0].color" },
    { paramNameKey: "WR2", precision: 0, min: 1, styleKey: "lines[1].color" },
    { paramNameKey: "WR3", precision: 0, min: 1, styleKey: "lines[2].color" },
    { paramNameKey: "WR4", precision: 0, min: 1, styleKey: "lines[3].color" },
    { paramNameKey: "WR5", precision: 0, min: 1, styleKey: "lines[4].color" }
  ]
}, pd = /* @__PURE__ */ p('<div class="klinecharts-pro-indicator-setting-modal-content"></div>'), vd = /* @__PURE__ */ p("<span></span>"), _d = (e) => {
  const [t, n] = w(n1.clone(e.params.calcParams)), r = (i) => md[i];
  return L(R1, {
    get title() {
      return e.params.indicatorName;
    },
    width: 360,
    get buttons() {
      return [{
        type: "confirm",
        children: a("confirm", e.locale),
        onClick: () => {
          const i = r(e.params.indicatorName), s = [];
          n1.clone(t()).forEach((l, c) => {
            !n1.isValid(l) || l === "" ? "default" in i[c] && s.push(i[c].default) : s.push(l);
          }), e.onConfirm(s), e.onClose();
        }
      }];
    },
    get onClose() {
      return e.onClose;
    },
    get children() {
      const i = pd.cloneNode(!0);
      return _(i, () => r(e.params.indicatorName).map((s, l) => [(() => {
        const c = vd.cloneNode(!0);
        return _(c, () => a(s.paramNameKey, e.locale)), c;
      })(), L(o9, {
        style: {
          width: "200px"
        },
        get value() {
          return t()[l] ?? "";
        },
        get precision() {
          return s.precision;
        },
        get min() {
          return s.min;
        },
        onChange: (c) => {
          const h = n1.clone(t());
          h[l] = c, n(h);
        }
      })])), i;
    }
  });
}, $d = /* @__PURE__ */ p('<svg viewBox="0 0 1024 1024"><path d="M945.066667 898.133333l-189.866667-189.866666c55.466667-64 87.466667-149.333333 87.466667-241.066667 0-204.8-168.533333-373.333333-373.333334-373.333333S96 264.533333 96 469.333333 264.533333 842.666667 469.333333 842.666667c91.733333 0 174.933333-34.133333 241.066667-87.466667l189.866667 189.866667c6.4 6.4 14.933333 8.533333 23.466666 8.533333s17.066667-2.133333 23.466667-8.533333c8.533333-12.8 8.533333-34.133333-2.133333-46.933334zM469.333333 778.666667C298.666667 778.666667 160 640 160 469.333333S298.666667 160 469.333333 160 778.666667 298.666667 778.666667 469.333333 640 778.666667 469.333333 778.666667z"></path></svg>'), bd = /* @__PURE__ */ p('<img alt="symbol">'), Ld = /* @__PURE__ */ p("<li><div><span></span></div></li>"), xd = (e) => {
  const [t, n] = w(""), [r] = j9(t, e.datafeed.searchSymbols.bind(e.datafeed));
  return L(R1, {
    get title() {
      return a("symbol_search", e.locale);
    },
    width: 460,
    get onClose() {
      return e.onClose;
    },
    get children() {
      return [L(o9, {
        class: "klinecharts-pro-symbol-search-modal-input",
        get placeholder() {
          return a("symbol_code", e.locale);
        },
        get suffix() {
          return $d.cloneNode(!0);
        },
        get value() {
          return t();
        },
        onChange: (i) => {
          const s = `${i}`;
          n(s);
        }
      }), L(Pe, {
        class: "klinecharts-pro-symbol-search-modal-list",
        get loading() {
          return r.loading;
        },
        get dataSource() {
          return r() ?? [];
        },
        renderItem: (i) => (() => {
          const s = Ld.cloneNode(!0), l = s.firstChild, c = l.firstChild;
          return s.$$click = () => {
            e.onSymbolSelected(i), e.onClose();
          }, _(l, L(H, {
            get when() {
              return i.logo;
            },
            get children() {
              const h = bd.cloneNode(!0);
              return Q(() => y1(h, "src", i.logo)), h;
            }
          }), c), _(c, () => i.shortName ?? i.ticker, null), _(c, () => `${i.name ? `(${i.name})` : ""}`, null), _(s, () => i.exchange ?? "", null), Q(() => y1(c, "title", i.name ?? "")), s;
        })()
      })];
    }
  });
};
_1(["click"]);
const kd = /* @__PURE__ */ p('<div class="klinecharts-pro-mobile-more-modal-content"><div class="item"><svg viewBox="0 0 22 22" xmlns="http://www.w3.org/2000/svg"><rect x="1" y="1" width="20" height="20" rx="4" ry="4" fill="none" stroke="currentColor" stroke-width="2"></rect><path d="m15.92 13.9-3.65-3.55a.773.773 0 0 1 0-1.12c.32-.33.79-.33 1.11 0l3.02 3.38c.31.32.31.81 0 1.13-.16.16-.48.16-.48.16Z" fill="currentColor" stroke="1"></path><path d="m12.43 14.23-.48-.17q-.48-.48 0-.96l4.13-4.36c.31-.32.63-.32.95 0s.32.65 0 .97l-4.13 4.35c-.15.17-.31.17-.47.17Zm-1.11-9.52c.31 0 1.11.16 1.11.97 0 .64-.95.48-1.27.48-1.43 0-1.91.81-2.07 1.45l-.31 1.94h1.58c.32 0 .64.32.64.64 0 .33-.32.65-.64.65H8.62l-1.11 4.99c-.16 1.46-1.11 1.46-1.59 0L7.03 11H5.6c-.31 0-.63-.32-.63-.65 0-.64.32-.8.63-.8h1.75l.48-2.1c.31-2.74 2.69-2.9 3.49-2.74Z" fill="noncurrentColore" stroke="1"></path></svg><span></span></div><div class="item"><svg width="20" height="20" viewBox="0 0 20 20"><path d="M18.5446,9.09091C18.3333,6.61616,17.2887,4.31818,15.5751,2.63889C13.8498,0.94697,11.6197,0,9.28404,0C8.02817,0,6.81925,0.265151,5.66901,0.782828C5.65728,0.782828,5.65728,0.795454,5.64554,0.795454C5.6338,0.795454,5.6338,0.808081,5.62207,0.808081C4.53052,1.31313,3.55634,2.0202,2.71127,2.92929C1.85446,3.85101,1.18545,4.91162,0.715963,6.11111C0.246479,7.33586,0,8.64899,0,10C0,10.8712,0.105634,11.7172,0.305164,12.5379C0.305164,12.5631,0.316901,12.5884,0.328638,12.6136C0.739437,14.2298,1.51408,15.7197,2.62911,16.9571C4.07277,18.548,5.92723,19.5581,7.93427,19.8737C7.95775,19.8737,7.96948,19.8864,7.99296,19.8864C8.3216,19.9369,8.66197,19.9747,9.00235,19.9747L9.21362,19.9747C9.61268,19.9747,10.3756,19.9369,11.0094,19.697C11.1737,19.6338,11.3028,19.5076,11.3732,19.3434C11.4437,19.1793,11.4554,18.9899,11.3967,18.8131C11.3028,18.5354,11.0563,18.346,10.7864,18.346C10.716,18.346,10.6338,18.3586,10.5634,18.3838C10.0939,18.5606,9.46009,18.5859,9.20188,18.5859L9.09624,18.5859C9.20188,18.2702,9.23709,17.9167,9.15493,17.5505C9.00235,16.8939,8.50939,16.3384,7.58216,15.7955L7.19484,15.5682C6.57277,15.2146,6.23239,15.0253,6.03286,14.7348C5.83333,14.4444,5.69249,13.9899,5.51643,12.9798C5.38732,12.298,5.04695,11.7677,4.50704,11.4646C4.14319,11.2626,3.70892,11.149,3.19249,11.149C2.82864,11.149,2.42958,11.1995,2.00704,11.3005C1.79578,11.351,1.59624,11.4141,1.42019,11.4646C1.33803,10.9848,1.30282,10.4798,1.30282,9.97475C1.30282,6.93182,2.76995,4.26768,4.98826,2.72727C5,3.00505,5.05869,3.29545,5.17606,3.57323C5.48122,4.26768,6.10329,4.7096,7.01878,4.89899C7.06573,4.91162,7.10094,4.91162,7.13615,4.91162L7.1831,4.91162C7.26526,4.91162,7.57042,4.92424,7.88732,5.0505C8.3216,5.2399,8.56808,5.55555,8.65023,6.04798C8.84977,7.61364,9.07277,10.4293,8.79108,11.3384C8.76761,11.4141,8.75587,11.4899,8.75587,11.5657C8.75587,11.9444,9.0493,12.2601,9.40141,12.2601C9.57747,12.2601,9.74179,12.1843,9.85915,12.0581C9.97653,11.9318,12.6174,9.05303,13.3216,8.09343C13.4038,7.97979,13.4859,7.87878,13.5798,7.76515C13.9202,7.33586,14.2723,6.90656,14.4014,6.26262C14.554,5.56818,14.4014,4.79798,13.9437,3.85101C13.615,3.16919,13.5563,2.86616,13.5446,2.75252C13.5563,2.7399,13.5798,2.72727,13.6033,2.71464C15.6221,4.10353,17.0188,6.43939,17.2535,9.19192C17.2887,9.55808,17.5587,9.82323,17.8991,9.82323L17.9577,9.82323C18.3099,9.8106,18.5681,9.48232,18.5446,9.09091ZM3.19249,12.5631C3.48592,12.5631,3.72066,12.6136,3.89671,12.7146C4.08451,12.8283,4.19014,12.9924,4.23709,13.2702C4.43662,14.3434,4.61268,15.0631,5,15.6061C5.37559,16.1364,5.85681,16.4015,6.58451,16.8182L6.60798,16.8308C6.71362,16.8939,6.84272,16.9571,6.96009,17.0328C7.69953,17.4621,7.86385,17.7525,7.89906,17.8914C7.93427,18.0303,7.85211,18.2323,7.74648,18.4343C4.91784,17.8535,2.65258,15.6944,1.73709,12.8283C2.15962,12.702,2.71127,12.5631,3.19249,12.5631ZM12.7934,4.5202C13.4272,5.83333,13.1455,6.18687,12.5822,6.89394C12.4883,7.00758,12.3944,7.12121,12.3005,7.24747C11.9484,7.72727,11.0211,8.77525,10.2113,9.68434C10.2113,9.24242,10.1878,8.73737,10.1526,8.19444C10.0704,6.95707,9.92958,5.90909,9.92958,5.87121L9.92958,5.83333C9.75352,4.83586,9.20188,4.11616,8.3216,3.76263C7.82864,3.56061,7.37089,3.53535,7.19484,3.53535C6.73709,3.43434,6.4554,3.24495,6.33803,2.99242C6.19718,2.68939,6.29108,2.24747,6.38498,1.9697C7.28873,1.59091,8.26291,1.37626,9.28404,1.37626C10.3873,1.37626,11.4437,1.61616,12.4061,2.04545C12.3357,2.18434,12.277,2.34848,12.2535,2.5505C12.2066,3.04293,12.3709,3.64899,12.7934,4.5202Z"></path></svg><span></span></div><div class="item"><svg viewBox="0 0 20 20"><path d="M19.7361,12.542L18.1916,11.2919C18.2647,10.8678,18.3025,10.4347,18.3025,10.0017C18.3025,9.56861,18.2647,9.13555,18.1916,8.71142L19.7361,7.46135C19.9743,7.26938,20.0615,6.95686,19.9554,6.6756L19.9342,6.61756C19.5074,5.49026,18.8755,4.45449,18.0549,3.53926L18.0124,3.49238C17.8096,3.26692,17.4819,3.1821,17.1848,3.28032L15.2677,3.92544C14.5603,3.3763,13.7704,2.94324,12.9168,2.63966L12.5466,0.742229C12.49,0.449802,12.2472,0.222111,11.9383,0.168536L11.8746,0.157375C10.6461,-0.0524583,9.35391,-0.0524583,8.1254,0.157375L8.06174,0.168536C7.75284,0.222111,7.50997,0.449802,7.45338,0.742229L7.08082,2.64859C6.2343,2.95217,5.44909,3.383,4.74641,3.92991L2.81522,3.28032C2.52047,3.1821,2.19036,3.26469,1.98757,3.49238L1.94513,3.53926C1.12455,4.45672,0.492609,5.49249,0.0658141,6.61756L0.0445921,6.6756C-0.0615171,6.95463,0.0257283,7.26715,0.263885,7.46135L1.82723,8.72482C1.75413,9.14448,1.71876,9.57308,1.71876,9.99944C1.71876,10.428,1.75413,10.8566,1.82723,11.2741L0.263885,12.5375C0.025729,12.7295,-0.0615164,13.042,0.0445929,13.3233L0.0658148,13.3813C0.49261,14.5064,1.12455,15.5444,1.94513,16.4596L1.98757,16.5065C2.19036,16.732,2.51812,16.8168,2.81522,16.7186L4.74641,16.069C5.44909,16.6159,6.2343,17.0489,7.08082,17.3503L7.45338,19.2567C7.50997,19.5491,7.75284,19.7768,8.06174,19.8303L8.1254,19.8415C8.74084,19.9464,9.37042,20,10,20C10.6296,20,11.2615,19.9464,11.8746,19.8415L11.9383,19.8303C12.2472,19.7768,12.49,19.5491,12.5466,19.2567L12.9168,17.3592C13.7704,17.0556,14.5603,16.6248,15.2677,16.0734L17.1848,16.7186C17.4795,16.8168,17.8096,16.7342,18.0124,16.5065L18.0549,16.4596C18.8755,15.5422,19.5074,14.5064,19.9342,13.3813L19.9554,13.3233C20.0615,13.0487,19.9743,12.7362,19.7361,12.542ZM16.5175,8.97483C16.5764,9.3119,16.6071,9.65791,16.6071,10.0039C16.6071,10.3499,16.5764,10.6959,16.5175,11.033L16.3618,11.9281L18.1233,13.3545C17.8568,13.9372,17.5196,14.4863,17.1188,14.9975L14.9305,14.2631L14.1901,14.839C13.6266,15.2765,12.9994,15.6203,12.3203,15.8614L11.4219,16.1806L10.9998,18.3459C10.3372,18.4173,9.66045,18.4173,8.9955,18.3459L8.57342,16.1761L7.6821,15.8524C7.01008,15.6114,6.38521,15.2676,5.82637,14.8323L5.08596,14.2541L2.88361,14.9953C2.48275,14.4841,2.14791,13.9327,1.8791,13.3523L3.65938,11.9125L3.50611,11.0196C3.44952,10.687,3.41887,10.3432,3.41887,10.0039C3.41887,9.66237,3.44716,9.32083,3.50611,8.98822L3.65938,8.09531L1.8791,6.6555C2.14556,6.07288,2.48275,5.52374,2.88361,5.01255L5.08596,5.75367L5.82637,5.17551C6.38521,4.74022,7.01008,4.39645,7.6821,4.15536L8.57578,3.83615L8.99786,1.66638C9.66045,1.59495,10.3372,1.59495,11.0021,1.66638L11.4242,3.83168L12.3226,4.1509C12.9994,4.39198,13.6289,4.73575,14.1925,5.17328L14.9329,5.7492L17.1211,5.01479C17.522,5.52598,17.8568,6.07734,18.1256,6.65773L16.3642,8.08416L16.5175,8.97483ZM10.0024,5.85189C7.7104,5.85189,5.85231,7.61092,5.85231,9.78068C5.85231,11.9504,7.7104,13.7095,10.0024,13.7095C12.2943,13.7095,14.1524,11.9504,14.1524,9.78068C14.1524,7.61092,12.2943,5.85189,10.0024,5.85189ZM11.8699,11.5486C11.37,12.0196,10.7074,12.2808,10.0024,12.2808C9.29732,12.2808,8.63473,12.0196,8.13483,11.5486C7.6373,11.0754,7.36142,10.4481,7.36142,9.78068C7.36142,9.11323,7.6373,8.48596,8.13483,8.01272C8.63473,7.53948,9.29732,7.28054,10.0024,7.28054C10.7074,7.28054,11.37,7.53948,11.8699,8.01272C12.3674,8.48596,12.6433,9.11323,12.6433,9.78068C12.6433,10.4481,12.3674,11.0754,11.8699,11.5486Z"></path></svg><span></span></div></div>'), Ad = (e) => L(R1, {
  get title() {
    return a("more", e.locale);
  },
  isMobile: !0,
  get onClose() {
    return e.onClose;
  },
  get children() {
    const t = kd.cloneNode(!0), n = t.firstChild, r = n.firstChild, i = r.nextSibling, s = n.nextSibling, l = s.firstChild, c = l.nextSibling, h = s.nextSibling, m = h.firstChild, d = m.nextSibling;
    return n.$$click = () => {
      e.onIndicatorClick(), e.onClose();
    }, _(i, () => a("indicator", e.locale)), s.$$click = () => {
      e.onTimezoneClick(), e.onClose();
    }, _(c, () => a("timezone", e.locale)), h.$$click = () => {
      e.onSettingClick(), e.onClose();
    }, _(d, () => a("setting", e.locale)), t;
  }
});
_1(["click"]);
const wd = /* @__PURE__ */ p('<i class="icon-close klinecharts-pro-load-icon"></i>'), Md = /* @__PURE__ */ p('<div class="klinecharts-pro-content"><div class="klinecharts-pro-widget"></div></div>'), Sd = /* @__PURE__ */ p('<div class="klinecharts-pro-quick-order-menu"><button type="button">Buy <!> @ <!> Limit</button><button type="button">Buy <!> @ <!> Stop</button><button type="button">Create new order...</button><button type="button">Copy Price (<!>)</button><button type="button">Draw horizontal line on </button></div>'), Td = /* @__PURE__ */ p('<div class="klinecharts-pro-quick-order-marker"><button type="button" class="klinecharts-pro-quick-order-plus">+</button></div>');
function $e(e, t, n, r) {
  t === "VOL" && (r = {
    gap: {
      bottom: 2
    },
    ...r
  });
  const i = (e == null ? void 0 : e.createIndicator({
    name: t,
    // @ts-expect-error
    createTooltipDataSource: ({
      indicator: s,
      defaultStyles: l
    }) => {
      const c = [];
      return s.visible ? (c.push(l.tooltip.icons[1]), c.push(l.tooltip.icons[2]), c.push(l.tooltip.icons[3])) : (c.push(l.tooltip.icons[0]), c.push(l.tooltip.icons[2]), c.push(l.tooltip.icons[3])), {
        icons: c
      };
    }
  }, n, r)) ?? null;
  if (i && t === "MA")
    try {
      e == null || e.overrideIndicator({
        name: "MA",
        calcParams: [7, 25, 99]
      }, i);
    } catch {
    }
  return i;
}
const Pd = (e) => {
  var k0, A0, w0, M0, S0, T0, P0, I0;
  let t, n = null, r;
  const [i, s] = w(!1), [l, c] = w(e.theme), [h, m] = w(e.styles), [d, $] = w(e.locale), [v, k] = w(e.symbol), [T, D] = w(e.period), [I, F] = w(!1), [K, P] = w([...e.mainIndicators]), [B, z] = w({}), [$1, a1] = w(!1), [f1, O] = w({
    key: e.timezone,
    text: $t(e.timezone, e.locale)
  }), [G, R] = w(!1), [N, Z] = w(), [u1, l1] = w(""), [q, b1] = w(e.drawingBarVisible), [g, t1] = w(!1), [S, Y] = w(!1), [j, d1] = w(!1), [g1, O1] = w({
    quickOrder: ((k0 = e.orderTools) == null ? void 0 : k0.quickOrder) ?? !0,
    openOrders: ((A0 = e.orderTools) == null ? void 0 : A0.openOrders) ?? !0,
    positions: ((w0 = e.orderTools) == null ? void 0 : w0.positions) ?? !0,
    orderHistory: ((M0 = e.orderTools) == null ? void 0 : M0.orderHistory) ?? !0
  }), [L1, x1] = w(null), [T1, A1] = w(!1), [Ce, re] = w(!1), [ie, ae] = w({
    visible: !1,
    indicatorName: "",
    paneId: "",
    calcParams: []
  });
  let o1 = /* @__PURE__ */ new Map(), h1 = /* @__PURE__ */ new Map();
  const Ke = (o, u, C) => {
    const f = n == null ? void 0 : n.getIndicatorByPaneId(u, o);
    return {
      name: o,
      shortName: (f == null ? void 0 : f.shortName) || o,
      paneId: u,
      type: C,
      calcParams: (f == null ? void 0 : f.calcParams) || [],
      precision: (f == null ? void 0 : f.precision) ?? 4,
      visible: (f == null ? void 0 : f.visible) ?? !0,
      styles: f == null ? void 0 : f.styles,
      figures: f == null ? void 0 : f.figures
    };
  }, p1 = (o, u, C, f) => {
    if (e.onIndicatorChange)
      if (f === "add" || f === "change")
        setTimeout(() => {
          const x = Ke(o, u, C);
          e.onIndicatorChange({
            action: f,
            indicator: x
          });
        }, 50);
      else {
        const x = {
          name: o,
          shortName: o,
          paneId: u,
          type: C,
          calcParams: [],
          precision: 4,
          visible: !1,
          styles: void 0,
          figures: void 0
        };
        e.onIndicatorChange({
          action: f,
          indicator: x
        });
      }
  }, E = (o) => ({
    line: 2,
    segment: 2,
    arrow: 2,
    rect: 2,
    triangle: 3,
    polygon: 3,
    circle: 2,
    ellipse: 2,
    arc: 3,
    fibonacciLine: 2,
    fibonacciArc: 3,
    fibonacciFan: 3,
    fibonacciZone: 2,
    fibonacciSpiral: 2,
    xabcd: 4,
    abcd: 4,
    gartley: 5,
    bat: 5,
    butterfly: 5,
    shark: 5,
    cypher: 5,
    text: 1,
    priceLine: 1,
    priceChannelLine: 3
  })[o] || 1, r1 = (o, u = /* @__PURE__ */ new WeakSet()) => {
    if (o == null)
      return o;
    if (u.has(o))
      return "[Circular]";
    if (typeof o != "object")
      return o;
    if (u.add(o), Array.isArray(o))
      return o.map((f) => r1(f, u));
    const C = {};
    for (const f in o)
      if (!(f === "__proto__" || f === "constructor" || f === "prototype"))
        try {
          const x = o[f];
          if (typeof x == "function")
            continue;
          C[f] = r1(x, u);
        } catch (x) {
          C[f] = `[Error: ${x.message}]`;
        }
    return C;
  }, ye = (o) => {
    if (!o)
      return null;
    try {
      return {
        id: o.id || "",
        type: o.name || "",
        name: o.name || "",
        points: (o.points || []).map((u) => ({
          timestamp: u.timestamp || 0,
          value: u.value || 0,
          dataIndex: u.dataIndex || 0
        })),
        extendData: r1(o.extendData || {}),
        styles: r1(o.styles || {}),
        visible: o.visible ?? !0,
        lock: o.lock ?? !1,
        mode: o.mode || N0.Normal
      };
    } catch (u) {
      return console.error("Error extracting overlay data:", u), null;
    }
  }, N1 = (o) => {
    var u, C, f;
    try {
      const x = (u = n == null ? void 0 : n.getOverlayById) == null ? void 0 : u.call(n, o);
      if (!x)
        return;
      const A = ye(x);
      if (A) {
        const y = o1.get(o), b = ((C = y == null ? void 0 : y.points) == null ? void 0 : C.length) || 0, M = ((f = A.points) == null ? void 0 : f.length) || 0;
        o1.set(o, A);
        const U = E(A.type);
        if (M >= U) {
          const X = h1.get(o);
          X && !X.complete && (X.complete = !0, X.checkInterval && (clearInterval(X.checkInterval), X.checkInterval = void 0));
        }
      }
    } catch (x) {
      console.error(`Error updating overlay tracking for ${o}:`, x);
    }
  }, l9 = (o, u) => {
    if (h1.has(o))
      return;
    const C = {
      monitoring: !0,
      complete: !1,
      lastPointCount: 0
    };
    h1.set(o, C), N1(o);
    const f = () => {
      N1(o);
    };
    document.addEventListener("mouseup", f), document.addEventListener("touchend", f), setTimeout(() => {
      var A;
      const x = h1.get(o);
      if (x && !x.complete) {
        x.checkInterval && clearInterval(x.checkInterval), x.mouseUpHandler && (document.removeEventListener("mouseup", x.mouseUpHandler), document.removeEventListener("touchend", x.mouseUpHandler)), N1(o);
        const y = o1.get(o);
        if (y) {
          const b = E(y.type), M = ((A = y.points) == null ? void 0 : A.length) || 0;
          M < b && console.warn(`âš ï¸ ${y.type} ${o} has only ${M} point(s), should have ${b}`);
        }
      }
    }, 3e4);
  };
  let H1 = {
    saveDrawings: (o, u) => {
      try {
        const C = `kline_drawings_${o}`, x = {
          drawings: u.map((A) => {
            var U;
            const y = {
              ...A
            };
            y.extendData && (y.extendData = r1(y.extendData)), y.styles && (y.styles = r1(y.styles));
            const b = E(A.type), M = ((U = A.points) == null ? void 0 : U.length) || 0;
            return M < b && console.warn(`âš ï¸ Saving ${A.type} with only ${M} point(s), needs ${b}`), y;
          }),
          timestamp: Date.now()
        };
      } catch (C) {
        console.error("Library: Error saving drawings:", C);
      }
    },
    loadDrawings: (o) => {
      try {
        const u = `kline_drawings_${o}`;
      } catch (u) {
        console.error("Library: Error loading drawings:", u);
      }
      return [];
    },
    clearDrawings: (o) => {
      try {
        const u = `kline_drawings_${o}`;
      } catch (u) {
        console.error("Library: Error clearing drawings:", u);
      }
    }
  };
  const m0 = () => {
    const o = v();
    if (o != null && o.ticker) {
      const u = Array.from(o1.values());
      H1.saveDrawings(o.ticker, u);
    }
  }, p0 = (o) => {
    var C, f;
    const u = {
      ...g1(),
      ...o
    };
    O1(u), (f = (C = e.orderTools) == null ? void 0 : C.onChange) == null || f.call(C, u);
  }, fe = (o) => {
    var C;
    const u = Math.min(Math.max(((C = v()) == null ? void 0 : C.pricePrecision) ?? 2, 0), 8);
    return o.toLocaleString(void 0, {
      minimumFractionDigits: u,
      maximumFractionDigits: u
    });
  }, u9 = (o) => {
    var C, f;
    const u = Number(o == null ? void 0 : o.y);
    if (!Number.isFinite(u))
      return NaN;
    try {
      const x = n == null ? void 0 : n.convertFromPixel([{
        x: (o == null ? void 0 : o.x) ?? 0,
        y: u
      }], {
        paneId: "candle_pane"
      }), A = Number((C = x == null ? void 0 : x[0]) == null ? void 0 : C.value);
      if (Number.isFinite(A) && A > 0)
        return A;
    } catch {
    }
    try {
      const x = n == null ? void 0 : n.convertFromPixel([{
        x: (o == null ? void 0 : o.x) ?? 0,
        y: u
      }], {
        paneId: "candle_pane",
        absolute: !0
      }), A = Number((f = x == null ? void 0 : x[0]) == null ? void 0 : f.value);
      if (Number.isFinite(A) && A > 0)
        return A;
    } catch {
    }
    return NaN;
  }, v0 = (o) => {
    if (!g1().quickOrder || (o == null ? void 0 : o.paneId) !== "candle_pane" || !t) {
      if (Ce() || T1())
        return;
      x1(null), A1(!1);
      return;
    }
    const u = Number(o.y), C = u9(o), f = t.clientHeight;
    if (!Number.isFinite(u) || !Number.isFinite(C) || C <= 0 || u < 0 || u > f) {
      if (Ce() || T1())
        return;
      x1(null), A1(!1);
      return;
    }
    x1({
      y: u,
      price: C
    });
  }, je = (o) => {
    var C, f;
    const u = L1();
    u && ((f = (C = e.orderTools) == null ? void 0 : C.onQuickOrderAction) == null || f.call(C, {
      action: o,
      price: u.price,
      symbol: v()
    }), A1(!1));
  }, d9 = async () => {
    var u;
    const o = L1();
    if (o) {
      try {
        await ((u = navigator.clipboard) == null ? void 0 : u.writeText(String(o.price)));
      } catch {
      }
      A1(!1);
    }
  }, h9 = () => {
    const o = L1();
    o && (n == null || n.createOverlay({
      name: "horizontalStraightLine",
      points: [{
        value: o.price
      }],
      lock: !1
    }), A1(!1));
  };
  let _0 = (S0 = e.orderTools) == null ? void 0 : S0.quickOrder, $0 = (T0 = e.orderTools) == null ? void 0 : T0.openOrders, b0 = (P0 = e.orderTools) == null ? void 0 : P0.positions, L0 = (I0 = e.orderTools) == null ? void 0 : I0.orderHistory;
  m1(() => {
    var A, y, b, M;
    const o = (A = e.orderTools) == null ? void 0 : A.quickOrder, u = (y = e.orderTools) == null ? void 0 : y.openOrders, C = (b = e.orderTools) == null ? void 0 : b.positions, f = (M = e.orderTools) == null ? void 0 : M.orderHistory, x = {};
    typeof o == "boolean" && o !== _0 && (_0 = o, x.quickOrder = o), typeof u == "boolean" && u !== $0 && ($0 = u, x.openOrders = u), typeof C == "boolean" && C !== b0 && (b0 = C, x.positions = C), typeof f == "boolean" && f !== L0 && (L0 = f, x.orderHistory = f), Object.keys(x).length > 0 && O1({
      ...g1(),
      ...x
    });
  }), e.ref({
    setTheme: c,
    getTheme: () => l(),
    setStyles: m,
    getStyles: () => n.getStyles(),
    setLocale: $,
    getLocale: () => d(),
    setTimezone: (o) => {
      O({
        key: o,
        text: $t(e.timezone, d())
      });
    },
    getTimezone: () => f1().key,
    setSymbol: k,
    getSymbol: () => v(),
    setPeriod: D,
    getPeriod: () => T(),
    getMainIndicators: () => K(),
    getSubIndicators: () => B(),
    setMainIndicators: P,
    setSubIndicators: z,
    overrideIndicator: (o, u) => {
      n == null || n.overrideIndicator(o, u);
    },
    createOverlay: (o) => {
      var C;
      const u = (C = n == null ? void 0 : n.createOverlay) == null ? void 0 : C.call(n, o);
      return typeof u == "string" ? u : null;
    },
    removeOverlay: (o) => {
      var u;
      if ((u = n == null ? void 0 : n.removeOverlay) == null || u.call(n, o), o.id) {
        o1.delete(o.id);
        const C = h1.get(o.id);
        C && (C.checkInterval && clearInterval(C.checkInterval), C.mouseUpHandler && (document.removeEventListener("mouseup", C.mouseUpHandler), document.removeEventListener("touchend", C.mouseUpHandler)), h1.delete(o.id)), m0();
      }
    },
    removeAllOverlay: () => {
      o1.forEach((o, u) => {
        var f;
        (f = n == null ? void 0 : n.removeOverlay) == null || f.call(n, {
          id: u
        });
        const C = h1.get(u);
        C && (C.checkInterval && clearInterval(C.checkInterval), C.mouseUpHandler && (document.removeEventListener("mouseup", C.mouseUpHandler), document.removeEventListener("touchend", C.mouseUpHandler)));
      }), o1.clear(), h1.clear();
    },
    getAllOverlay: () => Array.from(o1.values()),
    getOverlay: (o) => o1.get(o) || null,
    overrideOverlay: (o) => {
      n && "overrideOverlay" in n && typeof n.overrideOverlay == "function" ? n.overrideOverlay(o) : console.warn("overrideOverlay method not available on widget");
    },
    convertToPixel: (o, u) => n ? n.convertToPixel(o, u) : Array.isArray(o) ? [] : {},
    convertFromPixel: (o, u) => n ? n.convertFromPixel(o, u) : [],
    getVisibleRange: () => n ? n.getVisibleRange() : {
      from: 0,
      to: 0
    },
    getDataList: () => n ? n.getDataList() : [],
    getSize: (o, u) => n ? n.getSize(o, u) : null,
    subscribeAction: (o, u) => {
      n && n.subscribeAction(o, u);
    },
    unsubscribeAction: (o, u) => {
      n && n.unsubscribeAction(o, u);
    },
    setIndicatorModalVisible: F,
    setTimezoneModalVisible: a1,
    setSettingModalVisible: R,
    getOrderToolsState: () => g1(),
    setOrderToolsState: (o) => {
      p0(o);
    },
    dispose: () => {
      t && E0(t);
    },
    resize: () => {
      n && "resize" in n && typeof n.resize == "function" ? n.resize() : console.warn("resize method not available on widget");
    },
    getSettings: () => {
      var C, f, x, A, y, b, M, U, X, C1, D1, E1, ge, se, oe, B1;
      if (!n)
        return {};
      const o = n.getStyles(), u = (C = o.candle) == null ? void 0 : C.bar;
      return {
        // Candle settings
        candleType: (f = o.candle) == null ? void 0 : f.type,
        candleBarStyle: u == null ? void 0 : u.style,
        // bar.style might be LineType
        showLastPrice: (y = (A = (x = o.candle) == null ? void 0 : x.priceMark) == null ? void 0 : A.last) == null ? void 0 : y.show,
        showHighestPrice: (U = (M = (b = o.candle) == null ? void 0 : b.priceMark) == null ? void 0 : M.high) == null ? void 0 : U.show,
        showLowestPrice: (D1 = (C1 = (X = o.candle) == null ? void 0 : X.priceMark) == null ? void 0 : C1.low) == null ? void 0 : D1.show,
        // Indicator settings
        showIndicatorLastValue: (ge = (E1 = o.indicator) == null ? void 0 : E1.lastValueMark) == null ? void 0 : ge.show,
        // Axis settings - yAxis.reverse is boolean according to YAxisStyle interface
        priceAxisType: (se = o.yAxis) == null ? void 0 : se.type,
        reverseCoordinate: (oe = o.yAxis) == null ? void 0 : oe.reverse,
        // Grid settings
        showGrids: (B1 = o.grid) == null ? void 0 : B1.show,
        timestamp: Date.now()
      };
    },
    setSettings: (o) => {
      var C, f, x, A, y, b, M, U, X, C1, D1;
      if (!n)
        return;
      const u = {};
      if (o.candleType !== void 0 && (u.candle = {
        ...u.candle,
        type: o.candleType
      }), o.candleBarStyle !== void 0) {
        const E1 = ((C = u.candle) == null ? void 0 : C.bar) || {};
        u.candle = {
          ...u.candle,
          bar: {
            ...E1,
            style: o.candleBarStyle
          }
          // Use any since ChangeColor doesn't have style
        };
      }
      o.showLastPrice !== void 0 && (u.candle = {
        ...u.candle,
        priceMark: {
          ...(f = u.candle) == null ? void 0 : f.priceMark,
          last: {
            ...(A = (x = u.candle) == null ? void 0 : x.priceMark) == null ? void 0 : A.last,
            show: o.showLastPrice
          }
        }
      }), o.showHighestPrice !== void 0 && (u.candle = {
        ...u.candle,
        priceMark: {
          ...(y = u.candle) == null ? void 0 : y.priceMark,
          high: {
            ...(M = (b = u.candle) == null ? void 0 : b.priceMark) == null ? void 0 : M.high,
            show: o.showHighestPrice
          }
        }
      }), o.showLowestPrice !== void 0 && (u.candle = {
        ...u.candle,
        priceMark: {
          ...(U = u.candle) == null ? void 0 : U.priceMark,
          low: {
            ...(C1 = (X = u.candle) == null ? void 0 : X.priceMark) == null ? void 0 : C1.low,
            show: o.showLowestPrice
          }
        }
      }), o.showIndicatorLastValue !== void 0 && (u.indicator = {
        ...u.indicator,
        lastValueMark: {
          ...(D1 = u.indicator) == null ? void 0 : D1.lastValueMark,
          show: o.showIndicatorLastValue
        }
      }), o.priceAxisType !== void 0 && (u.yAxis = {
        ...u.yAxis,
        type: o.priceAxisType
      }), o.reverseCoordinate !== void 0 && (u.yAxis = {
        ...u.yAxis,
        reverse: o.reverseCoordinate
      }), o.showGrids !== void 0 && (u.grid = {
        ...u.grid,
        show: o.showGrids
      }), n.setStyles(u);
    },
    resetSettings: () => {
      var C, f, x, A, y, b, M;
      if (!n)
        return;
      n.getStyles();
      const o = {
        candle: {
          type: m9.CandleSolid,
          // Default to solid candles
          priceMark: {
            last: {
              show: !0
            },
            high: {
              show: !0
            },
            low: {
              show: !0
            }
          }
        },
        indicator: {
          lastValueMark: {
            show: !0
          }
        },
        yAxis: {
          type: p9.Normal,
          reverse: !1
        },
        grid: {
          show: !0,
          horizontal: {
            show: !0,
            color: "#1e293b",
            size: 0.5
          },
          vertical: {
            show: !0,
            color: "#1e293b",
            size: 0.5
          }
        }
      }, u = N();
      if (u) {
        const U = {
          candle: {
            type: (C = u.candle) == null ? void 0 : C.type,
            bar: (f = u.candle) == null ? void 0 : f.bar,
            priceMark: (x = u.candle) == null ? void 0 : x.priceMark
          },
          indicator: {
            lastValueMark: (A = u.indicator) == null ? void 0 : A.lastValueMark
          },
          yAxis: {
            type: (y = u.yAxis) == null ? void 0 : y.type,
            reverse: (b = u.yAxis) == null ? void 0 : b.reverse
          },
          grid: {
            show: (M = u.grid) == null ? void 0 : M.show
          }
        };
        n.setStyles(U);
      } else
        n.setStyles(o);
    },
    // === Drawing Methods ===
    saveDrawings: (o) => {
      const u = Array.from(o1.values());
      u.forEach((C, f) => {
        var y;
        const x = E(C.type), A = ((y = C.points) == null ? void 0 : y.length) || 0;
        A < x && console.warn(`âš ï¸ ${C.type} ${C.id} has only ${A} point(s), should have ${x}`);
      }), H1.saveDrawings(o, u);
    },
    loadDrawings: (o) => {
      H1.loadDrawings(o).forEach((C, f) => {
        var x;
        try {
          const A = {
            name: C.type,
            points: C.points || [],
            extendData: C.extendData,
            styles: C.styles,
            visible: C.visible ?? !0,
            lock: C.lock ?? !1,
            mode: C.mode ?? N0.Normal
          }, y = n == null ? void 0 : n.createOverlay(A), b = typeof y == "string" ? y : null;
          b && (o1.set(b, {
            ...C,
            id: b
          }), h1.set(b, {
            monitoring: !1,
            complete: !0,
            lastPointCount: ((x = C.points) == null ? void 0 : x.length) || 0
          }));
        } catch (A) {
          console.error(`   âŒ Error restoring ${C.type}:`, A);
        }
      });
    },
    getDrawings: (o) => H1.loadDrawings(o),
    clearDrawings: (o) => {
      H1.clearDrawings(o);
    },
    // Auto-save on overlay events
    enableAutoSave: (o, u = !0) => {
    }
  });
  const x0 = () => {
    n == null || n.resize();
  }, ze = (o, u, C) => {
    let f = u, x = f;
    switch (o.timespan) {
      case "minute": {
        f = f - f % (60 * 1e3), x = f - C * o.multiplier * 60 * 1e3;
        break;
      }
      case "hour": {
        f = f - f % (60 * 60 * 1e3), x = f - C * o.multiplier * 60 * 60 * 1e3;
        break;
      }
      case "day": {
        f = f - f % (60 * 60 * 1e3), x = f - C * o.multiplier * 24 * 60 * 60 * 1e3;
        break;
      }
      case "week": {
        const y = new Date(f).getDay(), b = y === 0 ? 6 : y - 1;
        f = f - b * 60 * 60 * 24;
        const M = new Date(f);
        f = (/* @__PURE__ */ new Date(`${M.getFullYear()}-${M.getMonth() + 1}-${M.getDate()}`)).getTime(), x = C * o.multiplier * 7 * 24 * 60 * 60 * 1e3;
        break;
      }
      case "month": {
        const A = new Date(f), y = A.getFullYear(), b = A.getMonth() + 1;
        f = (/* @__PURE__ */ new Date(`${y}-${b}-01`)).getTime(), x = C * o.multiplier * 30 * 24 * 60 * 60 * 1e3;
        const M = new Date(x);
        x = (/* @__PURE__ */ new Date(`${M.getFullYear()}-${M.getMonth() + 1}-01`)).getTime();
        break;
      }
      case "year": {
        const y = new Date(f).getFullYear();
        f = (/* @__PURE__ */ new Date(`${y}-01-01`)).getTime(), x = C * o.multiplier * 365 * 24 * 60 * 60 * 1e3;
        const b = new Date(x);
        x = (/* @__PURE__ */ new Date(`${b.getFullYear()}-01-01`)).getTime();
        break;
      }
    }
    return [x, f];
  };
  return i0(() => {
    if (window.addEventListener("resize", x0), n = g9(t, {
      customApi: {
        formatDate: (y, b, M, U) => {
          switch (T().timespan) {
            case "minute":
              return U === me.XAxis ? n1.formatDate(y, b, "HH:mm") : n1.formatDate(y, b, "YYYY-MM-DD HH:mm");
            case "hour":
              return U === me.XAxis ? n1.formatDate(y, b, "MM-DD HH:mm") : n1.formatDate(y, b, "YYYY-MM-DD HH:mm");
            case "day":
            case "week":
              return n1.formatDate(y, b, "YYYY-MM-DD");
            case "month":
              return U === me.XAxis ? n1.formatDate(y, b, "YYYY-MM") : n1.formatDate(y, b, "YYYY-MM-DD");
            case "year":
              return U === me.XAxis ? n1.formatDate(y, b, "YYYY") : n1.formatDate(y, b, "YYYY-MM-DD");
          }
          return n1.formatDate(y, b, "YYYY-MM-DD HH:mm");
        }
      }
    }), n) {
      const y = n.getDom("candle_pane", D0.Main);
      if (y) {
        let M = document.createElement("div");
        if (M.className = "klinecharts-pro-watermark", n1.isString(e.watermark)) {
          const U = e.watermark.replace(/(^\s*)|(\s*$)/g, "");
          M.innerHTML = U;
        } else
          M.appendChild(e.watermark);
        y.appendChild(M);
      }
      const b = n.getDom("candle_pane", D0.YAxis);
      r = document.createElement("span"), r.className = "klinecharts-pro-price-unit", b == null || b.appendChild(r);
    }
    let o = !1;
    const u = () => {
      const y = v();
      if (y != null && y.ticker)
        try {
          const b = Array.from(o1.values());
          H1.saveDrawings(y.ticker, b);
        } catch (b) {
          console.error("âŒ Error refreshing local storage:", b);
        }
    }, C = (y) => {
      o || (o = !0, y.preventDefault());
    };
    setTimeout(() => {
      t && t.addEventListener("contextmenu", C);
    }, 1e3), document.addEventListener("contextmenu", (y) => {
      t && t.contains(y.target) && C(y);
    });
    const f = n == null ? void 0 : n.removeOverlay;
    n && f && (n.removeOverlay = function(...y) {
      const b = f.apply(this, y), M = y[0];
      let U;
      if (typeof M == "string" ? U = M : M && typeof M == "object" && M.id && (U = M.id), U) {
        o1.delete(U);
        const X = h1.get(U);
        X && (X.checkInterval && clearInterval(X.checkInterval), X.mouseUpHandler && (document.removeEventListener("mouseup", X.mouseUpHandler), document.removeEventListener("touchend", X.mouseUpHandler)), h1.delete(U)), u();
      }
      return b;
    }), K().forEach((y) => {
      $e(n, y, !0, {
        id: "candle_pane"
      });
    });
    const x = {};
    e.subIndicators.forEach((y) => {
      const b = $e(n, y, !0);
      b && (x[y] = b);
    }), z(x), n == null || n.loadMore((y) => {
      s(!0), (async () => {
        try {
          const M = T(), [U] = ze(M, y, 1), [X] = ze(M, U, 500), C1 = await e.datafeed.getHistoryKLineData(v(), M, X, U);
          n == null || n.applyMoreData(C1, C1.length > 0);
        } finally {
          s(!1);
        }
      })();
    }), n == null || n.subscribeAction(Ze.OnTooltipIconClick, (y) => {
      if (y.indicatorName)
        switch (y.iconId) {
          case "visible": {
            n == null || n.overrideIndicator({
              name: y.indicatorName,
              visible: !0
            }, y.paneId);
            const b = y.paneId === "candle_pane" ? "main" : "sub";
            p1(y.indicatorName, y.paneId, b, "change");
            break;
          }
          case "invisible": {
            n == null || n.overrideIndicator({
              name: y.indicatorName,
              visible: !1
            }, y.paneId);
            const b = y.paneId === "candle_pane" ? "main" : "sub";
            p1(y.indicatorName, y.paneId, b, "change");
            break;
          }
          case "setting": {
            const b = n == null ? void 0 : n.getIndicatorByPaneId(y.paneId, y.indicatorName);
            ae({
              visible: !0,
              indicatorName: y.indicatorName,
              paneId: y.paneId,
              calcParams: b.calcParams
            });
            break;
          }
          case "close":
            if (y.paneId === "candle_pane") {
              const b = [...K()];
              n == null || n.removeIndicator("candle_pane", y.indicatorName), b.splice(b.indexOf(y.indicatorName), 1), P(b), p1(y.indicatorName, "candle_pane", "main", "remove");
            } else {
              const b = {
                ...B()
              };
              n == null || n.removeIndicator(y.paneId, y.indicatorName), delete b[y.indicatorName], z(b), p1(y.indicatorName, y.paneId, "sub", "remove");
            }
        }
    }), n == null || n.subscribeAction(Ze.OnCrosshairChange, v0);
    const A = n == null ? void 0 : n.createOverlay;
    n && A && (n.createOverlay = function(...y) {
      const b = y[0], M = A.apply(this, y), U = typeof M == "string" ? M : null;
      return U && (l9(U, b.name || "unknown"), N1(U), m0()), M;
    });
  }), j1(() => {
    window.removeEventListener("resize", x0), n == null || n.unsubscribeAction(Ze.OnCrosshairChange, v0), h1.clear(), o1.clear(), E0(t);
  }), m1(() => {
    const o = v();
    o != null && o.priceCurrency ? (r.innerHTML = o == null ? void 0 : o.priceCurrency.toLocaleUpperCase(), r.style.display = "flex") : r.style.display = "none", n == null || n.setPriceVolumePrecision((o == null ? void 0 : o.pricePrecision) ?? 2, (o == null ? void 0 : o.volumePrecision) ?? 0);
  }), m1((o) => {
    const u = v(), C = T();
    let f = !0;
    return j1(() => {
      f = !1;
    }), o && e.datafeed.unsubscribe(o.symbol, o.period), s(!0), Y(!0), (async () => {
      try {
        const [A, y] = ze(C, (/* @__PURE__ */ new Date()).getTime(), 500), b = await e.datafeed.getHistoryKLineData(u, C, A, y);
        if (!f)
          return;
        n == null || n.applyNewData(b, b.length > 0), e.datafeed.subscribe(u, C, (M) => {
          n == null || n.updateData(M);
        });
      } finally {
        f && (s(!1), Y(!1));
      }
    })(), {
      symbol: u,
      period: C
    };
  }), m1(() => {
    const o = l();
    n == null || n.setStyles(o);
    const u = o === "dark" ? "#929AA5" : "#76808F";
    n == null || n.setStyles({
      candle: {
        tooltip: {
          rect: {
            offsetLeft: 0,
            paddingLeft: 0
          }
        }
      },
      indicator: {
        tooltip: {
          icons: [{
            id: "visible",
            position: pe.Middle,
            marginLeft: 8,
            marginTop: 7,
            marginRight: 0,
            marginBottom: 0,
            paddingLeft: 0,
            paddingTop: 0,
            paddingRight: 0,
            paddingBottom: 0,
            icon: "",
            fontFamily: "icomoon",
            size: 14,
            color: u,
            activeColor: u,
            backgroundColor: "transparent",
            activeBackgroundColor: "rgba(22, 119, 255, 0.15)"
          }, {
            id: "invisible",
            position: pe.Middle,
            marginLeft: 8,
            marginTop: 7,
            marginRight: 0,
            marginBottom: 0,
            paddingLeft: 0,
            paddingTop: 0,
            paddingRight: 0,
            paddingBottom: 0,
            icon: "",
            fontFamily: "icomoon",
            size: 14,
            color: u,
            activeColor: u,
            backgroundColor: "transparent",
            activeBackgroundColor: "rgba(22, 119, 255, 0.15)"
          }, {
            id: "setting",
            position: pe.Middle,
            marginLeft: 6,
            marginTop: 7,
            marginBottom: 0,
            marginRight: 0,
            paddingLeft: 0,
            paddingTop: 0,
            paddingRight: 0,
            paddingBottom: 0,
            icon: "",
            fontFamily: "icomoon",
            size: 14,
            color: u,
            activeColor: u,
            backgroundColor: "transparent",
            activeBackgroundColor: "rgba(22, 119, 255, 0.15)"
          }, {
            id: "close",
            position: pe.Middle,
            marginLeft: 6,
            marginTop: 7,
            marginRight: 0,
            marginBottom: 0,
            paddingLeft: 0,
            paddingTop: 0,
            paddingRight: 0,
            paddingBottom: 0,
            icon: "",
            fontFamily: "icomoon",
            size: 14,
            color: u,
            activeColor: u,
            backgroundColor: "transparent",
            activeBackgroundColor: "rgba(22, 119, 255, 0.15)"
          }]
        }
      }
    });
  }), m1(() => {
    n == null || n.setLocale(d());
  }), m1(() => {
    n == null || n.setTimezone(f1().key);
  }), m1(() => {
    h() && (n == null || n.setStyles(h()), Z(Mi(n.getStyles())));
  }), [wd.cloneNode(!0), L(Il, {
    get locale() {
      return e.locale;
    },
    get symbol() {
      return v();
    },
    get spread() {
      return q();
    },
    get period() {
      return T();
    },
    get periods() {
      return e.periods;
    },
    onMenuClick: async () => {
      try {
        await z9(() => b1(!q())), n == null || n.resize();
      } catch {
      }
    },
    onSymbolClick: () => {
      t1(!g());
    },
    get onMobilePeriodClick() {
      return e.onMobilePeriodClick;
    },
    onMobileMoreClick: () => {
      e.onMobileMoreClick ? e.onMobileMoreClick() : d1(!0);
    },
    onPeriodChange: D,
    onIndicatorClick: () => {
      F((o) => !o);
    },
    onTimezoneClick: () => {
      a1((o) => !o);
    },
    onSettingClick: () => {
      R((o) => !o);
    },
    onScreenshotClick: () => {
      if (n) {
        const o = e.screenshotBackgroundColor || (e.theme === "dark" ? "#11131E" : "#ffffff"), u = n.getConvertPictureUrl(!0, "jpeg", o);
        l1(u);
      }
    },
    get showOrderToolsMenu() {
      var o;
      return ((o = e.orderTools) == null ? void 0 : o.visible) ?? !1;
    },
    get orderToolsState() {
      return g1();
    },
    onOrderToolsStateChange: p0
  }), (() => {
    const o = Md.cloneNode(!0), u = o.firstChild;
    return _(o, L(H, {
      get when() {
        return S();
      },
      get children() {
        return L(a9, {});
      }
    }), u), _(o, L(H, {
      get when() {
        return q();
      },
      get children() {
        return L(cd, {
          get locale() {
            return e.locale;
          },
          onDrawingItemClick: (C) => {
            n == null || n.createOverlay(C);
          },
          onModeChange: (C) => {
            n == null || n.overrideOverlay({
              mode: C
            });
          },
          onLockChange: (C) => {
            n == null || n.overrideOverlay({
              lock: C
            });
          },
          onVisibleChange: (C) => {
            n == null || n.overrideOverlay({
              visible: C
            });
          },
          onRemoveClick: (C) => {
            n == null || n.removeOverlay({
              groupId: C
            });
          }
        });
      }
    }), u), K1((C) => t = C, u), _(o, L(H, {
      get when() {
        return L1();
      },
      keyed: !0,
      children: (C) => (() => {
        const f = Td.cloneNode(!0), x = f.firstChild;
        return f.addEventListener("mouseleave", () => re(!1)), f.addEventListener("mouseenter", () => re(!0)), x.$$click = (A) => {
          A.stopPropagation(), A1((y) => !y);
        }, _(f, L(H, {
          get when() {
            return T1();
          },
          get children() {
            const A = Sd.cloneNode(!0), y = A.firstChild, b = y.firstChild, M = b.nextSibling, U = M.nextSibling, X = U.nextSibling;
            X.nextSibling;
            const C1 = y.nextSibling, D1 = C1.firstChild, E1 = D1.nextSibling, ge = E1.nextSibling, se = ge.nextSibling;
            se.nextSibling;
            const oe = C1.nextSibling, B1 = oe.nextSibling, C9 = B1.firstChild, O0 = C9.nextSibling;
            O0.nextSibling;
            const Qe = B1.nextSibling;
            return Qe.firstChild, y.$$click = () => je("limit"), _(y, () => v().shortName ?? v().name ?? v().ticker, M), _(y, () => fe(C.price), X), C1.$$click = () => je("stop"), _(C1, () => v().shortName ?? v().name ?? v().ticker, E1), _(C1, () => fe(C.price), se), oe.$$click = () => je("create"), B1.$$click = d9, _(B1, () => fe(C.price), O0), Qe.$$click = h9, _(Qe, () => fe(C.price), null), A;
          }
        }), null), Q((A) => {
          const y = `${Math.max(0, C.y - 18)}px`, b = g1().quickOrder ? "block" : "none";
          return y !== A._v$ && f.style.setProperty("top", A._v$ = y), b !== A._v$2 && f.style.setProperty("display", A._v$2 = b), A;
        }, {
          _v$: void 0,
          _v$2: void 0
        }), f;
      })()
    }), null), Q(() => y1(u, "data-drawing-bar-visible", q())), o;
  })(), L(H, {
    get when() {
      return g();
    },
    get children() {
      return L(xd, {
        get locale() {
          return e.locale;
        },
        get datafeed() {
          return e.datafeed;
        },
        onSymbolSelected: (o) => {
          k(o);
        },
        onClose: () => {
          t1(!1);
        }
      });
    }
  }), L(H, {
    get when() {
      return I();
    },
    get children() {
      return L(ld, {
        get locale() {
          return e.locale;
        },
        get mainIndicators() {
          return K();
        },
        get subIndicators() {
          return B();
        },
        onClose: () => {
          F(!1);
        },
        onMainIndicatorChange: (o) => {
          const u = [...K()];
          o.added ? ($e(n, o.name, !0, {
            id: "candle_pane"
          }), u.push(o.name), p1(o.name, "candle_pane", "main", "add")) : (n == null || n.removeIndicator("candle_pane", o.name), u.splice(u.indexOf(o.name), 1), p1(o.name, "candle_pane", "main", "remove")), P(u);
        },
        onSubIndicatorChange: (o) => {
          const u = {
            ...B()
          };
          if (o.added) {
            const C = $e(n, o.name);
            C && (u[o.name] = C, p1(o.name, C, "sub", "add"));
          } else
            o.paneId && (n == null || n.removeIndicator(o.paneId, o.name), delete u[o.name], p1(o.name, o.paneId, "sub", "remove"));
          z(u);
        }
      });
    }
  }), L(H, {
    get when() {
      return $1();
    },
    get children() {
      return L(dd, {
        get locale() {
          return e.locale;
        },
        get timezone() {
          return f1();
        },
        onClose: () => {
          a1(!1);
        },
        onConfirm: O
      });
    }
  }), L(H, {
    get when() {
      return G();
    },
    get children() {
      return L(yd, {
        get locale() {
          return e.locale;
        },
        get currentStyles() {
          return n1.clone(n.getStyles());
        },
        onClose: () => {
          R(!1);
        },
        onChange: (o) => {
          n == null || n.setStyles(o);
        },
        onRestoreDefault: (o) => {
          const u = {};
          o.forEach((C) => {
            const f = C.key;
            We(u, f, n1.formatValue(N(), f));
          }), n == null || n.setStyles(u);
        }
      });
    }
  }), L(H, {
    get when() {
      return u1().length > 0;
    },
    get children() {
      return L(gd, {
        get locale() {
          return e.locale;
        },
        get url() {
          return u1();
        },
        onClose: () => {
          l1("");
        }
      });
    }
  }), L(H, {
    get when() {
      return ie().visible;
    },
    get children() {
      return L(_d, {
        get locale() {
          return e.locale;
        },
        get params() {
          return ie();
        },
        onClose: () => {
          ae({
            visible: !1,
            indicatorName: "",
            paneId: "",
            calcParams: []
          });
        },
        onConfirm: (o) => {
          const u = ie();
          n == null || n.overrideIndicator({
            name: u.indicatorName,
            calcParams: o
          }, u.paneId);
          const C = u.paneId === "candle_pane" ? "main" : "sub";
          p1(u.indicatorName, u.paneId, C, "change");
        }
      });
    }
  }), L(H, {
    get when() {
      return j();
    },
    get children() {
      return L(Ad, {
        get locale() {
          return e.locale;
        },
        onIndicatorClick: () => {
          F(!0);
        },
        onTimezoneClick: () => {
          a1(!0);
        },
        onSettingClick: () => {
          R(!0);
        },
        onClose: () => {
          d1(!1);
        }
      });
    }
  })];
};
_1(["click"]);
const Id = /* @__PURE__ */ p('<svg class="logo" viewBox="0 0 80 92"><path d="M28.148808359375,51.7280513671875L22.963588359375,51.7280513671875C21.572648359375002,51.7280513671875,20.445068359375,52.6220613671875,20.445068359375,53.7248813671875L20.445068359375,72.3979013671875C20.445068359375,73.5007013671875,21.572648359375002,74.39470136718751,22.963588359375,74.39470136718751L33.926568359375,74.39470136718751C35.317468359375,74.39470136718751,36.445068359375,73.5007013671875,36.445068359375,72.3979013671875L36.445068359375,53.7248813671875C36.445068359375,52.6220613671875,35.317468359375,51.7280513671875,33.926568359375,51.7280513671875L28.741398359374998,51.7280513671875L28.741398359374998,46.2963223671875C28.741398359374998,46.1665793671875,28.608748359375,46.0614013671875,28.445108359375,46.0614013671875C28.281468359375,46.0614013671875,28.148808359375,46.1665793671875,28.148808359375,46.2963223671875L28.148808359375,51.7280513671875ZM28.741398359374998,74.3948013671875L28.741398359374998,79.82650136718749C28.741398359374998,79.9563013671875,28.608748359375,80.0614013671875,28.445108359375,80.0614013671875C28.281468359375,80.0614013671875,28.148808359375,79.9563013671875,28.148808359375,79.82650136718749L28.148808359375,74.3948013671875L28.741398359374998,74.3948013671875Z"></path><path d="M51.148808359374996,44.7280513671875L45.963588359375,44.7280513671875C44.572648359375,44.7280513671875,43.445068359375,45.6220613671875,43.445068359375,46.7248813671875L43.445068359375,65.3979013671875C43.445068359375,66.5007013671875,44.572648359375,67.39470136718751,45.963588359375,67.39470136718751L56.926568359375,67.39470136718751C58.317468359375,67.39470136718751,59.445068359375,66.5007013671875,59.445068359375,65.3979013671875L59.445068359375,46.7248813671875C59.445068359375,45.6220613671875,58.317468359375,44.7280513671875,56.926568359375,44.7280513671875L51.741398359375,44.7280513671875L51.741398359375,39.2963223671875C51.741398359375,39.1665793671875,51.608748359375,39.0614013671875,51.445108359375,39.0614013671875C51.281468359375,39.0614013671875,51.148808359374996,39.1665793671875,51.148808359374996,39.2963223671875L51.148808359374996,44.7280513671875ZM51.741398359375,67.3948013671875L51.741398359375,72.82650136718749C51.741398359375,72.9563013671875,51.608748359375,73.0614013671875,51.445108359375,73.0614013671875C51.281468359375,73.0614013671875,51.148808359374996,72.9563013671875,51.148808359374996,72.82650136718749L51.148808359374996,67.3948013671875L51.741398359375,67.3948013671875Z"></path><path d="M17.7274,90.6541C17.5901,90.6541,17.4517,90.6436,17.3121,90.6225C9.93219,89.5095,4.80718,86.7136,2.07787,82.3084C-1.1223,77.1437,0.241766,71.6314,0.56829,70.5137C5.37624,46.647,15.0785,38.4945,21.5025,33.0957C22.9683,31.8633,24.2342,30.7995,25.1676,29.7672C25.4105,29.4984,25.6051,29.2154,25.7556,28.9202C24.7465,29.2231,24.1971,29.4326,24.1703,29.4429C22.908,29.9368,21.4777,29.3247,20.9761,28.076C20.4756,26.8272,21.0897,25.4146,22.352,24.9172C22.5042,24.8571,23.5312,24.4607,25.3073,23.9616C24.087,21.4425,21.7693,18.7949,19.7125,16.6431L19.2819,16.1902C16.2438,12.9776,14.6017,4.80159,14.3036,3.19471C14.1306,2.26212,14.4636,1.30796,15.1814,0.679657C15.8995,0.0512175,16.8976,-0.159672,17.8125,0.123747C22.7731,1.66274,24.2638,1.81255,27.2321,2.11098C28.7357,2.26195,29.83,3.59029,29.6762,5.07662C29.5236,6.56295,28.182,7.64786,26.6784,7.49454C24.4992,7.27569,22.9517,7.09896,20.724,6.56646C21.4493,9.09088,22.3803,11.5427,23.2771,12.4919L23.6876,12.9237C25.3757,14.69,28.9691,18.45,30.7016,22.7299C35.0392,21.9433,40.8791,21.3359,47.7817,21.7249C48.2004,20.7386,48.8054,19.7953,49.5907,18.9135C49.7137,18.7754,49.8498,18.6502,49.9988,18.539C53.6142,15.8508,57.5491,12.857,59.7803,11.0758C58.1028,11.2502,56.1034,11.0278,53.9124,9.70882C53.2439,9.30622,52.5992,8.89427,51.9662,8.48933C48.4668,6.25164,46.497,5.12109,43.4234,5.94853C41.9647,6.34058,40.4622,5.48975,40.0659,4.04789C39.6695,2.60604,40.5296,1.11853,41.9871,0.726471C47.5602,-0.773825,51.4796,1.73271,54.9364,3.9434L54.9364,3.9434C55.5284,4.32176,56.1318,4.70797,56.7564,5.08482C58.3843,6.06556,59.4858,5.76127,61.2899,5.13865C62.3511,4.77234,63.5567,4.35687,64.8675,4.53476C66.3321,4.73254,67.4406,5.56933,67.9103,6.83096C68.7444,9.07333,67.1035,11.5533,65.5797,13.2374C64.6729,14.2394,60.0845,17.7606,56.4519,20.4957C56.9477,20.3369,57.4767,20.2511,58.026,20.2511C59.4281,20.2511,60.6982,20.8102,61.621,21.7153C65.4948,20.6901,67.87,17.9563,67.9033,17.9175C68.78,16.8888,70.3322,16.7577,71.3721,17.6226C72.412,18.4886,72.5457,20.0253,71.6702,21.054C71.5221,21.2286,69.5063,23.5492,66.0787,25.233C69.5399,26.8822,72.9993,29.682,74.1841,34.4145C74.5106,35.7206,73.7062,37.0407,72.3859,37.3638C72.1871,37.4117,71.9884,37.4351,71.792,37.4351C70.687,37.4351,69.6826,36.6932,69.4046,35.5848C68.4378,31.7217,64.8144,29.7431,61.7619,28.7456C60.8298,29.7349,59.5009,30.3535,58.026,30.3535C55.8642,30.3535,54.0162,29.0245,53.2713,27.1474C53.2022,27.138,53.1331,27.1287,53.0642,27.1195C54.232,29.5936,57.0851,31.9259,58.1868,32.665C58.3157,32.7516,58.4423,32.8523,58.5547,32.9599C66.5865,40.6151,72.4887,48.8133,76.0971,57.3287C76.6815,58.7074,76.0249,60.2932,74.6313,60.8702C74.2976,61.01,73.9388,61.082,73.576,61.082C72.5065,61.082,71.4914,60.4582,71.0525,59.4213C67.7577,51.6455,62.331,44.1074,54.9203,37.0116C53.6073,36.1009,48.0984,31.9917,47.2065,26.583C40.9421,26.2679,35.6187,26.8278,31.6725,27.5336C31.6197,29.527,30.9225,31.5172,29.2456,33.3731C28.0614,34.6827,26.5968,35.915,25.0446,37.2188C21.9414,39.8269,18.2648,42.9169,14.8104,48.1192C11.356,53.3215,8.12389,60.6361,5.9098,71.6934C5.88732,71.8035,5.85893,71.9123,5.82344,72.0188C5.81634,72.041,4.57886,76.0413,6.77344,79.5289C8.6332,82.4828,12.4557,84.4139,18.1367,85.2705C19.6297,85.4953,20.6566,86.8762,20.4295,88.3532C20.2213,89.6944,19.0559,90.6541,17.7274,90.6541ZM35.1195,7.03101C33.3502,7.03101,31.9158,5.61208,31.9158,3.86173C31.9158,2.11139,33.3502,0.69245,35.1195,0.69245C36.8889,0.69245,38.3233,2.11139,38.3233,3.86173C38.3233,5.61208,36.8889,7.03101,35.1195,7.03101ZM57.6848,23.1892L58.414,24.4754C58.8984,24.3623,59.3923,24.3435,59.8644,24.4203C60.2191,24.5005,60.5087,24.7182,60.6663,25.0229C60.8636,25.3394,60.8993,25.7346,60.7646,26.1094C60.5988,26.5176,60.2972,26.8749,59.9085,27.1235L60.31,27.8316L59.7886,28.1294L59.3994,27.443C58.9257,27.7175,58.399,27.883,57.8664,27.9247L57.3744,27.0569C57.6378,27.0741,57.9071,27.048,58.1704,26.9797C58.4501,26.9251,58.7239,26.8323,58.9829,26.7044L58.2801,25.4647C57.8047,25.5877,57.3167,25.6065,56.8549,25.5197C56.4913,25.4263,56.196,25.1971,56.0328,24.8814C55.8433,24.5561,55.8127,24.1572,55.9484,23.7789C56.088,23.373,56.3763,23.0149,56.7584,22.7726L56.4166,22.1699L56.938,21.8721L57.2727,22.4625C57.6615,22.2376,58.0888,22.0901,58.5254,22.0301L59.0042,22.8746C58.5548,22.8828,58.103,22.9906,57.6848,23.1892ZM56.9319,24.2961Q57.1278,24.6417,57.7863,24.5856L57.1695,23.4978Q56.6982,23.884,56.9319,24.2961ZM58.9077,25.3462L59.4981,26.3875L59.499,26.3891Q59.9965,26.0045,59.7628,25.5923Q59.573,25.2576,58.9077,25.3462ZM73.2212,66.5065C73.2212,68.2569,74.6555,69.6758,76.4249,69.6758C78.1943,69.6758,79.6286,68.2569,79.6286,66.5065C79.6286,64.7562,78.1943,63.3372,76.4249,63.3372C74.6555,63.3372,73.2212,64.7562,73.2212,66.5065ZM35.9465,91.8045C35.0734,91.8045,34.2038,91.7987,33.3378,91.7858C31.827,91.7636,30.6203,90.5359,30.6428,89.0402C30.6653,87.5457,31.9158,86.3297,33.4183,86.3742C49.6344,86.6059,65.7512,84.6175,67.6134,84.037C72.1953,82.4184,74.5295,79.3603,74.5295,74.9575C74.5295,73.463,75.754,72.2517,77.2648,72.2517C78.7755,72.2517,80,73.463,80,74.9575C80,81.5992,76.148,86.7686,69.4317,89.142C67.0041,89.9999,51.0955,91.8046,35.9465,91.8045ZM25.2731,92C23.5037,92,22.0693,90.5811,22.0693,88.8307C22.0693,87.0804,23.5037,85.6615,25.2731,85.6615C27.0424,85.6615,28.4768,87.0804,28.4768,88.8307C28.4768,90.5811,27.0424,92,25.2731,92Z"></path></svg>'), Od = Id.cloneNode(!0);
class Ud {
  constructor(t) {
    ce(this, "_chartApi", null);
    if (n1.isString(t.container)) {
      if (this._container = document.getElementById(t.container), !this._container)
        throw new Error("Container is null");
    } else
      this._container = t.container;
    this._container.classList.add("klinecharts-pro"), this._container.setAttribute("data-theme", t.theme ?? "light");
    const n = this;
    X9(() => L(Pd, {
      ref: (r) => {
        n._chartApi = r;
      },
      get styles() {
        return t.styles ?? {};
      },
      get watermark() {
        return t.watermark ?? Od;
      },
      get theme() {
        return t.theme ?? "light";
      },
      get locale() {
        return t.locale ?? "zh-CN";
      },
      get drawingBarVisible() {
        return t.drawingBarVisible ?? (typeof window < "u" && window.innerWidth > 768);
      },
      get symbol() {
        return t.symbol;
      },
      get period() {
        return t.period;
      },
      get periods() {
        return t.periods ?? [{
          multiplier: 1,
          timespan: "minute",
          text: "1m"
        }, {
          multiplier: 5,
          timespan: "minute",
          text: "5m"
        }, {
          multiplier: 15,
          timespan: "minute",
          text: "15m"
        }, {
          multiplier: 1,
          timespan: "hour",
          text: "1H"
        }, {
          multiplier: 2,
          timespan: "hour",
          text: "2H"
        }, {
          multiplier: 4,
          timespan: "hour",
          text: "4H"
        }, {
          multiplier: 1,
          timespan: "day",
          text: "D"
        }, {
          multiplier: 1,
          timespan: "week",
          text: "W"
        }, {
          multiplier: 1,
          timespan: "month",
          text: "M"
        }, {
          multiplier: 1,
          timespan: "year",
          text: "Y"
        }];
      },
      get timezone() {
        return t.timezone ?? "Asia/Shanghai";
      },
      get mainIndicators() {
        return t.mainIndicators ?? ["MA"];
      },
      get subIndicators() {
        return t.subIndicators ?? ["VOL"];
      },
      get datafeed() {
        return t.datafeed;
      },
      get onIndicatorChange() {
        return t.onIndicatorChange;
      },
      get onMobilePeriodClick() {
        return t.onMobilePeriodClick;
      },
      get onMobileMoreClick() {
        return t.onMobileMoreClick;
      },
      get screenshotBackgroundColor() {
        return t.screenshotBackgroundColor;
      },
      get orderTools() {
        return t.orderTools ?? {
          visible: !1,
          quickOrder: !0,
          openOrders: !0,
          positions: !0,
          orderHistory: !0
        };
      }
    }), this._container);
  }
  // Drawing methods
  createOverlay(t) {
    var n, r;
    return ((r = (n = this._chartApi) == null ? void 0 : n.createOverlay) == null ? void 0 : r.call(n, t)) ?? null;
  }
  removeOverlay(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.removeOverlay) == null || r.call(n, t);
  }
  removeAllOverlay() {
    var t, n;
    (n = (t = this._chartApi) == null ? void 0 : t.removeAllOverlay) == null || n.call(t);
  }
  getAllOverlay() {
    var t, n;
    return ((n = (t = this._chartApi) == null ? void 0 : t.getAllOverlay) == null ? void 0 : n.call(t)) || [];
  }
  getOverlay(t) {
    var n, r;
    return ((r = (n = this._chartApi) == null ? void 0 : n.getOverlay) == null ? void 0 : r.call(n, t)) ?? null;
  }
  overrideOverlay(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.overrideOverlay) == null || r.call(n, t);
  }
  // Utility methods
  dispose() {
    var t, n;
    (n = (t = this._chartApi) == null ? void 0 : t.dispose) == null || n.call(t);
  }
  resize() {
    var t, n;
    (n = (t = this._chartApi) == null ? void 0 : t.resize) == null || n.call(t);
  }
  getMainIndicators() {
    return this._chartApi.getMainIndicators();
  }
  overrideIndicator(t, n) {
    const r = this._chartApi;
    r && typeof r.overrideIndicator == "function" ? r.overrideIndicator(t, n) : console.warn("overrideIndicator method not available on chart API");
  }
  setMainIndicators(t) {
    return this._chartApi.setMainIndicators(t);
  }
  getSubIndicators() {
    return this._chartApi.getSubIndicators();
  }
  setSubIndicators(t) {
    return this._chartApi.setSubIndicators(t);
  }
  setTheme(t) {
    var n;
    (n = this._container) == null || n.setAttribute("data-theme", t), this._chartApi.setTheme(t);
  }
  getTheme() {
    return this._chartApi.getTheme();
  }
  setStyles(t) {
    this._chartApi.setStyles(t);
  }
  getStyles() {
    return this._chartApi.getStyles();
  }
  setLocale(t) {
    this._chartApi.setLocale(t);
  }
  getLocale() {
    return this._chartApi.getLocale();
  }
  setTimezone(t) {
    this._chartApi.setTimezone(t);
  }
  getTimezone() {
    return this._chartApi.getTimezone();
  }
  setSymbol(t) {
    this._chartApi.setSymbol(t);
  }
  getSymbol() {
    return this._chartApi.getSymbol();
  }
  setPeriod(t) {
    this._chartApi.setPeriod(t);
  }
  getPeriod() {
    return this._chartApi.getPeriod();
  }
  getSettings() {
    return this._chartApi.getSettings();
  }
  setSettings(t) {
    this._chartApi.setSettings(t);
  }
  resetSettings() {
    this._chartApi.resetSettings();
  }
  saveDrawings(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.saveDrawings) == null || r.call(n, t);
  }
  loadDrawings(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.loadDrawings) == null || r.call(n, t);
  }
  getDrawings(t) {
    var n, r;
    return ((r = (n = this._chartApi) == null ? void 0 : n.getDrawings) == null ? void 0 : r.call(n, t)) || [];
  }
  clearDrawings(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.clearDrawings) == null || r.call(n, t);
  }
  enableAutoSave(t, n = !0) {
    var r, i;
    (i = (r = this._chartApi) == null ? void 0 : r.enableAutoSave) == null || i.call(r, t, n);
  }
  setIndicatorModalVisible(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.setIndicatorModalVisible) == null || r.call(n, t);
  }
  setTimezoneModalVisible(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.setTimezoneModalVisible) == null || r.call(n, t);
  }
  setSettingModalVisible(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.setSettingModalVisible) == null || r.call(n, t);
  }
  getOrderToolsState() {
    var t, n;
    return ((n = (t = this._chartApi) == null ? void 0 : t.getOrderToolsState) == null ? void 0 : n.call(t)) ?? {
      quickOrder: !0,
      openOrders: !0,
      positions: !0,
      orderHistory: !0
    };
  }
  setOrderToolsState(t) {
    var n, r;
    (r = (n = this._chartApi) == null ? void 0 : n.setOrderToolsState) == null || r.call(n, t);
  }
  // Forwarded klinecharts chart methods for synchronizing DOM overlays
  // with the canvas (price/pixel conversion, visible-range, data, actions).
  convertToPixel(t, n) {
    return this._chartApi.convertToPixel(t, n);
  }
  convertFromPixel(t, n) {
    return this._chartApi.convertFromPixel(t, n);
  }
  getVisibleRange() {
    return this._chartApi.getVisibleRange();
  }
  getDataList() {
    var t, n;
    return ((n = (t = this._chartApi) == null ? void 0 : t.getDataList) == null ? void 0 : n.call(t)) ?? [];
  }
  getSize(t, n) {
    var r, i;
    return ((i = (r = this._chartApi) == null ? void 0 : r.getSize) == null ? void 0 : i.call(r, t, n)) ?? null;
  }
  subscribeAction(t, n) {
    var r, i;
    (i = (r = this._chartApi) == null ? void 0 : r.subscribeAction) == null || i.call(r, t, n);
  }
  unsubscribeAction(t, n) {
    var r, i;
    (i = (r = this._chartApi) == null ? void 0 : r.unsubscribeAction) == null || i.call(r, t, n);
  }
}
B9.forEach((e) => {
  v9(e);
});
export {
  Ed as DefaultDatafeed,
  Ud as KLineChartPro,
  Bd as loadLocales
};
//# sourceMappingURL=klinecharts-pro.js.map
