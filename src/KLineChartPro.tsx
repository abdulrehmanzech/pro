...
export default class KLineChartPro implements ChartPro {
    ...
    render(
      () => (
        <ChartProComponent
          ...
          drawingBarVisible={options.drawingBarVisible ?? (typeof window !== 'undefined' && window.innerWidth > 768)}
          ...
          orderTools={options.orderTools ?? { visible: false, openOrders: true, positions: true, orderHistory: true }}
        />
      ),
      this._container
    );
  ...
  createOverlay(overlay: OverlayCreate): string | null {
    return this._chartApi?.createOverlay?.(overlay) ?? null;
  }

  removeOverlay(options: { groupId?: string; id?: string }): void {
    this._chartApi?.removeOverlay?.(options);
  }

  removeAllOverlay(): void {
    this._chartApi?.removeAllOverlay?.();
  }

  getAllOverlay(): OverlayInfo[] {
    return this._chartApi?.getAllOverlay?.() || [];
  }

  getOverlay(id: string): OverlayInfo | null {
    return this._chartApi?.getOverlay?.(id) ?? null;
  }

  overrideOverlay(options: { [key: string]: any }): void {
    this._chartApi?.overrideOverlay?.(options);
  }
  ...
  getOrderToolsState() {
    return this._chartApi?.getOrderToolsState?.() ?? { openOrders: true, positions: true, orderHistory: true };
  }

  setOrderToolsState(state: { openOrders?: boolean; positions?: boolean; orderHistory?: boolean }): void {
  ...
  convertToPixel(
    ...
    return this._chartApi!.convertToPixel(points, finder);
  }
  ...
  convertFromPixel(
    ...
    return this._chartApi!.convertFromPixel(coordinates, finder);
  }
  ...
  unsubscribeAction(type: ActionType, callback?: ActionCallback): void {
    this._chartApi?.unsubscribeAction?.(type, callback);
  }
}
