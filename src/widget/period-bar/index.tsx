...
import { SymbolInfo, Period, OrderToolsState, ChartToolbarActionOptions } from "../../types";
...
const PeriodBar: Component<PeriodBarProps> = (props) => {
  ...
  onMount(() => {
    ...
    if (ref) {
      (ref as HTMLElement).addEventListener("scroll", checkScroll);
      // Check initial state after a small delay for rendering
  ...
  onCleanup(() => {
    ...
    if (ref) {
      (ref as HTMLElement).removeEventListener("scroll", checkScroll);
    }
  });
  ...
  const visiblePeriods = createMemo(() => {
    const periods = props.periods
      .filter((p) => {
        ...
        if (!secondaryText || secondaryText === activeText) {
          const fallback = props.periods.find(
            (item) => item.text !== activeText,
          );
    ...
    // Trigger scroll check when periods change
  ...
  createEffect(() => {
    ...
    // Check scroll on any period change
    setTimeout(checkScroll, 50);
  });
    ...
    >
      ...
      >
        <button
          ...
          type="button"
          aria-label="Toggle drawing sidebar"
        ...
        >
                    ...
                    <label class="klinecharts-pro-order-tools-item">
                      <span class="klinecharts-pro-order-tools-checkbox-box">
                        <input
                          ...
                          onChange={(event) => {
                            props.onOrderToolsStateChange?.({
                              orderHistory: event.currentTarget.checked,
                            });
                          }}
                        />
                        <span class="klinecharts-pro-order-tools-checkbox-fill" />
          ...
          <Show when={!isMobile()}>
            <div class="item tools klinecharts-pro-timezone-tool" onClick={props.onTimezoneClick}>
          ...
          <Show when={!isMobile()}>
            <Show when={props.chartToolbarAction} keyed>
              {(action) => (
                <button
                  type="button"
                  class={`item tools klinecharts-pro-chart-toolbar-action ${action.className ?? ""}`}
                  title={action.title}
                  aria-label={action.ariaLabel ?? action.title}
                  style={action.style}
                  onClick={action.onClick}
                >
                  <span innerHTML={action.iconSvg} />
                </button>
              )}
            </Show>
